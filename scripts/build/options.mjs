...
const parsed = minimist(process.argv.slice(2), {
	...
	default: {
		...
		inspect:
			process.env.inspect || process.env["inspect-brk"] || process.env.i,
		host: process.env.TYPESCRIPT_HOST || process.env.host || "node",
		browser:
			process.env.browser ||
			process.env.b ||
			(os.platform() === "win32" ? "edge" : "chrome"),
		...
		workers:
			+(process.env.workerCount ?? 0) ||
			os.cpus().length - (ci ? 0 : 1) ||
			1,
		failed: false,
		keepFailed: false,
		lkg: false,
...
if (!options.bundle && !options.typecheck) {
	throw new Error(
		"--no-typecheck cannot be passed when bundling is disabled",
	);
}
