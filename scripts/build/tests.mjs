...
import { exec, ExecError, rimraf } from "./utils.mjs";

const mochaJs = path.resolve(
	findUpRoot(),
	"node_modules",
	"mocha",
	"bin",
...
export async function runConsoleTests(
	...
	if (!cmdLineOptions.dirty) {
		if (options.watching) {
			console.log(
				chalk.yellowBright(`[watch] cleaning test directories...`),
			);
		}
		await cleanTestDirs();
	...
	if (!runInParallel) {
		...
		if (failed) {
			const grep = fs
				.readFileSync(".failed-tests", "utf8")
				.split(/\r?\n/g)
				.map((test) => test.trim())
	...
	try {
		...
		if (coverage) {
			await exec(
				"npm",
				["--prefer-offline", "exec", "--", "c8", "report"],
				{ token: options.token },
			);
		}
	} catch (e) {
		errorStatus =
			e instanceof ExecError ? (e.exitCode ?? undefined) : undefined;
		error = /** @type {Error} */ (e);
	} finally {
		if (coverage) {
	...
	if (error !== undefined) {
		...
		if (options.watching) {
			...
			process.exitCode =
				typeof errorStatus === "number" ? errorStatus : 2;
			throw error;
		}
	}
}
