...
class FailedTestsReporter extends Mocha.reporters.Base {
	...
	constructor(runner, options) {
		...
		const reporterOptions = (this.reporterOptions =
			options?.reporterOptions || {});
		if (reporterOptions.file === undefined)
			reporterOptions.file = ".failed-tests";
		if (reporterOptions.keepFailed === undefined)
			reporterOptions.keepFailed = false;
		if (reporterOptions.reporter) {
			...
			if (typeof reporterOptions.reporter === "function") {
				...
				try {
					...
					reporter = require(
						path.resolve(process.cwd(), reporterOptions.reporter),
					);
				}
			}
	...
	static writeFailures(file, passes, failures, keepFailed, done) {
		const failingTests = new Set(
			fs.existsSync(file) ? readTests() : undefined,
		);
		...
		if (failingTests.size > 0) {
			const failed = Array.from(failingTests).sort().join(os.EOL);
