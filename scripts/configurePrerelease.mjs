...
function main() {
	...
	if (args.length < 3) {
		...
		console.log("Usage:");
		console.log(
			`\tnode ${thisProgramName} <dev|insiders> <package.json location> <file containing version>`,
		);
		return;
	}
	...
	const modifiedTsFileContents = updateTsFile(
		tsFilePath,
		tsFileContents,
		majorMinor,
		patch,
	...
	packageJsonValue.gitHead = execFileSync("git", ["rev-parse", "HEAD"], {
		encoding: "utf8",
	}).trim();
	writeFileSync(
		packageJsonFilePath,
		JSON.stringify(packageJsonValue, /*replacer:*/ undefined, /*space:*/ 4),
	);
...
function updateTsFile(
	...
	const versionRgx =
		/export const version(?:: string)? = `\$\{versionMajorMinor\}\.(\d)(-\w+)?`;/;
