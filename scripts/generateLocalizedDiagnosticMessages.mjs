...
async function main() {
	...
	async function visitDirectory(name) {
		...
		if (!result || !result.LCX || !result.LCX.$_TgtCul) {
			console.error(
				"Unexpected XML file structure. Expected to find result.LCX.$_TgtCul.",
			);
			process.exit(1);
		}
		const outputDirectoryName = getPreferredLocaleName(
			result.LCX.$_TgtCul,
		).toLowerCase();
		if (!outputDirectoryName) {
			console.error(
				`Invalid output locale name for '${result.LCX.$_TgtCul}'.`,
			);
			process.exit(1);
		}
		await writeFile(
		...
		await writeFile(
			...
			getLCGFileXML(
				Object.entries(JSON.parse(contents))
					...
					.reduce(
						(s, [key, value]) => s + getItemXML(key, value),
						"",
					),
			),
		);
