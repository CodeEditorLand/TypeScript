...
async function main() {
	if (process.argv.length < 3) {
		console.log("Usage:");
		console.log(
			"\tnode processDiagnosticMessages.mjs <diagnostic-json-input-file>",
		);
		return;
	}
	...
	async function writeFile(fileName, contents) {
		...
		try {
			const existingContents = await fs.promises.readFile(
				filePath,
				"utf-8",
			);
	...
	const inputStr = await fs.promises.readFile(inputFilePath, {
		encoding: "utf-8",
	});
	...
	const infoFileOutput = buildInfoFileOutput(
		diagnosticMessages,
		inputFilePath,
	);
...
function buildInfoFileOutput(messageTable, inputFilePathRel) {
	...
	messageTable.forEach(
		(
			...
			const argReportsUnnecessary = reportsUnnecessary
