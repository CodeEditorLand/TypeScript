...
export function discoverTypings(
	...
	packageNameToTypingLocation.forEach((typing, name) => {
		...
		if (
			inferredTypings.get(name) === false &&
	...
	function getTypingNames(
		...
		const packagesFolderPath = combinePaths(
			projectRootPath,
			modulesDirName,
		);
		...
		if (log)
			log(
				`Searching for typing names in ${packagesFolderPath}; all files: ${JSON.stringify(dependencyManifestNames)}`,
			);
		...
		for (const manifestPath of dependencyManifestNames) {
			...
			if (ownTypes) {
				...
				if (host.fileExists(absolutePath)) {
					if (log)
						log(
							`    Package '${manifest.name}' provides its own types.`,
						);
	...
	function getTypingNamesFromSourceFileNames(fileNames: string[]) {
		const fromFileNames = mapDefined(fileNames, (j) => {
			...
			const inferredTypingName = removeFileExtension(
				toFileNameLowerCase(getBaseFileName(j)),
			);
		...
		if (hasJsxFile) {
			if (log)
				log(
					`Inferred 'react' typings due to presence of '.jsx' extension`,
				);
