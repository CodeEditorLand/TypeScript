...
export function isNameOfFunctionDeclaration(node: Node): boolean {
	return (
		isIdentifier(node) &&
		tryCast(node.parent, isFunctionLike)?.name === node
	);
}
	...
	switch (n.kind) {
		...
		case SyntaxKind.ConstructorType:
			return isCompletedNode(
				(n as SignatureDeclaration).type,
				sourceFile,
			);
		...
		case SyntaxKind.IfStatement:
			if ((n as IfStatement).elseStatement) {
				return isCompletedNode(
					(n as IfStatement).elseStatement,
					sourceFile,
				);
			}
			return isCompletedNode(
				(n as IfStatement).thenStatement,
				sourceFile,
			);
		...
		case SyntaxKind.ExpressionStatement:
			return (
				isCompletedNode(
					(n as ExpressionStatement).expression,
					sourceFile,
		...
		case SyntaxKind.WhileStatement:
			return isCompletedNode(
				(n as IterationStatement).statement,
				sourceFile,
			);
...
function getAdjustedLocation(node: Node, forRename: boolean): Node {
	...
	if (
		isModifier(node) &&
		(forRename || node.kind !== SyntaxKind.DefaultKeyword)
...
export function getTouchingPropertyName(
	...
	return getTouchingToken(
		...
		position,
		(n) =>
			isPropertyNameLiteral(n) ||
			isKeyword(n.kind) ||
			isPrivateIdentifier(n),
	);
}
...
function getTokenAtPositionWorker(
		...
		const i = binarySearchKey(
			...
			(middle, _) => {
				...
				const start = allowPositionInLeadingTrivia
					...
					: children[middle].getStart(
							sourceFile,
							/*includeJsDocComment*/ true,
						);
				if (start > position) {
	...
	function nodeContainsPosition(node: Node, start?: number, end?: number) {
		...
		if (
			...
			const previousToken = findPrecedingToken(
				position,
				sourceFile,
				node,
			);
		...
		const i = binarySearchKey(
			...
			(middle, _) => {
				...
				if (position < children[middle].end) {
					...
					if (
						!children[middle - 1] ||
						position >= children[middle - 1].end
					) {
						return Comparison.EqualTo;
					}
					return Comparison.GreaterThan;
...
export function isInTemplateString(sourceFile: SourceFile, position: number) {
	const token = getTokenAtPosition(sourceFile, position);
	return (
		isTemplateLiteralKind(token.kind) &&
		position > token.getStart(sourceFile)
	);
}
...
function areIntersectedTypesAvoidingStringReduction(
	checker: TypeChecker,
	t1: Type,
	t2: Type,
) {
	return (
		!!(t1.flags & TypeFlags.String) &&
		checker.isEmptyAnonymousObjectType(t2)
	);
}
...
/** @internal */
export function moduleResolutionUsesNodeModules(
...
export function findModifier(
	...
	return canHaveModifiers(node)
		? find(node.modifiers, (m): m is Modifier => m.kind === kind)
		: undefined;
}
...
export function insertImports(
	...
	let sortKind = isArray(imports)
		? OrganizeImports.detectImportDeclarationSorting(imports, preferences)
		: SortKind.Both;
	...
	const sortedNewImports = isArray(imports)
		? stableSort(imports, (a, b) =>
				OrganizeImports.compareImportsOrRequireStatements(
					a,
					b,
					comparer,
				),
	...
	if (!existingImportStatements.length) {
		...
		(sortKind = OrganizeImports.detectImportDeclarationSorting(
			existingImportStatements,
			preferences,
		))
	) {
...
export function documentSpansEqual(
	...
	return (
		(useCaseSensitiveFileNames
			? equateStringsCaseSensitive
			: equateStringsCaseInsensitive)(a.fileName, b.fileName) &&
		textSpansEqual(a.textSpan, b.textSpan)
	);
}

/** @internal */
export function getDocumentSpansEqualityComparer(
	useCaseSensitiveFileNames: boolean,
): EqualityComparer<DocumentSpan> {
...
export function getMappedLocation(
	...
	return (
		mapsTo &&
		(!fileExists || fileExists(normalizePath(mapsTo.fileName))
			? mapsTo
			: undefined)
	);
}
...
function getDisplayPartWriter(): DisplayPartsSymbolWriter {
	...
	return {
		displayParts: () => {
			const finalText =
				displayParts.length &&
				displayParts[displayParts.length - 1].text;
			if (
				...
				if (
					!isWhiteSpaceLike(
						finalText.charCodeAt(finalText.length - 1),
					)
				) {
					displayParts.push(
		...
		writeOperator: (text) =>
			writeKind(text, SymbolDisplayPartKind.operator),
...
export function buildLinkParts(
	...
	if (!link.name) {
		...
		const targetSymbol =
			symbol && checker ? getSymbolTarget(symbol, checker) : undefined;
		...
		if (decl) {
			...
			const separator =
				suffix === 0 ||
				(link.text.charCodeAt(suffix) === CharacterCodes.bar &&
					name.charCodeAt(name.length - 1) !== CharacterCodes.space)
					? " "
					: "";
...
export function getNewLineOrDefaultFromHost(
	host: FormattingHost,
	formatSettings: FormatCodeSettings | undefined,
) {
	return formatSettings?.newLineCharacter || host.getNewLine?.() || lineFeed;
}
...
/** @internal */
export function nodeToDisplayParts(
	node: Node,
	enclosingDeclaration: Node,
): SymbolDisplayPart[] {
...
export function getSymbolTarget(symbol: Symbol, checker: TypeChecker): Symbol {
	...
	while (
		isAliasSymbol(next) ||
		(isTransientSymbol(next) && next.links.target)
	) {
...
function getSynthesizedDeepCloneWorker<T extends Node>(
	...
	const nodesClone: <T extends Node>(
		ns: NodeArray<T> | undefined,
	) => NodeArray<T> | undefined = replaceNode
		? (ns) =>
				ns &&
...
export function createPackageJsonImportFilter(
	...
	function allowsImportingAmbientModule(
		...
		const declaringSourceFile =
			moduleSymbol.valueDeclaration.getSourceFile();
...
function getDefaultLikeExportNameFromDeclaration(
	symbol: Symbol,
): string | undefined {
	return firstDefined(symbol.declarations, (d) => {
		...
		if (isExportAssignment(d)) {
			return tryCast(skipOuterExpressions(d.expression), isIdentifier)
				?.text;
		}
		// "export { ~ as default }"
...
export function isNonGlobalDeclaration(declaration: Declaration) {
	...
	return (
		...
		!findAncestor(
			declaration,
			(d) => isModuleDeclaration(d) && isGlobalScopeAugmentation(d),
		)
	);
}
...
/** @internal */
export type DiagnosticOrDiagnosticAndArguments =
	| DiagnosticMessage
	| DiagnosticAndArguments;
/** @internal */
export function diagnosticToString(
	diag: DiagnosticOrDiagnosticAndArguments,
): string {
	return isArray(diag)
		? formatStringFromArgs(
				getLocaleSpecificMessage(diag[0]),
				diag.slice(1) as DiagnosticArguments,
			)
