...
function getRenameInfoForNode(
	...
	if (isStringLiteralLike(node) && tryGetImportFromModuleSpecifier(node)) {
		return preferences.allowRenameOfImportPath
...
function wouldRenameInOtherNodeModules(
	...
	if (
		...
		const importSpecifier =
			symbol.declarations &&
			find(symbol.declarations, (decl) => isImportSpecifier(decl));
		if (
			importSpecifier &&
			!(importSpecifier as ImportSpecifier).propertyName
		) {
	...
	if (originalPackage === undefined) {
		...
		if (
			some(declarations, (declaration) =>
				isInsideNodeModules(declaration.getSourceFile().path),
			)
		) {
	...
	for (const declaration of declarations) {
		const declPackage = getPackagePathComponents(
			declaration.getSourceFile().path,
		);
		if (declPackage) {
