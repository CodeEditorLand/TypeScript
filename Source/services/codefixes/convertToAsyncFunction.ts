...
function convertToAsyncFunction(
	...
	const pos = skipTrivia(
		sourceFile.text,
		moveRangePastModifiers(functionToConvert).pos,
	);
	changes.insertModifierAt(sourceFile, pos, SyntaxKind.AsyncKeyword, {
		suffix: " ",
	});
	...
	forEachChild(nodeToRename, function visit(node: Node) {
		...
		if (symbol) {
			...
			if (
				...
				const firstParameter = firstOrUndefined(
					lastCallSignature.parameters,
				);
				const ident =
					(firstParameter?.valueDeclaration &&
						isParameter(firstParameter.valueDeclaration) &&
						tryCast(
							firstParameter.valueDeclaration.name,
							isIdentifier,
						)) ||
				...
				const synthName = getNewNameIfConflict(
					ident,
					collidingSymbolMap,
				);
			...
			// We only care about identifiers that are parameters, variable declarations, or binding elements
				...
				if (
					...
					const newName = getNewNameIfConflict(
						node,
						collidingSymbolMap,
					);
	...
	return getSynthesizedDeepCloneWithReplacements(
		...
		(original) => {
			if (
				...
				const renameInfo =
					symbol &&
					identsToRenameMap.get(String(getSymbolId(symbol)));
				if (
					renameInfo &&
					renameInfo.text !==
...
function getNewNameIfConflict(
	name: Identifier,
	originalNames: ReadonlyMap<string, Symbol[]>,
): SynthIdentifier {
	...
	if (continuationArgName && !shouldReturn(node, transformer)) {
		if (isSynthIdentifier(continuationArgName)) {
			...
			transformer.synthNamesMap.forEach((val, key) => {
				if (
					val.identifier.text === continuationArgName.identifier.text
				) {
					const newSynthName =
						createUniqueSynthName(continuationArgName);
		...
		return [
			factory.createExpressionStatement(
				factory.createAssignment(
					getSynthesizedDeepClone(
						referenceSynthIdentifier(variableName),
					),
					rightHandSide,
				),
			),
		...
		factory.createVariableStatement(
			...
			factory.createVariableDeclarationList(
				[
					factory.createVariableDeclaration(
						getSynthesizedDeepClone(
							declareSynthBindingName(variableName),
						),
						/*exclamationToken*/ undefined,
						typeAnnotation,
						rightHandSide,
		...
		case SyntaxKind.Identifier: // identifier includes undefined
			...
			if (continuationArgName) {
				continuationArgName.types.push(
					transformer.checker.getAwaitedType(returnType) ||
						returnType,
				);
			}
			...
			if (isBlock(funcBody)) {
				...
				if (returnType) {
					...
					if (!shouldReturn(parent, transformer)) {
						...
						if (continuationArgName) {
							continuationArgName.types.push(
								transformer.checker.getAwaitedType(
									returnType,
								) || returnType,
							);
						}
...
function getArgBindingName(
	...
	function getSymbol(node: Node): Symbol | undefined {
		return (
			tryCast(node, canHaveSymbol)?.symbol ??
