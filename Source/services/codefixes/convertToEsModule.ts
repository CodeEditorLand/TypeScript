...
registerCodeFix({
	errorCodes: [
		Diagnostics
			.File_is_a_CommonJS_module_it_may_be_converted_to_an_ES_module.code,
	],
...
function collectExportRenames(
	...
	forEachExportReference(sourceFile, (node) => {
		...
		if (
			!res.has(text) &&
			(isIdentifierANonContextualKeyword(node.name) ||
				checker.resolveName(
					text,
					node,
					SymbolFlags.Value,
...
function classExpressionToDeclaration(
	...
	return factory.createClassDeclaration(
		concatenate(
			additionalModifiers,
			getSynthesizedDeepClones(cls.modifiers),
		),
		name,
