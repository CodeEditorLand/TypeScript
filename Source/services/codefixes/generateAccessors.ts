...
/** @internal */
export type AcceptedDeclaration =
	| ParameterPropertyDeclaration
	| PropertyDeclaration
	| PropertyAssignment;
/** @internal */
...
/** @internal */
export type AccessorOrRefactorErrorInfo =
	| AccessorInfo
	| refactor.RefactorErrorInfo;
/** @internal */
...
export function generateAccessorFromProperty(
	...
	if (isClassLike(container)) {
		...
		if (isSourceFileJS(file)) {
			...
			fieldModifiers = factory.createModifiersFromModifierFlags(
				prepareModifierFlagsForField(modifierFlags),
			);
		}
		if (canHaveDecorators(declaration)) {
			fieldModifiers = concatenate(
...
export function getAccessorConvertiblePropertyAtPosition(
	...
	if (
		((getEffectiveModifierFlags(declaration) & ModifierFlags.Modifier) |
			meaning) !==
		meaning
	) {
		return {
...
function updatePropertyAssignmentDeclaration(
	...
	let assignment = factory.updatePropertyAssignment(
		declaration,
		fieldName,
		declaration.initializer,
	);
	...
	if (
		...
		(assignment as Mutable<PropertyAssignment>).exclamationToken =
			undefined;
	}
...
function getDeclarationType(
	...
	if (
		...
		if (
			...
			const types = isUnionTypeNode(typeNode)
				? typeNode.types
				: [typeNode];
...
export function getAllSupers(
	...
	while (decl) {
		...
		const superSymbol =
			superElement &&
			checker.getSymbolAtLocation(superElement.expression);
