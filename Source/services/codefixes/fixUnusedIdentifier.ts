	...
	getAllCodeActions: (context) => {
		...
		return codeFixAll(context, errorCodes, (changes, diag) => {
			...
			switch (context.fixId) {
				...
				case fixIdDelete: {
					if (
						token.kind === SyntaxKind.InferKeyword ||
						isImport(token)
					) {
						break; // Can't delete
						...
						canDeleteEntireVariableStatement(sourceFile, token)
					) {
						deleteEntireVariableStatement(
							changes,
							sourceFile,
...
function deleteDestructuring(
	...
	if (
		...
		parent.initializer &&
		isCallLikeExpression(parent.initializer)
	) {
		if (
			isVariableDeclarationList(parent.parent) &&
			length(parent.parent.declarations) > 1
		) {
...
function tryDeleteDeclaration(
	...
	if (isIdentifier(token)) {
		FindAllReferences.Core.eachSymbolReferenceInFile(
			...
			(ref: Node) => {
				if (
					isPropertyAccessExpression(ref.parent) &&
					ref.parent.name === ref
				)
					ref = ref.parent;
...
function tryDeleteDeclarationWorker(
	...
	if (isParameter(parent)) {
		...
		Debug.assert(
			node !== sourceFile,
			"should not delete whole source file",
		);
...
function mayDeleteParameter(
	...
	switch (parent.kind) {
			...
			if (entries) {
				for (const entry of entries) {
					for (const reference of entry.references) {
						if (
							reference.kind === FindAllReferences.EntryKind.Node
						) {
							// argument in super(...)
							const isSuperCall =
							...
							const isSuperMethodCall =
								...
								isSuperKeyword(
									reference.node.parent.expression,
								) &&
								isCallExpression(
									reference.node.parent.parent,
								) &&
								reference.node.parent.parent.arguments.length >
									index;
		...
		case SyntaxKind.FunctionDeclaration: {
			if (
				parent.name &&
				isCallbackLike(checker, sourceFile, parent.name)
			) {
