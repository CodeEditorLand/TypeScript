...
registerCodeFix({
	...
	getCodeActions(context) {
		...
		return info.map(({ fix, symbolName, errorIdentifierText }) =>
			codeActionForFix(
				...
				symbolName,
				fix,
				/*includeSymbolNameInDescription*/ symbolName !==
					errorIdentifierText,
				program,
				preferences,
			),
	...
	getAllCodeActions: (context) => {
		const { sourceFile, program, preferences, host, cancellationToken } =
			context;
...
export interface ImportAdder {
	...
	writeFixes: (
		...
		oldFileQuotePreference?: QuotePreference,
	) => void;
}
...
function createImportAdderWorker(
			...
			case ImportFixKind.AddNew: {
				...
				switch (importKind) {
					...
					case ImportKind.Named:
						const prevValue = (entry.namedImports ||=
							new Map()).get(symbolName);
						entry.namedImports.set(
							symbolName,
							reduceAddAsTypeOnlyValues(prevValue, addAsTypeOnly),
						);
	...
	function writeFixes(
		...
		if (
			sourceFile.imports.length === 0 &&
			oldFileQuotePreference !== undefined
		) {
			...
			({ importClauseOrBindingPattern, defaultImport, namedImports }) => {
				doAddExistingFix(
					...
					arrayFrom(
						...
						([name, addAsTypeOnly]) => ({ addAsTypeOnly, name }),
					),
					preferences,
				);
			},
		...
		newImports.forEach(
			(
				...
				const getDeclarations = useRequire
					? getNewRequires
					: getNewImports;
...
export function getImportCompletionAction(
	...
	if (exportMapKey) {
		...
		exportInfos = getExportInfoMap(
			sourceFile,
			host,
			program,
			preferences,
			cancellationToken,
		).get(sourceFile.path, exportMapKey);
		Debug.assertIsDefined(
			exportInfos,
			"Some exportInfo should match the specified exportMapKey",
		);
	} else {
	...
	return getExportInfoMap(
		...
		(info) => {
			if (
				skipAlias(
					info[0].symbol,
					getChecker(info[0].isFromPackageJson),
				) === symbol &&
				info.some(
					(i) =>
						i.moduleSymbol === moduleSymbol ||
						i.symbol.parent === moduleSymbol,
				)
			) {
				return info;
			}
...
function getImportFixes(
	...
	host: LanguageServiceHost,
	preferences: UserPreferences,
	importMap = createExistingImportMap(
		program.getTypeChecker(),
		sourceFile,
		program.getCompilerOptions(),
	),
	...
	const checker = program.getTypeChecker();
	const existingImports = flatMap(
		exportInfos,
		importMap.getImportsForExportInfo,
	);
	const useNamespace =
		usagePosition !== undefined &&
		tryUseExistingNamespaceImport(existingImports, usagePosition);
	...
	if (addToExisting) {
		...
		return {
			...
			fixes: [
				...(useNamespace ? [useNamespace] : emptyArray),
				addToExisting,
			],
		};
	}
...
function tryAddToExistingImport(
	...
	for (const existingImport of existingImports) {
		...
		if (!fix) continue;
		const isTypeOnly = isTypeOnlyImportDeclaration(
	...
	function getAddToExistingImportFix({
		...
		symbol,
		targetFlags,
	}: FixAddToExistingImportInfo): FixAddToExistingImport | undefined {
		if (
...
function createExistingImportMap(
	...
	for (const moduleSpecifier of importingFile.imports) {
		...
		if (isVariableDeclarationInitializedToRequire(i.parent)) {
			const moduleSymbol =
				checker.resolveExternalModuleName(moduleSpecifier);
			if (moduleSymbol) {
				(importMap ||= createMultiMap()).add(
					getSymbolId(moduleSymbol),
					i.parent,
				);
			}
		} else if (
	...
	return {
		getImportsForExportInfo: ({
			...
			targetFlags,
			symbol,
		}: SymbolExportInfo): readonly FixAddToExistingImportInfo[] => {
			...
			if (
				...
				return emptyArray;
			const matchingDeclarations = importMap?.get(
				getSymbolId(moduleSymbol),
			);
...
function getNewImportFixes(
	...
	const rejectNodeModulesRelativePaths =
		moduleResolutionUsesNodeModules(moduleResolution);
	const getModuleSpecifiers = fromCacheOnly
		? (moduleSymbol: Symbol) => ({
				moduleSpecifiers:
					moduleSpecifiers.tryGetModuleSpecifiersFromCache(
						moduleSymbol,
						sourceFile,
	...
	const fixes = flatMap(exportInfo, (exportInfo, i) => {
		...
		return mapDefined(
			...
			(
				moduleSpecifier,
			): FixAddNewImport | FixAddJsdocTypeImport | undefined => {
				if (
					rejectNodeModulesRelativePaths &&
					pathContainsNodeModules(moduleSpecifier)
				) {
					return undefined;
				}
				if (
					!importedSymbolHasValueMeaning &&
					isJs &&
					usagePosition !== undefined
				) {
				...
				if (
					...
					// fs.writeFile
					const exportEquals = checker.resolveExternalModuleSymbol(
						exportInfo.moduleSymbol,
					);
					let namespacePrefix;
					if (exportEquals !== exportInfo.moduleSymbol) {
						namespacePrefix = getDefaultExportInfoWorker(
							exportEquals,
							checker,
							compilerOptions,
						)?.name;
...
function getFixInfos(
	...
	if (
		...
		return (
			fix && [{ fix, symbolName, errorIdentifierText: symbolToken.text }]
		);
	} else {
...
function getUmdSymbol(token: Node, checker: TypeChecker): Symbol | undefined {
	...
	const { parent } = token;
	if (
		(isJsxOpeningLikeElement(parent) && parent.tagName === token) ||
...
function getSymbolNamesToImport(
	...
	if (
		...
		if (needsJsxNamespaceFix(jsxNamespace, symbolToken, checker)) {
			const needsComponentNameFix =
				!isIntrinsicJsxName(symbolToken.text) &&
				!checker.resolveName(
					symbolToken.text,
					symbolToken,
					SymbolFlags.Value,
		...
		case ImportFixKind.AddNew: {
			...
			const namespaceLikeImport =
				...
				importKind === ImportKind.CommonJS
					? {
							importKind,
							name: qualification?.namespacePrefix || symbolName,
							addAsTypeOnly,
						}
					: undefined;
			insertImports(
...
function getModuleSpecifierText(
	...
	return promotedDeclaration.kind === SyntaxKind.ImportEqualsDeclaration
		...
		: cast(promotedDeclaration.parent.moduleSpecifier, isStringLiteral)
				.text;
}
...
function promoteFromTypeOnly(
		...
		case SyntaxKind.ImportSpecifier:
			if (aliasDeclaration.isTypeOnly) {
				...
				if (aliasDeclaration.parent.elements.length > 1 && sortKind) {
					...
					if (
						insertionIndex !==
						aliasDeclaration.parent.elements.indexOf(
							aliasDeclaration,
						)
					) {
				...
				changes.deleteRange(sourceFile, {
					...
					end: getTokenPosOfNode(
						aliasDeclaration.propertyName ?? aliasDeclaration.name,
					),
				});
				return aliasDeclaration;
			} else {
	...
	function promoteImportClause(importClause: ImportClause) {
		...
		if (!compilerOptions.allowImportingTsExtensions) {
			const moduleSpecifier = tryGetModuleSpecifierFromDeclaration(
				importClause.parent,
			);
			const resolvedModule =
				moduleSpecifier &&
				program.getResolvedModuleFromModuleSpecifier(moduleSpecifier)
					?.resolvedModule;
			if (resolvedModule?.resolvedUsingTsExtension) {
				...
				changes.replaceNode(
					sourceFile,
					moduleSpecifier!,
					factory.createStringLiteral(changedExtension),
				);
			}
		}
		if (convertExistingToTypeOnly) {
			...
			if (namedImports && namedImports.elements.length > 1) {
				if (
					OrganizeImports.detectImportSpecifierSorting(
...
function doAddExistingFix(
	...
	if (namedImports.length) {
		...
		if (typeof preferences.organizeImportsIgnoreCase === "boolean") {
			...
			const targetImportSorting =
				OrganizeImports.detectImportSpecifierSorting(
					existingSpecifiers,
					preferences,
				);
			if (targetImportSorting !== SortKind.Both) {
				ignoreCaseForSorting =
					targetImportSorting === SortKind.CaseInsensitive;
			}
		}
		if (ignoreCaseForSorting === undefined) {
			ignoreCaseForSorting =
				OrganizeImports.detectSorting(sourceFile, preferences) ===
			...
			namedImports.map((namedImport) =>
				factory.createImportSpecifier(
					(!clause.isTypeOnly || promoteFromTypeOnly) &&
						shouldUseTypeOnly(namedImport, preferences),
					/*propertyName*/ undefined,
		...
		if (
			specifierSort &&
			!(ignoreCaseForSorting && specifierSort === SortKind.CaseSensitive)
		) {
			for (const spec of newSpecifiers) {
				...
				const insertionIndex =
					promoteFromTypeOnly && !spec.isTypeOnly
						? 0
						: OrganizeImports.getImportSpecifierInsertionIndex(
								existingSpecifiers,
								spec,
...
function shouldUseTypeOnly(
	info: { addAsTypeOnly: AddAsTypeOnly },
	preferences: UserPreferences,
): boolean {
	return (
		needsTypeOnly(info) ||
		(!!preferences.preferTypeOnlyAutoImports &&
			info.addAsTypeOnly !== AddAsTypeOnly.NotAllowed)
	);
}
