...
function getInfo(
	...
	let declaration = findAncestorMatchingSpan(
		related.file,
	...
	if (
		...
		isTypeParameterDeclaration(declaration.parent)
	) {
	...
	if (isTypeParameterDeclaration(declaration)) {
		...
		const constraint =
			tryGetConstraintType(checker, token) ||
...
function tryGetConstraintType(checker: TypeChecker, node: Node) {
	...
	const contextualType = isExpression(node)
