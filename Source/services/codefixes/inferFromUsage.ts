...
function inferTypeFromReferences(
				...
				for (const call of calls) {
					if (call.argumentTypes.length <= parameterIndex) {
						...
						for (
							...
							i < call.argumentTypes.length;
							i++
						) {
							types.push(
								checker.getBaseTypeOfLiteralType(
									call.argumentTypes[i],
								),
							);
						}
					} else {
		...
		switch (parent.operatorToken.kind) {
			...
			case SyntaxKind.QuestionQuestionToken:
				if (
					...
					addCandidateType(
						usage,
						checker.getTypeAtLocation(parent.right),
					);
				}
				break;
	...
	function inferTypeFromPropertyElementExpression(
		...
		if (node === parent.argumentExpression) {
			...
			const indexType = checker.getTypeAtLocation(
	...
	function inferTypeFromPropertyAssignment(
		...
		addCandidateThisType(
			usage,
			checker.getTypeAtLocation(nodeWithRealType),
		);
	}
		...
		for (const i of inferences) {
			for (const { high, low } of priorities) {
				if (high(i)) {
					Debug.assert(
						!low(i),
						"Priority can't have both low and high",
					);
					toRemove.push(low);
				}
			}
	...
	function combineTypes(inferences: readonly Type[]): Type {
		...
		const priorities: Priority[] = [
			{
				high: (t) =>
					t === checker.getStringType() ||
					t === checker.getNumberType(),
	...
	function combineAnonymousTypes(anons: AnonymousType[]) {
		...
		for (const anon of anons) {
			...
			calls.push(
				...checker.getSignaturesOfType(anon, SignatureKind.Call),
			);
			constructs.push(
	...
	function inferTypes(usage: Usage): Type[] {
		...
		if (callsType && candidateTypes) {
			// TODO: should this be `some(candidateTypes)`?
			types.push(
				checker.getUnionType(
	...
	function allPropertiesAreAssignableToUsage(type: Type, usage: Usage) {
		...
		return !forEachEntry(usage.properties, (propUsage, name) => {
			const source = checker.getTypeOfPropertyOfType(
				type,
				name as string,
			);
			if (!source) {
	...
	function inferTypeParameters(
		...
		if (genericType === typeParameter) {
			...
			const usageArgs = checker.getTypeArguments(
				usageType as TypeReference,
			);
			const types = [];
	...
	function inferFromSignatures(
		...
		for (let i = 0; i < genericSig.parameters.length; i++) {
			...
			const targetType =
				tryCast(usageParam, isTransientSymbol)?.links.type ||
	...
	function getSignatureFromCalls(calls: CallUsage[]): Signature {
		...
		for (let i = 0; i < length; i++) {
			...
			symbol.links.type = combineTypes(
				calls.map(
					(call) =>
						call.argumentTypes[i] || checker.getUndefinedType(),
				),
			);
