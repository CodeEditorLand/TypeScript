		...
		return createCombinedCodeActions(
			textChanges.ChangeTracker.with(context, (changes) => {
				eachDiagnostic(context, errorCodes, (diag) => {
					...
					if (
						...
						if (info.kind === InfoKind.TypeLikeDeclaration) {
							...
							if (
								!infos.some((i) => i.token.text === token.text)
							) {
								infos.push(info);
							}
						}
				...
				typeDeclToMembers.forEach((infos, declaration) => {
					const supers = isTypeLiteralNode(declaration)
						? undefined
						: getAllSupers(declaration, checker);
					for (const info of infos) {
						...
						if (call && !isPrivateIdentifier(token)) {
							...
							if (
								isJSFile &&
								!isInterfaceDeclaration(parentDeclaration) &&
...
function getInfo(
	...
	if (
		...
		const targetType =
			checker.getContextualType(token) ||
	...
	if (isIdentifier(token) && isJsxOpeningLikeElement(token.parent)) {
		...
		const attributes = getUnmatchedAttributes(
			checker,
			target,
			token.parent,
		);
	...
	if (isIdentifier(token)) {
		...
		if (type && getObjectFlags(type) & ObjectFlags.Anonymous) {
			const signature = firstOrUndefined(
	...
	const declaration =
		...
		(find(
			symbol.declarations,
			(d) => isInterfaceDeclaration(d) || isTypeLiteralNode(d),
		) as InterfaceDeclaration | TypeLiteralNode | undefined);
	if (
		declaration &&
		!isSourceFileFromLibrary(program, declaration.getSourceFile())
	) {
		const makeStatic =
			!isTypeLiteralNode(declaration) &&
			((leftExpressionType as TypeReference).target ||
				leftExpressionType) !== checker.getDeclaredTypeOfSymbol(symbol);
		if (
			makeStatic &&
			(isPrivateIdentifier(token) || isInterfaceDeclaration(declaration))
		)
			return undefined;

		const declSourceFile = declaration.getSourceFile();
		const modifierFlags = isTypeLiteralNode(declaration)
			? ModifierFlags.None
			: (makeStatic ? ModifierFlags.Static : ModifierFlags.None) |
				(startsWithUnderscore(token.text)
	...
	if (
		enumDeclaration &&
		!(leftExpressionType.flags & TypeFlags.EnumLike) &&
...
function addMissingMemberInJs(
	...
	if (makeStatic) {
		...
		if (lastProp) {
			...
			changeTracker.insertMemberAtStart(
				sourceFile,
				classDeclaration,
				property,
			);
...
function createAddIndexSignatureAction(
	...
	const changes = textChanges.ChangeTracker.with(context, (t) =>
		t.insertMemberAtStart(sourceFile, node, indexSignature),
	);
...
function addMethodDeclaration(
	...
	const containingMethodDeclaration = tryGetContainingMethodDeclaration(
		parentDeclaration,
		callExpression,
	);
...
function addEnumMemberDeclaration(
	...
	const enumMember = factory.createEnumMember(
		token,
		hasStringInitializer
			? factory.createStringLiteral(token.text)
			: undefined,
	);
...
function addFunctionDeclaration(
	...
	if (functionDeclaration === undefined) {
		Debug.fail(
			"fixMissingFunctionDeclaration codefix got unexpected error.",
		);
	}
...
function addJsxAttributes(
	...
	const options = {
		prefix:
			jsxAttributesNode.pos === jsxAttributesNode.end ? " " : undefined,
	};
	changes.replaceNode(
		context.sourceFile,
		jsxAttributesNode,
...
function tryGetValueFromType(
	...
	if (type.flags & TypeFlags.EnumLike) {
		const enumMember = type.symbol.exports
			? firstOrUndefinedIterator(type.symbol.exports.values())
			: type.symbol;
