...
registerCodeFix({
	...
	getCodeActions(context) {
		...
		const { name, declarations, newParameters, newOptionalParameters } =
			info;
		...
		if (length(newParameters)) {
			append(
				...
				createCodeFixAction(
					...
					[
						length(newParameters) > 1
							? Diagnostics.Add_missing_parameters_to_0
		...
		if (length(newOptionalParameters)) {
			append(
				...
				createCodeFixAction(
					...
					[
						length(newOptionalParameters) > 1
							? Diagnostics.Add_optional_parameters_to_0
	...
	getAllCodeActions: (context) =>
		codeFixAll(context, errorCodes, (changes, diag) => {
			...
			if (info) {
				const { declarations, newParameters, newOptionalParameters } =
					info;
...
function getInfo(
	...
	if (
		...
		length(callExpression.arguments) === 0
	) {
		return undefined;
	}
	...
	const convertibleSignatureDeclarations = filter(
		type.symbol.declarations,
		isConvertibleSignatureDeclaration,
	);
	if (convertibleSignatureDeclarations === undefined) {
		return undefined;
	}

	const nonOverloadDeclaration = lastOrUndefined(
		convertibleSignatureDeclarations,
	);
	if (
	...
	for (let i = 0, pos = 0, paramIndex = 0; i < argumentsLength; i++) {
		...
		const expr = isAccessExpression(arg)
			? getNameOfAccessExpression(arg)
			: arg;
		const type = checker.getWidenedType(
			checker.getBaseTypeOfLiteralType(checker.getTypeAtLocation(arg)),
		);
		const parameter =
			pos < parametersLength
				? nonOverloadDeclaration.parameters[pos]
				: undefined;
		if (
			parameter &&
		...
		if (length(declaration.parameters)) {
			changes.replaceNodeRangeWithNodes(
				...
				updateParameters(declaration, newParameters),
				{
					...
					indentation: 0,
					leadingTriviaOption:
						textChanges.LeadingTriviaOption.IncludeAll,
...
function isConvertibleSignatureDeclaration(
	node: Node,
): node is ConvertibleSignatureDeclaration {
	switch (node.kind) {
	...
	for (const declaration of declarations) {
		if (isOverload(declaration)) {
			if (
				...
				length(implementation.parameters)
			) {
...
function isOverload(declaration: ConvertibleSignatureDeclaration) {
	return (
		isConvertibleSignatureDeclaration(declaration) &&
...
function isOptionalPos(
	...
	return (
		...
		some(
			declarations,
			(d) =>
				pos < length(d.parameters) &&
				!!d.parameters[pos] &&
				d.parameters[pos].questionToken === undefined,
		)
	);
}
