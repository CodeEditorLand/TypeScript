...
const errorCodes = [Diagnostics.This_condition_will_always_return_0.code];
...
registerCodeFix({
	...
	getCodeActions(context) {
		...
		const changes = textChanges.ChangeTracker.with(context, (t) =>
			doChange(t, sourceFile, arg, expression),
		);
		return [
			createCodeFixAction(
				fixId,
	...
	getAllCodeActions: (context) => {
		return codeFixAll(context, errorCodes, (changes, diag) => {
			const info = getInfo(
				context.program,
				diag.file,
				createTextSpan(diag.start, diag.length),
			);
			if (info) {
...
function getInfo(
	...
	const related = find(
		...
		(related) => related.code === Diagnostics.Did_you_mean_0.code,
	);
	if (
		related === undefined ||
	...
	const token = findAncestorMatchingSpan(
		related.file,
		createTextSpan(related.start, related.length),
	);
