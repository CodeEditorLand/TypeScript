...
export function addNewNodeForMemberSymbol(
	...
	const effectiveModifierFlags = declaration
		? getEffectiveModifierFlags(declaration)
		: ModifierFlags.None;
	let modifierFlags = effectiveModifierFlags & ModifierFlags.Static;
	modifierFlags |=
		effectiveModifierFlags & ModifierFlags.Public
			? ModifierFlags.Public
			: effectiveModifierFlags & ModifierFlags.Protected
	...
	switch (kind) {
			...
			addClassElement(
				factory.createPropertyDeclaration(
					...
					declaration
						...
						: symbol.getName(),
					optional &&
						preserveOptional & PreserveOptionalFlags.Property
		...
		case SyntaxKind.MethodDeclaration:
			...
			Debug.assertIsDefined(declarations);
			const signatures = type.isUnion()
				? flatMap(type.types, (t) => t.getCallSignatures())
	...
	function createModifiers(): NodeArray<Modifier> | undefined {
		...
		if (shouldAddOverrideKeyword()) {
			modifiers = append(
				modifiers,
				factory.createToken(SyntaxKind.OverrideKeyword),
			);
		}
	...
	function shouldAddOverrideKeyword(): boolean {
		return !!(
			context.program.getCompilerOptions().noImplicitOverride &&
			declaration &&
	...
	function createBody(
		...
		return ambient
			? undefined
			: getSynthesizedDeepClone(block, /*includeTrivia*/ false) ||
	...
	function createDeclarationName(
		symbol: Symbol,
...
export function createSignatureDeclarationFromSignature(
	...
	if (importAdder) {
		...
		if (type) {
			const importableReference =
				tryGetAutoImportableReferenceFromTypeNode(type, scriptTarget);
			if (importableReference) {
				type = importableReference.typeNode;
				importSymbols(importAdder, importableReference.symbols);
			}
...
export function getArgumentTypesAndTypeParameters(
	...
	for (let i = 0; i < instanceTypes.length; i += 1) {
		...
		if (
			instanceType.isUnionOrIntersection() &&
			instanceType.types.some(typeContainsTypeParameter)
		) {
			const synthesizedTypeParameterName = createTypeParameterName(i);
			argumentTypeNodes.push(
				factory.createTypeReferenceNode(synthesizedTypeParameterName),
			);
		...
		const widenedInstanceType =
			checker.getBaseTypeOfLiteralType(instanceType);
...
export function tryGetAutoImportableReferenceFromTypeNode(
	...
	function visit(node: Node): Node {
		if (isLiteralImportTypeNode(node) && node.qualifier) {
			...
			const typeArguments = visitNodes(
				node.typeArguments,
				visit,
				isTypeNode,
			);
...
/** @internal */
export function findAncestorMatchingSpan(
	sourceFile: SourceFile,
	span: TextSpan,
): Node {
