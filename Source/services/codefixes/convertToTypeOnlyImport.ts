	...
	getCodeActions: function getCodeActionsToConvertToTypeOnlyImport(context) {
		...
		if (declaration) {
			const changes = textChanges.ChangeTracker.with(context, (t) =>
				doChange(t, context.sourceFile, declaration),
			);
			const importDeclarationChanges =
				declaration.kind === SyntaxKind.ImportSpecifier &&
				canConvertImportDeclarationForSpecifier(
					declaration,
					context.sourceFile,
					context.program,
				)
	...
	getAllCodeActions: function getAllCodeActionsToConvertToTypeOnlyImport(
		...
		return codeFixAll(context, errorCodes, (changes, diag) => {
			...
			if (
				...
				!fixedImportDeclarations.has(
					errorDeclaration.parent.parent.parent,
				) &&
				canConvertImportDeclarationForSpecifier(
					errorDeclaration,
					diag.file,
...
function getDeclaration(sourceFile: SourceFile, pos: number) {
	...
	return isImportSpecifier(parent) ||
		(isImportDeclaration(parent) && parent.importClause)
		? parent
		: undefined;
}
...
function canConvertImportDeclarationForSpecifier(
	...
	const nonTypeOnlySpecifiers = specifier.parent.elements.filter(
		(e) => !e.isTypeOnly,
	);
			...
			const newNamedBindings =
				importClause.namedBindings?.kind === SyntaxKind.NamedImports
					? factory.updateNamedImports(
							importClause.namedBindings,
							sameMap(importClause.namedBindings.elements, (e) =>
								factory.updateImportSpecifier(
									e,
									/*isTypeOnly*/ false,
									e.propertyName,
