...
function getInfo(
	...
	if (isIdentifier(token)) {
		...
		const moduleSpecifier = isStringLiteral(
...
function doChange(
	...
	program: Program,
	{ exportName, node, moduleSourceFile }: Info,
) {
	const exportDeclaration = tryGetExportDeclaration(
		moduleSourceFile,
		exportName.isTypeOnly,
	...
	if (exportDeclaration) {
		updateExport(changes, program, moduleSourceFile, exportDeclaration, [
			exportName,
		]);
...
function tryGetExportDeclaration(sourceFile: SourceFile, isTypeOnly: boolean) {
	const predicate = (node: Node): node is ExportDeclaration =>
		...
		((isTypeOnly && node.isTypeOnly) || !node.isTypeOnly);
...
function updateExport(
	...
	const allowTypeModifier =
		...
		!!(
			...
			find(namedExports, (e) => e.isTypeOnly)
		);
	changes.replaceNode(
		sourceFile,
		node,
...
function createExport(
	...
	changes.insertNodeAtEndOfScope(
		...
		factory.createExportDeclaration(
			...
			/*isTypeOnly*/ false,
			factory.createNamedExports(
				createExportSpecifiers(
					names,
					/*allowTypeModifier*/ getIsolatedModules(
...
function getNodeOfSymbol(symbol: Symbol) {
	...
	return variableStatement &&
		length(variableStatement.declarationList.declarations) === 1
		? variableStatement
		: declaration;
}
