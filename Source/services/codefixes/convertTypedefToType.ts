...
registerCodeFix({
	...
	getCodeActions(context) {
		...
		const changes = textChanges.ChangeTracker.with(context, (t) =>
			doChange(t, node, context.sourceFile, newLineCharacter),
		);
	...
	getAllCodeActions: (context) =>
		codeFixAll(context, errorCodes, (changes, diag) => {
			...
			const fixAll = true;
			if (node)
				doChange(changes, node, diag.file, newLineCharacter, fixAll);
		}),
});
...
function doChange(
	...
	if (!leftSibling && commentNode.comment) {
		pos = findEndOfTextBetween(
			commentNode,
			commentNode.getStart(),
			node.getStart(),
		);
...
function getLeftAndRightSiblings(typedefNode: JSDocTypedefTag): {
	leftSibling?: Node;
	rightSibling?: Node;
} {
	...
	const currentNodeIndex = commentNode
		.getChildren()
		.findIndex(
			(n) =>
				n.getStart() === typedefNode.getStart() &&
				n.getEnd() === typedefNode.getEnd(),
		);

	const leftSibling =
		currentNodeIndex > 0
			? commentNode.getChildAt(currentNodeIndex - 1)
			: undefined;
	const rightSibling =
		currentNodeIndex < maxChildIndex
			? commentNode.getChildAt(currentNodeIndex + 1)
			: undefined;
...
function findEndOfTextBetween(
	...
	const comment = jsDocComment
		.getText()
		.substring(
			from - jsDocComment.getStart(),
			to - jsDocComment.getStart(),
		);
...
function createSignatureFromTypeLiteral(
	typeLiteral: JSDocTypeLiteral,
...
function getPropertyName(tag: JSDocPropertyLikeTag): string | undefined {
	return tag.name.kind === SyntaxKind.Identifier
		? tag.name.text
		: tag.name.right.text;
}
...
export function getJSDocTypedefNodes(node: Node): readonly JSDocTag[] {
	if (hasJSDocNodes(node)) {
		return flatMap(node.jsDoc, (doc) =>
			doc.tags?.filter((tag) => isJSDocTypedefTag(tag)),
		);
	}
