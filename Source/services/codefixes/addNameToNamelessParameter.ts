...
function makeChange(
	...
	let typeNode: TypeNode = factory.createTypeReferenceNode(
	...
	const replacement = factory.createParameterDeclaration(
		...
		"arg" + i,
		param.questionToken,
		param.dotDotDotToken && !isArrayTypeNode(typeNode)
	...
	changeTracker.replaceRange(
		sourceFile,
		createRange(param.getStart(sourceFile), end),
		replacement,
	);
}
...
function tryGetNextParam(sourceFile: SourceFile, param: ParameterDeclaration) {
	...
	if (
		...
		isArrayBindingPattern(nextToken.parent) &&
		isParameter(nextToken.parent.parent)
	) {
