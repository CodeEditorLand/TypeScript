...
function doChange(
	...
	if (
		isFunctionDeclaration(ctorDeclaration) ||
		isFunctionExpression(ctorDeclaration)
	) {
		changes.replaceNode(
			sourceFile,
			ctorDeclaration,
			createClassFromFunction(ctorDeclaration),
		);
			...
			symbol.members.forEach((member, key) => {
				if (key === "constructor" && member.valueDeclaration) {
					...
					if (
						...
						changes.delete(
							sourceFile,
							member.valueDeclaration.parent,
						);
					}
					return;
				}
		...
		function createClassElement(
			...
			function createFunctionLikeExpressionMember(
				...
				if (isFunctionExpression(expression))
					return createFunctionExpressionMember(
						members,
						expression,
						name,
					);
	...
	function createClassFromFunction(
...
function getModifierKindFromSource(
	...
	return canHaveModifiers(source)
		? filter(
				source.modifiers,
				(modifier): modifier is Modifier => modifier.kind === kind,
			)
		: undefined;
}
