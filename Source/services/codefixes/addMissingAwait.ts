		...
		return createCodeFixActionWithoutFixAll(
			...
			awaitableInitializers.initializers.length === 1
				? [
						...
						awaitableInitializers.initializers[0].declarationSymbol
							.name,
					]
	...
	return some(
		...
		({ start, length, relatedInformation, code }) =>
			...
			some(
				...
				(related) =>
					related.code ===
					Diagnostics.Did_you_forget_to_use_await.code,
			),
	);
}
...
function makeChange(
	...
	if (
		isForOfStatement(insertionSite.parent) &&
		!insertionSite.parent.awaitModifier
	) {
	...
	if (isBinaryExpression(insertionSite)) {
		...
		if (
			fixedDeclarations &&
			isIdentifier(insertionSite.parent.expression)
		) {
		...
		if (
			...
			const symbol = checker.getSymbolAtLocation(
				insertionSite.parent.name,
			);
			if (
				symbol &&
				!tryAddToSet(fixedDeclarations, getSymbolId(symbol))
			) {
				return;
			}
		}
