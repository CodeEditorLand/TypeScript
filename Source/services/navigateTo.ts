...
export function getNavigateToItems(
	...
	for (const sourceFile of sourceFiles) {
		...
		if (
			shouldExcludeFile(sourceFile, !!excludeLibFiles, singleCurrentFile)
		) {
			continue;
		}
...
function shouldExcludeFile(
	...
	return (
		file !== singleCurrentFile &&
		excludeLibFiles &&
		(isInsideNodeModules(file.path) || file.hasNoDefaultLib)
	);
}
...
function shouldKeepItem(
	...
	switch (declaration.kind) {
		...
		case SyntaxKind.ImportEqualsDeclaration:
			...
			return (
				importer.escapedName !== imported.escapedName &&
