		...
		case SyntaxKind.SwitchStatement:
			return {
				start: find(
					...
					(node) => node.kind === SyntaxKind.SwitchKeyword,
				)!,
...
export function findReferencedSymbols(
	...
	const symbol = isDefinitionForReference(adjustedNode)
		? checker.getSymbolAtLocation(adjustedNode)
		: undefined;
...
function getImplementationReferenceEntries(
	...
	if (node.parent.kind === SyntaxKind.ShorthandPropertyAssignment) {
		...
		return getReferenceEntriesForNode(
			...
			sourceFiles,
			cancellationToken,
			{ implementations: true, use: FindReferencesUse.References },
		);
	}
}
		...
		switch (def.type) {
			...
			case DefinitionKind.Label: {
				...
				return {
					...
					displayParts: [
						displayPart(node.text, SymbolDisplayPartKind.text),
					],
				};
			}
			case DefinitionKind.Keyword: {
				...
				return {
					...
					displayParts: [
						{ text: name, kind: ScriptElementKind.keyword },
					],
				};
			}
...
function toReferencedSymbolEntry(
	...
	return {
		...
		isDefinition:
			...
			isDeclarationOfSymbol(entry.node, symbol),
	};
}
 ...
 * @internal
 */
export function isDeclarationOfSymbol(
	node: Node,
...
function declarationIsWriteAccess(decl: Declaration): boolean {
	...
	switch (decl.kind) {
		...
		case SyntaxKind.PropertyDeclaration:
			return (
				!!(decl as VariableDeclaration | PropertyDeclaration)
					.initializer || isCatchClause(decl.parent)
			);
...
export namespace Core {
			...
			if (!options.implementations && isStringLiteralLike(node)) {
				if (isModuleSpecifierLike(node)) {
					...
					const referencedFileName =
						program.getResolvedModuleFromModuleSpecifier(node)
			...
			for (const entry of references) {
				...
				result[refIndex] = {
					...
					references: reference.references
						...
						.sort((entry1, entry2) => {
							const entry1File = getSourceFileIndexOfEntry(
								program,
								entry1,
							);
							const entry2File = getSourceFileIndexOfEntry(
								program,
								entry2,
							);
	...
	function getReferencedSymbolsForModule(
			...
			(reference) => {
				if (reference.kind === "import") {
					...
					const range =
						...
						reference.referencingFile.statements[0] ||
						reference.referencingFile;
				...
				switch (decl.kind) {
					...
					case SyntaxKind.ModuleDeclaration:
						if (sourceFilesSet.has(decl.getSourceFile().fileName)) {
							references.push(
								nodeEntry((decl as ModuleDeclaration).name),
							);
						}
						break;
					default:
			...
			for (const decl of exported.declarations) {
				...
				if (sourceFilesSet.has(sourceFile.fileName)) {
					...
					const node =
						...
						isPropertyAccessExpression(decl.left)
		...
		return references.length
			? [
					{
						definition: { type: DefinitionKind.Symbol, symbol },
						references,
					},
				]
			: emptyArray;
	...
	function getReferencedSymbolsSpecial(
		...
		if (isImportMeta(node.parent) && node.parent.name === node) {
			return getAllReferencesForImportMeta(
				sourceFiles,
				cancellationToken,
			);
		}
	...
	function skipPastExportOrImportSpecifierOrUnion(
		...
		return firstDefined(symbol.declarations, (decl) => {
			if (!decl.parent) {
				...
				Debug.fail(
					`Unexpected symbol at ${Debug.formatSyntaxKind(node.kind)}: ${Debug.formatSymbol(symbol)}`,
				);
			}
	...
	export function eachExportReference(
		...
		const { importSearches, indirectUsers, singleReferences } =
			importTracker(
				exportSymbol,
				{
		...
		for (const singleReference of singleReferences) {
			if (
				isIdentifier(singleReference) &&
				isImportTypeNode(singleReference.parent)
			) {
				cb(singleReference);
			}
		}
	...
	export function eachSymbolReferenceInFile<T>(
		...
		for (const token of getPossibleSymbolReferenceNodes(
			...
			if (
				...
				checker.getShorthandAssignmentValueSymbol(token.parent) ===
					symbol ||
	...
	export function getTopMostDeclarationNamesInFile(
		...
		return candidates.reduce(
			...
			{ depth: Infinity, declarationNames: [] as Node[] },
		).declarationNames;
		...
		for (const sourceFile of sourceFiles) {
			for (const name of getPossibleSymbolReferenceNodes(
				...
				if (
					referenceSymbol &&
					checker
						.getRootSymbols(referenceSymbol)
						.some((s) => s === symbol)
				) {
					if (cb(name, call)) {
						return true;
	...
	function getPossibleSymbolReferenceNodes(
		...
		return mapDefined(
			...
			(pos) => {
				const referenceLocation = getTouchingPropertyName(
					sourceFile,
					pos,
				);
				return referenceLocation === sourceFile
					? undefined
					: referenceLocation;
			},
		);
		...
		switch (node.kind) {
			...
			case SyntaxKind.NumericLiteral:
				return (
					...
					(node as NumericLiteral).text.length ===
						searchSymbolName.length
				);
	...
	function getReferencesAtLocation(
		...
		if (
			isJSDocPropertyLikeTag(parent) &&
			parent.isNameFirst &&
			parent.typeExpression &&
			isJSDocTypeLiteral(parent.typeExpression.type) &&
			parent.typeExpression.type.jsDocPropertyTags &&
			length(parent.typeExpression.type.jsDocPropertyTags)
		) {
	...
	function getReferencesAtJSDocTypeLiteral(
	...
	function getReferencesAtExportSpecifier(
		...
		if (!propertyName) {
			...
			if (
				...
				addReference(
					name,
					Debug.checkDefined(exportSpecifier.symbol),
					state,
				);
			}
		} else {
		...
		if (
			...
			const isDefaultExport =
				referenceLocation.escapedText === "default" ||
		...
		if (
			...
			const imported =
				state.checker.getExportSpecifierLocalTargetSymbol(
					exportSpecifier,
				);
	...
	function addReference(
		...
		const { kind, symbol } =
			"kind" in relatedSymbol
				? relatedSymbol
				: { kind: undefined, symbol: relatedSymbol }; // eslint-disable-line local/no-in-operator
	...
	function findOwnConstructorReferences(
		...
		if (constructorSymbol && constructorSymbol.declarations) {
			for (const decl of constructorSymbol.declarations) {
				...
				Debug.assert(
					decl.kind === SyntaxKind.Constructor && !!ctrKeyword,
				);
				addNode(ctrKeyword);
			}
		}
							...
							return (
								...
								(
									searchSpaceNode as
										| ClassLikeDeclaration
										| ObjectLiteralExpression
								).symbol === container.parent.symbol &&
								isStatic(container) === !!staticFlag
							);
						case SyntaxKind.SourceFile:
							return (
								container.kind === SyntaxKind.SourceFile &&
								!isExternalModule(container) &&
					...
					if (isStringLiteralLike(ref) && ref.text === node.text) {
						if (type) {
							...
							if (
								...
								(type === refType ||
									isStringLiteralPropertyReference(
										ref,
										checker,
									))
							) {
	...
	function isStringLiteralPropertyReference(
		node: StringLiteralLike,
		checker: TypeChecker,
	) {
	...
	function forEachRelatedSymbol<T>(
		...
		if (containingObjectLiteralElement) {
			 ...
			 */
			const shorthandValueSymbol =
				checker.getShorthandAssignmentValueSymbol(location.parent); // gets the local symbol
			...
			const res =
				...
				firstDefined(
					...
					(sym) =>
						fromRoot(sym, EntryKind.SearchedPropertyFoundLocal),
				);
			if (res) return res;
		...
		function getPropertySymbolOfObjectBindingPatternWithoutPropertyName(
			...
			if (
				...
				return getPropertySymbolFromBindingElement(
					checker,
					bindingElement,
				);
			}
				...
				firstDefined(
					...
					(typeReference) => {
						...
						return (
							...
							propertySymbol &&
							(firstDefined(
								checker.getRootSymbols(propertySymbol),
								cb,
							) ||
								recur(type.symbol))
						);
	...
	function isStaticSymbol(symbol: Symbol): boolean {
		...
		const modifierFlags = getEffectiveModifierFlags(
			symbol.valueDeclaration,
		);
	...
	function getRelatedSymbol(
		...
		return forEachRelatedSymbol(
			...
			/*onlyIncludeBindingElementAtReferenceLocation*/ state.options
				.use !== FindReferencesUse.Rename ||
				!!state.options.providePrefixAndSuffixTextForRename,
			(sym, rootSymbol, baseSymbol, kind): RelatedSymbol | undefined => {
				...
				if (baseSymbol) {
					...
					if (
						isStaticSymbol(referenceSymbol) !==
					...
					? // For a base type, use the symbol for the derived type. For a synthetic (e.g. union) property, use the union symbol.
						{
							symbol:
								rootSymbol &&
								!(getCheckFlags(sym) & CheckFlags.Synthetic)
									? rootSymbol
									: sym,
							kind,
						}
