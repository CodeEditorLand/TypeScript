...
export function getDefinitionAtPosition(
	...
	if (
		...
		(isIdentifier(node) &&
			isJSDocOverrideTag(parent) &&
			parent.tagName === node)
	) {
		return (
			getDefinitionFromOverriddenMember(typeChecker, node) || emptyArray
		);
	}
	...
	switch (node.kind) {
		...
		case SyntaxKind.CaseKeyword:
			const switchStatement = findAncestor(
				node.parent,
				isSwitchStatement,
			);
			if (switchStatement) {
				return [
					createDefinitionInfoFromSwitch(switchStatement, sourceFile),
				];
			}
			break;
	}
	...
	if (node.kind === SyntaxKind.AwaitKeyword) {
		const functionDeclaration = findAncestor(node, (n) =>
			isFunctionLikeDeclaration(n),
		) as FunctionLikeDeclaration | undefined;
	...
	if (node.kind === SyntaxKind.YieldKeyword) {
		...
		const isGeneratorFunction =
			functionDeclaration && functionDeclaration.asteriskToken;
	...
	if (searchOtherFilesOnly && failedAliasResolution) {
		...
		const moduleSpecifier =
			importDeclaration &&
			tryGetModuleSpecifierFromDeclaration(importDeclaration);
		if (moduleSpecifier) {
			({ symbol, failedAliasResolution } = getSymbol(
				moduleSpecifier,
				typeChecker,
				stopAtAlias,
			));
	...
	if (
		...
		every(
			symbol.declarations,
			(d) => d.getSourceFile().fileName === sourceFile.fileName,
		)
	)
		return undefined;
	...
	if (
		...
		return name === undefined
			...
			: flatMap(type.isUnion() ? type.types : [type], (t) => {
					...
					return (
						prop && getDefinitionFromSymbol(typeChecker, prop, node)
					);
				});
	}
	...
	return concatenate(
		fileReferenceDefinition,
		objectLiteralElementDefinition.length
...
function getDefinitionFromOverriddenMember(
	...
	const base = isClassExpression(expression)
		? expression.symbol
	...
	if (typeReferenceDirective) {
		const reference = program
			...
			.get(
				...
				typeReferenceDirective.resolutionMode ||
...
function shouldUnwrapFirstTypeArgumentTypeDefinitionFromAlias(
	typeChecker: TypeChecker,
	type: Type,
): boolean {
	if (!type.aliasSymbol) {
...
function getFirstTypeArgumentDefinitions(
	...
	if (
		...
		shouldUnwrapFirstTypeArgumentTypeDefinitionFromTypeReference(
			typeChecker,
	...
	if (
		shouldUnwrapFirstTypeArgumentTypeDefinitionFromAlias(
			typeChecker,
			type,
		) &&
...
export function getTypeDefinitionAtPosition(
	...
	const [resolvedType, typeDefinitions] =
	...
	return typeDefinitions.length
		? [
				...getFirstTypeArgumentDefinitions(
					typeChecker,
					resolvedType,
					node,
					failedAliasResolution,
...
function isExpandoDeclaration(node: Declaration): boolean {
	...
	return (
		...
		getAssignmentDeclarationKind(containingAssignment) ===
...
function createDefinitionInfoFromSwitch(
