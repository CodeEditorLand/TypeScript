		...
		function handleToken(): void {
			switch (token) {
				...
				case SyntaxKind.CloseBraceToken:
					...
					// then we aren't trying to keep track of a previously scanned template head.
					if (templateStack.length > 0) {
						const lastTemplateStackToken =
							lastOrUndefined(templateStack);

						if (
							lastTemplateStackToken === SyntaxKind.TemplateHead
						) {
...
function hasValueSideModule(symbol: Symbol): boolean {
	return some(
		...
		(declaration) =>
			...
			getModuleInstanceState(declaration) ===
			...
			for (const tag of docComment.tags) {
				...
				switch (tag.kind) {
					...
					case SyntaxKind.JSDocPropertyTag:
						...
						commentStart =
							(prop.isNameFirst && prop.typeExpression?.end) ||
							prop.name.end;
						break;
					case SyntaxKind.JSDocTemplateTag:
						processJSDocTemplateTag(tag as JSDocTemplateTag);
						pos = tag.end;
						commentStart = (tag as JSDocTemplateTag).typeParameters
							.end;
						break;
					case SyntaxKind.JSDocTypedefTag:
						const type = tag as JSDocTypedefTag;
						commentStart =
							(type.typeExpression?.kind ===
					...
					case SyntaxKind.JSDocCallbackTag:
						commentStart = (tag as JSDocCallbackTag).typeExpression
							.end;
						break;
					...
					case SyntaxKind.JSDocEnumTag:
						commentStart = (tag as JSDocThisTag | JSDocEnumTag)
							.typeExpression.end;
						break;
					case SyntaxKind.JSDocReturnTag:
						...
						pos = tag.end;
						commentStart =
							(tag as JSDocReturnTag).typeExpression?.end ||
							commentStart;
						break;
					...
					case SyntaxKind.JSDocImplementsTag:
						commentStart = (
							tag as JSDocImplementsTag | JSDocAugmentsTag
						).class.end;
						break;
					case SyntaxKind.JSDocThrowsTag:
						...
						pos = tag.end;
						commentStart =
							(tag as JSDocThrowsTag).typeExpression?.end ||
							commentStart;
						break;
				}
			...
			if (token) {
				switch (token.parent.kind) {
					...
					case SyntaxKind.TypeParameter:
						if (
							(token.parent as TypeParameterDeclaration).name ===
							token
						) {
					...
					case SyntaxKind.InterfaceDeclaration:
						if (
							(token.parent as InterfaceDeclaration).name ===
							token
						) {
					...
					case SyntaxKind.ModuleDeclaration:
						if (
							(token.parent as ModuleDeclaration).name === token
						) {
					...
					case SyntaxKind.Parameter:
						if (
							(token.parent as ParameterDeclaration).name ===
							token
						) {
