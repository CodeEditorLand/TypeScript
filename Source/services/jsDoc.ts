		...
		for (const jsdoc of getCommentHavingNodes(declaration)) {
			const inheritDoc =
				...
				find(
					jsdoc.tags,
					(t) =>
						t.kind === SyntaxKind.JSDocTag &&
						(t.tagName.escapedText === "inheritDoc" ||
							t.tagName.escapedText === "inheritdoc"),
				);
			...
			let newparts = jsdoc.comment
				? getDisplayPartsFromComment(jsdoc.comment, checker)
				: [];
			if (inheritDoc && inheritDoc.comment) {
				newparts = newparts.concat(
					getDisplayPartsFromComment(inheritDoc.comment, checker),
				);
			}
...
function getJSDocPropertyTagsInfo(
	...
	return flatMap(nodes, (propTag) =>
		concatenate(
			[
				{
					name: propTag.tagName.text,
					text: getCommentDisplayParts(propTag, checker),
				},
			],
...
function tryGetJSDocPropertyTags(node: JSDocTag) {
	return isJSDocPropertyLikeTag(node) &&
		node.isNameFirst &&
		node.typeExpression &&
		isJSDocTypeLiteral(node.typeExpression.type)
		? node.typeExpression.type.jsDocPropertyTags
		: undefined;
}
...
function getCommentDisplayParts(
	...
	switch (kind) {
			...
			if (length(templateTag.typeParameters)) {
				...
				const lastTypeParameter =
					templateTag.typeParameters[
						templateTag.typeParameters.length - 1
					];
		...
		case SyntaxKind.JSDocSatisfiesTag:
			return withNode(
				(tag as JSDocTypeTag | JSDocSatisfiesTag).typeExpression,
			);
		case SyntaxKind.JSDocTypedefTag:
		case SyntaxKind.JSDocCallbackTag:
		case SyntaxKind.JSDocPropertyTag:
...
export function getDocCommentTemplateAtPosition(
	...
	const commentOwnerJsDoc =
		hasJSDocNodes(commentOwner) && commentOwner.jsDoc
			? commentOwner.jsDoc
			: undefined;
	const lastJsDoc = lastOrUndefined(commentOwnerJsDoc);
	if (
		commentOwner.getStart(sourceFile) < position ||
		(lastJsDoc && existingDocComment && lastJsDoc !== existingDocComment)
	) {
		return undefined;
	}
	...
	switch (commentOwner.kind) {
		...
		case SyntaxKind.PropertySignature: {
			...
			return host.type && isFunctionTypeNode(host.type)
				? {
						commentOwner,
						parameters: host.type.parameters,
						hasReturn: hasReturn(host.type, options),
					}
				: { commentOwner };
		}
		...
		case SyntaxKind.BinaryExpression: {
			...
			if (
				getAssignmentDeclarationKind(be) ===
