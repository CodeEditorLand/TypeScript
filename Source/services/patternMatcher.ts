...
export function createPatternMatcher(
	...
	if (
		dotSeparatedSegments.length === 1 &&
		dotSeparatedSegments[0].totalTextChunk.text === ""
	) {
		return {
			getMatchForLastSegmentOfPattern: () =>
				createPatternMatch(
	...
	if (
		dotSeparatedSegments.some(
			(segment) => !segment.subWordTextChunks.length,
		)
	)
		return undefined;
...
function matchTextChunk(
	...
	if (chunk.isLowerCase) {
		...
		for (const span of wordSpans) {
			if (
				partStartsWith(candidate, span, chunk.text, /*ignoreCase*/ true)
			) {
				return createPatternMatch(
					PatternMatchKind.substring,
...
function matchSegment(
	...
	if (
		every(
			...
			(ch) =>
				ch !== CharacterCodes.space && ch !== CharacterCodes.asterisk,
		)
	) {
		...
		for (
			...
			if (gotOneMatchThisCandidate) {
				...
				if (
					...
					!isUpperCaseLetter(
						chunk.text.charCodeAt(
							chunkCharacterSpans[currentChunkSpan].start,
						),
					)
				) {
					break;
			...
			firstMatch =
				firstMatch === undefined ? currentCandidate : firstMatch;
	...
	for (let start = 0; start <= n; start++) {
		if (
			every(
				value,
				(valueChar, i) =>
					toLowerCase(str.charCodeAt(i + start)) === valueChar,
			)
		) {
			return start;
		}
...
function breakPatternIntoTextChunks(pattern: string): TextChunk[] {
	...
	for (let i = 0; i < pattern.length; i++) {
		...
		if (isWordChar(ch)) {
			...
			if (wordLength > 0) {
				result.push(
					createTextChunk(pattern.substr(wordStart, wordLength)),
				);
				wordLength = 0;
			}
		}
