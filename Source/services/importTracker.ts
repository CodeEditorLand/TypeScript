			...
			for (const direct of theseDirectImports) {
				...
				switch (direct.kind) {
					...
					case SyntaxKind.ImportDeclaration:
						...
						const namedBindings =
							direct.importClause &&
							direct.importClause.namedBindings;
						if (
							namedBindings &&
					...
					case SyntaxKind.ExportDeclaration:
						if (!direct.exportClause) {
							...
							handleDirectImports(
								getContainingModuleSymbol(direct, checker),
							);
						} else if (
							direct.exportClause.kind ===
	...
	function isExported(node: Node, stopAtAmbientModule = false) {
		return findAncestor(node, (node) => {
			if (stopAtAmbientModule && isAmbientModuleDeclaration(node))
				return "quit";
			return (
				canHaveModifiers(node) && some(node.modifiers, isExportModifier)
			);
		});
	}
...
function getSearchesFromDirectImports(
	...
	function searchForNamedImport(
		...
		for (const element of namedBindings.elements) {
			...
			if (propertyName) {
				...
				if (
					!isForRename ||
					name.escapedText === exportSymbol.escapedName
				) {
				...
				const localSymbol =
					element.kind === SyntaxKind.ExportSpecifier &&
	...
	function isNameMatch(name: __String): boolean {
		...
		return (
			...
			(exportKind !== ExportKind.Named &&
				name === InternalSymbolName.Default)
		);
	}
}
			...
			switch (statement.kind) {
				...
				case SyntaxKind.ImportDeclaration: {
					...
					if (
						...
						isStringLiteral(decl.moduleSpecifier)
					) {
...
export function getImportOrExportSymbol(
		...
		if (symbol.exportSymbol) {
			...
			if (
				...
				if (
					...
					const lhsSymbol = checker.getSymbolAtLocation(
						exportNode.name,
					)!;
					...
					return exportInfo(
						symbol,
						getExportKindForDeclaration(exportNode),
					);
				}
		...
		function getSpecialPropertyExport(
			...
			const sym = useLhsSymbol
				? checker.getSymbolAtLocation(
						getNameOfAccessExpression(
							cast(node.left, isAccessExpression),
						),
					)
				: symbol;
	...
	function getImport(): ImportedSymbol | undefined {
		...
		if (importedSymbol.escapedName === "export=") {
			importedSymbol = getExportEqualsLocalSymbol(
				importedSymbol,
				checker,
			);
...
function isNodeImport(node: Node): boolean {
	...
	switch (parent.kind) {
		...
		case SyntaxKind.NamespaceImport:
			Debug.assert(
				(parent as ImportClause | NamespaceImport).name === node,
			);
			return true;
	...
	if (symbol.declarations) {
		for (const declaration of symbol.declarations) {
			if (
				...
				return checker.getExportSpecifierLocalTargetSymbol(
					declaration.name,
				)!;
			}
		}
