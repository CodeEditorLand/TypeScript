...
export function spanInSourceFileAtLocation(
	...
	if (
		...
		if (
			!preceding ||
			sourceFile.getLineAndCharacterOfPosition(preceding.getEnd())
				.line !== lineOfPosition
		) {
			return undefined;
		}
	...
	function textSpan(startNode: Node, endNode?: Node) {
		const lastDecorator = canHaveDecorators(startNode)
			? findLast(startNode.modifiers, isDecorator)
			: undefined;
	...
	function spanInNode(node: Node | undefined): TextSpan | undefined {
		if (node) {
			...
			switch (node.kind) {
				...
				case SyntaxKind.DefaultClause:
					...
					return spanInNode(
						(node as CaseOrDefaultClause).statements[0],
					);
				...
				case SyntaxKind.Decorator:
					return spanInNodeArray(
						(parent as HasDecorators).modifiers,
						node,
						isDecorator,
					);
				...
				default:
					...
					// [a, b, c] = expression
					if (
						isArrayLiteralOrObjectLiteralDestructuringPattern(node)
					) {
					...
					switch (node.parent.kind) {
						case SyntaxKind.PropertyAssignment:
							...
							if (
								(node.parent as PropertyAssignment).name ===
									node &&
								!isArrayLiteralOrObjectLiteralDestructuringPattern(
									node.parent.parent,
								)
								...
								return spanInNode(
									(node.parent as PropertyAssignment)
										.initializer,
								);
							}
							break;
						case SyntaxKind.TypeAssertionExpression:
							...
							if ((node.parent as TypeAssertion).type === node) {
								return spanInNextNode(
									(node.parent as TypeAssertion).type,
								);
							}
							break;
						case SyntaxKind.VariableDeclaration:
		...
		function textSpanFromVariableDeclaration(
			...
			if (
				...
				variableDeclaration.parent.declarations[0] ===
					variableDeclaration
			) {
		...
		function spanInVariableDeclaration(
			...
			if (
				variableDeclaration.parent.parent.kind ===
			...
			if (
				(hasOnlyExpressionInitializer(variableDeclaration) &&
		...
		function spanInBlock(block: Block): TextSpan | undefined {
			switch (block.parent.kind) {
				case SyntaxKind.ModuleDeclaration:
					if (
						getModuleInstanceState(
							block.parent as ModuleDeclaration,
						) !== ModuleInstanceState.Instantiated
					) {
						return undefined;
					}
		...
		function spanInCloseBraceToken(node: Node): TextSpan | undefined {
			switch (node.parent.kind) {
				...
				case SyntaxKind.CaseBlock:
					...
					if (lastClause) {
						return spanInNode(
							lastOrUndefined(lastClause.statements),
						);
					}
					return undefined;
				...
				case SyntaxKind.ObjectBindingPattern:
					...
					return spanInNode(
						lastOrUndefined(bindingPattern.elements) ||
							bindingPattern,
					);
				...
				default:
					if (
						...
						// Breakpoint in last binding element or binding pattern if it contains no elements
						const objectLiteral =
							node.parent as ObjectLiteralExpression;
						return textSpan(
							lastOrUndefined(objectLiteral.properties) ||
								objectLiteral,
						);
					}
					return spanInNode(node.parent);
			...
			switch (node.parent.kind) {
				...
				default:
					if (
						...
						// Breakpoint in last binding element or binding pattern if it contains no elements
						const arrayLiteral =
							node.parent as ArrayLiteralExpression;
						return textSpan(
							lastOrUndefined(arrayLiteral.elements) ||
								arrayLiteral,
						);
					}
