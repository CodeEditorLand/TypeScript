...
function createNameAndKindSet(): NameAndKindSet {
	...
	function add(value: NameAndKind) {
		...
		if (
			!existing ||
			kindPrecedence[existing.kind] < kindPrecedence[value.kind]
		) {
...
function convertStringLiteralCompletions(
	...
	switch (completion.kind) {
		...
		case StringLiteralCompletionKind.Types: {
			const quoteChar =
				contextToken.kind === SyntaxKind.NoSubstitutionTemplateLiteral
			...
			const entries = completion.types.map((type) => ({
				...
				replacementSpan:
					getReplacementSpanForContextToken(contextToken),
			}));
			return {
				isGlobalCompletion: false,
...
function kindModifiersFromExtension(
	...
	switch (extension) {
		...
		case Extension.TsBuildInfo:
			return Debug.fail(
				`Extension ${Extension.TsBuildInfo} is unsupported.`,
			);
		case undefined:
...
function getStringLiteralCompletionEntries(
	...
	switch (parent.kind) {
		...
		case SyntaxKind.PropertyAssignment:
			...
			return (
				fromContextualType() || fromContextualType(ContextFlags.None)
			);
		...
		case SyntaxKind.JsxAttribute:
			if (!isRequireCallArgument(node) && !isImportCall(parent)) {
				const argumentInfo =
					SignatureHelp.getArgumentInfoForCompletions(
		...
		case SyntaxKind.CaseClause:
			const tracker = newCaseClauseTracker(
				typeChecker,
				(parent as CaseClause).parent.clauses,
			);
			...
			const literals = contextualTypes.types.filter(
				(literal) => !tracker.hasValue(literal.value),
			);
			return {
	...
	function fromUnionableLiteralType(
		...
		switch (grandParent.kind) {
			...
			case SyntaxKind.UnionType: {
				const result = fromUnionableLiteralType(
				...
				const alreadyUsedTypes =
					getAlreadyUsedTypesInStringLiteralUnion(
				...
				if (result.kind === StringLiteralCompletionKind.Properties) {
					return {
						...
						symbols: result.symbols.filter(
							(sym) => !contains(alreadyUsedTypes, sym.name),
						),
				...
				return {
					...
					types: result.types.filter(
						(t) => !contains(alreadyUsedTypes, t.value),
					),
					isNewIdentifier: false,
				};
			}
...
function getStringLiteralCompletionsFromSignature(
	...
	const editingArgument = isJsxOpeningLikeElement(call)
		? Debug.checkDefined(findAncestor(arg.parent, isJsxAttribute))
		: arg;
	const candidates =
	...
	const types = flatMap(candidates, (candidate) => {
		if (
			...
			return;
		let type = candidate.getTypeParameterAtPosition(
		...
		if (isJsxOpeningLikeElement(call)) {
			const propType = checker.getTypeOfPropertyOfType(
				type,
				getTextOfJsxAttributeName(
					(editingArgument as JsxAttribute).name,
				),
			);
			if (propType) {
				type = propType;
	...
	return length(types)
		? { kind: StringLiteralCompletionKind.Types, types, isNewIdentifier }
		: undefined;
}
...
function addReplacementSpans(
	...
	return names.map(
		({ name, kind, extension }): PathCompletion =>
			name.includes(directorySeparator) ||
			name.includes(altDirectorySeparator)
...
function getSupportedExtensionsForModuleResolution(
	...
	const ambientModulesExtensions = !typeChecker
		? []
		: mapDefined(typeChecker.getAmbientModules(), (module) => {
...
function getBaseDirectoriesFromRootDirs(
	...
	return deduplicate<string>(
		[
			...
			scriptDirectory,
		].map((baseDir) => removeTrailingDirectorySeparator(baseDir)),
		equateStringsCaseSensitive,
		compareStringsCaseSensitive,
	);
}
...
function getCompletionEntriesForDirectoryFragment(
	...
	if (!moduleSpecifierIsRelative) {
		...
		if (packageJsonPath) {
			...
			if (typeof typesVersions === "object") {
				const versionPaths =
					getPackageJsonTypesVersionsPaths(typesVersions)?.paths;
				if (versionPaths) {
					const packageDirectory = getDirectoryPath(packageJsonPath);
					const pathInPackage = absolutePath.slice(
						ensureTrailingDirectorySeparator(packageDirectory)
							.length,
					);
					if (
	...
	if (files) {
		for (let filePath of files) {
			...
			if (
				exclude &&
				comparePaths(filePath, exclude, scriptDirectory, ignoreCase) ===
					Comparison.EqualTo
			) {
				continue;
			}
			...
			result.add(
				nameAndKind(name, ScriptElementKind.scriptElement, extension),
			);
		}
	}
...
function getFilenameWithExtensionOption(
	...
	const nonJsResult =
		moduleSpecifiers.tryGetRealFileNameForNonJsDeclarationFileName(name);
	if (nonJsResult) {
		return {
			name: nonJsResult,
			extension: tryGetExtensionFromPath(nonJsResult),
		};
	}
	...
	if (
		(endingPreference === ModuleSpecifierEnding.Minimal ||
			endingPreference === ModuleSpecifierEnding.Index) &&
		fileExtensionIsOneOf(name, [
			Extension.Js,
			Extension.Jsx,
			Extension.Ts,
			Extension.Tsx,
		...
		return {
			...
			extension: tryGetExtensionFromPath(name),
		};
	}
...
function addCompletionEntriesFromPathsOrExports(
		...
		if (patterns) {
			...
			const isLongestMatch =
				...
				(matchedPath === undefined ||
					comparePaths(key, matchedPath) === Comparison.LessThan);
			if (isLongestMatch) {
			...
			if (
				...
				pathResults.push({
					...
					results: getCompletionsForPathMapping(
						...
						extensionOptions,
						host,
					).map(({ name, kind, extension }) =>
						nameAndKind(name, kind, extension),
					),
				});
			}
		}
	...
	pathResults.forEach((pathResult) =>
		pathResult.results.forEach((r) => result.add(r)),
	);
...
function getCompletionEntriesForNonRelativeModules(
	...
	if (baseUrl) {
		const absolute = normalizePath(
			combinePaths(host.getCurrentDirectory(), baseUrl),
		);
		getCompletionEntriesForDirectoryFragment(
			fragment,
			absolute,
		...
		if (!foundGlobal) {
			...
			if (
				...
				ancestorLookup = (ancestor) => {
					...
					const packageDirectory = combinePaths(
						ancestor,
						"node_modules",
						packagePath,
					);
...
function getCompletionsForPathMapping(
	...
	if (!endsWith(path, "*")) {
		...
		return !path.includes("*")
			? justPathMappingName(path, ScriptElementKind.scriptElement)
			: emptyArray;
	}
	...
	if (remainingFragment === undefined) {
		...
		return starIsFullPathComponent
			? justPathMappingName(pathPrefix, ScriptElementKind.directory)
...
function getModulesForPathsPattern(
	...
	const declarationExtension =
		normalizedSuffix &&
		getDeclarationEmitExtensionForPath("_" + normalizedSuffix);
	...
	const baseDirectory = normalizePath(
		combinePaths(packageDirectory, expandedPrefixDirectory),
	);
	const completePrefix = fragmentHasPath
		? baseDirectory
		: ensureTrailingDirectorySeparator(baseDirectory) +
			...
			if (trimmedWithPattern) {
				if (containsSlash(trimmedWithPattern)) {
					return directoryResult(
						getPathComponents(
							removeLeadingDirectorySeparator(trimmedWithPattern),
						)[1],
					);
				}
				const { name, extension } = getFilenameWithExtensionOption(
	...
	const directories = normalizedSuffix
		? emptyArray
		: mapDefined(tryGetDirectories(host, baseDirectory), (dir) =>
				dir === "node_modules" ? undefined : directoryResult(dir),
			);
...
function getAmbientModuleCompletions(
	...
	const nonRelativeModuleNames = ambientModules.filter(
		(moduleName) =>
			startsWith(moduleName, fragment) && !moduleName.includes("*"),
	);
...
function getTripleSlashReferenceCompletion(
	...
	return addReplacementSpans(
		toComplete,
		range.pos + prefix.length,
		arrayFrom(names.values()),
	);
}
	...
	function getCompletionEntriesFromDirectories(directory: string): void {
		...
		for (const typeDirectoryName of tryGetDirectories(host, directory)) {
			...
			if (fragmentDirectory === undefined) {
				...
				const baseDirectory = combinePaths(
					directory,
					typeDirectoryName,
				);
	...
	for (const packageJson of findPackageJsons(scriptPath, host)) {
		...
		for (const key of nodeModulesDependencyKeys) {
			...
			for (const dep in dependencies) {
				if (
					hasProperty(dependencies, dep) &&
					!startsWith(dep, "@types/")
				) {
					result.push(dep);
				}
			}
...
function isPathRelativeToScript(path: string) {
	if (path && path.length >= 2 && path.charCodeAt(0) === CharacterCodes.dot) {
		const slashIndex =
			path.length >= 3 && path.charCodeAt(1) === CharacterCodes.dot
				? 2
				: 1;
