...
const defaultToNamedAction = {
	...
	description: getLocaleSpecificMessage(
		Diagnostics.Convert_default_export_to_named_export,
	),
...
const namedToDefaultAction = {
	...
	description: getLocaleSpecificMessage(
		Diagnostics.Convert_named_export_to_default_export,
	),
...
function getInfo(
	...
	const exportingModuleSymbol = getExportingModuleSymbol(
		exportNode.parent,
		checker,
	);
	const flags =
	...
	switch (parent.kind) {
		...
		case SyntaxKind.ImportClause: {
			...
			if (!namedBindings) {
				...
				const quotePreference = isStringLiteral(
...
function getExportingModuleSymbol(
	...
	if (
		...
		isExternalModuleAugmentation(symbol.valueDeclaration)
	) {
