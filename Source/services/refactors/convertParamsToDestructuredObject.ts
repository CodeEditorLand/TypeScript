		...
		for (const entry of referenceEntries) {
            ...
            */
			if (
				contains(
					contextualSymbols,
					getSymbolTargetAtLocation(entry.node),
				)
			) {
			...
			if (
				contextualSymbol &&
				contains(contextualSymbols, contextualSymbol)
			) {
		...
		switch (parent.kind) {
			...
			case SyntaxKind.PropertyAccessExpression:
				...
				if (
					...
					if (
						callOrNewExpression &&
						callOrNewExpression.expression ===
							propertyAccessExpression
					) {
						return callOrNewExpression;
					}
...
function isTopLevelJSDoc(node: Node): boolean {
	...
	if (containingJSDoc) {
		...
		return (
			!!containingNonJSDoc &&
			isFunctionLikeDeclaration(containingNonJSDoc)
		);
	}
	return false;
}
...
function createNewArgument(
	...
	const properties = map(nonRestArguments, (arg, i) => {
		...
		const property = createPropertyOrShorthandAssignment(
			parameterName,
			arg,
		);
...
function getFunctionNames(
	...
	switch (functionDeclaration.kind) {
		...
		case SyntaxKind.Constructor:
			...
			if (
				functionDeclaration.parent.kind === SyntaxKind.ClassExpression
			) {
