...
function concatConsecutiveString(
	...
	while (index < nodes.length) {
		...
		if (isStringLiteralLike(node)) {
			...
			text += node.text;
			rawText += escapeRawStringForTemplate(
				getTextOfNode(node).slice(1, -1),
			);
			indexes.push(index);
			index++;
	...
	for (let i = begin; i < nodes.length; i++) {
		...
		if (isTemplateExpression(currentNode)) {
			const spans = map(currentNode.templateSpans, (span, index) => {
				...
				const isLastSpan =
					index === currentNode.templateSpans.length - 1;
				const text =
					span.literal.text + (isLastSpan ? subsequentText : "");
				const rawText =
