				...
				actions: [
					{
						...
						notApplicableReason: getStringError(
							rangeToExtract.errors,
						),
					},
				],
...
export function getRangeToExtract(
	...
	if (start !== end) {
		...
		return {
			targetRange: { range: statements, facts: rangeFacts, thisNode },
		};
	}
	...
	function checkRootNode(node: Node): Diagnostic[] | undefined {
		if (
			isIdentifier(isExpressionStatement(node) ? node.expression : node)
		) {
			return [
	...
	function checkNode(nodeToCheck: Node): Diagnostic[] | undefined {
		...
		if (
			...
			!(
				isExpressionNode(nodeToCheck) &&
				isExtractableExpression(nodeToCheck)
			) &&
		...
		function visit(node: Node) {
			...
			switch (node.kind) {
				...
				default:
					if (
						...
						permittedJumps |=
							PermittedJumps.Break | PermittedJumps.Continue;
					}
					break;
			}
			...
			switch (node.kind) {
				...
				case SyntaxKind.ContinueStatement: {
					const label = (node as BreakStatement | ContinueStatement)
						.label;
					if (label) {
...
function collectEnclosingScopes(range: TargetRange): Scope[] {
	...
	if (
		...
		!(range.facts & RangeFacts.UsesThisInFunction)
	) {
...
function getPossibleExtractions(
	...
	const extractions = scopes.map((scope, i): ScopeExtractions => {
		...
		if (i === 0 && !isClassLike(scope)) {
			constantDescription = formatStringFromArgs(
				getLocaleSpecificMessage(
					Diagnostics.Extract_to_0_in_enclosing_scope,
				),
...
function extractFunctionInScope(
	...
	const typeParametersAndDeclarations = arrayFrom(
		...
		(type) => ({
			type,
			declaration: getFirstDeclarationBeforePosition(
				type,
	...
	const typeParameters: readonly TypeParameterDeclaration[] | undefined =
		sortedTypeParametersAndDeclarations.length === 0
			...
			: mapDefined(
					...
					({ declaration }) =>
						declaration as TypeParameterDeclaration,
				);
	...
	let call: Expression = factory.createCallExpression(
		callThis
			? factory.createPropertyAccessExpression(called, "call")
			: called,
	...
	if (exposedVariableDeclarations.length && !writes) {
		...
		if (exposedVariableDeclarations.length === 1) {
			...
			for (const variableDeclaration of exposedVariableDeclarations) {
				...
				const variableType: TypeNode | undefined =
					checker.typeToTypeNode(
				...
				commonNodeFlags =
					commonNodeFlags & variableDeclaration.parent.flags;
			}
						...
						factory.createVariableDeclarationList(
							[
								factory.createVariableDeclaration(
									...
									/*exclamationToken*/ undefined,
									getTypeDeepCloneUnionUndefined(
										variableDeclaration.type,
									),
								),
							],
	...
	function getTypeDeepCloneUnionUndefined(
		...
		return isUnionTypeNode(withoutParens) &&
			find(
				withoutParens.types,
				(t) => t.kind === SyntaxKind.UndefinedKeyword,
			)
			? clone
...
function extractConstantInScope(
	...
	const localNameText =
		...
		!identifierToKeywordKind(node.name)
			? node.name.text
			: getUniqueName(
					isClassLike(scope) ? "newProperty" : "newLocal",
					file,
				);
	const isJS = isInJSFile(scope);
	...
	if (isClassLike(scope)) {
		...
		if (oldVariableDeclaration) {
			...
			// Declare
			const nodeToInsertBefore = getNodeToInsertConstantBefore(
				node,
				scope,
			);
...
function getFirstDeclarationBeforePosition(
	type: Type,
	position: number,
...
function transformFunctionBody(
	...
	if (hasWritesOrVariableDeclarations || substitutions.size) {
		const rewrittenStatements = visitNodes(
			statements,
			visitor,
			isStatement,
		).slice();
...
function collectReadsAndWrites(
		...
		for (
			...
			if (isDeclarationWithTypeParameters(curr)) {
				for (const typeParameterDecl of getEffectiveTypeParameterDeclarations(
					...
					if (
						allTypeParameterUsages.has(typeParameter.id.toString())
					) {
	...
	for (let i = 0; i < scopes.length; i++) {
		...
		if (
			targetRange.facts & RangeFacts.UsesThisInFunction &&
			isClassLike(scopes[i])
		) {
			functionErrorsPerScope[i].push(
				createDiagnosticForNode(
					targetRange.thisNode!,
					Messages.cannotExtractFunctionsContainingThisToMethod,
				),
			);
		}
	...
	function recordTypeParameterUsages(type: Type) {
		...
		for (const visitedType of visitedTypes) {
			if (visitedType.isTypeParameter()) {
				allTypeParameterUsages.set(
					visitedType.id.toString(),
					visitedType,
				);
			}
		}
	...
	function collectUsages(node: Node, valueUsage = Usage.Read) {
		...
		if (isAssignmentExpression(node)) {
			...
			recordUsage(
				node,
				valueUsage,
				/*isTypeNode*/ isPartOfTypeNode(node),
			);
		} else {
			forEachChild(node, collectUsages);
		}
	...
	function recordUsagebySymbol(
		...
		if (
			targetRange.facts & RangeFacts.IsGenerator &&
			usage === Usage.Write
		) {
		...
		for (let i = 0; i < scopes.length; i++) {
			...
			if (!substitutionsPerScope[i].has(symbolId)) {
				const substitution =
					tryReplaceWithQualifiedNameOrPropertyAccess(
	...
	function checkForUsedDeclarations(node: Node) {
		...
		if (
			...
			(isReadonlyArray(targetRange.range) &&
				targetRange.range.includes(node as Statement))
		) {
			return;
		}
