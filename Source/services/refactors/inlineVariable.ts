		...
		const edits = textChanges.ChangeTracker.with(context, (tracker) => {
			for (const node of references) {
				tracker.replaceNode(
					file,
					node,
					getReplacementExpression(node, replacement),
				);
			}
...
function getInliningInfo(
	...
	if (
		isInitializedVariable(parent) &&
		isVariableDeclarationInVariableStatement(parent) &&
		...
		if (checker.getMergedSymbol(parent.symbol).declarations?.length !== 1) {
			return {
				error: getLocaleSpecificMessage(
					Diagnostics.Variables_with_multiple_declarations_cannot_be_inlined,
				),
			};
		}
		...
		return (
			references && {
				references,
				declaration: parent,
				replacement: parent.initializer,
			}
		);
	}
	...
	if (tryWithReferenceToken) {
		...
		if (
			!isInitializedVariable(declaration) ||
			!isVariableDeclarationInVariableStatement(declaration) ||
	...
	return {
		error: getLocaleSpecificMessage(
			Diagnostics.Could_not_find_variable_to_inline,
		),
	};
}
...
function getReferenceNodes(
	...
	const cannotInline = FindAllReferences.Core.eachSymbolReferenceInFile(
		...
		(ref) => {
			...
			if (
				FindAllReferences.isWriteAccessForReference(ref) &&
				!isShorthandPropertyAssignment(ref.parent)
			) {
				return true;
			}
			...
			if (
				isExportSpecifier(ref.parent) ||
				isExportAssignment(ref.parent)
			) {
				return true;
			}
...
function getReplacementExpression(reference: Node, replacement: Expression) {
	...
	if (
		isExpression(parent) &&
		(getExpressionPrecedence(replacement) <
			getExpressionPrecedence(parent) ||
			needsParentheses(parent))
	) {
	...
	// E.g.: const f = () => {}; f(); -> (() => {})();
	if (
		isFunctionLike(replacement) &&
		(isCallLikeExpression(parent) || isPropertyAccessExpression(parent))
	) {
		return factory.createParenthesizedExpression(replacement);
	}
	...
	if (
		...
		(isNumericLiteral(replacement) ||
			isObjectLiteralExpression(replacement))
	) {
		return factory.createParenthesizedExpression(replacement);
	}
