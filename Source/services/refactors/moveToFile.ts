...
registerRefactor(refactorNameForMoveToFile, {
	...
	getEditsForAction: function getRefactorEditsToMoveToFile(
		...
		Debug.assert(
			actionName === refactorNameForMoveToFile,
			"Wrong refactor invoked",
		);
		const statements = Debug.checkDefined(getStatementsToMove(context));
		const { host, program } = context;
		Debug.assert(
			interactiveRefactorArguments,
			"No interactive refactor arguments available",
		);
		const targetFile = interactiveRefactorArguments.targetFile;
		if (hasJSFileExtension(targetFile) || hasTSFileExtension(targetFile)) {
			if (
				host.fileExists(targetFile) &&
				program.getSourceFile(targetFile) === undefined
			) {
				return error(
					getLocaleSpecificMessage(
						Diagnostics.Cannot_move_statements_to_the_selected_file,
					),
				);
			}
		...
		return error(
			getLocaleSpecificMessage(
				Diagnostics.Cannot_move_to_file_selected_file_is_invalid,
			),
		);
	},
});
...
function doChange(
	...
	if (!host.fileExists(targetFile)) {
		...
		const targetSourceFile = Debug.checkDefined(
		...
		getNewStatementsAndRemoveFromOldFile(
			...
			getUsageInfo(
				...
				toMove.all,
				checker,
				getExistingLocals(targetSourceFile, toMove.all, checker),
			),
			changes,
			toMove,
			program,
...
function getNewStatementsAndRemoveFromOldFile(
	...
	if (
		oldFile.externalModuleIndicator === undefined &&
		oldFile.commonJsModuleIndicator === undefined &&
	...
	const targetFileName =
		typeof targetFile === "string" ? targetFile : targetFile.fileName;
						...
						append(
							...
							filterImport(
								...
								factory.createStringLiteral(
									moduleSpecifierFromImport(i).text,
								),
								(name) =>
									importsToCopy.has(
		...
		for (const statement of sourceFile.statements) {
			forEachImportInStatement(statement, (importNode) => {
				...
				deleteUnusedImports(
					...
					changes,
					shouldMove,
				); // These will be changed to imports from the new file

				const pathToTargetFileWithExtension = resolvePath(
					getDirectoryPath(oldFile.path),
					targetFileName,
				);
				const newModuleSpecifier = getModuleSpecifier(
...
function updateNamespaceLikeImport(
	...
	FindAllReferences.Core.eachSymbolReferenceInFile(
		...
		(ref) => {
			...
			if (
				movedSymbols.has(checker.getSymbolAtLocation(ref.parent.name)!)
			) {
				toChange.push(ref);
			}
		},
...
function updateNamespaceLikeImportNode(
	...
	const newModuleString = makeStringLiteral(
		newModuleSpecifier,
		quotePreference,
	);
	switch (node.kind) {
...
/** @internal */
export function moduleSpecifierFromImport(
	i: SupportedImport,
): StringLiteralLike {
...
/** @internal */
export function forEachImportInStatement(
	statement: Statement,
...
export function createOldFileImportsFromTargetFile(
	...
	targetFileNeedExport.forEach((symbol) => {
		if (symbol.escapedName === InternalSymbolName.Default) {
			defaultImport = factory.createIdentifier(
				symbolNameNoDefault(symbol)!,
			);
		} else {
...
export function makeImportOrRequire(
	...
	const pathToTargetFile = resolvePath(
		getDirectoryPath(sourceFile.path),
		targetFileNameWithExtension,
	);
	...
	if (useEs6Imports) {
		...
		return makeImportIfNecessary(
			defaultImport,
			specifiers,
			pathToTargetFileWithCorrectExtension,
			quotePreference,
		);
	} else {
	...
	return flatMap(toMove, (statement) => {
		if (
			...
			forEachTopLevelDeclaration(statement, (d) =>
				needExport.has(
					Debug.checkDefined(tryCast(d, canHaveSymbol)?.symbol),
				),
			)
		) {
			const exports = addExport(
				getSynthesizedDeepClone(statement),
				useEs6Exports,
			);
	...
	if (useEs6Exports) {
		return (
			...
			!!(
				name &&
				sourceFile.symbol &&
				sourceFile.symbol.exports?.has(name.escapedText)
			)
		);
	}
	return (
...
function deleteUnusedImportsInVariableDeclaration(
	...
	switch (name.kind) {
				...
				if (
					...
					changes.delete(
						...
						isVariableDeclarationList(varDecl.parent) &&
							length(varDecl.parent.declarations) === 1
		...
		case SyntaxKind.ObjectBindingPattern:
			if (
				name.elements.every(
					(e) => isIdentifier(e.name) && isUnused(e.name),
				)
			) {
				changes.delete(
					sourceFile,
...
/** @internal */
export type TopLevelDeclarationStatement =
...
function addEs6Export(
	...
	switch (d.kind) {
		...
		case SyntaxKind.ClassDeclaration:
			const decorators = canHaveDecorators(d)
				? getDecorators(d)
				: undefined;
...
/** @internal */
export function nameOfTopLevelDeclaration(
...
/** @internal */
export function getTopLevelDeclarationStatement(
	d: TopLevelDeclaration,
): TopLevelDeclarationStatement {
	switch (d.kind) {
		case SyntaxKind.VariableDeclaration:
...
function getRangeToMove(context: RefactorContext): RangeToMove | undefined {
	...
	if (
		endNodeIndex !== -1 &&
		range.end <= statements[endNodeIndex].getStart()
	) {
		endNodeIndex--;
	}
	const endingOverloadRangeToMove = getOverloadRangeToMove(
		file,
		statements[endNodeIndex],
	);
	...
	return {
		toMove: statements.slice(
			startNodeIndex,
			endNodeIndex === -1 ? statements.length : endNodeIndex + 1,
		),
		afterLast:
			endNodeIndex === -1 ? undefined : statements[endNodeIndex + 1],
	};
}

/** @internal */
export function getStatementsToMove(
	context: RefactorContext,
): ToMove | undefined {
	...
	for (const statement of toMove) {
		forEachReference(
			...
			(symbol, isValidTypeOnlyUseSite) => {
				...
				for (const decl of symbol.declarations) {
					if (isInImport(decl)) {
						const prevIsTypeOnly =
							oldImportsNeededByTargetFile.get(symbol);
						oldImportsNeededByTargetFile.set(
							symbol,
							prevIsTypeOnly === undefined
								? isValidTypeOnlyUseSite
...
function inferNewFileName(
	importsFromNewFile: Set<Symbol>,
	movedSymbols: Set<Symbol>,
): string {
	return (
		forEachKey(importsFromNewFile, symbolNameNoDefault) ||
		forEachKey(movedSymbols, symbolNameNoDefault) ||
		"newFile"
	);
}
	...
	switch (statement.kind) {
		...
		case SyntaxKind.VariableStatement:
			return firstDefined(
				...
				(decl) =>
					forEachTopLevelDeclarationInBindingName(decl.name, cb),
			);
...
function moveStatementsToTargetFile(
	...
	if (targetExports) {
		...
		for (const node of toMove.all) {
			if (
				isTopLevelDeclarationStatement(node) &&
				...
				forEachTopLevelDeclaration(node, (declaration) => {
					const targetDeclarations = canHaveSymbol(declaration)
						? targetExports.get(declaration.symbol.escapedName)
								?.declarations
						: undefined;
					const exportDeclaration = firstDefined(
						targetDeclarations,
						(d) =>
					...
					if (
						...
						targetToSourceExports.set(
							exportDeclaration,
							(
								targetToSourceExports.get(exportDeclaration) ||
								new Set()
							).add(declaration),
						);
					}
		...
		for (const [exportDeclaration, topLevelDeclarations] of arrayFrom(
			targetToSourceExports,
		)) {
			if (
				exportDeclaration.exportClause &&
				isNamedExports(exportDeclaration.exportClause) &&
				...
				const updatedElements = filter(
					...
					(elem) =>
						find(
							...
							(d) =>
								isTopLevelDeclaration(d) &&
								topLevelDeclarations.has(d),
						) === undefined,
				);
	...
	if (lastReExport) {
		...
		changes.insertNodesAfter(
			targetFile,
			targetFile.statements[targetFile.statements.length - 1],
			statements,
		);
	}
}
...
function getOverloadRangeToMove(sourceFile: SourceFile, statement: Statement) {
	if (isFunctionLikeDeclaration(statement)) {
		...
		if (
			...
			!contains(declarations, statement)
		) {
			return undefined;
		}
		const firstDecl = declarations[0];
		const lastDecl = declarations[length(declarations) - 1];
		const statementsToMove = mapDefined(declarations, (d) =>
			getSourceFileOfNode(d) === sourceFile && isStatement(d)
				? d
				: undefined,
		);
		const end = findIndex(
			sourceFile.statements,
			(s) => s.end >= lastDecl.end,
		);
		const start = findIndex(
			sourceFile.statements,
			(s) => s.end >= firstDecl.end,
		);
...
function getExistingLocals(
	...
	for (const moduleSpecifier of sourceFile.imports) {
		...
		if (
			isVariableDeclarationInitializedToRequire(declaration.parent) &&
			isObjectBindingPattern(declaration.parent.name)
		) {
