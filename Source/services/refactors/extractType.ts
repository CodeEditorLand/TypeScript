...
function getRangeToExtract(
	...
	const firstType = getFirstTypeAt(
		file,
		startPosition,
		range,
		isCursorRequest,
	...
	if (
		(isUnionTypeNode(expandedFirstType.parent) ||
			isIntersectionTypeNode(expandedFirstType.parent)) &&
		...
		addRange(
			...
			expandedFirstType.parent.types.filter((type) => {
				return nodeOverlapsWithStartEnd(
					type,
					file,
					range.pos,
					range.end,
...
function getFirstTypeAt(
	...
	for (const f of currentNodes) {
		const current = f();
		const overlappingRange = nodeOverlapsWithStartEnd(
			current,
			file,
			range.pos,
			range.end,
			...
			if (
				...
				!flattenedTypeMembers.every(
					(type) =>
						...
						addToSeen(
							seen,
							getNameFromPropertyName(type.name) as string,
						),
				)
			) {
				return undefined;
...
function collectTypeParameters(
	...
	const selectionRange = {
		pos: selectionArray[0].pos,
		end: selectionArray[selectionArray.length - 1].end,
	};
		...
		if (isTypeReferenceNode(node)) {
			...
			if (isIdentifier(node.exprName)) {
				...
				if (
					...
					!rangeContainsSkipTrivia(selectionRange, node.parent, file)
				) {
					return true;
				}
			}
...
function doTypedefChange(
	...
	if (isJSDoc(enclosingNode)) {
		...
		const newLineCharacter = getNewLineOrDefaultFromHost(
			context.host,
		...
		changes.insertNodeAt(file, enclosingNode.getStart(file), jsDoc, {
			suffix:
				...
				newLineCharacter +
				file.text.slice(
					getPrecedingNonSpaceCharacterPosition(file.text, pos - 1),
					pos,
				),
		});
	} else {
...
function getNodesToEdit(info: ExtractInfo) {
	if (isArray(info.selection)) {
		return {
			...
			newTypeNode: isUnionTypeNode(info.selection[0].parent)
				? factory.createUnionTypeNode(info.selection)
...
function getEnclosingNode(node: Node, isJS: boolean) {
	return (
		findAncestor(node, isStatement) ||
		(isJS ? findAncestor(node, isJSDoc) : undefined)
	);
}
...
function getExpandedSelectionNode(firstType: Node, enclosingNode: Node) {
	...
	return (
		findAncestor(firstType, (node) => {
			if (node === enclosingNode) return "quit";
			if (
				isUnionTypeNode(node.parent) ||
				isIntersectionTypeNode(node.parent)
			) {
				return true;
			}
			return false;
