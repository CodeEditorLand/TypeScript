...
export function provideInlayHints(context: InlayHintsContext): InlayHint[] {
	...
	function addTypeHints(
		hintText: string | InlayHintDisplayPart[],
		position: number,
	) {
		result.push({
	...
	function visitVariableLikeDeclaration(
		...
		if (hintParts) {
			const hintText =
				typeof hintParts === "string"
					? hintParts
					: hintParts.map((part) => part.text).join("");
			const isVariableNameMatchesType =
				preferences.includeInlayVariableTypeHintsWhenTypeMatchesName ===
					false &&
	...
	function visitCallOrNewExpression(expr: CallExpression | NewExpression) {
		...
		for (const originalArg of args) {
			...
			const identifierInfo = checker.getParameterIdentifierInfoAtPosition(
				signature,
				signatureParamPos,
			);
			...
			if (identifierInfo) {
				const {
					parameter,
					parameterName,
					isRestParameter: isFirstVariadicArgument,
				} = identifierInfo;
				...
				if (
					!isParameterNameNotSameAsArgument &&
	...
	function visitFunctionLikeForParameterType(node: FunctionLikeDeclaration) {
		...
		for (
			...
			const typeHints = getParameterDeclarationTypeHints(
	...
	function typeToInlayHintParts(type: Type): InlayHintDisplayPart[] | string {
			...
			switch (node.kind) {
				case SyntaxKind.Identifier:
					...
					const name =
						...
						node.symbol.declarations.length &&
				...
				case SyntaxKind.MappedType:
					...
					if (node.readonlyToken) {
						if (node.readonlyToken.kind === SyntaxKind.PlusToken) {
							...
							node.readonlyToken.kind === SyntaxKind.MinusToken
						) {
							parts.push({ text: "-" });
						}
				...
				case SyntaxKind.ImportType:
					...
					if (node.assertions) {
						...
						visitDisplayPartList(
							node.assertions.assertClause.elements,
							", ",
						);
						parts.push({ text: " }" });
					}
		 ...
		 */
		function visitParametersAndTypeParameters(
			signatureDeclaration: SignatureDeclarationBase,
		) {
		...
		function visitDisplayPartList<T extends Node>(
			nodes: NodeArray<T>,
			separator: string,
		) {
			...
			switch (node.kind) {
				...
				case SyntaxKind.TemplateTail: {
					const rawText =
						...
						escapeTemplateSubstitution(
							escapeString(node.text, CharacterCodes.backtick),
						);
					switch (node.kind) {
						case SyntaxKind.TemplateHead:
							return "`" + rawText + "${";
