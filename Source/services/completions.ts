...
function originIncludesSymbolName(
	...
	return (
		originIsExport(origin) ||
		originIsResolvedExport(origin) ||
		originIsComputedPropertyName(origin)
	);
}
...
function originIsTypeOnlyAlias(
	origin: SymbolOriginInfo | undefined,
): origin is SymbolOriginInfoTypeOnlyImport {
...
function originIsComputedPropertyName(
	...
	return !!(
		origin && origin.kind & SymbolOriginInfoKind.ComputedPropertyName
	);
}
...
function resolvingModuleSpecifiers<TReturn>(
	...
	host.log?.(
		`${logPrefix}: response is ${skippedAny ? "incomplete" : "complete"}`,
	);
	...
	function tryResolve(
		...
		if (isFromAmbientModule) {
			const result = resolver.getModuleSpecifierForBestExportInfo(
				exportInfo,
				position,
		...
		resolvedFromCacheCount +=
			exportInfo.length - (result?.computedWithoutCacheCount || 0);
...
function continuePreviousIncompleteResponse(
	...
	const exportMap = getExportInfoMap(
		file,
		host,
		program,
		preferences,
		...
		(context) => {
			const entries = mapDefined(previousResponse.entries, (entry) => {
				...
				if (!result || result === "failed") {
					host.log?.(
						`Unexpected failure resolving auto import for '${entry.name}' from '${entry.source}'`,
					);
					return undefined;
				}
	...
	previousResponse.flags =
		(previousResponse.flags || 0) | CompletionInfoFlags.IsContinuation;
	previousResponse.optionalReplacementSpan =
		getOptionalReplacementSpan(touchNode);
...
function getJSDocParameterCompletions(
	...
	const isSnippet =
		preferences.includeCompletionsWithSnippetText || undefined;
	const paramTagCount = countWhere(
		jsDoc.tags,
		(tag) => isJSDocParameterTag(tag) && tag.getEnd() <= position,
	);
	return mapDefined(func.parameters, (param) => {
		...
		if (isIdentifier(param.name)) {
			...
			const snippetTextResult = isSnippet
				...
				: undefined;
			let displayText = displayTextResult.join(
				getNewLineCharacter(options) + "* ",
			);
			let snippetText = snippetTextResult?.join(
				getNewLineCharacter(options) + "* ",
			);
			if (tagNameOnly) {
				// Remove `@`
...
function generateJSDocParamTagsForDestructuring(
	...
	function elementWorker(
		...
		if (
			(!element.propertyName && isIdentifier(element.name)) ||
...
function getJSDocParamAnnotation(
	...
	if (isJs) {
		...
		if (isObject) {
			...
			if (initializer) {
				const inferredType = checker.getTypeAtLocation(
					initializer.parent,
				);
...
function completionInfoFromData(
	...
	if (
		caseClause &&
		(isCaseKeyword(contextToken!) ||
			isNodeDescendantOf(contextToken!, caseClause.expression))
	) {
		const tracker = newCaseClauseTracker(
			checker,
			caseClause.parent.clauses,
		);
			...
			if (
				...
				if (value !== undefined && tracker.hasValue(value)) {
					symbolToOriginInfoMap[i] = {
						kind: SymbolOriginInfoKind.Ignore,
					};
				}
			}
		});
	...
	if (keywordFilters !== KeywordCompletionFilters.None) {
		for (const keywordEntry of getKeywordCompletions(
			...
			if (
				(isTypeOnlyLocation &&
					isTypeKeyword(stringToToken(keywordEntry.name)!)) ||
				(!isTypeOnlyLocation &&
	...
	for (const literal of literals) {
		const literalEntry = createCompletionEntryForLiteral(
			sourceFile,
			preferences,
			literal,
		);
...
function getExhaustiveCaseSnippets(
	...
	if (
		switchType &&
		switchType.isUnion() &&
		every(switchType.types, (type) => type.isLiteral())
	) {
			...
			if (type.flags & TypeFlags.EnumLiteral) {
				...
				const enumValue =
					...
					checker.getConstantValue(
						type.symbol.valueDeclaration as EnumMember,
					);
				...
				const expr = typeNodeToExpression(
					typeNode,
					target,
					quotePreference,
				);
		...
		const newClauses = map(elements, (element) =>
			factory.createCaseClause(element, []),
		);
		const newLineChar = getNewLineOrDefaultFromHost(
			host,
		...
		const firstClause = printer.printNode(
			EmitHint.Unspecified,
			newClauses[0],
			sourceFile,
		);
	...
	switch (typeNode.kind) {
			...
			switch (literal.kind) {
				...
				case SyntaxKind.NumericLiteral:
					return factory.createNumericLiteral(
						literal.text,
						(literal as NumericLiteral).numericLiteralFlags,
					);
			}
			return undefined;
		case SyntaxKind.ParenthesizedType:
			...
			return (
				...
				(isIdentifier(exp)
					? exp
					: factory.createParenthesizedExpression(exp))
			);
		case SyntaxKind.TypeQuery:
			return entityNameToExpression(
				(typeNode as TypeQueryNode).exprName,
				languageVersion,
				quotePreference,
			);
		case SyntaxKind.ImportType:
			Debug.fail(
				`We should not get an import type after calling 'codefix.typeToAutoImportableTypeNode'.`,
			);
	}
...
function entityNameToExpression(
	...
	const unescapedName = unescapeLeadingUnderscores(
		entityName.right.escapedText,
	);
...
function createCompletionEntry(
	...
	if (isJsxInitializer) {
		...
		if (typeof isJsxInitializer !== "boolean") {
			replacementSpan = createTextSpanFromNode(
				isJsxInitializer,
				sourceFile,
			);
		}
	...
	if (origin && originIsPromise(origin) && propertyAccessToConvert) {
		...
		const isInAwaitExpression = tryCast(
			propertyAccessToConvert.parent,
			isAwaitExpression,
		);
		const wrapNode = isInAwaitExpression
			? propertyAccessToConvert.parent
			: propertyAccessToConvert.expression;
		replacementSpan = createTextSpanFromBounds(
			wrapNode.getStart(sourceFile),
	...
	if (
		...
		contextToken &&
		findPrecedingToken(contextToken.pos, sourceFile, contextToken)?.kind !==
	...
	const parentNamedImportOrExport = findAncestor(
		location,
		isNamedImportsOrExports,
	);
	if (parentNamedImportOrExport?.kind === SyntaxKind.NamedImports) {
		...
		if (
			...
			possibleToken &&
			(possibleToken === SyntaxKind.AwaitKeyword ||
				isNonContextualKeyword(possibleToken))
		) {
	...
	return (
		...
		(isClassLike(location) ||
			(location.parent &&
				...
				location.parent.getLastToken(sourceFile) ===
...
function getEntryForMemberCompletion(
	...
	const {
		...
		range: eraseRange,
		decorators: presentDecorators,
	} = getPresentModifiers(contextToken, sourceFile, position);
	...
	const isAbstract =
		presentModifiers & ModifierFlags.Abstract &&
	...
	if (completionNodes.length) {
		...
		let allowedModifiers =
			modifiers | ModifierFlags.Override | ModifierFlags.Public;
		if (!isMethod) {
		...
		if (
			...
			allowedAndPresent & ModifierFlags.Public
		) {
			modifiers &= ~ModifierFlags.Protected;
		}
		// `public` modifier is optional and can be dropped.
		if (
			allowedAndPresent !== ModifierFlags.None &&
			!(allowedAndPresent & ModifierFlags.Public)
		) {
			modifiers &= ~ModifierFlags.Public;
		}
		modifiers |= allowedAndPresent;
		completionNodes = completionNodes.map((node) =>
			factory.replaceModifiers(node, modifiers),
		);
		...
		if (presentDecorators?.length) {
			...
			if (canHaveDecorators(lastNode)) {
				completionNodes[completionNodes.length - 1] =
					factory.replaceDecoratorsAndModifiers(
						lastNode,
						(presentDecorators as ModifierLike[]).concat(
							getModifiers(lastNode) || [],
						),
					);
			}
		}
...
function getPresentModifiers(
    ...
    */
	if (
		...
		contextToken.parent.modifiers
	) {
		modifiers |=
			modifiersToFlags(contextToken.parent.modifiers) &
	...
	return {
		modifiers,
		decorators,
		range: range.pos !== position ? range : undefined,
	};
}
...
function getEntryForObjectLiteralMethodCompletion(
	...
	const method = createObjectLiteralMethod(
		symbol,
	...
	const labelDetails = {
		detail: signaturePrinter.printNode(
			EmitHint.Unspecified,
			methodSignature,
			sourceFile,
		),
...
function createObjectLiteralMethod(
	...
	const quotePreference = getQuotePreference(sourceFile, preferences);
	const builderFlags =
		NodeBuilderFlags.OmitThisParameter |
		(quotePreference === QuotePreference.Single
			? NodeBuilderFlags.UseSingleQuotesForStringLiteralType
			: NodeBuilderFlags.None);
	...
	switch (declaration.kind) {
		...
		case SyntaxKind.MethodDeclaration: {
			let effectiveType =
				type.flags & TypeFlags.Union &&
				(type as UnionType).types.length < 10
...
function createSnippetPrinter(printerOptions: PrinterOptions) {
	...
	const writer: EmitTextWriter = {
		...
		writeStringLiteral: (s) =>
			escapingWrite(s, () => baseWriter.writeStringLiteral(s)),
		writeSymbol: (s, symbol) =>
			escapingWrite(s, () => baseWriter.writeSymbol(s, symbol)),
		writeParameter: (s) =>
			escapingWrite(s, () => baseWriter.writeParameter(s)),
		writeComment: (s) => escapingWrite(s, () => baseWriter.writeComment(s)),
		writeProperty: (s) =>
			escapingWrite(s, () => baseWriter.writeProperty(s)),
	};
	...
	function printSnippetList(
		...
		return escapes
			? textChanges.applyChanges(unescaped, escapes)
			: unescaped;
	}
	...
	function printAndFormatSnippetList(
		...
		const allChanges = escapes
			? stableSort(concatenate(changes, escapes), (a, b) =>
					compareTextSpans(a.span, b.span),
				)
			: changes;
...
function getInsertTextAndReplacementSpanForImportCompletion(
	...
	const quotedModuleSpecifier = escapeSnippetText(
		quote(sourceFile, preferences, origin.moduleSpecifier),
	);
	...
	const isImportSpecifierTypeOnly =
...
function isRecommendedCompletionMatch(
	...
	return (
		...
		(!!(localSymbol.flags & SymbolFlags.ExportValue) &&
			checker.getExportSymbolOfSymbol(localSymbol) ===
				recommendedCompletion)
	);
}
...
export function getCompletionEntriesFromSymbols(
	...
	for (let i = 0; i < symbols.length; i++) {
		...
		if (
			...
			symbolAppearsToBeTypeOnly(symbol)
		) {
			continue;
		}
		...
		const originalSortText =
			...
			SortText.LocationPriority;
		const sortText = isDeprecated(symbol, typeChecker)
			? SortText.Deprecated(originalSortText)
			: originalSortText;
		...
		if (!isSourceFile(location)) {
			...
			if (
				...
				((isTypeParameterDeclaration(variableOrParameterDeclaration) &&
					...
					(isParameter(variableOrParameterDeclaration) &&
						isParameter(symbolDeclaration)))
			) {
				...
				if (
					symbolDeclarationPos >=
						variableOrParameterDeclaration.pos &&
					parameters &&
			...
			if (
				...
				(symbolToSortTextMap[getSymbolId(symbolOrigin)] ===
...
function getSymbolCompletionFromEntryId(
	...
	const literal = find(
		literals,
		(l) =>
			completionNameForLiteral(sourceFile, preferences, l) ===
			entryId.name,
	);
		...
		firstDefined(symbols, (symbol, index): SymbolCompletion | undefined => {
			...
			return info &&
				...
				((entryId.source === CompletionSource.ClassMemberSnippet &&
					...
					(entryId.source ===
						CompletionSource.ObjectLiteralMethodSnippet &&
						symbol.flags &
							(SymbolFlags.Property | SymbolFlags.Method)) ||
					getSourceFromOrigin(origin) === entryId.source ||
					entryId.source ===
						CompletionSource.ObjectLiteralMemberWithComma)
				? {
...
export function getCompletionEntryDetails(
	...
	switch (symbolCompletion.type) {
		...
		case "symbol": {
			...
			const symbolName = originIsComputedPropertyName(origin)
				? origin.symbolName
				: symbol.name;
...
function getCompletionEntryCodeActionsAndSourceDisplay(
	...
	if (data?.moduleSpecifier) {
		if (
			previousToken &&
			getImportStatementCompletionInfo(
...
function getContextualType(
	...
	switch (previousToken.kind) {
		...
		case SyntaxKind.CaseKeyword:
			...
			return caseClause
				? getSwitchedType(caseClause, checker)
				: undefined;
...
function getCompletionData(
	...
	if (insideComment) {
		if (hasDocComment(sourceFile, position)) {
			if (
				sourceFile.text.charCodeAt(position - 1) === CharacterCodes.at
			) {
	...
	if (contextToken) {
		const importStatementCompletionInfo = getImportStatementCompletionInfo(
			contextToken,
			sourceFile,
		);
				...
				return {
					...
					keywordCompletions: [
						keywordToCompletionEntry(
							importStatementCompletionInfo.keywordCompletion,
						),
					],
					isNewIdentifierLocation:
						importStatementCompletionInfo.isNewIdentifierLocation,
				};
			}
		...
		if (
			importStatementCompletionInfo.replacementSpan &&
			preferences.includeCompletionsForImportStatements &&
			preferences.includeCompletionsWithInsertText
		) {
		...
		if (
			!importStatementCompletionInfo.replacementSpan &&
			isCompletionListBlocker(contextToken)
		) {
			log(
		...
		if (
			...
			switch (parent.kind) {
				case SyntaxKind.PropertyAccessExpression:
					...
					const leftmostAccessExpression =
						getLeftmostAccessExpression(propertyAccessToConvert);
					if (
	...
	if (isRightOfDot || isRightOfQuestionDot) {
		...
		const tagName = (contextToken.parent.parent as JsxElement)
	...
	const isLiteralExpected =
		!tryCast(previousToken, isStringLiteralLike) &&
		!isJsxIdentifierExpected;
	const literals = !isLiteralExpected
		? []
		: mapDefined(
				contextualType &&
					(contextualType.isUnion()
	...
	function tryGetTypeExpressionFromTag(
		tag: JSDocTag,
	): JSDocTypeExpression | ExpressionWithTypeArguments | undefined {
							...
							? // At `namespace N.M/**/`, if this is the only declaration of `M`, don't include `M` as a completion.
								(symbol) =>
									...
									!symbol.declarations?.every(
										(d) => d.parent === node.parent,
									)
							: isRhsOfImportDeclaration
								? // Any kind is allowed when dotting off namespace in internal import equals declaration
									(symbol) =>
										isValidTypeAccess(symbol) ||
										isValidValueAccess(symbol)
								: isTypeLocation || insideJsDocTagTypeExpression
									? isValidTypeAccess
									: isValidValueAccess;
	...
	function addPropertySymbol(
		...
		if (computedPropertyName) {
			const leftMostName = getLeftMostName(
			...
			const firstAccessibleSymbolId =
				firstAccessibleSymbol && getSymbolId(firstAccessibleSymbol);
			if (
				firstAccessibleSymbolId &&
				addToSeen(seenPropertySymbols, firstAccessibleSymbolId)
			) {
				...
				if (
					...
					const { moduleSpecifier } =
						(importSpecifierResolver ||=
							codefix.createImportSpecifierResolver(
								sourceFile,
								program,
								host,
								preferences,
				...
				if (
					firstAccessibleSymbolId &&
					seenPropertySymbols.has(firstAccessibleSymbolId)
				) {
					return;
				}
		...
		function addSymbolSortInfo(symbol: Symbol) {
			if (isStaticProperty(symbol)) {
				symbolToSortTextMap[getSymbolId(symbol)] =
					SortText.LocalDeclarationPriority;
			}
		}
	...
	function tryGetJsxCompletionSymbols(): GlobalsSearch {
		...
		const attrsType =
			jsxContainer &&
			typeChecker.getContextualType(jsxContainer.attributes);
	...
	function getGlobalCompletions(): void {
		...
		const scopeNode =
			getScopeNode(contextToken, adjustedPosition, sourceFile) ||
			sourceFile;
		...
		if (
			...
			const thisType = typeChecker.tryGetThisTypeAt(
				...
				isClassLike(scopeNode.parent)
					? (scopeNode as ThisContainer)
					: undefined,
			);
			if (
				thisType &&
				...
				for (const symbol of getPropertiesForCompletion(
					...
					symbols.push(symbol);
					symbolToSortTextMap[getSymbolId(symbol)] =
						SortText.SuggestedClassMembers;
				}
			}
		}
		collectAutoImports();
			...
			switch (contextToken.kind) {
				...
				case SyntaxKind.EqualsToken:
					return (
						...
						parentKind === SyntaxKind.TypeParameter
					);
	...
	function collectAutoImports() {
		...
		const isAfterTypeOnlyImportSpecifierModifier =
			previousToken === contextToken && importStatementCompletion;
		...
		resolvingModuleSpecifiers(
			"collectAutoImports",
			host,
			(importSpecifierResolver ||= codefix.createImportSpecifierResolver(
				sourceFile,
				program,
				host,
				preferences,
			...
			(context) => {
				exportInfo.search(
					...
					(symbolName, targetFlags) => {
						...
						if (
							!detailsEntryId &&
							isStringANonContextualKeyword(symbolName)
						)
							return false;
						if (
							!isTypeOnlyLocation &&
						...
						if (
							...
							!(
								targetFlags &
								(SymbolFlags.Module | SymbolFlags.Type)
							)
						)
							return false;
						...
						if (
							isRightOfOpenTag &&
							(firstChar < CharacterCodes.A ||
								firstChar > CharacterCodes.Z)
						)
							return false;
					...
					(info, symbolName, isFromAmbientModule, exportMapKey) => {
						if (
							...
							!some(
								info,
								(i) =>
									detailsEntryId.source ===
									stripQuotes(i.moduleSymbol.name),
							)
						) {
							return;
						}
						...
						let exportInfo = info[0],
							moduleSpecifier;
						if (result !== "skipped") {
							({ exportInfo = info[0], moduleSpecifier } =
								result);
						}
				...
				flags |= context.resolvedAny()
					? CompletionInfoFlags.ResolvedModuleSpecifiers
					: 0;
				flags |= context.resolvedBeyondLimit()
					? CompletionInfoFlags.ResolvedModuleSpecifiersBeyondLimit
					: 0;
			},
		);
	...
	function pushAutoImportSymbol(
		...
		symbolToSortTextMap[symbolId] = importStatementCompletion
			? SortText.LocationPriority
			: SortText.AutoImportSuggestions;
		symbols.push(symbol);
	}
			...
			switch (tokenKind) {
				case SyntaxKind.CommaToken:
					return (
						...
						containingNodeKind ===
							SyntaxKind.ArrayLiteralExpression || // [a, |
				...
				case SyntaxKind.OpenParenToken:
					return (
						...
						containingNodeKind === SyntaxKind.NewExpression || // new C(a|
						containingNodeKind ===
							SyntaxKind.ParenthesizedExpression || // const x = (a|
	...
	function tryGetObjectTypeLiteralInTypeArgumentCompletionSymbols():
		...
		symbols = concatenate(
			...
			filter(
				members,
				(s) => !existingMemberEscapedNames.has(s.escapedName),
			),
		);
	...
	function tryGetObjectLikeCompletionSymbols(): GlobalsSearch | undefined {
		...
		const objectLikeContainer = tryGetObjectLikeCompletionContainer(
			contextToken,
			position,
			sourceFile,
		);
		...
		if (objectLikeContainer.kind === SyntaxKind.ObjectLiteralExpression) {
			...
			isNewIdentifierLocation =
				!!hasStringIndexType || !!hasNumberIndextype;
			...
			const rootDeclaration = getRootDeclaration(
				objectLikeContainer.parent,
			);
			...
			let canGetType =
				...
				!!getEffectiveTypeAnnotationNode(rootDeclaration) ||
		...
		if (typeMembers && typeMembers.length > 0) {
			...
			const filteredMembers = filterObjectMembersList(
				typeMembers,
		...
		const importAttributes =
			...
			contextToken.kind === SyntaxKind.CommaToken
				...
				: contextToken.kind === SyntaxKind.ColonToken
					? tryCast(contextToken.parent.parent, isImportAttributes)
					: undefined;
		if (importAttributes === undefined) return GlobalsSearch.Continue;

		const existing = new Set(
			importAttributes.elements.map(getNameFromImportAttribute),
		);
		symbols = filter(
			typeChecker
				.getTypeAtLocation(importAttributes)
				.getApparentProperties(),
			(attr) => !existing.has(attr.escapedName),
		);
		return GlobalsSearch.Success;
	}
				...
				case SyntaxKind.JsxSpreadAttribute:
					if (
						...
						if (contextToken.kind === SyntaxKind.GreaterThanToken) {
							...
							if (
								!(parent as JsxOpeningLikeElement)
									.typeArguments ||
								(precedingToken &&
									precedingToken.kind ===
	...
	function isInDifferentLineThanContextToken(
		contextToken: Node,
		position: number,
	): boolean {
		return (
			sourceFile.getLineEndOfPosition(contextToken.getEnd()) < position
		);
	}
	...
	function isSolelyIdentifierDefinitionLocation(contextToken: Node): boolean {
		...
		const ancestorClassLike = findAncestor(
			contextToken.parent,
			isClassLike,
		);
		if (
		...
		return (
			...
			!(
				(isClassLike(contextToken.parent) ||
					isInterfaceDeclaration(contextToken.parent) ||
		...
		symbols.forEach((m) => {
			if (m.flags & SymbolFlags.Optional) {
				...
				symbolToSortTextMap[symbolId] =
					symbolToSortTextMap[symbolId] ?? SortText.OptionalMember;
			}
		});
	}
	...
	function setSortTextToMemberDeclaredBySpreadAssignment(
		...
		for (const contextualMemberSymbol of contextualMemberSymbols) {
			if (
				membersDeclaredBySpreadAssignment.has(
					contextualMemberSymbol.name,
				)
			) {
				symbolToSortTextMap[getSymbolId(contextualMemberSymbol)] =
					SortText.MemberDeclaredBySpreadAssignment;
			}
		}
	}
	...
	function transformObjectLiteralMembersSortText(start: number): void {
		for (let i = start; i < symbols.length; i++) {
			...
			if (displayName) {
				const originalSortText =
					symbolToSortTextMap[symbolId] ?? SortText.LocationPriority;
				const { name } = displayName;
				symbolToSortTextMap[symbolId] = SortText.ObjectLiteralProperty(
					originalSortText,
					name,
				);
			}
	...
	function isCurrentlyEditingNode(node: Node): boolean {
		return (
			node.getStart(sourceFile) <= position && position <= node.getEnd()
		);
	}
}
...
function tryGetObjectLikeCompletionContainer(
	...
	if (contextToken) {
		...
		switch (contextToken.kind) {
			...
			case SyntaxKind.Identifier:
				if (
					(contextToken as Identifier).text === "async" &&
					isShorthandPropertyAssignment(contextToken.parent)
				) {
					...
					const ancestorNode = findAncestor(
						parent,
						isPropertyAssignment,
					);
					if (
						ancestorNode?.getLastToken(sourceFile) ===
							contextToken &&
	...
	const moduleSymbol = data.ambientModuleName
		...
		: data.fileName
			? checker.getMergedSymbol(
					Debug.checkDefined(
						containingProgram.getSourceFile(data.fileName),
					).symbol,
				)
			: undefined;
	...
	switch (kind) {
		case CompletionKind.MemberLike:
			return originIsComputedPropertyName(origin)
				? { name: origin.symbolName, needsConvertPropertyAccess: false }
				: undefined;
		case CompletionKind.ObjectPropertyDeclaration:
			// TODO: GH#18169
			return {
...
function getKeywordCompletions(
	...
	return (
		...
		(_keywordCompletions[index] = getTypescriptKeywordCompletions(
			...
			(entry) => !isTypeScriptOnlyKeyword(stringToToken(entry.name)!),
		))
	);
}
...
function keywordForNode(node: Node): SyntaxKind {
	return isIdentifier(node)
		? (identifierToKeywordKind(node) ?? SyntaxKind.Unknown)
		: node.kind;
}
...
function tryGetObjectTypeDeclarationCompletionContainer(
	...
	switch (location.kind) {
		...
		case SyntaxKind.PrivateIdentifier:
			...
			break;
		case SyntaxKind.Identifier: {
			const originalKeywordKind = identifierToKeywordKind(
	...
	switch (contextToken.kind) {
		...
		case SyntaxKind.CloseBraceToken: // class c { method() { } | }
			...
			return isFromObjectTypeDeclaration(location) &&
				(location.parent as ClassElement | TypeElement).name ===
					location
		...
		default:
			if (isObjectTypeDeclaration(location)) {
				...
				return isValidKeyword(contextToken.kind) ||
					...
					(isIdentifier(contextToken) &&
						isValidKeyword(
							identifierToKeywordKind(contextToken) ??
								SyntaxKind.Unknown,
						))
	...
	switch (node.kind) {
		case SyntaxKind.PropertySignature:
			return checker.getTypeOfPropertyOfContextualType(
				t,
				(node as PropertySignature).symbol.escapedName,
			);
...
function getImportStatementCompletionInfo(
	...
	return {
		...
		isTopLevelTypeOnly:
			!!tryCast(candidate, isImportDeclaration)?.importClause
				?.isTypeOnly ||
...
function getSingleLineReplacementSpanForImportCompletionNode(
	...
	Debug.assert(
		top.kind !== SyntaxKind.ImportKeyword &&
			top.kind !== SyntaxKind.ImportSpecifier,
	);
	...
	const potentialSplitPoint =
		top.kind === SyntaxKind.ImportDeclaration
			? (getPotentiallyInvalidImportSpecifier(
					top.importClause?.namedBindings,
				) ?? top.moduleSpecifier)
			: top.moduleReference;
...
function getPotentiallyInvalidImportSpecifier(
...
function canCompleteFromNamedBindings(namedBindings: NamedImportBindings) {
	...
	if (isNamedImports(namedBindings)) {
		...
		const invalidNamedImport =
			getPotentiallyInvalidImportSpecifier(namedBindings);
		const validImports = invalidNamedImport
			? namedBindings.elements.indexOf(invalidNamedImport)
...
function getVariableOrParameterDeclaration(
	contextToken: Node | undefined,
	location: Node,
) {
