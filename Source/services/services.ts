...
class NodeObject implements Node {
	...
	public getChildren(sourceFile?: SourceFileLike): Node[] {
		...
		return (
			this._children ||
			(this._children = createChildren(this, sourceFile))
		);
	}
	...
	while (pos < end) {
		...
		if (textPos <= end) {
			if (token === SyntaxKind.Identifier) {
				...
				Debug.fail(
					`Did not expect ${Debug.formatSyntaxKind(parent.kind)} to have an Identifier in its trivia`,
				);
			}
...
{
	public override kind: SyntaxKind.PrivateIdentifier =
		SyntaxKind.PrivateIdentifier;
	public escapedText!: __String;
	declare _primaryExpressionBrand: any;
	declare _memberExpressionBrand: any;
...
function hasJSDocInheritDocTag(node: Node) {
	return getJSDocTags(node).some(
		(tag) =>
			tag.tagName.text === "inheritDoc" ||
			tag.tagName.text === "inheritdoc",
	);
}
				...
				(symbol) => {
					if (!seenSymbols.has(symbol)) {
						...
						if (
							...
							return symbol.getContextualJsDocTags(
								declaration,
								checker,
							);
						}
...
export function createLanguageService(
	...
	function synchronizeHostDataWorker(): void {
		...
		const hasInvalidatedResolutions: HasInvalidatedResolutions =
			host.hasInvalidatedResolutions || returnFalse;
		const hasInvalidatedLibResolutions =
			maybeBind(host, host.hasInvalidatedLibResolutions) || returnFalse;
		...
		let parsedCommandLines:
			| Map<Path, ParsedCommandLine | false>
			| undefined;
		...
		function getOrCreateSourceFileByPath(
			...
			if (!shouldCreateNewSourceFile) {
				...
				if (oldSourceFile) {
					...
					if (
						scriptKind === oldSourceFile.scriptKind ||
						releasedScriptKinds!.has(oldSourceFile.resolvedPath)
					) {
	...
	function updateIsDefinitionOfReferencedSymbols(
		...
		for (const referencedSymbol of referencedSymbols) {
			for (const ref of referencedSymbol.references) {
				...
				if (
					knownSymbolSpans.has(ref) ||
					FindAllReferences.isDeclarationOfSymbol(refNode, symbol)
				) {
					...
					const mappedSpan = getMappedDocumentSpan(
						ref,
						sourceMapper,
						maybeBind(host, host.fileExists),
					);
					if (mappedSpan) {
		...
		function getNodeForSpan(docSpan: DocumentSpan): Node | undefined {
			...
			const rawNode = getTouchingPropertyName(
				sourceFile,
				docSpan.textSpan.start,
			);
			const adjustedNode = FindAllReferences.Core.getAdjustedNode(
				rawNode,
	...
	function getQuickInfoAtPosition(
		...
		if (!symbol || typeChecker.isUnknownSymbol(symbol)) {
			...
			return (
				type && {
					...
					textSpan: createTextSpanFromNode(
						nodeForQuickInfo,
						sourceFile,
					),
		...
		return {
			kind: symbolKind,
			kindModifiers: SymbolDisplay.getSymbolModifiers(
				typeChecker,
				symbol,
			),
	...
	function findRenameLocations(
		...
		if (
			...
			return [openingElement, closingElement].map(
				(node): RenameLocation => {
			...
			const providePrefixAndSuffixTextForRename =
	...
	function getReferencesWorker<T>(
		...
		const sourceFiles =
			options &&
			options.use === FindAllReferences.FindReferencesUse.Rename
				? program
						.getSourceFiles()
						.filter(
							(sourceFile) =>
	...
	function getFileReferences(fileName: string): ReferenceEntry[] {
		...
		return FindAllReferences.Core.getReferencesForFileName(
			...
			program,
			program.getSourceFiles(),
		).map(FindAllReferences.toReferenceEntry);
	}
	...
	function getDocCommentTemplateAtPosition(
		...
		const formatSettings = formatOptions
			? formatting.getFormatContext(formatOptions, host).options
			: undefined;
	...
	function getLinkedEditingRangeAtPosition(
		...
		if (isJsxFragment(token.parent.parent)) {
			...
			if (
				containsParseError(openFragment) ||
				containsParseError(closeFragment)
			)
				return undefined;
			...
			return {
				ranges: [
					{ start: openPos, length: 0 },
					{ start: closePos, length: 0 },
				],
			...
			return {
				ranges: [
					{ start: openTagStart, length: openTagEnd - openTagStart },
					{
						start: closeTagStart,
	...
	function getLinesForRange(sourceFile: SourceFile, textRange: TextRange) {
		return {
			...
			firstLine: sourceFile.getLineAndCharacterOfPosition(textRange.pos)
				.line,
			lastLine: sourceFile.getLineAndCharacterOfPosition(textRange.end)
				.line,
		};
	}
	...
	function toggleLineComment(
		...
		for (let i = firstLine; i <= lastLine; i++) {
			...
			if (regExec) {
				...
				if (
					lineText.substr(regExec.index, openComment.length) !==
					openComment
				) {
					...
					textChanges.push(
						...toggleMultilineComment(
							fileName,
							{
								...
								end: sourceFile.getLineEndOfPosition(
									lineStarts[i],
								),
							},
							isCommenting,
	...
	function toggleMultilineComment(
		...
		if (isCommenting || !hasComment) {
			...
			for (let i = 1; i < positions.length - 1; i++) {
				...
				if (
					text.substr(positions[i], openMultiline.length) !==
					openMultiline
				) {
					textChanges.push({
						newText: openMultiline,
			...
			for (const pos of positions) {
				const from =
					pos - closeMultiline.length > 0
						? pos - closeMultiline.length
						: 0;
				const offset =
	...
	function getSpanOfEnclosingComment(
		...
		const sourceFile = syntaxTreeCache.getCurrentSourceFile(fileName);
		const range = formatting.getRangeOfEnclosingComment(
			sourceFile,
			position,
		);
		return range &&
	...
	function getTodoComments(
		...
		if (descriptors.length > 0 && !isNodeModulesFile(sourceFile.fileName)) {
			...
			while ((matchArray = regExp.exec(fileContents))) {
				...
				if (
					isLetterOrDigit(
						fileContents.charCodeAt(
							matchPosition + descriptor.text.length,
						),
					)
				) {
					continue;
		...
		const files = mapDefined(allFiles, (file) =>
			...
			!(
				...
				(extension === Extension.Dts &&
					startsWith(getBaseFileName(file.fileName), "lib.") &&
					extensionFromPath(file.fileName) === Extension.Dts)
			) &&
			extension === extensionFromPath(file.fileName)
				? file.fileName
				: undefined,
		);
...
function initializeNameTable(sourceFile: SourceFile): void {
	...
	sourceFile.forEachChild(function walk(node) {
		if (
			...
			nameTable.set(
				text,
				nameTable.get(text) === undefined ? node.pos : -1,
			);
...
export function getPropertySymbolsFromContextualType(
	...
	const filteredTypes =
		isObjectLiteralExpression(node.parent) || isJsxAttributes(node.parent)
			? filter(
			...
			: contextualType.types;
	const discriminatedPropertySymbols = mapDefined(filteredTypes, (t) =>
		t.getProperty(name),
	);
	if (
		unionSymbolOk &&
