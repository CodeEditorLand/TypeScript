...
export function getSmartSelectionRange(
	...
	outer: while (true) {
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (
				getTokenPosOfNode(node, sourceFile, /*includeJsDoc*/ true) > pos
			) {
				break outer;
			}
			...
			if (positionShouldSnapToNode(sourceFile, pos, node)) {
				...
				if (
					...
					(isSyntaxList(node) &&
						isVariableDeclarationList(parentNode)) ||
				...
				let start = isBetweenMultiLineBookends
					? prevNode.getEnd()
					: node.getStart();
				...
				if (isSyntaxList(node)) {
					...
					if (
						firstChild &&
						hasJSDocNodes(firstChild) &&
						firstChild.jsDoc?.length &&
						firstChild.getStart() !== node.pos
					) {
						start = Math.min(
							start,
							first(firstChild.jsDoc).getStart(),
						);
					}
				}
				pushSelectionRange(start, end);
...
function getSelectionChildren(node: Node): readonly Node[] {
	...
	if (isPropertySignature(node)) {
		...
		const firstJSDocChild =
			children[0]?.kind === SyntaxKind.JSDoc ? children[0] : undefined;
		const withJSDocSeparated = firstJSDocChild
			? children.slice(1)
			: children;
		...
		return firstJSDocChild
			? [firstJSDocChild, createSyntaxList(splittedChildren)]
			: splittedChildren;
	}
	...
	if (isParameter(node)) {
		...
		const groupedWithQuestionToken = groupChildren(
			...
			(child) =>
				child === groupedDotDotDotAndName[0] ||
				child === node.questionToken,
		);
