...
export namespace SmartIndenter {
	...
	export function getIndentation(
		...
		if (
			...
			const actualIndentation =
				getActualIndentationForListItemBeforeComma(
					precedingToken,
					sourceFile,
					options,
				);
		...
		if (
			containerList &&
			!rangeContainsRange(containerList, precedingToken)
		) {
			const useTheSameBaseIndentation = [
				SyntaxKind.FunctionExpression,
				SyntaxKind.ArrowFunction,
			].includes(currentToken.parent.kind);
			const indentSize = useTheSameBaseIndentation
				? 0
				: options.indentSize!;
			return (
	...
	function getIndentationForNodeWorker(
		...
		while (parent) {
			...
			if (useActualIndentation) {
				// check if current node is a list item - if yes, take indentation from it
				const firstListChild = getContainingList(
					current,
					sourceFile,
				)?.[0];
				...
				const listIndentsChild =
					!!firstListChild &&
					getStartLineAndCharacterForNode(firstListChild, sourceFile)
						.line > containingListOrParentStart.line;
	...
	export function childIsUnindentedBranchOfConditionalExpression(
		...
		if (
			...
			if (child === parent.whenTrue) {
				...
				return (
					conditionEndLine === trueStartLine &&
					trueEndLine === childStartLine
				);
			}
		}
		return false;
	...
	function getListByRange(
		...
		switch (node.kind) {
			...
			case SyntaxKind.ArrayBindingPattern:
				return getList(
					(node as ObjectBindingPattern | ArrayBindingPattern)
						.elements,
				);
		}
	...
	function deriveActualIndentationFromList(
		...
		let lineAndCharacter = getStartLineAndCharacterForNode(
			node,
			sourceFile,
		);
	...
	export function nodeWillIndentChild(
		...
		switch (parent.kind) {
			...
			case SyntaxKind.ArrowFunction:
				if (
					sourceFile &&
					childKind === SyntaxKind.ParenthesizedExpression
				) {
					return rangeIsOnOneLine(sourceFile, child!);
				}
				return childKind !== SyntaxKind.Block;
