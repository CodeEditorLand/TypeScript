...
function findEnclosingNode(range: TextRange, sourceFile: SourceFile): Node {
	...
	function find(n: Node): Node {
		const candidate = forEachChild(
			n,
			(c) =>
				startEndContainsRange(c.getStart(sourceFile), c.end, range) &&
				c,
		);
		if (candidate) {
...
function getOwnOrInheritedDelta(
	...
	while (n) {
		...
		if (
			SmartIndenter.shouldIndentChildNode(options, n, child, sourceFile)
		) {
...
function formatSpanWorker(
	...
	if (
		...
		const tokenInfo = formattingScanner.isOnEOF()
		...
		if (tokenInfo && tokenInfo.pos === previousRangeTriviaEnd!) {
			...
			const parent =
				findPrecedingToken(tokenInfo.end, sourceFile, enclosingNode)
	...
	function tryComputeIndentationForListItem(
		...
		if (
			...
			if (startLine !== parentStartLine || startPos === column) {
				...
				const baseIndentSize =
					SmartIndenter.getBaseIndentation(options);
	...
	function getFirstNonDecoratorTokenOfNode(node: Node) {
		if (canHaveModifiers(node)) {
			const modifier = find(
				node.modifiers,
				isModifier,
				findIndex(node.modifiers, isDecorator),
			);
	...
	function getDynamicIndentation(
		...
		function shouldAddDelta(
			...
			return (
				...
				!(
					hasDecorators(node) &&
					kind === getFirstNonDecoratorTokenOfNode(node)
				)
			);
		}
	...
	function processNode(
		...
		function processChildNode(
			...
			if (
				!rangeOverlapsWithStartEnd(originalRange, child.pos, child.end)
			) {
		...
		function processChildNodes(
			...
			if (
				!rangeOverlapsWithStartEnd(originalRange, nodes.pos, nodes.end)
			) {
			...
			if (listStartToken !== SyntaxKind.Unknown) {
				...
				while (
					...
					if (tokenInfo.token.end > nodes.pos) {
						...
						if (
							indentationOnLastIndentedLine !== Constants.Unknown
						) {
		...
		function consumeTokenAndAdvanceScanner(
				...
				if (!rangeHasError) {
					if (lineAction === LineAction.None) {
						...
						const prevEndLine =
							savePreviousRange &&
							sourceFile.getLineAndCharacterOfPosition(
								savePreviousRange.end,
							).line;
						indentToken =
							lastTriviaWasNewLine &&
							tokenStart.line !== prevEndLine;
					} else {
			...
			if (indentToken) {
				const tokenIndentation =
					isTokenInRange &&
					!rangeContainsError(currentTokenInfo.token)
	...
	function processTrivia(
		...
		for (const triviaItem of trivia) {
			if (
				...
				const triviaItemStart =
					sourceFile.getLineAndCharacterOfPosition(triviaItem.pos);
				processRange(
					triviaItem,
					triviaItemStart,
					parent,
				...
				if (dynamicIndentation) {
					switch (lineAction) {
						case LineAction.LineRemoved:
							...
							if (
								currentParent.getStart(sourceFile) ===
								currentItem.pos
							) {
	...
	function indentMultilineComment(
		...
		for (let i = startIndex; i < parts.length; i++, startLine++) {
			...
			const newIndentation =
				nonWhitespaceCharacterAndColumn.column + delta;
			if (newIndentation > 0) {
	...
	function trimTrailingWhitespacesForLines(
		...
		for (let line = line1; line < line2; line++) {
			...
			if (whitespaceStart !== -1) {
				...
				recordDelete(
					whitespaceStart,
					lineEndPosition + 1 - whitespaceStart,
				);
			}
		}
	}
...
export function getIndentationString(
	...
	if (!options.convertTabsToSpaces) {
		...
		if (internedTabsIndentation[tabs] === undefined) {
			internedTabsIndentation[tabs] = tabString = repeatString(
				"\t",
				tabs,
			);
		} else {
