...
function isOptionEnabled(
	...
	return (context) =>
		context.options &&
		hasProperty(context.options, optionName) &&
...
function isOptionalPropertyContext(context: FormattingContext) {
	return (
		isPropertyDeclaration(context.contextNode) &&
		context.contextNode.questionToken
	);
}
...
function isNextTokenParentJsxAttribute(context: FormattingContext): boolean {
	return (
		context.nextTokenParent.kind === SyntaxKind.JsxAttribute ||
...
function isNextTokenParentNotJsxNamespacedName(
	context: FormattingContext,
): boolean {
	return context.nextTokenParent.kind !== SyntaxKind.JsxNamespacedName;
}

function isNextTokenParentJsxNamespacedName(
	context: FormattingContext,
): boolean {
	return context.nextTokenParent.kind === SyntaxKind.JsxNamespacedName;
}
...
function isStartOfVariableDeclarationList(context: FormattingContext): boolean {
	return (
		context.currentTokenParent.kind ===
...
function isNotPropertyAccessOnIntegerLiteral(
	...
	return (
		!isPropertyAccessExpression(context.contextNode) ||
