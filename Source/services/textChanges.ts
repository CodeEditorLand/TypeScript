		...
		if (comments) {
			...
			for (const comment of comments) {
				...
				if (
					...
					getLineOfLocalPosition(sourceFile, comment.pos) >
						nodeEndLine
				) {
					break;
				}
...
export class ChangeTracker {
	...
	public deleteNodeRange(
		...
		const endPosition = getAdjustedEndPosition(
			sourceFile,
			endNode,
			options,
		);
	...
	public insertNodeBefore(
		...
		this.insertNodeAt(
			...
			newNode,
			this.getOptionsForInsertNodeBefore(
				before,
				newNode,
				blankLineBetween,
			),
	...
	public insertCommentBeforeLine(
		...
		const lineStartPosition = getStartPositionOfLine(
			lineNumber,
			sourceFile,
		);
	...
	public insertNodeAtConstructorStartAfterSuperCall(
		...
		const superCallStatement = find(
			ctr.body!.statements,
			(stmt) =>
				isExpressionStatement(stmt) && isSuperCall(stmt.expression),
		);
	...
	private insertNodeAtStartWorker(
		...
		const indentation =
			this.guessIndentationFromExistingMembers(sourceFile, node) ??
			this.computeIndentationForNewMember(sourceFile, node);
		this.insertNodeAt(
			sourceFile,
			getMembersOrProperties(node).pos,
			newElement,
			this.getInsertNodeAtStartInsertOptions(
				sourceFile,
				node,
				indentation,
			),
	...
	private guessIndentationFromExistingMembers(
		...
		for (const member of getMembersOrProperties(node)) {
			if (
				rangeStartPositionsAreOnSameLine(lastRange, member, sourceFile)
			) {
	...
	private getInsertNodeAtStartInsertOptions(
		...
		return {
			...
			suffix: insertTrailingComma
				? ","
				: isInterfaceDeclaration(node) && isEmpty
					? ";"
					: "",
		};
	}
	...
	private insertNodeAfterWorker(
		...
		if (needSemicolonBetween(after, newNode)) {
			...
			if (
				sourceFile.text.charCodeAt(after.end - 1) !==
...
function endPositionToDeleteNodeInList(
	...
	if (isSeparator(node, token)) {
		const prevToken = findPrecedingToken(
		...
		if (isSeparator(prevNode, prevToken)) {
			...
			if (
				positionsAreOnSameLine(
					prevToken.getStart(sourceFile),
					token.getStart(sourceFile),
					sourceFile,
				)
			) {
				return isLineBreak(sourceFile.text.charCodeAt(pos - 1))
					? pos - 1
					: pos;
			}
...
namespace changesToText {
		...
		return mapDefined(
			...
			(changesInFile) => {
				...
				const normalized = stableSort(
					changesInFile,
					(a, b) =>
						a.range.pos - b.range.pos || a.range.end - b.range.end,
				);
				...
				const textChanges = mapDefined(normalized, (c) => {
					...
					const targetSourceFile =
						c.kind === ChangeKind.ReplaceWithSingleNode
							? (getSourceFileOfNode(getOriginalNode(c.node)) ??
								c.sourceFile)
							: c.kind === ChangeKind.ReplaceWithMultipleNodes
								? (getSourceFileOfNode(
										getOriginalNode(c.nodes[0]),
									) ?? c.sourceFile)
								: c.sourceFile;
	...
	export function newFileChangesWorker(
		...
		const nonFormattedText = flatMap(insertions, (insertion) =>
	...
	function computeNewText(
		...
		const noIndent =
			...
			getLineStartPositionForPosition(pos, targetSourceFile) === pos
				? text
				: text.replace(/^\s+/, "");
		return (
			(options.prefix || "") +
...
const textChangesTransformationContext: TransformationContext = {
	...
	factory: createNodeFactory(
		nullTransformationContext.factory.flags |
