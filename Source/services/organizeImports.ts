...
export function organizeImports(
	...
	const processImportsOfSameModuleSpecifier = (
		...
		if (shouldSort)
			importGroup = stableSort(importGroup, (s1, s2) =>
				compareImportsOrRequireStatements(s1, s2, comparer),
			);
		return importGroup;
	};
	...
	topLevelImportGroupDecls.forEach((importGroupDecl) =>
		organizeImportsWorker(
			importGroupDecl,
			processImportsOfSameModuleSpecifier,
		),
	);
	...
	if (mode !== OrganizeImportsMode.RemoveUnused) {
		...
		getTopLevelExportGroups(sourceFile).forEach((exportGroupDecl) =>
			organizeImportsWorker(exportGroupDecl, (group) =>
				coalesceExportsWorker(group, comparer, preferences),
			),
		);
	}
	...
	for (const ambientModule of sourceFile.statements.filter(isAmbientModule)) {
		...
		if (mode !== OrganizeImportsMode.RemoveUnused) {
			...
			organizeImportsWorker(ambientModuleExportDecls, (group) =>
				coalesceExportsWorker(group, comparer, preferences),
			);
		}
	}
	...
	>(
		...
		const oldImportGroups = shouldCombine
			? group(
					oldImportDecls,
					(importDecl) =>
						getExternalModuleName(importDecl.moduleSpecifier)!,
				)
			: [oldImportDecls];
		const sortedImportGroups = shouldSort
			? stableSort(oldImportGroups, (group1, group2) =>
		...
		const newImportDecls = flatMap(sortedImportGroups, (importGroup) =>
			...
			importGroup[0].moduleSpecifier === undefined
		...
		if (newImportDecls.length === 0) {
			...
			changeTracker.deleteNodes(
				...
				{
					leadingTriviaOption:
						textChanges.LeadingTriviaOption.Exclude,
					trailingTriviaOption:
						textChanges.TrailingTriviaOption.Include,
				},
			...
			const replaceOptions = {
				...
				suffix: getNewLineOrDefaultFromHost(
					host,
...
function groupByNewlineContiguous<
	T extends ImportDeclaration | ExportDeclaration,
>(sourceFile: SourceFile, decls: T[]): T[][] {
		...
		return (
			(jsxElementsPresent &&
				(identifier.text === jsxNamespace ||
					(jsxFragmentFactory &&
						identifier.text === jsxFragmentFactory)) &&
...
function coalesceImportsWorker(
	...
	for (const group of [regularImports, typeOnlyImports]) {
		...
		const sortedNamespaceImports = stableSort(namespaceImports, (i1, i2) =>
			comparer(
				i1.importClause.namedBindings.name.text,
				i2.importClause.namedBindings.name.text,
			),
		);
		...
		const sortedImportSpecifiers = factory.createNodeArray(
			...
			firstNamedImport?.importClause.namedBindings.elements
				.hasTrailingComma,
		);
...
function getCategorizedImports(importGroup: readonly ImportDeclaration[]) {
	...
	for (const importDeclaration of importGroup) {
		...
		if (name) {
			group.defaultImports.push(
				importDeclaration as ImportDeclarationWithDefaultImport,
			);
		}

		if (namedBindings) {
			if (isNamespaceImport(namedBindings)) {
				group.namespaceImports.push(
					importDeclaration as ImportDeclarationWithNamespaceImport,
				);
			} else {
				group.namedImports.push(
					importDeclaration as ImportDeclarationWithNamedImports,
				);
			}
		}
	}
...
function coalesceExportsWorker(
	exportGroup: readonly ExportDeclaration[],
	comparer: Comparer<string>,
	preferences?: UserPreferences,
) {
	...
	for (const exportGroup of [namedExports, typeOnlyExports]) {
		...
		const sortedExportSpecifiers = sortSpecifiers(
			newExportSpecifiers,
			comparer,
			preferences,
		);
...
function sortSpecifiers<T extends ImportOrExportSpecifier>(
	...
	return stableSort(specifiers, (s1, s2) =>
		compareImportOrExportSpecifiers(s1, s2, comparer, preferences),
	);
}
...
>(
	...
	switch (preferences?.organizeImportsTypeOrder) {
		case "first":
			return (
				compareBooleans(s2.isTypeOnly, s1.isTypeOnly) ||
				comparer(s1.name.text, s2.name.text)
			);
		case "inline":
...
function detectSortingWorker(
	importGroups: ImportDeclaration[][],
	preferences: UserPreferences,
): SortKind {
	...
	for (const importGroup of importGroups) {
		...
		const declarationWithNamedImports = find(
			importGroup,
			(i) =>
				tryCast(i.importClause?.namedBindings, isNamedImports)?.elements
					.length! > 1,
		);
		if (declarationWithNamedImports) {
			const namedImportSort = detectImportSpecifierSorting(
				(
					declarationWithNamedImports.importClause!
...
class ImportSpecifierSortingCache
	implements
		MemoizeCache<[readonly ImportSpecifier[], UserPreferences], SortKind>
{
...
export const detectImportSpecifierSorting = memoizeCached(
	(
		...
		if (preferences.organizeImportsTypeOrder !== "inline") {
			...
			if (
				regularCaseSensitivity === SortKind.None ||
				typeCaseSensitivity === SortKind.None
			) {
				return SortKind.None;
			}
...
export function getImportDeclarationInsertionIndex(
	...
	const index = binarySearch(sortedImports, newImport, identity, (a, b) =>
		compareImportsOrRequireStatements(a, b, comparer),
	);
...
export function getImportSpecifierInsertionIndex(
	...
	const index = binarySearch(sortedImports, newImport, identity, (s1, s2) =>
		compareImportOrExportSpecifiers(s1, s2, comparer, preferences),
	);
	return index < 0 ? ~index : index;
}
...
function getImportKindOrder(s1: AnyImportOrRequireStatement) {
	switch (s1.kind) {
		case SyntaxKind.ImportDeclaration:
			...
			if (
				s1.importClause.namedBindings?.kind ===
...
function getOrganizeImportsUnicodeStringComparer(
	ignoreCase: boolean,
	preferences: UserPreferences,
): Comparer<string> {
...
/** @internal */
export function getOrganizeImportsComparer(
	preferences: UserPreferences,
	ignoreCase: boolean,
): Comparer<string> {
...
function getOrganizeImportsComparerWithDetection(
	...
	const ignoreCase =
		typeof preferences.organizeImportsIgnoreCase === "boolean"
...
function getTopLevelExportGroups(sourceFile: SourceFile) {
		...
		if (isExportDeclaration(statements[i])) {
			...
			if (exportDecl.moduleSpecifier) {
				...
				while (i < len && isExportDeclaration(statements[i])) {
					topLevelExportGroups[groupIndex].push(
						statements[i++] as ExportDeclaration,
					);
				}
				groupIndex++;
			}
	...
	return flatMap(topLevelExportGroups, (exportGroupDecls) =>
		groupByNewlineContiguous(sourceFile, exportGroupDecls),
	);
}
