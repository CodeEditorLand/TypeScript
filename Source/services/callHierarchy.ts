...
export type AssignedExpression =
	| (ClassExpression & {
			name: undefined;
			parent: VariableLike & { name: Identifier };
	  })
	| (FunctionExpression & {
			name: undefined;
			parent: VariableLike & { name: Identifier };
...
function getCallHierarchyItemName(
	...
	const declName = isAssignedExpression(node)
		? node.parent.name
		: Debug.checkDefined(
...
function getCallHierarchItemContainerName(
	...
	if (isAssignedExpression(node)) {
		if (
			...
			return isClassExpression(node.parent.parent)
				? getAssignedName(node.parent.parent)?.getText()
...
export function resolveCallHierarchyDeclaration(
	...
	while (true) {
		...
		if (isPossibleCallHierarchyDeclaration(location)) {
			...
			return (
				ancestor &&
				findImplementationOrAllInitialDeclarations(
					typeChecker,
					ancestor,
				)
			);
		...
		if (isDeclarationName(location)) {
			...
			if (
				isVariableLike(location.parent) &&
		...
		if (
			...
			isAssignedExpression(location.initializer)
		) {
	...
	function collect(node: Node | undefined) {
		...
		switch (node.kind) {
			...
			case SyntaxKind.Parameter:
				...
				collect(
					(node as VariableDeclaration | ParameterDeclaration).name,
				);
				collect(
					(node as VariableDeclaration | ParameterDeclaration)
						.initializer,
				);
				return;
