...
function getSymbolKindOfConstructorPropertyMethodAccessorFunctionOrVar(
	...
	if (flags & SymbolFlags.Variable) {
		if (isFirstDeclarationOfSymbolParameter(symbol)) {
			...
			symbol.valueDeclaration &&
			isVarUsing(symbol.valueDeclaration as VariableDeclaration)
		) {
			return ScriptElementKind.variableUsingElement;
		} else if (
			symbol.valueDeclaration &&
			isVarAwaitUsing(symbol.valueDeclaration as VariableDeclaration)
		) {
...
export function getSymbolModifiers(
	...
	if (symbol.flags & SymbolFlags.Alias) {
		...
		if (resolvedSymbol !== symbol) {
			forEach(
				getNormalizedSymbolModifiers(resolvedSymbol),
				(modifier) => {
...
function getSymbolDisplayPartsDocumentationAndSymbolKindWorker(
	...
	if (
		...
		if (isCallOrNewExpression(location)) {
			...
			callExpressionLike = location.parent as
				| CallExpression
				| NewExpression;
		} else if (
			location.parent &&
		...
		if (callExpressionLike) {
			...
			const useConstructSignatures =
				...
				(isCallExpression(callExpressionLike) &&
					callExpressionLike.expression.kind ===
				...
				if (useConstructSignatures && symbolFlags & SymbolFlags.Class) {
					...
					if (useConstructSignatures) {
						if (signature.flags & SignatureFlags.Abstract) {
							displayParts.push(
								keywordPart(SyntaxKind.AbstractKeyword),
							);
							displayParts.push(spacePart());
						}
				...
				switch (symbolKind) {
					...
					case ScriptElementKind.localVariableElement:
						...
						if (useConstructSignatures) {
							...
							displayParts.push(
								keywordPart(SyntaxKind.NewKeyword),
							);
							displayParts.push(spacePart());
						}
			...
			if (locationIsSymbolDeclaration) {
				...
				if (
					!typeChecker.isImplementationOfOverload(functionDeclaration)
				) {
					signature =
	...
	if (symbolFlags & SymbolFlags.Enum) {
		...
		if (
			some(
				symbol.declarations,
				(d) => isEnumDeclaration(d) && isEnumConst(d),
			)
		) {
		...
		if (symbol.parent) {
			...
			if (declaration) {
				if (isFunctionLike(declaration)) {
					addInPrefix();
					const signature =
						typeChecker.getSignatureFromDeclaration(declaration)!; // TODO: GH#18217
		...
		if (symbol.declarations) {
			switch (symbol.declarations[0].kind) {
				...
				case SyntaxKind.ExportAssignment:
					...
					displayParts.push(
						keywordPart(
							(symbol.declarations[0] as ExportAssignment)
								.isExportEquals
		...
		if (symbolKind !== ScriptElementKind.unknown) {
			if (type) {
				...
				if (
					...
					if (
						type.symbol &&
						type.symbol.flags & SymbolFlags.TypeParameter &&
						symbolKind !== ScriptElementKind.indexSignatureElement
					) {
						const typeParameterParts = mapToDisplayParts(
							(writer) => {
								const param =
	...
	function addFullSymbolName(
		...
		if (symbolKind === ScriptElementKind.indexSignatureElement) {
			indexInfos =
				typeChecker.getIndexInfosOfIndexSymbol(symbolToDisplay);
		}
		...
		if (symbolToDisplay.flags & SymbolFlags.Signature && indexInfos) {
			if (symbolToDisplay.parent) {
				fullSymbolDisplayParts = symbolToDisplayParts(
					typeChecker,
					symbolToDisplay.parent,
				);
			}
			fullSymbolDisplayParts.push(
				punctuationPart(SyntaxKind.OpenBracketToken),
			);
			// Needed to handle more than one type of index
			indexInfos.forEach((info, i) => {
				// Needed to handle template literals
				fullSymbolDisplayParts.push(
					...typeToDisplayParts(typeChecker, info.keyType),
				);
				if (i !== indexInfos.length - 1) {
					fullSymbolDisplayParts.push(spacePart());
					fullSymbolDisplayParts.push(
						punctuationPart(SyntaxKind.BarToken),
					);
					fullSymbolDisplayParts.push(spacePart());
				}
			});
			fullSymbolDisplayParts.push(
				punctuationPart(SyntaxKind.CloseBracketToken),
			);
		} else {
			fullSymbolDisplayParts = symbolToDisplayParts(
				typeChecker,
		...
		if (symbolKind) {
			...
			if (
				...
				!some(
					...
					(d) =>
						isArrowFunction(d) ||
						((isFunctionExpression(d) || isClassExpression(d)) &&
							!d.name),
				)
			) {
				displayParts.push(spacePart());
	...
	function addSignatureDisplayParts(
		...
		if (
			...
			documentation =
				allSignatures[0].getDocumentationComment(typeChecker);
			tags = allSignatures[0]
				.getJsDocTags()
				.filter((tag) => tag.name !== "deprecated"); // should only include @deprecated JSDoc tag on the first overload (#49368)
		}
	}
