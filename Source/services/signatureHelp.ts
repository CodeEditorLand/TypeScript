...
function isSyntacticOwner(
	...
	switch (startingToken.kind) {
		...
		case SyntaxKind.CommaToken: {
			...
			return (
				!!containingList && contains(invocationChildren, containingList)
			);
		}
...
function getArgumentOrParameterListAndIndex(
	...
	if (
		...
		return (
			list && {
				list,
				argumentIndex: getArgumentIndex(list, node, checker),
			}
		);
	}
}
...
function getImmediatelyContainingArgumentInfo(
	...
	if (isCallOrNewExpression(parent)) {
		...
		const argumentIndex = isInsideTemplateLiteral(
			node,
			position,
			sourceFile,
		)
		...
		const spanIndex =
			templateSpan.parent.templateSpans.indexOf(templateSpan);
...
function tryGetParameterInfo(
	...
	const signature = lastOrUndefined(
		nonNullableContextualType.getCallSignatures(),
	);
...
function chooseBetterSymbol(s: Symbol): Symbol {
	return s.name === InternalSymbolName.Type
		? firstDefined(s.declarations, (d) =>
				isFunctionTypeNode(d)
					? tryCast(d.parent, canHaveSymbol)?.symbol
					: undefined,
			) || s
		: s;
}
...
function getArgumentIndex(
	...
	for (let pos = 0; pos < length(args); pos++) {
		...
		if (isSpreadElement(child)) {
			argumentIndex =
				argumentIndex +
				getSpreadElementCount(child, checker) +
				(pos > 0 ? pos : 0);
		} else {
...
function getSpreadElementCount(node: SpreadElement, checker: TypeChecker) {
	...
	if (checker.isTupleType(spreadType)) {
		const { elementFlags, fixedLength } = (spreadType as TupleTypeReference)
			.target;
		if (fixedLength === 0) {
			return 0;
		}
		const firstOptionalIndex = findIndex(
			elementFlags,
			(f) => !(f & ElementFlags.Required),
		);
...
function getSignatureHelpItem(
	...
	return map(infos, ({ isVariadic, parameters, prefix, suffix }) => {
		...
		const documentation =
			candidateSignature.getDocumentationComment(checker);
...
function returnTypeToDisplayParts(
	...
	return mapToDisplayParts((writer) => {
		...
		writer.writeSpace(" ");
		const predicate =
			checker.getTypePredicateOfSignature(candidateSignature);
		if (predicate) {
		...
		!checker.hasEffectiveRestParameter(candidateSignature)
			...
			: lists.length === 1
				...
				: (pList) =>
						!!(
							pList.length &&
							tryCast(pList[pList.length - 1], isTransientSymbol)
								?.links.checkFlags! & CheckFlags.RestParameter
						);
...
function createSignatureHelpParameterForParameter(
	...
	const isRest =
		isTransientSymbol(parameter) &&
		!!(parameter.links.checkFlags & CheckFlags.RestParameter);
	return {
		name: parameter.name,
