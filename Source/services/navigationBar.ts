				...
				case AssignmentDeclarationKind.ObjectDefinePrototypeProperty: {
					...
					const className =
						...
						AssignmentDeclarationKind.ObjectDefinePropertyValue
							...
							: ((
									defineCall
										.arguments[0] as PropertyAccessExpression
					...
					addChildrenRecursively(
						(node as CallExpression).arguments[2],
					);
					endNode();
					endNode();
					endNestedNodes(depth);
				...
				case AssignmentDeclarationKind.Property: {
					...
					if (
						...
						if (
							...
							isBindableStaticAccessExpression(assignmentTarget)
						) {
	...
	if (
		...
		if (
			...
			if (ctorFunction !== undefined) {
				...
				ctor.children =
					a.node === ctorFunction ? a.children : b.children;
				a.children =
					a.node === ctorFunction
...
function areSameModule(a: ModuleDeclaration, b: ModuleDeclaration): boolean {
	...
	return (
		...
		(a.body.kind !== SyntaxKind.ModuleDeclaration ||
			areSameModule(
				a.body as ModuleDeclaration,
				b.body as ModuleDeclaration,
			))
	);
}
		...
		if (name !== undefined) {
			...
			const args = cleanText(
				mapDefined(parent.arguments, (a) =>
					isStringLiteralLike(a) || isTemplateLiteral(a)
						? a.getText(curSourceFile)
						: undefined,
				).join(", "),
			);
