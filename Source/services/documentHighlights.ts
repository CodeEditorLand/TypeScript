...
export namespace DocumentHighlights {
	...
	function getSemanticDocumentHighlights(
		...
		return arrayFrom(
			mapDefinedIterator(map.entries(), ([fileName, highlightSpans]) => {
	...
	function getSyntacticDocumentHighlights(
		...
		return (
			highlightSpans && [
				{ fileName: sourceFile.fileName, highlightSpans },
			]
		);
	}
		...
		return findAncestor(statement, (node) => {
			switch (node.kind) {
				...
				case SyntaxKind.DoStatement:
					return (
						!statement.label ||
						isLabeledBy(node, statement.label.escapedText)
					);
				default:
	...
	function getBreakOrContinueStatementOccurrences(
		...
		if (owner) {
			switch (owner.kind) {
				...
				case SyntaxKind.DoStatement:
				case SyntaxKind.WhileStatement:
					return getLoopBreakContinueOccurrences(
						owner as IterationStatement,
					);
				case SyntaxKind.SwitchStatement:
					return getSwitchCaseDefaultOccurrences(
						owner as SwitchStatement,
					);
			}
		}
	...
	function getSwitchCaseDefaultOccurrences(
		...
		forEach(switchStatement.caseBlock.clauses, (clause) => {
			...
			forEach(
				aggregateAllBreakAndContinueStatements(clause),
				(statement) => {
					if (
						ownsBreakOrContinueStatement(switchStatement, statement)
					) {
						pushKeywordIf(
							keywords,
							statement.getFirstToken(),
		...
		for (let i = 0; i < keywords.length; i++) {
			if (
				...
				for (
					...
					if (
						!isWhiteSpaceSingleLine(sourceFile.text.charCodeAt(j))
					) {
	...
	function getIfElseKeywords(
		...
		while (true) {
			...
			for (let i = children.length - 1; i >= 0; i--) {
				if (
					pushKeywordIf(keywords, children[i], SyntaxKind.ElseKeyword)
				) {
					break;
				}
			}
