...
export function createCacheableExportInfoMap(
	...
	const cache: ExportInfoMap = {
		...
		add: (
			...
			if (moduleFile) {
				...
				if (nodeModulesPathParts) {
					...
					if (
						startsWith(
							importingFile,
							moduleFile.path.substring(
								0,
								topLevelNodeModulesIndex,
							),
						)
					) {
						const prevDeepestNodeModulesPath =
						...
						if (prevDeepestNodeModulesPath) {
							const prevDeepestNodeModulesIndex =
								prevDeepestNodeModulesPath.indexOf(
									nodeModulesPathPart,
								);
							if (
								topLevelNodeModulesIndex >
								prevDeepestNodeModulesIndex
							) {
			...
			const names =
				...
				isExternalModuleSymbol(namedSymbol)
			...
			const storedModuleSymbol =
				moduleSymbol.flags & SymbolFlags.Transient
					? undefined
					: moduleSymbol;
		...
		search: (importingFile, preferCapitalized, matches, action) => {
			...
			return forEachEntry(exportInfo, (info, key) => {
				...
				const name =
					(preferCapitalized && info[0].capitalizedSymbolName) ||
					symbolName;
	...
	function rehydrateCachedInfo(
		...
		const [cachedSymbol, cachedModuleSymbol] =
			symbols.get(id) || emptyArray;
...
function isImportablePath(
	...
	return (
		...
		(!!globalCachePath &&
			startsWith(
				getCanonicalFileName(globalCachePath),
				toNodeModulesParent,
			))
	);
}
...
function forEachExternalModule(
	...
	const realpathsWithSymlinks = host
		.getSymlinkCache?.()
			...
			if (
				...
				return (
					forEachAncestorDirectory(
						...
						(dirPath) => {
							const symlinks = realpathsWithSymlinks.get(
								ensureTrailingDirectorySeparator(dirPath),
							);
							if (symlinks) {
								return symlinks.some((s) =>
									excludePatterns.some((p) =>
										p.test(fileName.replace(dir, s)),
									),
								);
							}
	...
	for (const ambient of checker.getAmbientModules()) {
		if (
			...
			!(
				excludePatterns &&
				ambient.declarations?.every((d) =>
					isExcluded!(d.getSourceFile()),
				)
			)
		) {
...
export function getDefaultExportInfoWorker(
	...
	if (defaultExport.flags & SymbolFlags.Alias) {
		...
		if (aliased && aliased.parent) {
			...
			return getDefaultExportInfoWorker(
				aliased,
				checker,
				compilerOptions,
			);
	...
	return (
		...
		firstDefined(symbol.declarations, (declaration) => {
			if (isExportAssignment(declaration)) {
				...
				return (
					declaration.propertyName && declaration.propertyName.text
				);
			}
		})
	);
