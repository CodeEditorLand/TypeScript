...
export const HarnessLSCouldNotResolveModule =
	"HarnessLanguageService:: Could not resolve module";
...
export function replaceAll(
	...
	let result: string | undefined = (
		source as string & { replaceAll: typeof source.replace }
...
function handleLoggerGroup(
	logger: Logger,
	host: ts.server.ServerHost,
	logText: (s: string) => void,
	sanitizeLibs: true | undefined,
): Logger {
	...
	function log(s: string) {
		logText(
			(sanitizeLibs ? sanitizeLibFileText : ts.identity)(sanitizeLog(s)),
		);
	}

	function msg(
		s: string,
		type = ts.server.Msg.Err,
		write: (s: string) => void,
	) {
		s = `[${nowString(logger)}] ${s}`;
		if (!inGroup || firstInGroup)
			s = padStringRight(type + " seq", "          ") + s;
...
export function createLoggerWritingToConsole(
	host: ts.server.ServerHost,
	sanitizeLibs?: true,
) {
...
export function sanitizeLog(s: string): string {
	...
	s = s.replace(
		/getCompletionData: Get current token: \d+(?:\.\d+)?/g,
		`getCompletionData: Get current token: *`,
	);
	s = s.replace(
		/getCompletionData: Is inside comment: \d+(?:\.\d+)?/g,
		`getCompletionData: Is inside comment: *`,
	);
	s = s.replace(
		/getCompletionData: Get previous token: \d+(?:\.\d+)?/g,
	...
	s = s.replace(
		/getCompletionData: Semantic work: \d+(?:\.\d+)?/g,
	...
	s = s.replace(
		/getExportInfoMap: done in \d+(?:\.\d+)?/g,
		`getExportInfoMap: done in *`,
	);
	s = s.replace(
		/collectAutoImports: \d+(?:\.\d+)?/g,
		`collectAutoImports: *`,
	);
	s = s.replace(
		/continuePreviousIncompleteResponse: \d+(?:\.\d+)?/g,
		`continuePreviousIncompleteResponse: *`,
	);
	s = s.replace(/dependencies in \d+(?:\.\d+)?/g, `dependencies in *`);
	s = s.replace(/"exportMapKey":\s*"\d+ \d+ /g, (match) =>
		match.replace(/ \d+ /, ` * `),
	);
	s = s.replace(
