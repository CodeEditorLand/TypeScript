...
export function createWatchUtils<PollingWatcherData, FsWatcherData>(
	...
	const pollingWatches =
		initializeWatches<PollingWatcherData>(pollingWatchesName);
	const fsWatches = initializeWatches<FsWatcherData>(fsWatchesName);
	const fsWatchesRecursive = initializeWatches<FsWatcherData>(
		`${fsWatchesName}Recursive`,
	);
	...
	function initializeWatches<Data>(name: string): Watches<Data> {
			...
			if (!actuals.has(path)) {
				...
				if (canonicalPath !== path) {
					const existing =
						canonicalPathsToStrings!.get(canonicalPath);
					if (existing!.size === 1)
						canonicalPathsToStrings!.delete(canonicalPath);
		...
		function forEach(path: string, cb: (data: Data) => void) {
			...
			const canonicalPath = getCanonicalFileName(path);
			if (canonicalPath !== path)
				allData = addRange(allData, actuals.get(canonicalPath));
			canonicalPathsToStrings
				?.get(canonicalPath)
				?.forEach((canonicalSamePath) => {
					if (
						canonicalSamePath !== path &&
						canonicalSamePath !== canonicalPath
					) {
						allData = addRange(
							allData,
							actuals.get(canonicalSamePath),
						);
					}
				});
			allData?.forEach(cb);
	...
	function pollingWatch(path: string, data: PollingWatcherData) {
		return createWatcher(pollingWatches, path, data);
	}
...
export function serializeMultiMap<T>(
	...
	hasChange =
		diffMap(baseline, caption, serialized, multiMap, /*deleted*/ true) ||
		hasChange;
	if (hasChange) {
		serialized = new Map();
		multiMap.forEach((value, key) =>
			serialized!.set(key, new Array(...value)),
		);
	}
	return serialized;
}
...
function diffMap<T>(
	...
	let hasChange = false;
	if (map) {
		for (const key of arrayFrom(map.keys()).sort(
			compareStringsCaseSensitive,
		)) {
			...
			for (const value of values) {
				...
				if (!addedKey) {
					addBaseline(
						`${key}:${deleted || existing ? "" : " *new*"}`,
					);
					addedKey = true;
				}
				addBaseline(
					`  ${JSON.stringify(value)}${deleted || hasExisting || !existing ? "" : " *new*"}`,
				);
			}
		}
	}
