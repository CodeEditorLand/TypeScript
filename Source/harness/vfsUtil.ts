...
export class FileSystem {
		...
		if (axis === "ancestors-or-self" || axis === "ancestors") {
			...
			if (dirname !== path) {
				try {
					...
					if (
						!traversal.traverse ||
						traversal.traverse(dirname, stats)
					) {
						this._scan(
							dirname,
							stats,
			...
			if (
				...
				for (const file of this.readdirSync(path)) {
					try {
						...
						const stats = this._stat(
							this._walk(childpath, noFollow),
						);
						this._scan(
							childpath,
							stats,
	...
	public writeFileSync(
		...
		node.size = !node.buffer.encoding
			? node.buffer.data.byteLength
			: undefined;
		node.mtimeMs = time;
		node.ctimeMs = time;
	}
	...
	private static diffWorker(
		...
		if (changedLinks && !baseLinks)
			return FileSystem.trackCreatedInodes(
				container,
				changed,
				changedLinks,
			);
	...
	private static fileDiff(
		...
		const encoding =
			changedNode.buffer?.encoding ??
			baseNode.buffer?.encoding ??
		...
		if (changedBuffer === baseBuffer) {
			if (
				!options.includeChangedFileWithSameContent ||
				changedNode.mtimeMs === baseNode.mtimeMs
			)
				return false;
			container[basename] = new SameFileWithModifiedTime(
				changedBuffer.data,
		...
		if (
			(!changedBuffer.encoding &&
				!baseBuffer.encoding &&
				Buffer.compare(changedBuffer.data, baseBuffer.data) === 0) || // same buffer content
			(changedBuffer.encoding === baseBuffer.encoding &&
				changedBuffer.data === baseBuffer.data) // same string content
		) {
			if (!options.includeChangedFileWithSameContent) return false;
			container[basename] = new SameFileContentFile(changedBuffer.data, {
				encoding: changedBuffer.encoding,
			});
			return true;
		}

		container[basename] = new File(changedBuffer.data, {
			encoding: changedBuffer.encoding,
		});
		return true;
	}
	...
	private static trackCreatedInode(
		...
		if (isDirectory(node)) {
			...
			container[basename] = new File(buffer.data, {
				encoding: buffer.encoding ?? undefined,
			});
		}
		return true;
	}
	...
	private _getShadow(root: Inode) {
		const shadows =
			this._lazy.shadows ||
			(this._lazy.shadows = new Map<number, Inode>());
	...
	private _getBuffer(
		...
		if (!node.buffer) {
			...
			if (source && resolver) {
				...
				node.buffer = this._shadowRoot._getBuffer(
					node.shadowRoot,
					encoding,
				);
			} else {
	...
	private _resolve(path: string) {
		return this._cwd
			...
			: vpath.validate(
					path,
					vpath.ValidationFlags.Absolute |
						vpath.ValidationFlags.AllowWildcard,
				);
	}
...
export function createFromFileSystem(
	...
	if (documents) {
		for (const document of documents) {
			...
			if (symlink) {
				for (const link of symlink
					.split(",")
					.map((link) => link.trim())) {
					fs.mkdirpSync(vpath.dirname(link));
					fs.symlinkSync(
						vpath.resolve(fs.cwd(), document.file),
						link,
					);
				}
			}
...
export class File {
	...
	constructor(
		data: Buffer | string,
		{
			meta,
			encoding,
		}: { encoding?: string; meta?: Record<string, any> } = {},
	) {
		this.data = data;
...
type FileDataBuffer =
	| { encoding?: undefined; data: Buffer }
	| { encoding: BufferEncoding; data: string };

function ensureBufferEncoding(
	fileBuffer: FileDataBuffer,
	encoding: BufferEncoding | undefined,
) {
	if (fileBuffer.encoding === encoding) return;

	const buffer = !fileBuffer.encoding
		? fileBuffer.data
		: ts.sys.bufferFrom!(fileBuffer.data, fileBuffer.encoding);

	fileBuffer.encoding = encoding;
	fileBuffer.data = !encoding
		? (buffer as Buffer)
