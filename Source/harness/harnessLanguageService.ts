...
class SessionServerHost implements ts.server.ServerHost {
	...
	watchUtils = createWatchUtils<
		ServerHostFileWatcher,
		ServerHostDirectoryWatcher
	>(
		"watchedFiles",
		"watchedDirectories",
		ts.createGetCanonicalFileName(this.useCaseSensitiveFileNames),
					...
					module: () => ({
						create(info: ts.server.PluginCreateInfo) {
							...
							proxy.getQuickInfoAtPosition = function () {
								// eslint-disable-next-line prefer-spread, prefer-rest-params
								const parts =
									langSvc.getQuickInfoAtPosition.apply(
										langSvc,
										arguments,
									);
...
class FourslashSession extends ts.server.Session {
	constructor(
		opts: ts.server.SessionOptions,
		readonly baselineHost: (when: string) => void,
	) {
		super(opts);
	}
...
export class ServerLanguageServiceAdapter implements LanguageServiceAdapter {
	...
	constructor(
		...
		this.logger = createLoggerWithInMemoryLogs(
			serverHost,
			/*sanitizeLibs*/ true,
		);
