...
namespace SourceMapSpanWriter {
	...
	export function initializeSourceMapSpanWriter(
		...
		sourceMapRecorder.WriteLine(
			"mapUrl: " +
				ts.tryGetSourceMappingURL(
					ts.getLineInfo(jsFile.text, jsLineMap),
				),
		);
	...
	function getSourceMapSpanString(
		...
		if (ts.isSourceMapping(mapEntry)) {
			...
			if (
				...
				mapString +=
					" name (" + sourceMapNames![mapEntry.nameIndex!] + ")";
			} else {
				if (
	...
	export function recordNewSourceFileSpan(
		...
		sourceMapRecorder.WriteLine(
			"sourceFile:" +
				sourceMapSources[
					spansOnSingleLine[0].sourceMapSpan.sourceIndex!
				],
		);
		sourceMapRecorder.WriteLine(
			"-------------------------------------------------------------------",
	...
	function writeRecordedSpans() {
		...
		function writeSourceMapSourceText(
			...
			for (let i = 0; i < tsCodeLineMap.length; i++) {
				...
				sourceMapRecorder.Write(
					getTextOfLine(i, tsCodeLineMap, sourceText),
				);
		...
		function writeSpanDetails(
			...
			sourceMapRecorder.WriteLine(
				markerIds[index] +
					getSourceMapSpanString(currentSpan.sourceMapSpan),
			);
		}
