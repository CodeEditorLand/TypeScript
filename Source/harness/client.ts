...
export class SessionClient implements LanguageService {
			...
			entries: response.body!.entries.map<CompletionEntry>((entry) => {
				...
				if (entry.replacementSpan !== undefined) {
					const res: CompletionEntry = {
						...
						replacementSpan: this.decodeSpan(
	...
	getDefinitionAndBoundSpan(
		...
		const request =
			this.processRequest<protocol.DefinitionAndBoundSpanRequest>(
	...
	getSourceDefinitionAndBoundSpan(
		...
		const args: protocol.FileLocationRequestArgs =
			this.createFileLocationRequestArgs(fileName, position);
		const request =
			this.processRequest<protocol.FindSourceDefinitionRequest>(
	...
	private getDiagnostics(
		...
		const fakeSourceFile = {
			fileName: file,
			text: sourceText,
		} as SourceFile; // Warning! This is a huge lie!
	...
	findRenameLocations(
		...
		if (
			...
			if (
				providePrefixAndSuffixTextForRename !== undefined ||
				quotePreference !== undefined
			) {
				const oldPreferences = this.preferences;
	...
	private decodeNavigationTree(
		...
		return {
			...
			nameSpan:
				tree.nameSpan &&
				this.decodeSpan(tree.nameSpan, fileName, lineMap),
	...
	getNavigationTree(file: string): NavigationTree {
		...
		const response =
			this.processResponse<protocol.NavTreeResponse>(request);
	...
	getCodeFixesAtPosition(
		...
		const response =
			this.processResponse<protocol.CodeFixResponse>(request);
			...
			return {
				...
				displayParts: displayParts?.map(({ text, span }) => ({
					...
					span: span && {
						...
						length:
							this.lineOffsetToPosition(span.file, span.end) -
	...
	getApplicableRefactors(
		...
		const args: protocol.GetApplicableRefactorsRequestArgs =
			this.createFileLocationOrRangeRequestArgs(
				positionOrRange,
				fileName,
			);
		...
		const response =
			this.processResponse<protocol.GetApplicableRefactorsResponse>(
				request,
			);
		if (preferences) {
			// Restore preferences
	...
	getMoveToRefactoringFileSuggestions(
		...
		const request =
			this.processRequest<protocol.GetMoveToRefactoringFileSuggestionsRequest>(
				protocol.CommandTypes.GetMoveToRefactoringFileSuggestions,
				args,
			);
		const response =
			this.processResponse<protocol.GetMoveToRefactoringFileSuggestions>(
				request,
			);
		return {
			newFileName: response.body?.newFileName,
	...
	getEditsForRefactor(
		...
		const request =
			this.processRequest<protocol.GetEditsForRefactorRequest>(
	...
	prepareCallHierarchy(
		...
		const args = this.createFileLocationRequestArgs(fileName, position);
		const request =
			this.processRequest<protocol.PrepareCallHierarchyRequest>(
				protocol.CommandTypes.PrepareCallHierarchy,
				args,
			);
		const response =
			this.processResponse<protocol.PrepareCallHierarchyResponse>(
				request,
			);
		return (
			response.body &&
