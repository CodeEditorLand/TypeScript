...
export class CompilationResult {
	...
	constructor(
		...
		>({ comparer: this.vfs.stringComparer, sort: "insertion" }));
						...
						if (!vpath.isDeclaration(sourceFile.fileName)) {
							...
							const outputs: CompilationOutput = {
								...
								dts: dts.get(
									this.getOutputPath(
										sourceFile.fileName,
										ts.getDeclarationEmitExtensionForPath(
											sourceFile.fileName,
										),
									),
								),
	...
	public getSourceMap(path: string): documents.SourceMap | undefined {
		...
		if (this.options.sourceMap) {
			...
			return (
				document &&
				new documents.SourceMap(document.file, document.text)
			);
		}
	}
