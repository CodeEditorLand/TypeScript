...
function convertDocumentSpanToString<T extends ts.DocumentSpan>(
	...
	let text = prefix || "";
	for (const p in span) {
		if (p === "textSpan" || p === "fileName" || p === "contextSpan")
			continue;
		if (ts.contains(ignoredProperties, p)) continue;
		if (ts.hasProperty(span, p) && !!span[p]) {
			// Serialize truthy properties
...
interface BaselineDocumentSpansWithFileContentsOptions<
	T extends ts.DocumentSpan,
> {
...
export class TestCancellationToken implements ts.HostCancellationToken {
	...
	public resetCancelled(): void {
		this.numberOfCallsBeforeCancellation =
			TestCancellationToken.notCanceled;
	}
}
...
export class TestState {
	...
	constructor(
		...
		for (const file of testData.files) {
			...
			if (
				!startResolveFileRef &&
				file.fileOptions[MetadataOptionNames.resolveReference] ===
					"true"
			) {
		...
		if (configFileName) {
			const baseDir = ts.normalizePath(
		...
		if (compilationOptions.typeRoots) {
			compilationOptions.typeRoots = compilationOptions.typeRoots.map(
				(p) => ts.getNormalizedAbsolutePath(p, this.basePath),
			);
		}
		...
		if (startResolveFileRef) {
			...
			const resolvedResult =
				languageServiceAdapter.getPreProcessedFileInfo(
					startResolveFileRef.fileName,
					startResolveFileRef.content,
				);
			...
			const exts = ts.flatten(
				ts.getSupportedExtensions(compilationOptions),
			);
			...
			ts.forEach(importedFiles, (importedFile) => {
				...
				const importedFilePath =
					this.basePath + "/" + importedFile.fileName;
		...
		for (const file of testData.files) {
			ts.forEach(file.symlinks, (link) => {
				this.languageServiceAdapterHost.vfs.mkdirpSync(
					vpath.dirname(link),
				);
				this.languageServiceAdapterHost.vfs.symlinkSync(
					file.fileName,
					link,
				);
			});
	...
	private baseline(command: string, actual: string, ext?: string) {
		if (!this.baselineFromTest)
			this.baselineFromTest = [{ command, actual, ext }];
		...
		if (this.baselineFromTest) {
			Harness.Baseline.runBaseline(
				this.getBaselineFileNameForContainingTestFile(
					this.baselineFromTest[0].ext,
				),
				this.baselineFromTest
					.map(
						({ command, actual }) =>
							`// === ${command} ===\n${actual}`,
					)
	...
	public verifyErrorExistsBetweenMarkers(
		...
		if (exists !== shouldExist) {
			...
			throw new Error(
				`${shouldExist ? "Expected" : "Did not expect"} failure between markers: '${startMarkerName}', '${endMarkerName}'`,
			);
		}
	}

	public verifyOrganizeImports(
		newContent: string,
	...
	private messageAtLastKnownMarker(message: string) {
		const locationDescription =
			this.lastKnownMarker !== undefined
	...
	public verifyErrorExistsAfterMarker(
		...
		if (after) {
			predicate = (
				...
				errorMinChar >= startPos && errorLimChar >= startPos
					? true
					: false;
		} else {
			predicate = (
	...
	public verifyNoErrors() {
		ts.forEachKey(this.inputFiles, (fileName) => {
			...
			if (errors.length) {
				...
				this.raiseError(
					`Found an error: ${this.formatPosition(error.file!, error.start!)}: ${message}`,
				);
			}
		});
	}
	...
	public verifyErrorExistsAtRange(
		...
		if (!hasMatchingError) {
			this.raiseError(
				`No error with code ${code} found at provided range.`,
			);
		}
	}
	...
	public verifyEval(expr: string, value: any) {
		const emit = this.languageService.getEmitOutput(
			this.activeFile.fileName,
		);
		if (emit.outputFiles.length !== 1) {
			throw new Error(
				"Expected exactly one output from emit of " +
					this.activeFile.fileName,
			);
		}
	...
	private renderMarkers(
		...
		function bold(text: string) {
			return useTerminalBoldSequence
				? `\x1b[1;4m${text}\x1b[0;31m`
				: text;
		}
	}
	...
	private baselineGoToDefs(
		...
		const defIdMap = new Map<
			ts.DefinitionInfo | ts.ImplementationLocation,
			number
		>();
	...
	public baselineGoToDefinition(
		...
		this.baselineEachMarkerOrRange(
			...
			rangeText,
			(markerOrRange) =>
				this.baselineGoToDefs("/*GOTO DEF*/", markerOrRange, () =>
	...
	public baselineGetDefinitionAtPosition(
		...
		this.baselineEachMarkerOrRange(
			...
			rangeText,
			(markerOrRange) =>
				this.baselineGoToDefs("/*GOTO DEF POS*/", markerOrRange, () =>
	...
	public baselineGoToSourceDefinition(
		...
		if (this.testType !== FourSlashTestType.Server) {
			this.raiseError(
				"goToSourceDefinition may only be used in fourslash/server tests.",
			);
		}
		this.baselineEachMarkerOrRange(
	...
	public baselineGoToType(
		...
		this.baselineEachMarkerOrRange(
			...
			rangeText,
			(markerOrRange) =>
				this.baselineGoToDefs("/*GOTO TYPE*/", markerOrRange, () =>
	...
	public baselineGoToImplementation(
		...
		this.baselineEachMarkerOrRange(
			...
			rangeText,
			(markerOrRange) =>
				this.baselineGoToDefs("/*GOTO IMPL*/", markerOrRange, () =>
	...
	public baselineInlayHints(
		span: ts.TextSpan = {
			start: 0,
			length: this.activeFile.content.length,
		},
		preferences?: ts.UserPreferences,
	): void {
		...
		if (options.marker === undefined) {
			...
			if (ts.isArray(options.marker)) {
				...
				return {
					andApplyCodeAction: () => {
						this.raiseError(
							`Cannot apply code action when multiple markers are specified.`,
						);
					},
				};
			}
			this.goToMarker(options.marker);
			return this.verifyCompletionsWorker({
				...options,
	...
	private verifyCompletionsWorker(
		...
		const nameAndSourceToData = new Map<
			string,
			ts.CompletionEntryData | false
		>();
				...
				if (
					entries.some(
						(e) =>
							...
							e.data?.ambientModuleName ===
								entry.data?.ambientModuleName,
					)
				) {
		...
		if (ts.hasProperty(options, "exact")) {
			...
			for (const expectedEntry of options.unsorted) {
				...
				if (!found)
					throw this.raiseError(
						`Unsorted: completion '${name}' not found.`,
					);
				if (!found.length)
					throw this.raiseError(
			...
			if (options.includes) {
				for (const include of toArray(options.includes)) {
					const name =
						typeof include === "string" ? include : include.name;
					const found = nameToEntries.get(name);
					if (!found)
						throw this.raiseError(
		...
		return {
			andApplyCodeAction: (options: {
				...
				const data = nameAndSourceToData.get(
				...
				if (data === undefined) {
					this.raiseError(
						`No completion entry found for '${options.name}' from '${options.source}'`,
					);
				}
	...
	private verifyCompletionEntry(
		...
		if (actual.insertText !== expected.insertText) {
			this.raiseError(
				`At entry ${actual.name}: Completion insert text did not match: ${showTextDiff(expected.insertText || "", actual.insertText || "")}`,
			);
		}
		...
		} else if (ts.hasProperty(expected, "replacementSpan")) {
			// Expected `replacementSpan` is explicitly set as `undefined`.
			assert.equal(
				actual.replacementSpan,
				undefined,
		...
		if (
			...
			expected.kindModifiers !== undefined
		) {
			assert.equal(
				actual.kind,
				expected.kind,
		...
		assert.equal(
			actual.source,
			expected.source,
			`At entry ${actual.name}: Expected 'source' values to match`,
		);
		assert.equal(
			actual.sortText,
		...
		if (expected.text !== undefined) {
			...
			assert.equal(
				actualDetails.source &&
					ts.displayPartsToString(actualDetails.source),
				expected.sourceDisplay,
			...
			assert(
				expected.documentation === undefined &&
	...
	private verifyCompletionsAreExactly(
		...
		ts.zipWith(
			actual,
			expected,
			(completion, expectedCompletion, index) => {
				const name =
	...
	public setTypesRegistry(map: ts.MapLike<void>): void {
		this.languageServiceAdapterHost.typesRegistry = new Map(
			Object.entries(map),
		);
	}
	...
	private baselineEachMarkerOrRange(
		...
		if (rangeText !== undefined) {
			toArray(rangeText).forEach(
				(text) =>
					(done =
						this.baselineArray(
							command,
							this.rangesByText().get(text)!,
							worker,
						) || done),
			);
		}
	...
	private baselineArray<T>(
		...
		array.forEach((single) =>
			this.baseline(command, worker(single), ".baseline.jsonc"),
		);
		return !!array.length;
	}
	...
	public baselineFindAllReferences(
		...
		this.baselineEachMarkerOrRange(
			...
			(markerOrRange) => {
				...
				const defIdMap = new Map<
					| ts.ReferencedSymbolDefinitionInfo
	...
	public baselineGetFileReferences(fileNames: string[]) {
		this.baselineArray("getFileReferences", fileNames, (fileName) => {
			...
			return (
				`// fileName: ${fileName}\n\n` +
				this.getBaselineForDocumentSpansWithFileContents(references, {
			...
			markerInfo !== undefined
				? ts.isString(markerInfo.markerOrRange)
					...
					: isMarker(markerInfo.markerOrRange)
						...
						: {
								fileName: markerInfo.markerOrRange.fileName,
								position: markerInfo.markerOrRange.pos,
							}
				: undefined;
		...
		for (const group of spansByFile) {
			if (group.length) {
				...
				if (contentOfFile !== undefined) {
					fileBaselines.push(
						this.getBaselineContentForFile(
							group[0].fileName,
							contentOfFile,
							group,
							marker,
					...
					foundAdditionalSpan ||=
						!!additionalSpan &&
						additionalSpan.fileName === group[0].fileName;
				} else {
	...
	private getBaselineContentForFile<T extends ts.DocumentSpan>(
		...
		if (fileName === marker?.fileName)
			details.push({
				...
				locationMarker: markerInfo!.markerName,
			});
		...
		for (const span of group) {
			...
			if (span.contextSpan) {
				details.push({
					location: span.contextSpan.start,
					locationMarker: "<|",
					span,
					type: "contextStart",
				});
				if (
					canDetermineContextIdInline &&
					span.contextSpan.start > span.textSpan.start
				) {
			...
			details.push(
				{
					location: span.textSpan.start,
					locationMarker: "[|",
					span,
					type: "textStart",
				},
				{
					location: textSpanEnd,
			...
			if (
				...
				ts.documentSpansEqual(
					additionalSpan,
					span,
					this.languageServiceAdapterHost.useCaseSensitiveFileNames(),
				)
			) {
			...
			if (startPrefix) {
				if (
					...
					span.textSpan.start === marker?.position
				) {
					ts.Debug.assert(
						!detailPrefixes.has(details[0]),
						"Expected only single prefix at marker location",
					);
			...
			if (endSuffix) {
				if (
					fileName === marker?.fileName &&
					textSpanEnd === marker?.position
				) {
					ts.Debug.assert(
						!detailSuffixes.has(details[0]),
						"Expected only single suffix at marker location",
					);
		...
		let pos = 0;
		const sortedDetails = ts.stableSort(details, (a, b) =>
			ts.compareValues(a.location, b.location),
		);
		...
		sortedDetails.forEach((detail, index) => {
			...
			if (!span && deferredMarkerIndex === undefined) {
				...
				for (
					...
					if (
						sortedDetails[matchingEndPosIndex].location ===
							location &&
				...
				switch (type) {
					case "textStart":
						...
						if (contextId !== undefined) {
							...
							for (
								let textStartIndex = index - 1;
								textStartIndex >= 0;
								textStartIndex--
							) {
								const textStartDetail =
									sortedDetails[textStartIndex];
								if (
									textStartDetail.type === "contextStart" &&
									textStartDetail.span === span
								) {
		...
		function textWithContext(
			location: number | undefined,
			type: Detail["type"],
		) {
			if (!newContent && location === undefined)
				ts.Debug.fail("Unsupported");
			if (type !== "textEnd" && type !== "contextEnd") {
				// Calculate pos to location number of lines
				const posLine =
					posLineInfo?.pos === pos
						? posLineInfo.line
						: ts.computeLineOfPosition(
								lineStarts,
								pos,
								posLineInfo?.line,
							);
				...
				if (location !== undefined)
					posLineInfo = { pos: location, line: locationLine };
				let nLines = 0;
				if (newContent) nLines += TestState.nLinesContext + 1;
				if (location !== undefined)
					nLines += TestState.nLinesContext + 1;
				...
				if (locationLine - posLine > nLines) {
							...
							readableJsoncBaseline(
								newContent +
									content.slice(
										pos,
										lineStarts[
											posLine + TestState.nLinesContext
										],
									) +
					...
					if (location !== undefined) {
						newContent +=
							...
							content.slice(
								lineStarts[
									locationLine - TestState.nLinesContext + 1
								],
								location,
							);
					}
	...
	private assertObjectsEqual<T>(
		...
		const recur = <U>(actual: U, expected: U, path: string) => {
			const fail = (msg: string) => {
				this.raiseError(
					`${msgPrefix} At ${path}: ${msg} ${displayExpectedAndActualString(stringify(fullExpected), stringify(fullActual))}`,
				);
			};
			...
			for (const key in actual) {
				if (ts.hasProperty(actual as any, key)) {
					...
					if (typeof ak === "object" && typeof ek === "object") {
						...
						fail(
							`Expected '${key}' to be '${stringify(ek)}', got '${stringify(ak)}'`,
						);
					}
				}
			}
	...
	public getSuggestionDiagnostics(
		...
		this.testDiagnostics(
			expected,
			this.languageService.getSuggestionDiagnostics(
				this.activeFile.fileName,
			),
			"suggestion",
		);
			...
			(markerOrRange) => {
				const { fileName, position } = ts.isString(markerOrRange)
					...
					: isMarker(markerOrRange)
						? markerOrRange
						: {
								fileName: markerOrRange.fileName,
								position: markerOrRange.pos,
							};
				const {
					findInStrings = false,
				...
				const renameOptions = options
					? (options.findInStrings !== undefined
							? `// @findInStrings: ${findInStrings}\n`
							: "") +
						(options.findInComments !== undefined
						...
						undefined
							...
							: "") +
						(options.quotePreference !== undefined
				...
				return (
					...
					this.getBaselineForDocumentSpansWithFileContents(
						locations,
						{
							...
							endMarker: "RENAME|]",
							startMarkerPrefix: (span) =>
								span.prefixText
									? `/*START PREFIX*/${span.prefixText}`
									: "",
							endMarkerSuffix: (span) =>
								span.suffixText
									? `${span.suffixText}/*END SUFFIX*/`
									: "",
							ignoredDocumentSpanProperties: [
								"prefixText",
								"suffixText",
	...
	public verifySignatureHelpPresence(
		...
		if (expectPresent !== !!actual) {
			if (actual) {
				...
				this.raiseError(
					"Expected signature help, but none was returned.",
				);
			}
		}
	}
	...
	private verifySignatureHelpWorker(
		...
		const selectedItem =
			help.items[
				options.overrideSelectedItemIndex ?? help.selectedItemIndex
			];
		...
		assert.equal(
			...
			this.assertionMessageAtLastKnownMarker(
				"signature help overloads count",
			),
		);
					...
					selectedItem.parameters
						...
						.join(
							ts.displayPartsToString(
								selectedItem.separatorDisplayParts,
							),
						) +
	...
	private spanLines(
		...
		if (lineNumbers) {
			...
			lineNumberPrefixLength =
				(contextEnd.line + 1).toString().length + 2;
		} else {
			contextString = file.content.substring(
				contextStartPos,
				contextEndPos,
			);
			contextLineMap = ts.computeLineStarts(contextString);
		...
		for (
			...
			output.push(
				lineNumbers
					? `${`${lineNumber + 1}: `.padStart(lineNumberPrefixLength)}${spanLine}`
					: spanLine,
			);
			if (selection) {
				if (
				...
				const selectionLength = isEmpty
					...
					: Math.max(
							lineNumber < selectionEnd.line
								? spanLine.trimRight().length -
										selectionPadLength
								: selectionEnd.character - selectionPadLength,
							1,
						);
				const selectionLine = isEmpty
					? "<"
					: "^".repeat(selectionLength);
	...
	private spanInfoToString(
		...
		if (spanInfo) {
			...
			resultString +=
				...
				") to (" +
				this.getLineColStringAtPosition(
					ts.textSpanEnd(spanInfo),
					file,
				) +
				")";
	...
	private baselineCurrentFileLocations(
		...
		const addSpanInfoString = () => {
			if (previousSpanInfo) {
				...
				let thisLineMarker =
					ts.repeatString(" ", startColumn!) +
					ts.repeatString("~", length!);
		...
		for (; pos < this.activeFile.content.length; pos++) {
			if (pos === 0 || pos === fileLineMap[nextLine]) {
				...
				currentLine =
					...
					">" +
					this.activeFile.content.substring(
						pos,
						fileLineMap[nextLine],
					) +
					"\n    ";
				startColumn = 0;
				length = 0;
			}
			const spanInfo = this.spanInfoToString(
				getSpanAtPos(pos),
				prefixString,
			);
	...
	public verifyGetEmitOutput(expectedOutputFiles: readonly string[]): void {
		...
		for (const { name, text } of outputFiles) {
			...
			if (fromTestFile !== text) {
				this.raiseError(
					`Emit output for ${name} is not as expected: ${showTextDiff(fromTestFile, text)}`,
				);
			}
		}
	}
	...
	public baselineQuickInfo() {
		const result = ts.arrayFrom(
			...
			([name, marker]) => ({
				marker: { ...marker, name },
				item: this.languageService.getQuickInfoAtPosition(
					marker.fileName,
					marker.position,
				),
			}),
			...
			({ displayParts, documentation, tags }) => [
				...
				...(tags?.length
					? tags
							.map(
								(p) =>
									`@${p.name} ${p.text?.map((dp) => dp.text).join("") ?? ""}`,
							)
							.join("\n")
							.split("\n")
					: []),
	...
	public baselineSignatureHelp() {
		const result = ts.arrayFrom(
			...
			([name, marker]) => ({
				marker: { ...marker, name },
				item: this.languageService.getSignatureHelpItems(
					marker.fileName,
					marker.position,
					/*options*/ undefined,
				),
		...
		const annotations = this.annotateContentWithTooltips(
			...
			(item, previous) => {
				...
				const tooltip = [];
				let signature = "";
				if (prefixDisplayParts.length)
					signature += prefixDisplayParts.map((p) => p.text).join("");
				const separator = separatorDisplayParts
					.map((p) => p.text)
					.join("");
				signature += parameters
					.map((p, i) => {
						const text = p.displayParts
							.map((dp) => dp.text)
							.join("");
						return i === item.argumentIndex
							? "**" + text + "**"
							: text;
					})
					.join(separator);
				if (suffixDisplayParts.length)
					signature += suffixDisplayParts.map((p) => p.text).join("");
				tooltip.push(signature);
				// only display signature documentation on the last argument when multiple arguments are marked
				if (
					previous?.applicableSpan.start !== item.applicableSpan.start
				) {
				...
				for (const tag of entry.tags ?? ts.emptyArray) {
					for (const part of tag.text ?? ts.emptyArray) {
						if (part.kind === "linkName") {
							...
							if (
								/lib(?:.*)\.d\.ts$/.test(link.target.fileName)
							) {
	...
	>(
		...
		sorted.sort((q1, q2) =>
			q1.marker.fileName === q1.marker.fileName
				? q1.marker.position > q2.marker.position
					? -1
					: 1
				: q1.marker.fileName > q1.marker.fileName
					? 1
		...
		for (const { marker, item } of sorted) {
			const span = (item ? getSpan(item) : undefined) ?? {
			...
			const underline =
				" ".repeat(startLc.character) + "^".repeat(span.length);
			let tooltip = [
				bar,
				...((item ? getToolTipContents(item, previous) : undefined) ?? [
					`No ${opName} at /*${marker.name}*/.`,
				]),
				bar,
			];
			tooltip = tooltip.map((l) => "| " + l);
			const lines =
				files.get(marker.fileName) ??
				this.getFileContent(marker.fileName).split(/\r?\n/);
		...
		const text = markers
			.map((marker) => {
				...
				while (selectionRange) {
					...
					const leadingWidth = ts
						.arrayFrom(masked)
						.findIndex(isRealCharacter);
	...
	public printBreakpointLocation(pos: number) {
		Harness.IO.log(
			"\n**Pos: " +
				pos +
				" " +
				this.spanInfoToString(
					this.getBreakpointStatementLocation(pos)!,
					"  ",
				),
		);
	}
	...
	public deleteChar(count = 1) {
		...
		for (let i = 0; i < count; i++) {
			...
			if (this.enableFormatting) {
				const edits =
					this.languageService.getFormattingEditsAfterKeystroke(
						this.activeFile.fileName,
						offset,
						ch,
						this.formatCodeSettings,
	...
	public deleteLineRange(startIndex: number, endIndexInclusive: number) {
		const startPos =
			this.languageServiceAdapterHost.lineAndCharacterToPosition(
				this.activeFile.fileName,
				{ line: startIndex, character: 0 },
			);
		const endPos =
			this.languageServiceAdapterHost.lineAndCharacterToPosition(
				this.activeFile.fileName,
				{ line: endIndexInclusive + 1, character: 0 },
			);
	...
	private checkPostEditInvariants() {
		...
		const incrementalSourceFile =
			this.languageService.getNonBoundSourceFile(
				this.activeFile.fileName,
			);
	...
	public setFormatOptions(
		...
		if (this.testType === FourSlashTestType.Server) {
			(
				this.languageService as ts.server.SessionClient
			).setFormattingOptions(this.formatCodeSettings);
		}
	...
	private editScriptAndUpdateMarkers(
		...
		for (const range of this.testData.ranges) {
			if (range.fileName === fileName) {
				range.pos = updatePosition(
					range.pos,
					editStart,
					editEnd,
					newText,
				);
				range.end = updatePosition(
					range.end,
					editStart,
					editEnd,
					newText,
	...
	public verifyCaretAtMarker(markerName = "") {
		...
		if (pos.position !== this.currentCaretPosition) {
			throw new Error(
				`verifyCaretAtMarker failed - expected to be at marker "/*${markerName}*/, but was at position ${this.currentCaretPosition}(${this.getLineColStringAtPosition(this.currentCaretPosition)})`,
			);
		}
	}
	...
	private classificationToIdentifier(classification: number) {
		...
		tokenTypes[ts.classifier.v2020.TokenType.typeParameter] =
			"typeParameter";
			...
			(expectedClassification, actualClassification) => {
				...
				if (expectedSpan) {
					const expectedLength =
						expectedSpan.end - expectedSpan.start;
	...
	public replaceWithSemanticClassifications(
		...
		for (const a of actual) {
			...
			replacement.push(
				`    c2.semanticToken("${identifier}", "${text}"), `,
			);
		}
		replacement.push(");");
	...
	private printOutliningSpansInline(spans: ts.OutliningSpan[]) {
		...
		ts.forEach(reverseSpans, (span) => {
			annotated =
				annotated.slice(0, span.pos) +
				span.text +
				annotated.slice(span.pos);
		});
		Harness.IO.log(`\nMockup:\n${annotated}`);
	}
	...
	private verifyNewContent(
		{
			newFileContent,
			newRangeContent,
		}: FourSlashInterface.NewContentOptions,
		...
		return ts.flatMap(
			...
			(diagnostic) => {
				...
				if (
					...
					if (
						!ts.textRangeContainsPositionInclusive(span, position)
					) {
						return;
					}
				}
		...
		for (const codeFix of codeFixes) {
			...
			for (const textChange of change.textChanges) {
				...
				const deletedText = originalContent.slice(
					span.start,
					span.start + textChange.span.length,
				);
	...
	public verifyBraceCompletionAtPosition(
		...
		const openBraceMap = new Map(
			Object.entries<ts.CharacterCodes>({
	...
	public baselineAutoImports(
		markerName: string,
		fullNamesForCodeFix?: string[],
		preferences?: ts.UserPreferences,
	) {
		const marker = this.getMarkerByName(markerName);
		...
		const autoImportCompletions = completions.entries.filter(
			(c) =>
				c.hasAction &&
				c.source &&
				c.sortText === ts.Completions.SortText.AutoImportSuggestions,
		);
		if (autoImportCompletions.length) {
			...
			autoImportCompletions.forEach((c) => {
				...
				assert(
					details.codeActions[0].changes[0].fileName ===
						this.activeFile.fileName,
				...
				const completionChange: ts.TextChange = {
					...
					span: c.replacementSpan ||
						completions.optionalReplacementSpan || {
							start: marker.position,
							length: 0,
						},
				};
				const sortedChanges = [...changes, completionChange].sort(
					(a, b) => a.span.start - b.span.start,
				);
				let newFileContent = this.activeFile.content;
				for (let i = sortedChanges.length - 1; i >= 0; i--) {
					newFileContent =
						newFileContent.substring(
							0,
							sortedChanges[i].span.start,
						) +
						sortedChanges[i].newText +
						newFileContent.substring(
							sortedChanges[i].span.start +
								sortedChanges[i].span.length,
						);
				}
		...
		if (fullNamesForCodeFix) {
			...
			const range = this.getRangesInFile()[0] ||
				getRangeOfIdentifierTouchingPosition(
			...
			for (const fullNameForCodeFix of fullNamesForCodeFix) {
				this.applyEdits(fileName, [
					{
						span: {
							start: 0,
							length: this.getFileContent(fileName).length,
						},
				...
				this.applyEdits(fileName, [
					{
						span: ts.createTextSpanFromRange(range),
						newText: fullNameForCodeFix,
					},
				]);
	...
	public verifyLinkedEditingRange(map: {
	...
	public baselineLinkedEditing(): void {
		...
		for (const f of files) {
			const result = getLinkedEditingBaselineWorker(
				f,
				offset,
		...
		function getLinkedEditingBaselineWorker(
			...
			for (let pos = 0; pos < activeFile.content.length; pos++) {
				...
				const existingPositions =
					linkedEditsInFile.get(linkedEditString) ?? [];
				linkedEditsInFile.set(linkedEditString, [
					...existingPositions,
					pos,
				]);
			}

			const linkedEditsByRange = [...linkedEditsInFile.entries()].sort(
				(a, b) => a[1][0] - b[1][0],
			);
			...
			inlineLinkedEditBaselines = inlineLinkedEditBaselines.sort(
				(a, b) => a.start - b.start,
			);
			const fileText = activeFile.content;
			baselineContent += fileText.slice(
				0,
				inlineLinkedEditBaselines[0].start,
			);
			...
			this.assertObjectsEqual(
				...
				expected.map(
					(e): ts.NavigateToItem => ({
						...
						containerKind:
							e.containerKind || ts.ScriptElementKind.unknown,
					}),
				),
			);
		}
	...
	public verifyNavigationBar(
		...
		this.verifyNavigationTreeOrBar(
			json,
			this.languageService.getNavigationBarItems(
				this.activeFile.fileName,
			),
			"Bar",
			options,
	...
	public printNavigationBar() {
		...
		for (const item of items) {
			Harness.IO.log(
				`${ts.repeatString(" ", item.indent)}name: ${item.text}, kind: ${item.kind}, childItems: ${item.childItems.map((child) => child.text)}`,
			);
		}
	}
		...
		this.baselineEachMarkerOrRangeArrayOrSingle(
			...
			(markerOrRange) => {
				...
				const highlights = this.getDocumentHighlightsAtCurrentPosition(
					ts.map(options?.filesToSearch, ts.normalizePath) || [
						this.activeFile.fileName,
					],
				);

				// Write input files
				const filesToSearch = options
					? "// filesToSearch:\n" +
						options.filesToSearch
							.map((f) => "//   " + f)
							.join("\n") +
						"\n\n"
					: "";
				const baselineContent =
					this.getBaselineForGroupedDocumentSpansWithFileContents(
						highlights?.map((h) =>
							h.highlightSpans.map((s) =>
								s.fileName
									? (s as ts.DocumentSpan)
	...
	public verifyCodeFixAvailable(
		...
		if (negative) {
			...
			this.assertObjectsEqual(
				actuals,
				negative ? ts.emptyArray : expected,
			);
		}
	}
	...
	public verifyApplicableRefactorAvailableAtMarker(
		...
		const isAvailable =
			this.getApplicableRefactors(this.getMarkerByName(markerName))
				.length > 0;
	...
	public applyRefactor({
		...
		if (action.description !== actionDescription) {
			this.raiseError(
				`Expected action description to be ${JSON.stringify(actionDescription)}, got: ${JSON.stringify(action.description)}`,
			);
		}
		...
		for (const fileName in newFileContents) {
			const { renamePosition: rp, newContent } =
				TestState.parseNewContent(newFileContents[fileName]);
	...
	public moveToFile(options: FourSlashInterface.MoveToFileOptions): void {
		...
		const action = ts.first(refactor.actions);
		assert(
			action.name === "Move to file" &&
				action.description === "Move to file",
		);
	...
	private findFile(indexOrName: string | number): FourSlashFile {
		if (typeof indexOrName === "number") {
			...
			const { file, availableNames } =
				this.tryFindFileWorker(indexOrName);
			if (!file) {
				throw new Error(
	...
	private getApplicableRefactors(
		...
		return this.getApplicableRefactorsWorker(
			"position" in rangeOrMarker
				? rangeOrMarker.position
				: rangeOrMarker,
	...
	public commentSelection(newFileContent: string): void {
		...
		for (const range of this.getRanges()) {
			changes.push(
				...this.languageService.commentSelection(
					this.activeFile.fileName,
					range,
				),
			);
...
export function runFourSlashTestContent(
	...
	if (serverLogBaseliner)
		serverLogBaseliner.baseline = () => state.baselineTsserverLog();
...
function parseTestData(
	...
	for (let line of lines) {
		...
		if (line.substr(0, 4) === "////") {
			...
			const possiblySymlinks =
				Harness.TestCaseParser.parseSymlinkFromTest(line, symlinks);
			if (possiblySymlinks) {
