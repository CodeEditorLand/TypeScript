...
function createNodeIO(): IO {
	...
	function listFiles(
		path: string,
		spec: RegExp | undefined,
		options: { recursive?: boolean } = {},
	) {
		function filesInFolder(folder: string): string[] {
			const { files, directories } =
				ts.sys.getAccessibleFileSystemEntries!(folder);
			let paths: string[] = [];
...
export namespace Compiler {
	...
	export function getDefaultLibrarySourceFile(
		...
		if (!libFileNameSourceFileMap) {
			libFileNameSourceFileMap = new Map(
				Object.entries({
	...
	export function compileFiles(
		...
		if (harnessSettings) {
			...
			if (
				ts.isString(harnessSettings.typeScriptVersion) &&
				harnessSettings.typeScriptVersion
			) {
		...
		const programFileNames = inputFiles
			...
			.filter(
				(fileName) => !ts.fileExtensionIs(fileName, ts.Extension.Json),
			);
		...
		if (options.libFiles) {
			for (const fileName of options.libFiles.split(",")) {
				programFileNames.push(
					vpath.combine(vfs.testLibFolder, fileName),
				);
			}
		}
		...
		ts.assign(
			options,
			ts.convertToOptionsWithAbsolutePaths(options, (path) =>
				ts.getNormalizedAbsolutePath(path, currentDirectory),
			),
		);
	...
	export function prepareDeclarationCompilationContext(
		...
		function addDtsFile(file: TestFile, dtsFiles: TestFile[]) {
			if (
				vpath.isDeclaration(file.unitName) ||
				vpath.isJson(file.unitName)
			) {
				dtsFiles.push(file);
			} else if (
		...
		function findResultCodeFile(fileName: string) {
			...
			if (!outFile) {
				if (options.outDir) {
					...
					sourceFileName = ts.combinePaths(
						options.outDir,
						sourceFilePath,
					);
				} else {
	...
	export function* iterateErrorBaseline(
		...
		function outputErrorText(error: ts.Diagnostic) {
			...
			if (
				!error.file ||
				(!isDefaultLibraryFile(error.file.fileName) &&
					!vpath.isTsConfigFile(error.file.fileName))
			) {
				totalErrorsReportedInNonLibraryNonTsconfigFiles++;
			}
		}

		let topDiagnostics = minimalDiagnosticsToString(
			diagnostics,
		...
		yield [
			...
			topDiagnostics + IO.newLine() + IO.newLine(),
			diagnostics.length,
		];
		...
		for (const inputFile of inputFiles.filter(
				...
				fileErrors.forEach((errDiagnostic) => {
					...
					if (
						...
						const length =
							...
							err.start -
							Math.max(0, thisLineStart - err.start);
						...
						outputLines +=
							...
							"    " +
							line
								.substr(0, squiggleStart)
								.replace(/[^\s]/g, " ") +
							new Array(
								Math.min(length, line.length - squiggleStart) +
									1,
							).join("~");
			...
			const isDupe = dupeCase.has(
				sanitizeTestFilePath(inputFile.unitName),
			);
			yield [
		...
		const numLibraryDiagnostics = ts.countWhere(
			diagnostics,
			(diagnostic) => {
				return (
					!!diagnostic.file &&
		...
		const numTsconfigDiagnostics = ts.countWhere(
			diagnostics,
			(diagnostic) => {
				return (
					!!diagnostic.file &&
					vpath.isTsConfigFile(diagnostic.file.fileName)
				);
			},
		);
	...
	export function doTypeAndSymbolBaseline(
		...
		if (typesError && symbolsError) {
			throw new Error(
				typesError.stack + IO.newLine() + symbolsError.stack,
			);
		}
					...
					if (lastIndexWritten === undefined) {
						...
						if (
							!(
								...
								(codeLines[lastIndexWritten + 1].match(
									/^\s*[{|}]\s*$/,
								) ||
		...
		if (options.inlineSourceMap) {
			...
			if (
				...
				(options.sourceMap
					...
					: 0) +
					(declMaps
						? result.getNumberOfJsFiles(/*includeJson*/ true)
						: 0)
			) {
				throw new Error(
	...
	export function doJsEmitBaseline(
		...
		result.js.forEach((file) => {
			if (
				jsCode.length &&
				jsCode.charCodeAt(jsCode.length - 1) !==
					ts.CharacterCodes.lineFeed
			) {
				jsCode += "\r\n";
			}
			if (
	...
	if (option) {
		...
		if (option.type === "boolean") {
			return (
				...
				(booleanVaryByStarSettingValues = new Map(
					Object.entries({
						true: 1,
						false: 0,
					}),
...
export function getFileBasedTestConfigurations(
	...
	for (const varyByKey of varyBy) {
		if (ts.hasProperty(settings, varyByKey)) {
			...
			const entries = splitVaryBySettingValue(
				settings[varyByKey],
				varyByKey,
			);
			if (entries) {
...
export namespace TestCaseParser {
	...
	export function makeUnitsFromTest(
		...
		for (const line of lines) {
			...
			const possiblySymlinks = parseSymlinkFromTest(
				line,
				symlinks,
				vfs.srcFolder,
			);
		...
		const parseConfigHost: ts.ParseConfigHost = {
					...
					(dir) => {
						...
						for (const unit of testUnitData) {
							...
							if (
								...
								if (path.includes("/")) {
									const directoryName = path.substring(
										0,
										path.indexOf("/"),
									);
						...
						return {
							files,
							directories: ts.arrayFrom(directories),
						};
					},
					ts.identity,
				);
			...
			fileExists: (fileName) =>
				testUnitData.some(
					(data) =>
						data.name.toLowerCase() === fileName.toLowerCase(),
				),
			readFile: (name) =>
...
export namespace Baseline {
	...
	function baselinePath(
		...
		if (subfolder !== undefined) {
			...
			return (
				userSpecifiedRoot + baselineFolder + "/" + type + "/" + fileName
			);
		}
	}
	...
	function writeComparison(
		...
		if (expected !== encodedActual) {
			...
			const errorMessage =
				getBaselineFileChangedErrorMessage(relativeFileName);
			if (!!require && opts && opts.PrintDiff) {
				...
				throw new Error(
					`${errorMessage}${ts.ForegroundColorEscapeSequences.Grey}\n\n${patch}`,
				);
			} else {
	...
	function getBaselineFileChangedErrorMessage(
		relativeFileName: string,
	): string {
	...
	export function runMultifileBaseline(
		...
		if (
			...
			(referencedExtensions &&
				referencedExtensions.includes(".ts") &&
				!referencedExtensions.includes(".d.ts"))
		) {
...
export function isDefaultLibraryFile(filePath: string): boolean {
	...
	return (
		ts.startsWith(fileName, "lib.") &&
		ts.endsWith(fileName, ts.Extension.Dts)
	);
}
...
export function getConfigNameFromFileName(
	...
	return ts.find(
		["tsconfig.json" as const, "jsconfig.json" as const],
		(x) => x === flc,
	);
}
