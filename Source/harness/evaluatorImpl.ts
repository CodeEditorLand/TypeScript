...
const symbolNames = ["asyncIterator", "dispose", "asyncDispose", "metadata"];
	...
	private resolveAsDirectory(dir: string) {
		...
		if (this.isFile(packageFile)) {
			...
			if (json.main) {
				...
				const result =
					this.resolveAsFile(main) || this.resolveIndex(main);
				...
				switch (module.state) {
					...
					case SystemModuleState.Instantiated: {
						...
						for (const dependencyId of dependencies) {
							const dependency = this.load(
								this.resolve(dependencyId, base),
							);
					...
					case SystemModuleState.AllDependenciesAdded: {
						...
						for (const dependency of module.dependencies) {
							if (
								dependency.state ===
								SystemModuleState.Uninstantiated
							) {
								return;
							}
						}
...
type AmdDefineArgsUnnamedModule = [
	dependencies: string[],
	declare: AmdModuleDeclaration,
];
type AmdDefineArgsNamedModuleNoDependencies = [
	id: string,
	declare: AmdModuleDeclaration,
];
type AmdDefineArgsNamedModule = [
	id: string,
	dependencies: string[],
	declare: AmdModuleDeclaration,
];
...
function isAmdDefineArgsUnnamedModule(
	args: AmdDefineArgs,
): args is AmdDefineArgsUnnamedModule {
	return args.length === 2 && Array.isArray(args[0]);
}

function isAmdDefineArgsNamedModuleNoDependencies(
	args: AmdDefineArgs,
): args is AmdDefineArgsNamedModuleNoDependencies {
	return args.length === 2 && typeof args[0] === "string";
}

function isAmdDefineArgsNamedModule(
	args: AmdDefineArgs,
): args is AmdDefineArgsNamedModule {
...
class AmdLoader extends Loader<AmdModule> {
	...
	protected evaluate(text: string, _file: string, module: AmdModule): void {
		...
		const localDefine = (...args: AmdDefineArgs) => {
			if (isAmdDefineArgsUnnamedModuleNoDependencies(args)) {
				...
				isAmdDefineArgsNamedModuleNoDependencies(args) ||
				isAmdDefineArgsNamedModule(args)
			) {
	...
	private instantiateModule(
		module: AmdModule,
		dependencies: string[],
		callback?: AmdModuleDeclaration,
	) {
		try {
			module.requestedDependencies = dependencies;
			module.declaration =
				typeof callback === "function"
					? (callback as AmdModuleDefineCallback)
					: () => callback;
			...
			for (;;) {
				switch (module.state) {
					...
					case AmdModuleState.Instantiated: {
						...
						for (const dependencyId of dependencies) {
							if (
								dependencyId === "require" ||
								dependencyId === "exports" ||
								dependencyId === "module"
							) {
	...
	private evaluateModule(module: AmdModule, stack: AmdModule[]) {
		if (module.state < AmdModuleState.Linked)
			throw new Error("Invalid state for evaluation.");
		...
		try {
			...
			module.module.exports =
				module.declaration?.(...args) ?? module.module.exports;
			module.state = AmdModuleState.Ready;
		} catch (e) {
			this.handleError(module, e);
			throw e;
