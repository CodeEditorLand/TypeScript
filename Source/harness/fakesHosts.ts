	...
	public getDirectories(path: string) {
		...
		try {
			for (const file of this.vfs.readdirSync(path)) {
				if (
					this.vfs.statSync(vpath.combine(path, file)).isDirectory()
				) {
					result.push(file);
				}
			}
...
export class CompilerHost implements ts.CompilerHost {
	...
	public getSourceFile(
		...
		if (cacheKey) {
			...
			while (fs.shadowRoot) {
				try {
					const shadowRootStats = fs.shadowRoot.existsSync(
						canonicalFileName,
					)
...
function expectedDiagnosticMessageChainToText(
	...
	if (next) {
		...
		next.forEach(
			(kid) =>
				(text += expectedDiagnosticMessageChainToText(kid, indent)),
		);
	}
	return text;
}
...
export function patchHostForBuildInfoWrite<T extends ts.System>(
	sys: T,
	version: string,
) {
