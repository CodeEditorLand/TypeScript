...
export function reportDocumentRegistryStats(
		...
		bucketEntries.forEach((entry, path) => {
			if (ts.isDocumentRegistryEntry(entry)) {
				...
				entry.forEach((real, kind) =>
					str.push(
						`    ${path}: ${ts.Debug.formatScriptKind(kind)} ${real.languageServiceRefCount}`,
					),
				);
			}
		});
...
function verifyDocumentRegistryStats(
	...
	documentRegistry.getBuckets().forEach((bucketEntries, key) => {
		...
		bucketEntries.forEach((entry, path) => {
			...
			if (ts.isDocumentRegistryEntry(entry)) {
				ts.Debug.assert(
					expected?.size === 1 &&
						expected.has(entry.sourceFile.scriptKind) &&
						expected.get(entry.sourceFile.scriptKind) ===
	...
	function reportStats() {
		const str: string[] = [
			"",
			"Actual::",
			...reportDocumentRegistryStats(documentRegistry),
		];
		str.push("Expected::");
		stats?.forEach((statsByPath, key) => {
			...
			statsByPath.forEach((entry, path) =>
				entry.forEach((refCount, kind) =>
					str.push(
						`    ${path}: ${ts.Debug.formatScriptKind(kind)} ${refCount}`,
					),
				),
			);
		});
...
function verifyDocumentRegistry(service: ts.server.ProjectService) {
	...
	const collectStats = (project: ts.server.Project) => {
		if (project.autoImportProviderHost)
			collectStats(project.autoImportProviderHost);
		if (project.noDtsResolutionProject)
			collectStats(project.noDtsResolutionProject);
		...
		program.getSourceFiles().forEach((f) => {
			const keyWithMode =
				service.documentRegistry.getDocumentRegistryBucketKeyWithMode(
					key,
					f.impliedNodeFormat,
				);
...
function getResolutionCacheDetails<
	File,
	T extends ts.ResolutionWithFailedLookupLocations,
>(
	baseline: string[],
	cacheType: string,
	file: File,
...
function getResolvedModuleFileName(
	r: ts.ResolvedModuleWithFailedLookupLocations,
) {
	return r.resolvedModule?.resolvedFileName;
}

function getResolvedTypeRefFileName(
	r: ts.ResolvedTypeReferenceDirectiveWithFailedLookupLocations,
) {
...
function getProgramStructure(program: ts.Program | undefined) {
	...
	program
		?.getSourceFiles()
		.slice()
		.sort((f1, f2) => ts.comparePathsCaseSensitive(f1.path, f2.path))
		.forEach((f) => {
			baseline.push(
...
export function verifyProgramStructure(
	...
	ts.Debug.assert(
		actual === expected,
		`Program verification:: ${projectName}`,
	);
}
...
export function verifyResolutionCache(
	...
	type ExpectedResolution = ts.CachedResolvedModuleWithFailedLookupLocations &
		ts.CachedResolvedTypeReferenceDirectiveWithFailedLookupLocations;

	const expectedToResolution = new Map<
		ExpectedResolution,
		ts.ResolutionWithFailedLookupLocations
	>();
	const resolutionToExpected = new Map<
		ts.ResolutionWithFailedLookupLocations,
		ExpectedResolution
	>();
	const resolutionToRefs = new Map<
		ts.ResolutionWithFailedLookupLocations,
		ResolutionInfo[]
	>();
	...
	actual.resolvedLibraries.forEach((resolved, libFileName) => {
		const expectedResolution = collectResolution(
			...
			resolutionHostCacheHost.toPath(
				ts.getInferredLibraryNameResolveFrom(
	...
	resolutionToRefs.forEach((info, resolution) => {
		...
		verifySet(
			resolutionToExpected.get(resolution)!.files,
			resolution.files,
			`${projectName}:: Resolution files`,
		);
	});
	...
	expected.startCachingPerDirectoryResolution();
	actual.resolvedModuleNames.forEach((_resolutions, path) =>
		expected.removeResolutionsOfFile(path),
	);
	actual.resolvedTypeReferenceDirectives.forEach((_resolutions, path) =>
		expected.removeResolutionsOfFile(path),
	);
	expected.finishCachingPerDirectoryResolution(
	...
	resolutionToExpected.forEach((expected) => {
		ts.Debug.assert(
			!expected.refCount,
			`${projectName}:: All the resolution should be released`,
		);
		ts.Debug.assert(
			!expected.files?.size,
			`${projectName}:: Shouldnt ref to any files`,
		);
	});
	ts.Debug.assert(
	...
	function collectResolutionToRefFromCache<
		T extends ts.ResolutionWithFailedLookupLocations,
	>(
		cacheType: string,
		fileName: ts.Path,
		...
		storeExpcted: Map<
			ts.Path,
			ts.ModeAwareCache<ts.ResolutionWithFailedLookupLocations>
		>,
	) {
		ts.Debug.assert(
			actualProgram.getSourceFileByPath(fileName) ||
				ts.endsWith(fileName, ts.inferredTypesContainingFile),
		...
		cache?.forEach((resolved, name, mode) => {
			...
			if (!expectedCache)
				storeExpcted.set(
					fileName,
					(expectedCache = ts.createModeAwareCache()),
				);
	...
	>(
		...
		if (existing) {
			...
			resolutionToRefs.set(resolved, [
				{ cacheType, fileName, name, mode },
			]);
			expectedResolution = {
				resolvedModule: (resolved as any).resolvedModule,
				resolvedTypeReferenceDirective: (resolved as any)
	...
	function verifyMapOfResolutionSet(
		expected:
			| Map<ts.Path, Set<ts.ResolutionWithFailedLookupLocations>>
			| undefined,
		actual:
			| Map<ts.Path, Set<ts.ResolutionWithFailedLookupLocations>>
			| undefined,
	...
	function verifyResolutionSet(
		...
		expected?.forEach((resolution) =>
			ts.Debug.assert(
				actual?.has(
					expectedToResolution.get(resolution as ExpectedResolution)!,
				),
	...
	function verifyDirectoryWatchesOfFailedLookups(
		...
		verifyMap(
			expected,
			actual,
			(expected, actual, caption) => {
				ts.Debug.assert(
					expected?.refCount === actual?.refCount,
					`${projectName}:: ${caption}:: refCount`,
				);
				ts.Debug.assert(
					!!expected?.refCount,
					`${projectName}:: ${caption}:: expected refCount to be non zero`,
				);
				ts.Debug.assert(
	...
	function verifyFileWatcherOfAffectingLocation(
		...
		ts.Debug.assert(
			expected?.resolutions === actual?.resolutions,
			`${projectName}:: ${caption}:: resolutions`,
		);
		ts.Debug.assert(
			expected?.files === actual?.files,
			`${projectName}:: ${caption}:: files`,
		);
		verifySet(
			expected?.symlinks,
			actual?.symlinks,
			`${projectName}:: ${caption}:: symlinks`,
		);
	}
}
...
function verifyMap<Key extends string, Expected, Actual>(
	...
	expected?.forEach((expected, path) =>
		verifyValue(expected, actual?.get(path), `${caption}:: ${path}`),
	);
	actual?.forEach((actual, path) =>
		verifyValue(expected?.get(path), actual, `${caption}:: ${path}`),
	);
}
...
function verifyProgram(
	...
	const compilerHost = ts.createCompilerHostWorker(
		options,
		/*setParentNodes*/ undefined,
		service.host,
	);
	compilerHost.useSourceOfProjectReferenceRedirect =
		project.useSourceOfProjectReferenceRedirect?.bind(project);
	compilerHost.getCurrentDirectory =
		project.getCurrentDirectory.bind(project);
	...
	compilerHost.readFile = (fileName) => {
		...
		let fileSize: number;
		if (service.host.getFileSize)
			fileSize = service.host.getFileSize(fileName);
		else {
	...
	const resolutionHostCacheHost: ts.ResolutionCacheHost = {
		...
		globalCacheResolutionModuleName:
			project.globalCacheResolutionModuleName.bind(project),
...
function beforeResolveSingleModuleNameWithoutWatching(
	...
	const resolutionToData: ResolveSingleModuleNameWithoutWatchingData["resolutionToData"] =
		new Map();
	// Currently it doesnt matter if moduleResolutionCache itself changes or not so just verify resolutions:
	moduleResolutionCache.directoryToModuleNameMap
		.getOwnMap()
		.forEach((cache) => {
			cache.forEach((resolution) => {
				...
				resolutionToData.set(resolution, {
					failedLookupLocations:
						resolution.failedLookupLocations?.slice(),
					affectingLocations: resolution.affectingLocations?.slice(),
					resolutionDiagnostics:
						resolution.resolutionDiagnostics?.slice(),
				});
			});
		});
...
function afterResolveSingleModuleNameWithoutWatching(
	...
	if (existing) {
		verifyArrayLength(
			existing.failedLookupLocations,
			result.failedLookupLocations,
			"failedLookupLocations",
		);
		verifyArrayLength(
			existing.affectingLocations,
			result.affectingLocations,
			"affectingLocations",
		);
		verifyArrayLength(
			existing.resolutionDiagnostics,
	...
	verifyMap(
		...
		(expected, actual, caption) =>
			ts.Debug.assert(expected === actual, caption),
		`Expected packageJsonInfo to not change: ${moduleName} ${containingFile}`,
	);

	function verifyArrayLength<T>(
...
function onProjectCreation(project: ts.server.Project) {
	...
	(
		project as ts.ResolutionCacheHost
...
export function incrementalVerifier(service: ts.server.ProjectService) {
	service.verifyDocumentRegistry = withIncrementalVerifierCallbacks(
		service,
		verifyDocumentRegistry,
	);
	service.verifyProgram = withIncrementalVerifierCallbacks(
		service,
		verifyProgram,
	);
...
function withIncrementalVerifierCallbacks(
	...
	return (...args: any[]) => {
		const data = (
			service.host as IncrementalVerifierCallbacks
		).beforeVerification?.();
		callback(service, ...args);
		(service.host as IncrementalVerifierCallbacks).afterVerification?.(
			data,
		);
	};
}
