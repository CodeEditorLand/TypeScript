...
export abstract class TypingsInstaller {
	...
	installPackage(req: InstallPackageRequest) {
		...
		const cwd =
			forEachAncestorDirectory(
				...
				(directory) => {
					if (
						this.installTypingHost.fileExists(
	...
	private processCacheLocation(cacheLocation: string) {
		...
		const packageLockJson = combinePaths(
			cacheLocation,
			"package-lock.json",
		);
		if (this.log.isEnabled()) {
		...
		if (
			...
			if (this.log.isEnabled()) {
				this.log.writeLine(
					`Loaded content of '${packageJson}':${stringifyIndented(npmConfig)}`,
				);
				this.log.writeLine(
					`Loaded content of '${packageLockJson}':${stringifyIndented(npmLock)}`,
				);
			}
			if (npmConfig.devDependencies && npmLock.dependencies) {
				for (const key in npmConfig.devDependencies) {
					...
					this.packageNameToTypingLocation.set(
						packageName,
						newTyping,
					);
				}
	...
	protected ensurePackageDirectoryExists(directory: string) {
		...
		if (!this.installTypingHost.fileExists(npmConfigPath)) {
			...
			this.installTypingHost.writeFile(
				npmConfigPath,
				'{ "private": true }',
			);
		}
	...
	private installTypings(
		...
		if (filteredTypings.length === 0) {
			...
			this.sendResponse(
				this.createSetTypings(req, currentlyCachedTypings),
			);
			return;
		}
			...
			try {
				...
				for (const packageName of filteredTypings) {
					...
					const newVersion = new Version(
						distTags[`ts${versionMajorMinor}`] ||
							distTags[this.latestDistTag],
					);
					const newTyping: JsTyping.CachedTyping = {
						typingLocation: typingFile,
						version: newVersion,
	...
	private watchFiles(projectName: string, files: string[]) {
		...
		if (
			!existing ||
			forEachKey(newSet, (s) => !existing.has(s)) ||
			forEachKey(existing, (s) => !newSet.has(s))
		) {
