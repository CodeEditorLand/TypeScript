	...
	if (errorCount === 1) {
		messageAndArgs =
			filesInError[0] !== undefined
				? [Diagnostics.Found_1_error_in_0, firstFileReference!]
				: [Diagnostics.Found_1_error];
	} else {
		messageAndArgs =
			distinctFileNamesWithLines.length === 0
				? [Diagnostics.Found_0_errors, errorCount]
...
function createTabularErrorsDisplay(
	...
	const distinctFiles = filesInError.filter(
		(value, index, self) =>
			index ===
			self.findIndex((file) => file?.fileName === value?.fileName),
	);
	...
	fileToErrorCount.forEach((row) => {
		...
		const fileRef = prettyPathForFileError(
			file!,
...
export function explainFiles(program: Program, write: (s: string) => void) {
	...
	for (const file of program.getSourceFiles()) {
		...
		reasons
			...
			?.forEach((reason) =>
				write(
					`  ${fileIncludeReasonToDiagnostics(program, reason, relativeFileName).messageText}`,
				),
			);
		explainIfFileIsRedirectAndImpliedFormat(
			file,
			relativeFileName,
		)?.forEach((d) => write(`  ${d.messageText}`));
	}
}
			...
			case ModuleKind.CommonJS:
				if (file.packageJsonScope) {
					(result ??= []).push(
						chainDiagnosticMessages(
							/*details*/ undefined,
							file.packageJsonScope.contents.packageJsonContent
								.type
...
export function getMatchedFileSpec(program: Program, fileName: string) {
	...
	return find(
		...
		(fileSpec) =>
			program.getCanonicalFileName(
				getNormalizedAbsolutePath(fileSpec, basePath),
			) === filePath,
	);
}
...
export function fileIncludeReasonToDiagnostics(
		...
		switch (reason.kind) {
			case FileIncludeKind.Import:
				if (isReferenceFileLocation(referenceLocation)) {
					...
					referenceLocation.text === externalHelpersModuleNameText
				) {
					message = referenceLocation.packageId
	...
	switch (reason.kind) {
		...
		case FileIncludeKind.LibFile: {
			...
			const messageAndArgs: DiagnosticAndArguments = target
				? [Diagnostics.Default_library_for_target_0, target]
...
export const WatchType: WatchTypeRegistry = {
	...
	NoopConfigFileForInferredRoot:
		"Noop Config file for the inferred project root",
	MissingGeneratedFile: "Missing generated file",
	NodeModulesForModuleSpecifierCache:
		"node_modules for module specifier cache invalidation",
...
/** @internal */
export interface WatchFactoryWithLog<X, Y = undefined>
	extends WatchFactory<X, Y> {
...
export function createWatchFactory<Y = undefined>(
	...
	const result = getWatchFactory<WatchType, Y>(
		...
		watchLogLevel,
		writeLog,
	) as WatchFactoryWithLog<WatchType, Y>;
...
/** @internal */
export function getSourceFileVersionAsHashFromText(
	host: Pick<CompilerHost, "createHash">,
	text: string,
) {
...
export function setGetSourceFileAsHashVersioned(compilerHost: CompilerHost) {
	...
	compilerHost.getSourceFile = (...args) => {
		...
		if (result) {
			result.version = getSourceFileVersionAsHashFromText(
				compilerHost,
				result.text,
			);
		}
...
>(system: System, createProgram: CreateProgram<T> | undefined): ProgramHost<T> {
	...
	return {
		...
		getEnvironmentVariable: maybeBind(
			system,
		...
		storeFilesChangingSignatureDuringEmit:
			system.storeFilesChangingSignatureDuringEmit,
