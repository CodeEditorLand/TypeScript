	...
	createBaseSourceFileNode: (kind) =>
		new (SourceFileConstructor ||
			(SourceFileConstructor =
				objectAllocator.getSourceFileConstructor()))(kind, -1, -1),
...
const forEachChildTable: ForEachChildTable = {
	...
	[SyntaxKind.PropertyDeclaration]:
		function forEachChildInPropertyDeclaration<T>(
	...
	[SyntaxKind.VariableDeclaration]:
		function forEachChildInVariableDeclaration<T>(
	...
	[SyntaxKind.FunctionDeclaration]:
		function forEachChildInFunctionDeclaration<T>(
	...
	[SyntaxKind.SatisfiesExpression]:
		function forEachChildInSatisfiesExpression<T>(
	...
	[SyntaxKind.ExpressionStatement]:
		function forEachChildInExpressionStatement<T>(
	...
	[SyntaxKind.TemplateLiteralType]:
		function forEachChildInTemplateLiteralType<T>(
	...
	[SyntaxKind.CommaListExpression]:
		function forEachChildInCommaListExpression<T>(
...
namespace Parser {
	...
	var disallowInAndDecoratorContext =
	...
	function reparseTopLevelAwait(sourceFile: SourceFile) {
		...
		const baseSyntaxCursor =
			IncrementalParser.createSyntaxCursor(sourceFile);
	...
	function parseParameterWorker(
		...
		const modifiers = inOuterAwaitContext
			...
			: doOutsideOfAwaitContext(() =>
					parseModifiers(/*allowDecorators*/ true),
				);
	...
	function parsePropertyOrMethodSignature(
		...
		if (
			...
			if (token() === SyntaxKind.EqualsToken)
				(node as Mutable<PropertySignature>).initializer =
					parseInitializer();
		}
		parseTypeMemberSemicolon();
		return withJSDoc(finishNode(node, pos), hasJSDoc);
	...
	function parseJsxElementOrSelfClosingElementOrFragment(
		...
		if (opening.kind === SyntaxKind.JsxOpeningElement) {
			...
			const lastChild: JsxChild | undefined =
	...
	function parsePropertyAccessExpressionRest(
		...
		if (
			...
			const end =
				skipTrivia(sourceText, expression.typeArguments.end) + 1;
			parseErrorAt(
				pos,
				end,
			...
			if (typeArguments || token() === SyntaxKind.OpenParenToken) {
				...
				if (
					...
					typeArguments = (expression as ExpressionWithTypeArguments)
						.typeArguments;
					expression = (expression as ExpressionWithTypeArguments)
						.expression;
				}
		...
		switch (token()) {
			...
			default:
				if (modifiersIn) {
					...
					(missing as Mutable<MissingDeclaration>).modifiers =
						modifiersIn;
					return missing;
				}
	...
	function parseVariableDeclaration(
		...
		if (
			...
			exclamationToken =
				parseTokenNode<Token<SyntaxKind.ExclamationToken>>();
		}
		const type = parseTypeAnnotation();
	...
	function tryParseConstructorDeclaration(
		...
		return tryParse(() => {
			if (parseConstructorName()) {
				...
				(node as Mutable<ConstructorDeclaration>).typeParameters =
					typeParameters;
	...
	function parseMethodDeclaration(
		...
		(node as Mutable<MethodDeclaration>).exclamationToken =
			exclamationToken;
		return withJSDoc(finishNode(node, pos), hasJSDoc);
	}
	...
	function parseModuleOrNamespaceDeclaration(
		...
		const name =
			flags & NodeFlags.NestedNamespace
				? parseIdentifierName()
				: parseIdentifier();
	...
	function parseImportDeclarationOrImportEqualsDeclaration(
		...
		if (
			...
			(token() !== SyntaxKind.FromKeyword ||
				(isIdentifier() &&
					lookAhead(nextTokenIsFromKeywordOrEqualsToken))) &&
			(isIdentifier() ||
		...
		function parseJSDocCommentWorker(
			...
			function parseJSDocLinkName() {
				if (tokenIsIdentifierOrKeyword(token())) {
					...
					let name: EntityName | JSDocMemberName =
						parseIdentifierName();
					while (parseOptional(SyntaxKind.DotToken)) {
						name = finishNode(
							factory.createQualifiedName(
			...
			function parseCallbackTag(
				...
				const end =
					comment !== undefined ? getNodePos() : typeExpression.end;
				return finishNode(
...
/** @internal */
export function getDeclarationFileExtension(
	fileName: string,
): string | undefined {
...
export function tagNamesAreEquivalent(
	...
	return (
		...
		tagNamesAreEquivalent(
			...
			(rhs as PropertyAccessExpression)
				.expression as JsxTagNameExpression,
		)
	);
}
