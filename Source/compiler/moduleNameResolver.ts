...
function withPackageId(
	...
	if (r && packageInfo) {
		const packageJsonContent = packageInfo.contents
			.packageJsonContent as PackageJson;
		if (
			...
			packageId = {
				...
				subModuleName: r.path.slice(
					packageInfo.packageDirectory.length +
...
function createResolvedModuleWithFailedLookupLocationsHandlingSymlink(
	...
	if (
		...
		const { resolvedFileName, originalPath } =
	...
	if (resultFromCache) {
		if (!cache?.isReadonly) {
			...
			return {
				...resultFromCache,
				failedLookupLocations:
					initializeResolutionFieldForReadonlyCache(
						resultFromCache.failedLookupLocations,
						failedLookupLocations,
					),
...
/** @internal */
export function updateResolutionField<T>(
	to: T[] | undefined,
	value: T[] | undefined,
) {
...
function initializeResolutionFieldForReadonlyCache<T>(
...
function getOriginalAndResolvedFileName(
	fileName: string,
	host: ModuleResolutionHost,
	traceEnabled: boolean,
) {
...
function getCandidateFromTypeRoot(
	...
	const nameForLookup =
		endsWith(typeRoot, "/node_modules/@types") ||
		endsWith(typeRoot, "/node_modules/@types/")
...
export function resolveTypeReferenceDirective(
	...
	if (resolved) {
		...
		if (!options.preserveSymlinks)
			({ resolvedFileName, originalPath } =
				getOriginalAndResolvedFileName(fileName, host, traceEnabled));
		resolvedTypeReferenceDirective = {
			primary,
			resolvedFileName,
			originalPath,
	...
	if (containingDirectory && cache && !cache.isReadonly) {
		cache
			...
			.set(typeReferenceDirectiveName, /*mode*/ resolutionMode, result);
	...
	function traceResult(
		...
		if (!result.resolvedTypeReferenceDirective?.resolvedFileName) {
			...
			trace(
				...
				packageIdToString(
					result.resolvedTypeReferenceDirective.packageId,
				),
	...
	function primaryLookup(): PathAndPackageId | undefined {
		...
		if (typeRoots && typeRoots.length) {
			...
			return firstDefined(typeRoots, (typeRoot) => {
				const candidate = getCandidateFromTypeRoot(
					typeRoot,
					typeReferenceDirectiveName,
					moduleResolutionState,
				);
				...
				if (options.typeRoots) {
					...
					if (resolvedFromFile) {
						const packageDirectory = parseNodeModuleFromPath(
						...
						return resolvedTypeScriptOnly(
							withPackageId(packageInfo, resolvedFromFile),
						);
					}
				}
				return resolvedTypeScriptOnly(
	...
	function secondaryLookup(): PathAndPackageId | undefined {
		...
		if (initialLocationForSecondaryLookup !== undefined) {
			...
			if (
				!options.typeRoots ||
				!endsWith(containingFile!, inferredTypesContainingFile)
			) {
...
export function getConditions(
	...
	const conditions =
		resolutionMode === ModuleKind.ESNext ? ["import"] : ["require"];
				...
				if (host.directoryExists(root)) {
					for (const typeDirectivePath of host.getDirectories(root)) {
						...
						const isNotNeededPackage =
							...
							(readJson(packageJsonPath, host) as PackageJson)
								.typings === null;
...
export interface PerDirectoryResolutionCache<T> {
	...
	/** @internal */ directoryToModuleNameMap: CacheWithRedirects<
		Path,
		ModeAwareCache<T>
	>;
...
/** @internal */
export type PackageJsonInfoCacheEntry =
	| PackageJsonInfo
	| MissingPackageJsonInfo;

/** @internal */
export function isPackageJsonInfo(
	entry: PackageJsonInfoCacheEntry | undefined,
): entry is PackageJsonInfo {
...
export interface PackageJsonInfoCache {
	...
	): PackageJsonInfoCacheEntry | undefined;
	/** @internal */ setPackageJsonInfo(
		packageJsonPath: string,
		info: PackageJsonInfoCacheEntry,
	): void;
	/** @internal */ getInternalMap():
		| Map<Path, PackageJsonInfoCacheEntry>
		| undefined;
	clear(): void;
	/** @internal */ isReadonly?: boolean;
}

export type PerModuleNameCache =
...
export function getKeyForCompilerOptions(
	...
	return (
		affectingOptionDeclarations
			.map((option) =>
				compilerOptionValueToString(
					getCompilerOptionValue(options, option),
				),
			)
...
export function createCacheWithRedirects<K, V>(
	...
	function getOrCreateMap(
		redirectOptions: CompilerOptions,
		create: true,
	): Map<K, V>;
	function getOrCreateMap(
		redirectOptions: CompilerOptions,
...
function createPerDirectoryResolutionCache<T>(
	...
	const directoryToModuleNameMap = createCacheWithRedirects<
		Path,
		ModeAwareCache<T>
	...
	function getFromDirectoryCache(
		...
		return directoryToModuleNameMap
			.getMapOfCacheRedirects(redirectedReference)
			?.get(path)
			?.get(name, mode);
	}
}
...
export function createModeAwareCacheKey(
	...
	return (
		mode === undefined ? specifier : `${mode}|${specifier}`
	) as ModeAwareCacheKey;
}
/** @internal */
export function createModeAwareCache<T>(): ModeAwareCache<T> {
	...
	const memoizedReverseKeys = new Map<
		ModeAwareCacheKey,
		[specifier: string, mode: ResolutionMode]
	>();
...
function getOriginalOrResolvedModuleFileName(
...
function getOriginalOrResolvedTypeReferenceFileName(
	result: ResolvedTypeReferenceDirectiveWithFailedLookupLocations,
) {
	return (
...
function createNonRelativeNameResolutionCache<T>(
	...
	function getFromNonRelativeNameCache(
		...
		return moduleNameToDirectoryMap
			.getMapOfCacheRedirects(redirectedReference)
			?.get(createModeAwareCacheKey(nonRelativeModuleName, mode))
			?.get(directoryName);
	}
...
interface ModuleOrTypeReferenceResolutionCache<T>
	extends PerDirectoryResolutionCache<T>,
		NonRelativeNameResolutionCache<T>,
		PackageJsonInfoCache {
	getPackageJsonInfoCache(): PackageJsonInfoCache;
...
export function createModuleResolutionCache(
	...
	result.getOrCreateCacheForModuleName = (
		nonRelativeName,
		mode,
		redirectedReference,
	) =>
		result.getOrCreateCacheForNonRelativeName(
			nonRelativeName,
			mode,
			redirectedReference,
		);
...
export function resolveLibrary(
	...
	return resolveModuleName(
		libraryName,
		resolveFrom,
		getOptionsForLibraryResolution(compilerOptions),
		host,
		cache,
	);
}
...
export function resolveModuleName(
	...
	const containingDirectory = getDirectoryPath(containingFile);
	let result = cache?.getFromDirectoryCache(
		moduleName,
		resolutionMode,
		containingDirectory,
		redirectedReference,
	...
	if (result) {
		...
		switch (moduleResolution) {
			...
			default:
				return Debug.fail(
					`Unexpected moduleResolution: ${moduleResolution}`,
				);
		}
		...
		if (cache && !cache.isReadonly) {
			cache
				...
				.set(moduleName, resolutionMode, result);
...
function tryLoadModuleUsingPathsIfEligible(
	...
	if (paths && !pathIsRelative(moduleName)) {
		...
		const pathPatterns = configFile?.configFileSpecs
			? (configFile.configFileSpecs.pathPatterns ||=
					tryParsePatterns(paths))
			: undefined;
...
function nodeModuleNameResolverWorker(
	...
	if (
		traceEnabled &&
		moduleResolutionSupportsPackageJsonExportsAndImports(moduleResolution)
	) {
		trace(
			host,
	...
	if (
		...
		const wantedTypesButGotJs =
			result?.value &&
			extensions & (Extensions.TypeScript | Extensions.Declaration) &&
			!extensionIsOk(
				Extensions.TypeScript | Extensions.Declaration,
				result.value.resolved.extension,
			);
		if (
...
function loadFileNameFromPackageJsonField(
	...
	if (
		...
		(extensions & Extensions.Declaration &&
			fileExtensionIsOneOf(candidate, supportedDeclarationExtensions))
	) {
...
function tryAddingExtensions(
	...
	if (!onlyRecordFailures) {
		...
		if (directory) {
			onlyRecordFailures = !directoryProbablyExists(
				directory,
				state.host,
			);
		}
	...
	switch (originalExtension) {
		...
		case Extension.Dmts:
			return (
				(extensions & Extensions.TypeScript &&
					tryExtension(
						Extension.Mts,
						originalExtension === Extension.Mts ||
							originalExtension === Extension.Dmts,
					)) ||
				(extensions & Extensions.Declaration &&
					tryExtension(
						Extension.Dmts,
		...
		case Extension.Dcts:
			return (
				(extensions & Extensions.TypeScript &&
					tryExtension(
						Extension.Cts,
						originalExtension === Extension.Cts ||
							originalExtension === Extension.Dcts,
					)) ||
				(extensions & Extensions.Declaration &&
					tryExtension(
						Extension.Dcts,
				...
				(extensions & Extensions.JavaScript &&
					tryExtension(Extension.Cjs)) ||
				undefined
			);
		case Extension.Json:
			return (
				(extensions & Extensions.Declaration &&
					tryExtension(".d.json.ts")) ||
				(extensions & Extensions.Json &&
		...
		case Extension.Jsx:
			...
			return (
				(extensions & Extensions.TypeScript &&
					(tryExtension(
						Extension.Tsx,
						originalExtension === Extension.Tsx,
					) ||
						tryExtension(
							Extension.Ts,
							originalExtension === Extension.Tsx,
		...
		case "":
			return (
				(extensions & Extensions.TypeScript &&
					(tryExtension(
						Extension.Ts,
						originalExtension === Extension.Ts ||
							originalExtension === Extension.Dts,
					) ||
						tryExtension(
							Extension.Tsx,
							originalExtension === Extension.Ts ||
		...
		default:
			return (
				(extensions & Extensions.Declaration &&
					!isDeclarationFileName(candidate + originalExtension) &&
		...
		return path === undefined
			...
			: {
					...
					resolvedUsingTsExtension:
						!state.candidateIsFromPackageJsonField &&
						resolvedUsingTsExtension,
				};
	}
}
...
function tryFile(
	...
	return forEach(state.compilerOptions.moduleSuffixes, (suffix) =>
		tryFileLookup(
			fileNameNoExtension + suffix + ext,
			onlyRecordFailures,
			state,
		),
	);
...
export function getEntrypointsFromPackageJsonInfo(
	...
	if (
		!resolveJs &&
		packageJsonInfo.contents.resolvedEntrypoints !== undefined
	) {
	...
	const extensions =
		Extensions.TypeScript |
		Extensions.Declaration |
		(resolveJs ? Extensions.JavaScript : 0);
	...
	loadPackageJsonMainState.requestContainingDirectory =
	...
	const mainResolution = loadNodeModuleFromDirectoryWorker(
		...
		getVersionPathsOfPackageJsonInfo(
			packageJsonInfo,
			loadPackageJsonMainState,
		),
	);
	...
	if (
		...
		const conditionSets = deduplicate(
			[
				getConditions(options, ModuleKind.ESNext),
				getConditions(options, ModuleKind.CommonJS),
			],
			arrayIsEqualTo,
		);
	...
	// eslint-disable-next-line no-null/no-null
		...
		for (const key in exports) {
			loadEntrypointsFromTargetExports(
				(exports as MapLike<unknown>)[key],
			);
		}
	} else {
	...
	function loadEntrypointsFromTargetExports(
		...
		if (typeof target === "string" && startsWith(target, "./")) {
			if (target.includes("*") && state.host.readDirectory) {
				...
				if (
					partsAfterFirst.includes("..") ||
					partsAfterFirst.includes(".") ||
					partsAfterFirst.includes("node_modules")
				) {
					return false;
				}
				const resolvedTarget = combinePaths(
					scope.packageDirectory,
					target,
				);
...
export function getPackageJsonInfo(
	...
	if (directoryExists && host.fileExists(packageJsonPath)) {
		const packageJsonContent = readJson(
			packageJsonPath,
			host,
		) as PackageJson;
		if (traceEnabled) {
		...
		if (
			...
			!state.packageJsonInfoCache.isReadonly
		)
		...
		if (
			...
			state.packageJsonInfoCache.setPackageJsonInfo(packageJsonPath, {
				packageDirectory,
				directoryExists,
			});
...
function loadNodeModuleFromDirectoryWorker(
	...
	if (jsonContent) {
		if (state.isConfigLookup) {
			...
			packageFile =
				...
				(extensions &
					(Extensions.ImplementationFiles | Extensions.Declaration) &&
	...
	const loader: ResolutionKindSpecificLoader = (
		...
		const candidateIsFromPackageJsonField =
			state.candidateIsFromPackageJsonField;
	...
	const indexPath = combinePaths(
		candidate,
		state.isConfigLookup ? "tsconfig" : "index",
	);
...
function resolvedIfExtensionMatches(
	...
	return ext !== undefined && extensionIsOk(extensions, ext)
		? { path, ext, resolvedUsingTsExtension }
		: undefined;
}
	...
	return (
		...
		(extensions & Extensions.Declaration &&
			(extension === Extension.Dts ||
				extension === Extension.Dmts ||
				extension === Extension.Dcts)) ||
...
function loadModuleFromSelfNameReference(
	...
	const subpath = !length(trailingParts)
		? "."
	...
	if (
		getAllowJSCompilerOption(state.compilerOptions) &&
		!pathContainsNodeModules(directory)
	) {
	...
	if (subpath === ".") {
		...
		if (
			...
			(typeof scope.contents.packageJsonContent.exports === "object" &&
				noKeyStartsWithDot(
					scope.contents.packageJsonContent
						.exports as MapLike<unknown>,
				))
		) {
			mainExport = scope.contents.packageJsonContent.exports;
		} else if (
			hasProperty(
				scope.contents.packageJsonContent.exports as MapLike<unknown>,
				".",
			)
		) {
			mainExport = (
...
function getLoadModuleFromTargetImportOrExport(
	...
	function loadModuleFromTargetImportOrExport(
		...
		if (typeof target === "string") {
			...
			if (
				subpathParts.includes("..") ||
				subpathParts.includes(".") ||
				subpathParts.includes("node_modules")
			) {
				if (state.traceEnabled) {
					trace(
						state.host,
			...
			if (!Array.isArray(target)) {
				...
				for (const condition of getOwnKeys(
					target as MapLike<unknown>,
				)) {
					if (
						condition === "default" ||
						...
						const subTarget = (target as MapLike<unknown>)[
							condition
						];
						...
						if (result) {
							...
							traceIfEnabled(
								state,
								Diagnostics.Exiting_conditional_exports,
							);
							return result;
						} else {
							traceIfEnabled(
		...
		function tryLoadInputFileForPath(
			...
			if (
				!state.isConfigLookup &&
				(state.compilerOptions.declarationDir ||
				...
				(state.compilerOptions.configFile
					? containsPath(
							scope.packageDirectory,
							toAbsolutePath(
								state.compilerOptions.configFile.fileName,
							),
				...
				if (
					...
					(state.compilerOptions.composite &&
						state.compilerOptions.configFilePath)
				) {
					...
					const requestingFile = toAbsolutePath(
						combinePaths(
				...
				for (const commonSourceDirGuess of commonSourceDirGuesses) {
					...
					for (const candidateDir of candidateDirectories) {
						if (
							...
							const pathFragment = finalPath.slice(
								candidateDir.length + 1,
							); // +1 to also remove directory seperator
							const possibleInputBase = combinePaths(
								commonSourceDirGuess,
								pathFragment,
							);
								...
								if (fileExtensionIs(possibleInputBase, ext)) {
									...
									for (const possibleExt of inputExts) {
										if (
											...
											continue;
										const possibleInputWithInputExtension =
											changeAnyExtension(
												possibleInputBase,
												possibleExt,
												ext,
...
function loadModuleFromSpecificNodeModulesDirectory(
	...
	if (
		...
		(!(state.features & NodeResolutionFeatures.Exports) ||
			!hasProperty(
				(rootPackageInfo = getPackageJsonInfo(
					packageDirectory,
					!nodeModulesDirectoryExists,
					state,
				))?.contents.packageJsonContent ?? emptyArray,
				"exports",
			))
	) {
	...
	const loader: ResolutionKindSpecificLoader = (
		...
		let pathAndExtension =
			((rest || !(state.features & NodeResolutionFeatures.EsmMode)) &&
				loadModuleFromFile(
					extensions,
					candidate,
					onlyRecordFailures,
	...
	if (
		packageInfo &&
		packageInfo.contents.packageJsonContent.exports &&
		state.features & NodeResolutionFeatures.Exports
	) {
		return loadModuleFromExports(
			packageInfo,
...
function tryLoadModuleUsingPaths(
	...
	if (matchedPattern) {
		...
		const resolved = forEach(paths[matchedPatternText], (subst) => {
			const path = matchedStar
				? replaceFirstStar(subst, matchedStar)
				: subst;
	...
	if (result) {
		...
		return {
			value: result.resolvedModule && {
				...
				packageId: result.resolvedModule.packageId,
				resolvedUsingTsExtension:
					result.resolvedModule.resolvedUsingTsExtension,
			},
		};
	}
}
...
export function classicNameResolver(
	...
	const resolved =
		...
		tryResolve(
			Extensions.JavaScript |
				(compilerOptions.resolveJsonModule ? Extensions.Json : 0),
		);
...
function resolveFromTypeRoot(moduleName: string, state: ModuleResolutionState) {
	...
	for (const typeRoot of state.compilerOptions.typeRoots) {
		...
		const resolved = loadNodeModuleFromDirectory(
			Extensions.Declaration,
			candidate,
			!directoryExists,
			state,
...
function useCaseSensitiveFileNames(state: ModuleResolutionState) {
	return !state.host.useCaseSensitiveFileNames
		? true
		: typeof state.host.useCaseSensitiveFileNames === "boolean"
