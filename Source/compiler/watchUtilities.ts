...
export function createCachedDirectoryStructureHost(
	...
	function getCachedFileSystemEntriesForBaseDir(path: Path) {
		...
		if (!entries.sortedAndCanonicalizedFiles) {
			entries.sortedAndCanonicalizedFiles = entries.files
				.map(getCanonicalFileName)
				.sort() as SortedArray<Canonicalized>;
			entries.sortedAndCanonicalizedDirectories = entries.directories
				.map(getCanonicalFileName)
				.sort() as SortedArray<Canonicalized>;
		}
	...
	function hasEntry(
		entries: SortedReadonlyArray<Canonicalized>,
		name: Canonicalized,
	) {
	...
	function fileExists(fileName: string): boolean {
		...
		return (
			(result &&
				hasEntry(
					result.sortedAndCanonicalizedFiles,
					getCanonicalFileName(getBaseNameOfFileName(fileName)),
				)) ||
	...
	function createDirectory(dirPath: string) {
		...
		if (result) {
			...
			const canonicalizedDirectories =
				result.sortedAndCanonicalizedDirectories;
			// Case-sensitive comparison since already canonicalized
			if (
				insertSorted(
					canonicalizedDirectories,
	...
	function addOrDeleteFileOrDirectory(
		...
		if (
			...
			hasEntry(
				parentResult.sortedAndCanonicalizedDirectories,
				getCanonicalFileName(baseName),
			)
		) {
	...
	function updateFilesOfFileSystemEntry(
		...
		if (fileExists) {
			...
			if (sortedIndex >= 0) {
				...
				const unsortedIndex = parentResult.files.findIndex(
					(entry) =>
	...
	extendedConfigCache.forEach(({ extendedResult }, key) => {
		if (
			extendedResult.extendedSourceFiles?.some(
				(extendedFile) =>
					toPath(extendedFile) === extendedConfigFilePath,
			)
		) {
...
export function getWatchFactory<X, Y = undefined>(
	...
	const triggerInvokingFactory: WatchFactory<X, Y> | undefined =
		watchLogLevel !== WatchLogLevel.None
			? {
					...
					watchDirectory:
						createTriggerLoggingAddWatch("watchDirectory"),
				}
			: undefined;
	const factory =
	...
	function createExcludeWatcherWithLogging(
		...
		log(
			`ExcludeWatcher:: Added:: ${getWatchInfo(file, flags, options, detailInfo1, detailInfo2, getDetailWatchInfo)}`,
		);
		return {
			close: () =>
				log(
					`ExcludeWatcher:: Close:: ${getWatchInfo(file, flags, options, detailInfo1, detailInfo2, getDetailWatchInfo)}`,
				),
		};
	}
	...
	function createFileWatcherWithLogging(
		...
		log(
			`FileWatcher:: Added:: ${getWatchInfo(file, flags, options, detailInfo1, detailInfo2, getDetailWatchInfo)}`,
		);
		...
		return {
			close: () => {
				log(
					`FileWatcher:: Close:: ${getWatchInfo(file, flags, options, detailInfo1, detailInfo2, getDetailWatchInfo)}`,
				);
				watcher.close();
			},
		};
