...
function getOrCreateValueMapFromConfigFileMap<K extends string, V>(
	...
	return getOrCreateValueFromConfigFileMap(
		configFileMap,
		resolved,
		() => new Map(),
	);
...
export interface SolutionBuilderHostBase<T extends BuilderProgram>
	...
	/** @deprecated @internal */ beforeEmitBundle?(
		config: ParsedCommandLine,
	): void;
	/** @deprecated @internal */ afterEmitBundle?(
		config: ParsedCommandLine,
	): void;
...
export interface SolutionBuilder<T extends BuilderProgram> {
	...
	// Testing only
	/** @internal */ getUpToDateStatusOfProject(
		project: string,
	): UpToDateStatus;
...
interface SolutionBuilderState<T extends BuilderProgram>
	...
	readonly projectPendingBuild: Map<
	...
	readonly allWatchedWildcardDirectories: Map<
		ResolvedConfigFilePath,
		Map<string, WildcardDirectoryWatcher>
	>;
	...
	readonly allWatchedExtendedConfigFiles: Map<
		Path,
		SharedExtendedConfigFileWatcher<ResolvedConfigFilePath>
	>;
	readonly allWatchedPackageJsonFiles: Map<
		ResolvedConfigFilePath,
		Map<Path, FileWatcher>
	>;
...
function createSolutionBuilderState<T extends BuilderProgram>(
	...
	) as CompilerHost & ReadBuildProgramHost;
	...
	compilerHost.resolveModuleNameLiterals = maybeBind(
		host,
	...
	compilerHost.getModuleResolutionCache = maybeBind(
		host,
	...
	compilerHost.getBuildInfo = (fileName, configFilePath) =>
		getBuildInfo(
			...
			toResolvedConfigFilePath(
				state,
				configFilePath as ResolvedConfigFileName,
			),
...
function toPath<T extends BuilderProgram>(
...
function resolveProjectName<T extends BuilderProgram>(
	...
	return resolveConfigFileProjectName(
		resolvePath(state.compilerHost.getCurrentDirectory(), name),
	);
}
...
function getBuildOrder<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
) {
	return state.buildOrder || createStateBuildOrder(state);
}

function createStateBuildOrder<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
) {
	...
	mutateMapSkippingNewValues(
		state.outputTimeStamps,
		currentProjects,
		noopOnDelete,
	);
	mutateMapSkippingNewValues(
		state.lastCachedPackageJsonLookups,
		currentProjects,
		noopOnDelete,
	);
	...
	if (state.watch) {
		...
		mutateMapSkippingNewValues(
			...
			{
				onDeleteValue: (existingMap) =>
					existingMap.forEach(closeFileWatcher),
			},
		);
...
function getBuildOrderFor<T extends BuilderProgram>(
	...
	Debug.assert(
		!onlyReferences ||
			buildOrder[buildOrder.length - 1] === resolvedProject,
	);
...
function disableCache<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
) {
...
function createBuildOrUpdateInvalidedProject<T extends BuilderProgram>(
	...
	function createProgram() {
		...
		if (state.watch) {
			const internalMap = state.moduleResolutionCache
			...
			state.lastCachedPackageJsonLookups.set(
				...
				internalMap &&
					new Set(
						arrayFrom(internalMap.values(), (data) =>
							state.host.realpath &&
							(isPackageJsonInfo(data) || data.directoryExists)
								? state.host.realpath(
										combinePaths(
	...
	function emit(
		...
		// Actual Emit
		const { host, compilerHost } = state;
		const resultFlags = program.hasChangedEmitSignature?.()
		...
		outputFiles.forEach(({ name, text, writeByteOrderMark, data }) => {
			...
			const modifiedTime = data?.differsOnlyInMap
				? ts_getModifiedTime(state.host, name)
				: undefined;
			writeFile(
				writeFileCallback
					? { writeFile: writeFileCallback }
			...
			if (data?.differsOnlyInMap)
				state.host.setModifiedTime(name, modifiedTime!);
			else if (!isIncremental && state.watch) {
				(outputTimeStampMap ||= getOutputTimeStampMap(
					state,
					projectPath,
				)!).set(path, (now ||= getCurrentTime(state.host)));
			}
		});
	...
	function emitBundle(
		...
		if (state.options.verbose)
			reportStatus(
				state,
				Diagnostics.Updating_output_of_project_0,
				project,
			);
		...
		const existingBuildInfo =
			state.buildInfoCache.get(projectPath)!.buildInfo || undefined;
		outputFiles.forEach(({ name, text, writeByteOrderMark, data }) => {
			emittedOutputs.set(toPath(state, name), name);
			if (data?.buildInfo) {
				if (
					(data.buildInfo.program as ProgramBundleEmitBuildInfo)
						?.outSignature !==
					(existingBuildInfo?.program as ProgramBundleEmitBuildInfo)
						?.outSignature
				) {
...
function getModifiedTime<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
	fileName: string,
): Date {
	...
	if (state.watch) {
		if (existing)
			(existing as FileWatcherWithModifiedTime).modifiedTime = result;
		...
		const watcher = state.watchFile(
			...
			(fileName, eventKind, modifiedTime) => {
				const existing = Debug.checkDefined(
					state.filesWatched.get(path),
				);
				Debug.assert(isFileWatcherWithModifiedTime(existing));
				existing.modifiedTime = modifiedTime;
				existing.callbacks.forEach((cb) =>
					cb(fileName, eventKind, modifiedTime),
				);
			},
			pollingInterval,
			options,
...
function setBuildInfo<T extends BuilderProgram>(
	...
	const existing = getBuildInfoCacheEntry(
		state,
		buildInfoPath,
		resolvedConfigPath,
	);
...
function getUpToDateStatusWorker<T extends BuilderProgram>(
	...
	if (project.projectReferences) {
		...
		for (const ref of project.projectReferences) {
			...
			const resolvedRefPath = toResolvedConfigFilePath(
				state,
				resolvedRef,
			);
			const resolvedConfig = parseConfigFile(
				state,
				resolvedRef,
				resolvedRefPath,
			)!;
	...
	let buildInfoVersionMap:
		| ReturnType<typeof getBuildInfoFileVersionMap>
		| undefined;
	if (buildInfoPath) {
		const buildInfoCacheEntry = getBuildInfoCacheEntry(
			state,
			buildInfoPath,
			resolvedPath,
		);
		buildInfoTime =
			buildInfoCacheEntry?.modifiedTime ||
			ts_getModifiedTime(host, buildInfoPath);
		...
		if (buildInfo.program) {
			...
			if (
				(buildInfo.program as ProgramMultiFileEmitBuildInfo)
					.changeFileSet?.length ||
				(!project.options.noEmit
					? (buildInfo.program as ProgramMultiFileEmitBuildInfo)
							.affectedFilesPendingEmit?.length ||
						(buildInfo.program as ProgramMultiFileEmitBuildInfo)
							.emitDiagnosticsPerFile?.length
					: some(
							(buildInfo.program as ProgramMultiFileEmitBuildInfo)
		...
		if (buildInfoTime && buildInfoTime < inputTime) {
			...
			if (buildInfoProgram) {
				...
				if (!buildInfoVersionMap)
					buildInfoVersionMap = getBuildInfoFileVersionMap(
						buildInfoProgram,
						buildInfoPath!,
						host,
					);
				version = buildInfoVersionMap.fileInfos.get(
					toPath(state, inputFile),
				);
				const text = version
					? state.readFileWithCache(inputFile)
					: undefined;
				currentVersion =
					text !== undefined
						? getSourceFileVersionAsHashFromText(host, text)
						: undefined;
	...
	if (referenceStatuses) {
		for (const {
			...
			if (
				buildInfoCacheEntry &&
				hasSameBuildInfo(state, buildInfoCacheEntry, resolvedRefPath)
			) {
				return {
	...
	const packageJsonLookups =
		state.lastCachedPackageJsonLookups.get(resolvedPath);
	const dependentPackageFileStatus =
		packageJsonLookups &&
		forEachKey(packageJsonLookups, (path) =>
			checkConfigFileUpToDateStatus(
				state,
				path,
				oldestOutputFileTime,
...
function updateOutputTimestampsWorker<T extends BuilderProgram>(
	...
	if (buildInfoPath) {
		...
		if (!skipOutputs?.has(toPath(state, buildInfoPath))) {
			...
			state.host.setModifiedTime(
				buildInfoPath,
				(now = getCurrentTime(state.host)),
			);
			getBuildInfoCacheEntry(
				state,
				buildInfoPath,
				projectPath,
			)!.modifiedTime = now;
	...
	outputTimeStampMap?.forEach((_value, key) => {
		if (!skipOutputs?.has(key) && !modifiedOutputs!.has(key))
...
function getLatestChangedDtsTime<T extends BuilderProgram>(
	...
	const entry = Debug.checkDefined(
		state.buildInfoCache.get(resolvedConfigPath),
	);
	if (entry.latestChangedDtsTime !== undefined)
		return entry.latestChangedDtsTime || undefined;
	const latestChangedDtsTime =
		entry.buildInfo &&
		entry.buildInfo.program &&
		entry.buildInfo.program.latestChangedDtsFile
			? state.host.getModifiedTime(
					getNormalizedAbsolutePath(
						entry.buildInfo.program.latestChangedDtsFile,
						getDirectoryPath(entry.path),
					),
				)
			: undefined;
...
function queueReferencingProjects<T extends BuilderProgram>(
	...
	for (let index = projectIndex + 1; index < buildOrder.length; index++) {
		...
		for (const ref of nextProjectConfig.projectReferences) {
			const resolvedRefPath = resolveProjectName(state, ref.path);
			if (
				toResolvedConfigFilePath(state, resolvedRefPath) !== projectPath
			)
				continue;
					...
					case UpToDateStatusType.OutOfDateWithPrepend:
						if (
							...
							state.projectStatus.set(nextProjectPath, {
								...
								outOfDateOutputFileName:
									status.type ===
									UpToDateStatusType.OutOfDateWithPrepend
...
function build<T extends BuilderProgram>(
	...
	performance.measure(
		...
		"SolutionBuilder::beforeBuild",
		"SolutionBuilder::afterBuild",
	);
	return result;
}
...
function buildWorker<T extends BuilderProgram>(
	...
	writeFile: WriteFileCallback | undefined,
	getCustomTransformers:
		| ((project: string) => CustomTransformers)
		| undefined,
	onlyReferences: boolean | undefined,
): ExitStatus {
...
function clean<T extends BuilderProgram>(
	...
	performance.measure(
		...
		"SolutionBuilder::beforeClean",
		"SolutionBuilder::afterClean",
	);
	return result;
}
...
function buildNextInvalidatedProjectWorker<T extends BuilderProgram>(
	...
	if (invalidatedProject) {
		...
		while (state.projectPendingBuild.size) {
			...
			if (
				info.kind !== InvalidatedProjectKind.UpdateOutputFileStamps &&
				(changeDetected || projectsBuilt === 5)
			) {
			...
			const project = createInvalidatedProjectWithInfo(
				state,
				info,
				buildOrder,
			);
					...
					if (
						isIgnoredFileFromWildCardWatching({
							...
							program:
								...
								getCachedParsedConfigFile(state, resolvedPath)
									?.fileNames,
...
function startWatching<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
	buildOrder: AnyBuildOrder,
) {
...
function stopWatching<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
) {
...
function relName<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
	path: string,
): string {
...
function reportErrors<T extends BuilderProgram>(
	{ host }: SolutionBuilderState<T>,
...
function reportErrorSummary<T extends BuilderProgram>(
	state: SolutionBuilderState<T>,
	buildOrder: AnyBuildOrder,
) {
	...
	if (isCircularBuildOrder(buildOrder)) {
		...
		reportErrors(state, buildOrder.circularDiagnostics);
		if (canReportSummary)
			totalErrors += getErrorCountForSummary(
				buildOrder.circularDiagnostics,
			);
		if (canReportSummary)
			filesInError = [
				...filesInError,
