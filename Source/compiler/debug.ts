...
export namespace Debug {
	...
	export function assertLessThanOrEqual(
		...
		if (a > b) {
			fail(
				`Expected ${a} <= ${b}`,
				stackCrawlMark || assertLessThanOrEqual,
			);
		}
	}
	...
	export function assertOptionalToken(
		...
		if (
			shouldAssertFunction(AssertionLevel.Normal, "assertOptionalToken")
		) {
			assert(
	...
	const enumMemberCache = new Map<
		any,
		SortedReadonlyArray<[number, string]>
	>();
	...
	function getEnumMembers(enumObject: any) {
		...
		const sorted = stableSort<[number, string]>(result, (x, y) =>
	...
	export function formatRelationComparisonResult(
		result: RelationComparisonResult | undefined,
	): string {
		return formatEnum(
			result,
			(ts as any).RelationComparisonResult,
			/*isFlags*/ true,
		);
	}
	...
	export function formatSignatureCheckMode(
		mode: SignatureCheckMode | undefined,
	): string {
		return formatEnum(
			mode,
			(ts as any).SignatureCheckMode,
			/*isFlags*/ true,
		);
	}
				...
				__tsDebuggerDisplay: {
					value(this: FlowNodeBase) {
						...
						const remainingFlags =
							this.flags & ~(FlowFlags.Referenced - 1);
	...
	export function enableDebugInfo() {
		...
		Object.defineProperties(
			objectAllocator.getTypeConstructor().prototype,
			{
				// for use with vscode-js-debug's new customDescriptionGenerator in launch.json
				__tsDebuggerDisplay: {
					value(this: Type) {
			...
			switch (this.kind) {
				...
				case TypeMapKind.Array:
					return zipWith<DebugType, DebugType | string, unknown>(
						...
						(this.targets as readonly DebugType[]) ||
							map(this.sources, () => "any"),
						(s, t) =>
...
m2: ${(this.mapper2 as unknown as DebugTypeMapper).__debugToString().split("\n").join("\n    ")}`;
	...
	export function attachDebugPrototypeIfDebug(
		mapper: TypeMapper,
	): TypeMapper {
		if (isDebugging) {
	...
	export function formatControlFlowGraph(flowNode: FlowNode) {
		...
		function hasAntecedents(
			...
			return (
				!!(f.flags & FlowFlags.Label) && !!(f as FlowLabel).antecedents
			);
		}
		...
		function renderFlowNode(
			...
			if (hasNode(flowNode)) {
				...
				for (
					let i = flowNode.clauseStart;
					i < flowNode.clauseEnd;
					i++
				) {
					const clause =
						flowNode.switchStatement.caseBlock.clauses[i];
		...
		function renderGraph() {
			...
			const laneCount =
				nodes.reduce((x, n) => Math.max(x, n.lane), 0) + 1;
			...
			for (let column = 0; column < columnCount; column++) {
				for (let lane = 0; lane < lanes.length; lane++) {
					...
					const fill =
						connector & Connection.Left ? BoxCharacter.lr : " ";
					const node = grid[column][lane];
					if (!node) {
						if (column < columnCount - 1) {
							writeLane(
								lane,
								repeat(fill, columnWidths[column] + 1),
							);
						}
					} else {
