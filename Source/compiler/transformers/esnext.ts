...
/** @internal */
export function transformESNext(
				...
				append(
					...
					factory.createExportDeclaration(
						...
						factory.createNamedExports(
							arrayFrom(exportBindings.values()),
						),
					),
				);
			}
	...
	function visitForOfStatement(node: ForOfStatement) {
		if (isUsingVariableDeclarationList(node.initializer)) {
			...
			Debug.assert(
				forInitializer.declarations.length === 1,
				"ForInitializer may only have one declaration",
			);

			const forDecl = forInitializer.declarations[0];
			Debug.assert(
				!forDecl.initializer,
				"ForInitializer may not have an initializer",
			);
			...
			const usingVarStatement = factory.createVariableStatement(
				/*modifiers*/ undefined,
				usingVarList,
			);
			return visitNode(
	...
	function visitCaseOrDefaultClause(
	...
	function visitSwitchStatement(node: SwitchStatement) {
		...
		//  }
		//
		const usingKind = getUsingKindOfCaseOrDefaultClauses(
					...
					factory.updateSwitchStatement(
						...
						factory.updateCaseBlock(
							...
							node.caseBlock.clauses.map((clause) =>
								visitCaseOrDefaultClause(clause, envBinding),
							),
						),
					),
				],
	...
	function transformUsingDeclarations(
		...
		for (let i = start; i < end; i++) {
			...
			if (usingKind) {
				...
				for (let declaration of statement.declarationList
					.declarations) {
				...
				if (declarations.length) {
					const varList = factory.createVariableDeclarationList(
						declarations,
						NodeFlags.Const,
					);
	...
	function hoistImportOrExportOrHoistedDeclaration(
		node: Statement,
	...
	function hoistExportAssignment(node: ExportAssignment) {
		return node.isExportEquals
			? hoistExportEquals(node)
			: hoistExportDefault(node);
	}
	...
	function hoistClassDeclaration(node: ClassDeclaration) {
		...
		let expression: Expression =
			factory.converters.convertToClassExpression(node);
		if (node.name) {
			// given:
			//
			//  using x = expr;
	...
	function hoistVariableStatement(node: VariableStatement) {
		...
		for (const variable of node.declarationList.declarations) {
			...
			if (variable.initializer) {
				expressions = append(
					expressions,
					hoistInitializedVariable(variable),
				);
			}
		}
		if (expressions) {
	...
	function hoistBindingIdentifier(
		...
		// NOTE: `node` has already been visited
		const name = isGeneratedIdentifier(node)
			? node
		...
		if (isExport) {
			...
			const specifier = factory.createExportSpecifier(
				/*isTypeOnly*/ false,
				localName,
				exportName,
			);
	...
	function createDownlevelUsingStatements(
		...
		const envVarList = factory.createVariableDeclarationList(
			[envVar],
			NodeFlags.Const,
		);
		const envVarStatement = factory.createVariableStatement(
			/*modifiers*/ undefined,
			envVarList,
		);
			...
			finallyBlock = factory.createBlock(
				[
						...
						factory.createVariableDeclarationList(
							[
								factory.createVariableDeclaration(
									...
									/*exclamationToken*/ undefined,
									/*type*/ undefined,
									emitHelpers().createDisposeResourcesHelper(
										envBinding,
									),
								),
							],
					...
					factory.createIfStatement(
						result,
						factory.createExpressionStatement(
							factory.createAwaitExpression(result),
						),
					),
				],
				/*multiLine*/ true,
...
function countPrologueStatements(statements: readonly Statement[]) {
	for (let i = 0; i < statements.length; i++) {
		if (
			!isPrologueDirective(statements[i]) &&
			!isCustomPrologue(statements[i])
		) {
			return i;
		}
	}
...
function isUsingVariableDeclarationList(
	node: Node,
): node is VariableDeclarationList & { _usingBrand: any } {
	return (
		isVariableDeclarationList(node) &&
		getUsingKindOfVariableDeclarationList(node) !== UsingKind.None
	);
}

function getUsingKindOfVariableDeclarationList(node: VariableDeclarationList) {
	return (node.flags & NodeFlags.BlockScoped) === NodeFlags.AwaitUsing
		? UsingKind.Async
		: (node.flags & NodeFlags.BlockScoped) === NodeFlags.Using
			? UsingKind.Sync
			: UsingKind.None;
}
...
function getUsingKind(statement: Statement): UsingKind {
	return isVariableStatement(statement)
		? getUsingKindOfVariableStatement(statement)
		: UsingKind.None;
}
...
function getUsingKindOfCaseOrDefaultClauses(
