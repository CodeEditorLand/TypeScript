...
/** @internal */
export function transformESDecorators(
	...
	function shouldVisitNode(node: Node) {
		return (
			!!(node.transformFlags & TransformFlags.ContainsDecorators) ||
			(!!classThis &&
	...
	function modifierVisitor(
		node: ModifierLike,
	): VisitResult<Modifier | undefined> {
		switch (node.kind) {
			case SyntaxKind.Decorator: // elided, will be emitted as part of `visitClassDeclaration`
				return undefined;
			default:
	...
	function getHelperVariableName(node: ClassLikeDeclaration | ClassElement) {
		let declarationName =
			...
			!isGeneratedIdentifier(node.name)
				? idText(node.name)
				: node.name &&
					...
					: node.name &&
						  ...
						  isIdentifierText(node.name.text, ScriptTarget.ESNext)
						? node.name.text
						: isClassLike(node)
							? "class"
							: "member";
	...
	function createHelperVariable(
		node: ClassLikeDeclaration | ClassElement,
		suffix: string,
	) {
					...
					if (!staticMethodExtraInitializersName) {
						...
						const initializer =
							emitHelpers().createRunInitializersHelper(
								classThis ?? factory.createThis(),
								staticMethodExtraInitializersName,
							);
						setSourceMapRange(
							initializer,
							node.name ?? moveRangePastDecorators(node),
						);
			...
			if (isClassStaticBlockDeclaration(member)) {
				...
				if (hasStaticModifier(member)) {
					hasStaticInitializers ||=
						!!member.initializer || hasDecorators(member);
				} else {
					hasNonAmbientInstanceFields ||=
						!isAmbientPropertyDeclaration(member);
				}
			}
	...
	function transformClassLike(node: ClassLikeDeclaration) {
		...
		// 1. Class decorators are evaluated outside of the private name scope of the class.
		const classDecorators = transformAllDecoratorsOfDeclaration(
			getAllDecoratorsOfClass(node),
		);
		if (classDecorators) {
			...
			classDefinitionStatements.push(
				createLet(
					...
					factory.createArrayLiteralExpression(classDecorators),
				),
		...
		const extendsElement =
			extendsClause && firstOrUndefined(extendsClause.types);
		const extendsExpression =
			extendsElement &&
			visitNode(extendsElement.expression, visitor, isExpression);
		if (extendsExpression) {
			...
			const safeExtendsExpression =
				...
				isArrowFunction(unwrapped)
					...
					: extendsExpression;
			classDefinitionStatements.push(
				createLet(classInfo.classSuper, safeExtendsExpression),
			);
			const updatedExtendsElement =
				factory.updateExpressionWithTypeArguments(
					extendsElement,
					classInfo.classSuper,
					/*typeArguments*/ undefined,
				);
			const updatedExtendsClause = factory.updateHeritageClause(
				extendsClause,
		...
		if (pendingExpressions) {
			...
			for (let expression of pendingExpressions) {
				...
				expression = visitNode(
					expression,
					function thisVisitor(node: Node): Node {
						if (
							!(
								node.transformFlags &
								TransformFlags.ContainsLexicalThis
							)
						) {
							return node;
						}
						...
						switch (node.kind) {
							case SyntaxKind.ThisKeyword:
								if (!outerThis) {
									outerThis = factory.createUniqueName(
										"_outerThis",
										GeneratedIdentifierFlags.Optimistic,
									);
									classDefinitionStatements.unshift(
										createLet(
											outerThis,
											factory.createThis(),
										),
		...
		if (
			some(classInfo.pendingInstanceInitializers) &&
			...
			if (initializerStatements) {
				...
				const isDerivedClass = !!(
					...
					skipOuterExpressions(extendsClauseElement.expression)
						.kind !== SyntaxKind.NullKeyword
				);
				const constructorStatements: Statement[] = [];
				if (isDerivedClass) {
					const spreadArguments = factory.createSpreadElement(
					...
					constructorStatements.push(
						factory.createExpressionStatement(superCall),
					);
				}

				addRange(constructorStatements, initializerStatements);

				const constructorBody = factory.createBlock(
		...
		if (classInfo.memberInfos) {
			forEachEntry(classInfo.memberInfos, (memberInfo, member) => {
				if (isStatic(member)) {
					classDefinitionStatements.push(
						createLet(memberInfo.memberDecoratorsName),
					);
					...
					if (memberInfo.memberDescriptorName) {
						classDefinitionStatements.push(
							createLet(memberInfo.memberDescriptorName),
						);
					}
				}
			});
		...
		leadingBlockStatements = addRange(
			leadingBlockStatements,
		...
		if (
			...
			const classDescriptor = factory.createObjectLiteralExpression([
				valueProperty,
			]);
			const classDescriptorAssignment = factory.createAssignment(
				classInfo.classDescriptorName,
				classDescriptor,
			);
			const classNameReference = factory.createPropertyAccessExpression(
				renamedClassThis,
				"name",
			);
			...
			const esDecorateStatement =
				factory.createExpressionStatement(esDecorateHelper);
			setSourceMapRange(
				esDecorateStatement,
			...
			const classDescriptorValueReference =
				factory.createPropertyAccessExpression(
					classInfo.classDescriptorName,
					"value",
				);
			const classThisAssignment = factory.createAssignment(
				classInfo.classThis,
				classDescriptorValueReference,
			);
			const classReferenceAssignment = factory.createAssignment(
				classReference,
				classThisAssignment,
			);
			leadingBlockStatements.push(
				factory.createExpressionStatement(classReferenceAssignment),
			);
		}
		...
		leadingBlockStatements.push(
			createSymbolMetadata(renamedClassThis, classInfo.metadataReference),
		);
		...
		if (some(classInfo.pendingStaticInitializers)) {
			for (const initializer of classInfo.pendingStaticInitializers) {
				const initializerStatement =
					factory.createExpressionStatement(initializer);
				setSourceMapRange(
					initializerStatement,
					getSourceMapRange(initializer),
				);
				trailingBlockStatements = append(
		...
		if (classInfo.classExtraInitializersName) {
			const runClassInitializersHelper =
				emitHelpers().createRunInitializersHelper(
					renamedClassThis,
					classInfo.classExtraInitializersName,
				);
		...
		if (
			leadingBlockStatements &&
			trailingBlockStatements &&
			!classInfo.hasStaticInitializers
		) {
		...
		if (leadingStaticBlock || syntheticConstructor || trailingStaticBlock) {
			...
			const existingNamedEvaluationHelperBlockIndex = members.findIndex(
				isClassNamedEvaluationHelperBlock,
			);
			...
			members = setTextRange(
				factory.createNodeArray(newMembers),
				members,
			);
		}
		...
		if (classDecorators) {
			...
			const classReferenceVarDeclList =
				factory.createVariableDeclarationList([
					classReferenceDeclaration,
				]);
			const returnExpr = classInfo.classThis
				? factory.createAssignment(classReference, classInfo.classThis)
				: classReference;
			classDefinitionStatements.push(
			...
			classDefinitionStatements.push(
				factory.createReturnStatement(classExpression),
			);
		}
	...
	function visitClassDeclaration(
		...
		if (isDecoratedClassLike(node)) {
			...
			const className = originalClass.name
				? factory.createStringLiteralFromNode(originalClass.name)
			...
			if (isExport && isDefault) {
				...
				if (node.name) {
					...
					const varDecls = factory.createVariableDeclarationList(
						[varDecl],
						NodeFlags.Let,
					);
					...
					const exportStatement = factory.createExportDefault(
				...
				const iife = transformClassLike(node);
				const modifierVisitorNoExport = isExport
					? (node: ModifierLike) =>
							isExportModifier(node)
								? undefined
								: modifierVisitor(node)
					: modifierVisitor;
				const modifiers = visitNodes(
					node.modifiers,
					modifierVisitorNoExport,
					isModifier,
				);
				const declName = factory.getLocalName(
					node,
					/*allowComments*/ false,
				...
				if (isExport) {
					...
					const exportStatement =
						factory.createExternalModuleExport(declName);
					setOriginalNode(exportStatement, node);
					statements.push(exportStatement);
				}
			}
	...
	function transformConstructorBodyWorker(
		...
		if (isTryStatement(superStatement)) {
			...
			const tryBlockStatementsArray =
				factory.createNodeArray(tryBlockStatements);
			setTextRange(
			...
			statementsOut.push(
				factory.updateTryStatement(
					superStatement,
					factory.updateBlock(
						superStatement.tryBlock,
						tryBlockStatements,
					),
					visitNode(
						superStatement.catchClause,
						visitor,
						isCatchClause,
					),
	...
	function visitConstructorDeclaration(node: ConstructorDeclaration) {
		...
		if (node.body && classInfo) {
			// If there are instance extra initializers we need to add them to the body along with any
			// field initializers
			const initializerStatements = prepareConstructor(
				classInfo.class,
				classInfo,
			);
			if (initializerStatements) {
				...
				const superStatementIndices = findSuperStatementIndexPath(
					node.body.statements,
					nonPrologueStart,
				);
				if (superStatementIndices.length > 0) {
					...
					addRange(
						statements,
						visitNodes(node.body.statements, visitor, isStatement),
					);
				}
	...
	>(
		...
		if (memberDecorators) {
			...
			const memberDecoratorsAssignment = factory.createAssignment(
				memberDecoratorsName,
				memberDecoratorsArray,
			);
			...
			const kind = isGetAccessorDeclaration(member)
				? "getter"
			...
			if (isIdentifier(member.name) || isPrivateIdentifier(member.name)) {
				...
				propertyName = {
					computed: true,
					name: factory.createStringLiteralFromNode(member.name),
				};
			} else {
				const expression = member.name.expression;
				if (
					isPropertyNameLiteral(expression) &&
					!isIdentifier(expression)
				) {
					propertyName = {
						computed: true,
						name: factory.createStringLiteralFromNode(expression),
					};
				} else {
					enterName();
			...
			if (isMethodOrAccessor(member)) {
				...
				const methodExtraInitializersName = isStatic(member)
					? classInfo.staticMethodExtraInitializersName
					: classInfo.instanceMethodExtraInitializersName;
				...
				if (
					...
					descriptor = createDescriptor(
						...
						visitNodes(
							modifiers,
							(node) => tryCast(node, isAsyncModifier),
							isModifier,
						),
					);
					memberInfo.memberDescriptorName = descriptorName =
						createHelperVariable(member, "descriptor");
					descriptor = factory.createAssignment(
						descriptorName,
						descriptor,
					);
				}

				const esDecorateExpression =
				...
				initializersName = memberInfo.memberInitializersName ??=
	...
	function visitMethodDeclaration(node: MethodDeclaration) {
		enterClassElement(node);
		const { modifiers, name, descriptorName } =
	...
	): VisitResult<ClassStaticBlockDeclaration> {
		enterClassElement(node);
		...
		if (isClassNamedEvaluationHelperBlock(node)) {
			...
			if (classInfo) {
				...
				if (some(classInfo.pendingStaticInitializers)) {
					...
					const staticBlock =
						factory.createClassStaticBlockDeclaration(body);
	...
	function visitPropertyDeclaration(node: PropertyDeclaration) {
		...
		if (hasAccessorModifier(node) && descriptorName) {
			...
			const modifiersWithoutAccessor = visitNodes(
				modifiers,
				(node) =>
					node.kind !== SyntaxKind.AccessorKeyword ? node : undefined,
				isModifier,
			);
			...
			const getter = createGetAccessorDescriptorForwarder(
				modifiersWithoutAccessor,
				getterName,
				descriptorName,
			);
			...
			const setter = createSetAccessorDescriptorForwarder(
				modifiersWithoutAccessor,
				setterName,
				descriptorName,
			);
	...
	function visitCallExpression(node: CallExpression) {
		if (isSuperProperty(node.expression) && classThis) {
			...
			const argumentsList = visitNodes(
				node.arguments,
				visitor,
				isExpression,
			);
	...
	function isAnonymousClassNeedingAssignedName(
		...
		return (
			isClassExpression(node) && !node.name && isDecoratedClassLike(node)
		);
	}

	function canIgnoreEmptyStringLiteralInAssignedName(
		node: WrappedExpression<AnonymousFunctionDefinition>,
	) {
		...
		return (
			isClassExpression(innerExpression) &&
	...
	function visitBinaryExpression(node: BinaryExpression, discarded: boolean) {
		...
		if (isAssignmentExpression(node)) {
			...
			if (isNamedEvaluation(node, isAnonymousClassNeedingAssignedName)) {
				node = transformNamedEvaluation(
					context,
					node,
					canIgnoreEmptyStringLiteralInAssignedName(node.right),
				);
				return visitEachChild(node, visitor, context);
			}
		...
		if (node.operatorToken.kind === SyntaxKind.CommaToken) {
			const left = visitNode(
				node.left,
				discardedValueVisitor,
				isExpression,
			);
			const right = visitNode(
				node.right,
				discarded ? discardedValueVisitor : visitor,
				isExpression,
			);
			return factory.updateBinaryExpression(
				node,
		...
		if (
			...
			if (isSuperProperty(operand) && classThis && classSuper) {
				let setterName = isElementAccessExpression(operand)
					...
					: isIdentifier(operand.name)
						? factory.createStringLiteralFromNode(operand.name)
						: undefined;
				if (setterName) {
					let getterName = setterName;
					if (!isSimpleInlineableExpression(setterName)) {
	...
	function visitReferencedPropertyName(node: PropertyName) {
		...
		if (
			isPropertyNameLiteral(node.expression) &&
			!isIdentifier(node.expression)
		) {
			const referencedName = factory.createStringLiteralFromNode(
				node.expression,
			);
			const name = visitNode(node, visitor, isPropertyName);
			return { referencedName, name };
		...
		const key = emitHelpers().createPropKeyHelper(
			visitNode(node.expression, visitor, isExpression),
		);
		const assignment = factory.createAssignment(referencedName, key);
		const name = factory.updateComputedPropertyName(
			node,
			injectPendingExpressions(assignment),
		);
		return { referencedName, name };
	}
		...
		if (isSuperProperty(node) && classThis && classSuper) {
			const propertyName = isElementAccessExpression(node)
				...
				: isIdentifier(node.name)
					...
					: undefined;
			if (propertyName) {
				const paramName = factory.createTempVariable(
	...
	function visitAssignmentElement(
		...
		if (isAssignmentExpression(node, /*excludeCompoundAssignment*/ true)) {
			...
			const assignmentTarget = visitDestructuringAssignmentTarget(
				node.left,
			);
			...
			return visitDestructuringAssignmentTarget(
				node,
			) as ArrayAssignmentElement;
		}
	}
	...
	function visitAssignmentRestElement(node: SpreadElement) {
		if (isLeftHandSideExpression(node.expression)) {
			const expression = visitDestructuringAssignmentTarget(
				node.expression,
			);
	...
	function visitAssignmentProperty(node: PropertyAssignment) {
		...
		if (isLeftHandSideExpression(node.initializer)) {
			const assignmentElement = visitDestructuringAssignmentTarget(
				node.initializer,
			);
			return factory.updatePropertyAssignment(
				node,
	...
	function visitParenthesizedExpression(
		...
		const visitorFunc: Visitor<Node, Node> = discarded
			? discardedValueVisitor
			: visitor;
		const expression = visitNode(
			node.expression,
	...
	function injectPendingExpressionsCommon(
		pendingExpressions: Expression[] | undefined,
	...
	function injectPendingExpressions(expression: Expression) {
		const result = injectPendingExpressionsCommon(
			pendingExpressions,
			expression,
		);
	...
	function injectPendingInitializers(
		classInfo: ClassInfo,
		isStatic: boolean,
		expression: Expression | undefined,
	) {
	...
	function transformDecorator(decorator: Decorator) {
		...
		if (isAccessExpression(innerExpression)) {
			...
			return factory.restoreOuterExpressions(
				expression,
				factory.createFunctionBindCall(target, thisArg, []),
			);
		}
		return expression;
	}
	...
	function createDescriptorMethod(
		...
		const functionName = factory.createStringLiteralFromNode(
			name,
			/*isSingleQuote*/ undefined,
		);
		const namedFunction = emitHelpers().createSetFunctionNameHelper(
			func,
			functionName,
			prefix,
		);
		const method = factory.createPropertyAssignment(
	...
	function createMethodDescriptorForwarder(
		...
		modifiers = visitNodes(
			modifiers,
			(node) => (isStaticModifier(node) ? node : undefined),
			isModifier,
		);
	...
	function createMetadata(
		name: Identifier,
		classSuper: Identifier | undefined,
	) {
		const varDecl = factory.createVariableDeclaration(
			name,
			/*exclamationToken*/ undefined,
			...
			factory.createConditionalExpression(
				...
				factory.createCallExpression(
					...
					[
						classSuper
							? createSymbolMetadataReference(classSuper)
							: factory.createNull(),
					],
				),
	...
	function createSymbolMetadata(
		target: Identifier | ThisExpression,
		value: Identifier,
	) {
		...
		return setEmitFlags(
			factory.createIfStatement(
				value,
				factory.createExpressionStatement(defineProperty),
			),
