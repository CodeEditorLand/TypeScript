...
/** @internal */
export function transformES2015(
	...
	function isReturnVoidStatementInConstructorWithCapturedSuper(
		...
		return (
			(hierarchyFacts & HierarchyFacts.ConstructorWithSuperCall) !== 0 &&
	...
	function classWrapperStatementVisitor(
		...
		if (shouldVisitNode(node)) {
			...
			if (
				isPropertyDeclaration(original) &&
				hasStaticModifier(original)
			) {
	...
	function callExpressionVisitor(node: Node): VisitResult<Node | undefined> {
		if (node.kind === SyntaxKind.SuperKeyword) {
			return visitSuperKeyword(
				node as SuperExpression,
				/*isExpressionOfCall*/ true,
			);
		}
		return visitor(node);
	}
	...
	function returnCapturedThis(node: Node): ReturnStatement {
		return setOriginalNode(
			factory.createReturnStatement(createCapturedThis()),
			node,
		);
	}
			...
			return setOriginalNode(
				setTextRange(
					factory.createIdentifier(
						unescapeLeadingUnderscores(node.escapedText),
					),
					node,
				),
				node,
	...
	function visitBreakOrContinueStatement(
		...
		if (convertedLoopState) {
			...
			const canUseBreakOrContinue =
				...
				(!node.label &&
					convertedLoopState.allowedNonLabeledJumps! & jump);
	...
	function isUninitializedVariableStatement(node: Statement) {
		return (
			...
			every(
				...
				(decl) => isIdentifier(decl.name) && !decl.initializer,
			)
		);
	}
	...
	function transformConstructorBody(
		...
		const body = factory.createBlock(
			setTextRange(
				factory.createNodeArray([...prologue, ...statements]),
	...
	type CapturedThis = GeneratedIdentifier & {
		readonly escapedText: __String & "__this";
	};
	type SyntheticSuper = GeneratedIdentifier & {
		readonly escapedText: __String & "__super";
	};
	...
	type ThisCapturingVariableStatement = VariableStatement & {
		readonly declarationList: {
			readonly declarations: { 0: ThisCapturingVariableDeclaration };
		};
	};
	type ThisCapturingAssignmentExpression =
		AssignmentExpression<EqualsToken> & { readonly left: CapturedThis };
	type TransformedSuperCall = CallExpression & {
		readonly expression: PropertyAccessExpression & {
			readonly expression: SyntheticSuper;
		};
	};
	...
	type ThisCapturingTransformedSuperCallWithFallback =
		ThisCapturingAssignmentExpression & {
			readonly right: TransformedSuperCallWithFallback;
		};
	type ThisCapturingImplicitSuperCallWithFallback =
		ThisCapturingAssignmentExpression & {
	...
	function isThisCapturingVariableStatement(
		node: Node,
	): node is ThisCapturingVariableStatement {
		return (
			isVariableStatement(node) &&
			node.declarationList.declarations.length === 1 &&
			isThisCapturingVariableDeclaration(
	...
	function isThisCapturingVariableDeclaration(
		node: Node,
	): node is ThisCapturingVariableDeclaration {
		return (
			isVariableDeclaration(node) &&
			isCapturedThis(node.name) &&
			!!node.initializer
		);
	}

	/** Tests whether `node` is an AssignmentExpression like `_this = ...` */
	function isThisCapturingAssignment(
		node: Node,
	): node is ThisCapturingAssignmentExpression {
		return (
	...
	function isTransformedSuperCall(node: Node): node is TransformedSuperCall {
		return (
			...
			(idText(node.expression.name) === "call" ||
				idText(node.expression.name) === "apply") &&
	...
	function isTransformedSuperCallWithFallback(
		node: Node,
	): node is TransformedSuperCallWithFallback {
		return (
	...
	function isImplicitSuperCallWithFallback(
		node: Node,
	): node is ImplicitSuperCallWithFallback {
		return (
			isBinaryExpression(node) &&
			node.operatorToken.kind === SyntaxKind.BarBarToken &&
			node.right.kind === SyntaxKind.ThisKeyword &&
	...
	function isThisCapturingTransformedSuperCallWithFallback(
		...
		return (
			isThisCapturingAssignment(node) &&
			isTransformedSuperCallWithFallback(node.right)
		);
	}
	...
	function isThisCapturingImplicitSuperCallWithFallback(
		...
		return (
			isThisCapturingAssignment(node) &&
			isImplicitSuperCallWithFallback(node.right)
		);
	}

	function isTransformedSuperCallLike(
		node: Node,
	): node is TransformedSuperCallLike {
		return (
			isTransformedSuperCall(node) ||
		...
		for (let i = 0; i < body.statements.length - 1; i++) {
			...
			while (superCallIndex < body.statements.length) {
				...
				if (isExpressionStatement(statement)) {
					if (
						isTransformedSuperCallLike(
							skipOuterExpressions(statement.expression),
						)
					) {
						break;
					}
			...
			const following = body.statements[
				superCallIndex
			...
			const newStatements = factory.createNodeArray([
				...
				...body.statements.slice(
					thisCaptureStatementIndex + 1,
					superCallIndex,
	...
	function simplifyConstructorInlineSuperReturn(
		...
		for (const statement of original.statements) {
			if (
				...
				!getSuperCallFromStatement(statement)
			) {
				return body;
			}
		}
		...
		const canElideThisCapturingVariable =
		...
		for (let i = body.statements.length - 1; i > 0; i--) {
			...
			if (
				...
				isCapturedThis(statement.expression)
			) {
				const preceding = body.statements[i - 1];
				let expression: Expression | undefined;
				if (
					isExpressionStatement(preceding) &&
					isThisCapturingTransformedSuperCallWithFallback(
						skipOuterExpressions(preceding.expression),
					)
				) {
					...
					canElideThisCapturingVariable &&
					isThisCapturingVariableStatement(preceding)
				) {
					const varDecl = preceding.declarationList.declarations[0];
					if (
						isTransformedSuperCallLike(
							skipOuterExpressions(varDecl.initializer),
						)
					) {
	 ...
	 */
	function simplifyConstructorElideUnusedThisCapture(
		body: Block,
		original: Block,
	) {
		// given:
	...
	function injectSuperPresenceCheckWorker(
		...
		if (
			...
			idText(node.arguments[1]) === "arguments"
		) {
	...
	function isSufficientlyCoveredByReturnStatements(
		...
		// A block is covered if it has a last statement which is covered.
			...
			const lastStatement = lastOrUndefined(
				(statement as Block).statements,
			);
			if (
				lastStatement &&
	...
	function insertDefaultValueAssignmentForInitializer(
		...
		initializer = Debug.checkDefined(
			visitNode(initializer, visitor, isExpression),
		);
	...
	function transformAccessorsToExpression(
		...
		if (getAccessor) {
			...
			const getter = factory.createPropertyAssignment(
				"get",
				getterFunction,
			);
		...
		if (setAccessor) {
			...
			const setter = factory.createPropertyAssignment(
				"set",
				setterFunction,
			);
	...
	function shouldEmitExplicitInitializerForLetDeclaration(
		...
		const emittedAsTopLevel =
			...
			(isCapturedInFunction &&
				isDeclaredInLoop &&
				(hierarchyFacts & HierarchyFacts.IterationStatementBlock) !==
					0);
	...
	function visitVariableDeclaration(
		...
		if (isBindingPattern(node.name)) {
			updated = flattenDestructuringBinding(
				...
				FlattenLevel.All,
				/*rval*/ undefined,
				(ancestorFacts & HierarchyFacts.ExportedVariableStatement) !==
					0,
			);
		} else {
			updated = visitEachChild(node, visitor, context);
	...
	function visitLabeledStatement(
	...
	): VisitResult<Statement | undefined> {
		...
		return isIterationStatement(
			...
			? visitIterationStatement(
					statement,
					/*outermostLabeledStatement*/ node,
				)
	...
	function createFunctionForInitializerOfForStatement(
		...
		const containsYield =
			(node.initializer.transformFlags & TransformFlags.ContainsYield) !==
			0;
	...
	function createFunctionForBodyOfIterationStatement(
		...
		const containsYield =
			(node.statement.transformFlags & TransformFlags.ContainsYield) !==
			0;
		...
		if (isSimpleLoop) {
			...
			if (state.nonLocalJumps! & Jump.Return) {
				...
				if (outerState) {
					...
					returnStatement =
						factory.createReturnStatement(loopResultName);
				} else {
	...
	function processLoopVariableDeclaration(
		...
		if (isBindingPattern(name)) {
			...
			if (
				...
				const outParamName = factory.createUniqueName(
					"out_" + idText(name),
				);
				...
				if (isForStatement(container)) {
					if (
						...
						resolver.isBindingCapturedByNode(
							container.initializer,
							decl,
						)
					) {
	...
	function addStatementToStartOfBlock(
		...
		return factory.updateBlock(block, [
			statement,
	...
	function visitTypeScriptClassWrapper(node: CallExpression) {
		...
		const returnStatement = tryCast(
			elementAt(funcStatements, classBodyEnd),
			isReturnStatement,
		);
	...
	function transformAndSpreadElements(
		...
		for (let i = startsWithSpread ? 0 : 1; i < segments.length; i++) {
			...
			expression = helpers.createSpreadArrayHelper(
				expression,
				segment.expression,
				segment.kind === SpreadSegmentKind.UnpackedSpread &&
					!isArgumentList,
			);
		}
		return expression;
	...
	function visitTemplateExpression(node: TemplateExpression): Expression {
		let expression: Expression = factory.createStringLiteral(
			node.head.text,
		);
	...
	function visitSuperKeyword(
		...
		const expression =
			hierarchyFacts & HierarchyFacts.NonStaticClassElement &&
			!isExpressionOfCall
				? factory.createPropertyAccessExpression(
						setOriginalNode(createSyntheticSuper(), node),
						"prototype",
					)
				: createSyntheticSuper();
	...
	function substituteIdentifier(node: Identifier) {
		...
		if (
			...
			if (original && isNameOfDeclarationWithCollidingName(original)) {
				return setTextRange(
					factory.getGeneratedNameForNode(original),
					node,
				);
			}
		}
	...
	function substituteExpressionIdentifier(node: Identifier): Identifier {
		if (
			...
			if (
				declaration &&
				!(
					isClassLike(declaration) &&
					isPartOfClassBody(declaration, node)
				)
			) {
				return setTextRange(
	...
	function isPartOfClassBody(
		...
		while (currentNode) {
			...
			if (
				isClassElement(currentNode) &&
				currentNode.parent === declaration
			) {
				return true;
			}
	...
	function hasSynthesizedDefaultSuperCall(
		...
		const statementExpression = (statement as ExpressionStatement)
			.expression;
		if (
		...
		return (
			isIdentifier(expression) && expression.escapedText === "arguments"
		);
	}
}
