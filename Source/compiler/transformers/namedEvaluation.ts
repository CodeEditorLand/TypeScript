...
function getAssignedNameOfPropertyName(
	...
	if (
		isPropertyNameLiteral(name.expression) &&
		!isIdentifier(name.expression)
	) {
		const assignedName = factory.createStringLiteralFromNode(
			name.expression,
		);
		return { assignedName, name };
	}
	...
	const key = context
		...
		.createPropKeyHelper(name.expression);
 ...
 * @internal
 */
export function isClassNamedEvaluationHelperBlock(
	node: Node,
): node is ClassNamedEvaluationHelperBlock {
	if (
	...
	return (
		...
		isCallToHelper(
			...
			"___setFunctionName" as __String,
		) &&
		(statement.expression as CallExpression).arguments.length >= 2 &&
		(statement.expression as CallExpression).arguments[1] ===
			node.emitNode?.assignedName
	);
}
 ...
 * @internal
 */
export function classHasExplicitlyAssignedName(
 ...
 * @internal
 */
export function classHasDeclaredOrExplicitlyAssignedName(
	node: ClassLikeDeclaration,
): boolean {
...
export function injectClassNamedEvaluationHelperBlockIfMissing(
	...
	const { factory } = context;
	const namedEvaluationBlock = createClassNamedEvaluationHelperBlock(
		context,
		assignedName,
		thisExpression,
	);
...
function finishTransformNamedEvaluation(
	...
	if (
		...
		isStringLiteral(assignedName) &&
		isEmptyStringLiteral(assignedName)
	) {
		return expression;
	}
	...
	const updatedExpression = isClassExpression(innerExpression)
		...
		: context
				.getEmitHelperFactory()
				.createSetFunctionNameHelper(innerExpression, assignedName);
...
function transformNamedEvaluationOfPropertyAssignment(
	...
	const { factory } = context;
	const { assignedName, name } = getAssignedNameOfPropertyName(
		context,
		node.name,
		assignedNameText,
	);
...
function transformNamedEvaluationOfShorthandAssignmentProperty(
	...
	const { factory } = context;
	const assignedName =
		assignedNameText !== undefined
			? factory.createStringLiteral(assignedNameText)
...
export function transformNamedEvaluation<T extends NamedEvaluation>(
	...
	NamedEvaluation,
	Pick<T, "kind" | (keyof T & "operatorToken") | (keyof T & "name")>
>;
