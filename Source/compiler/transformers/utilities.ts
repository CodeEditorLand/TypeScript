...
export function getImportNeedsImportStarHelper(
	...
	return (
		...
		(!!(bindings.elements.length - defaultRefCount) &&
		...
		switch (node.kind) {
			...
			case SyntaxKind.VariableStatement:
				if (hasSyntacticModifier(node, ModifierFlags.Export)) {
					for (const decl of (node as VariableStatement)
	...
	private static toKey(name: Identifier | PrivateIdentifier): string {
		if (isGeneratedPrivateIdentifier(name) || isGeneratedIdentifier(name)) {
			...
			if (
				...
				const baseName =
					isMemberName(node) && node !== name
						? IdentifierNameMap.toKey(node)
 ...
 * @internal
 */
export function getSuperCallFromStatement(
	statement: Statement,
): SuperCall | undefined {
 ...
 * @internal
 */
export function findSuperStatementIndexPath(
	statements: NodeArray<Statement>,
	start: number,
) {
 ...
 * @internal
 */
export function getAllDecoratorsOfClass(
...
function getAllDecoratorsOfAccessors(
	...
	const { firstAccessor, secondAccessor, getAccessor, setAccessor } =
		getAllAccessorDeclarations(parent.members, accessor);
	const firstAccessorWithDecorators = hasDecorators(firstAccessor)
		? firstAccessor
		: secondAccessor && hasDecorators(secondAccessor)
			? secondAccessor
			: undefined;
...
function getAllDecoratorsOfMethod(
	method: MethodDeclaration | AccessorDeclaration,
): AllDecorators | undefined {
	if (!method.body) {
		return undefined;
	}
...
> {
	...
	readonly previous:
		| LexicalEnvironment<TEnvData, TPrivateEnvData, TPrivateEntry>
		| undefined;
}

/** @internal */
export function walkUpLexicalEnvironments<
	TEnvData,
	TPrivateEnvData,
	TPrivateEntry,
	U,
>(
	env:
		...
		| undefined,
	cb: (
		env: LexicalEnvironment<TEnvData, TPrivateEnvData, TPrivateEntry>,
	) => U,
): U | undefined {
	while (env) {
		const result = cb(env);
...
/** @internal */
export function newPrivateEnvironment<TData, TEntry>(
	data: TData,
): PrivateEnvironment<TData, TEntry> {
	return { data };
}
...
export function setPrivateIdentifier<TData, TEntry>(
	...
	if (isGeneratedPrivateIdentifier(name)) {
		...
		privateEnv.generatedIdentifiers.set(
...
>(
	...
	return walkUpLexicalEnvironments(env, (env) =>
		getPrivateIdentifier(env.privateEnv, name),
	);
}
