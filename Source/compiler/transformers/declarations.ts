...
export function transformDeclarations(context: TransformationContext) {
	...
	let necessaryTypeReferences:
		| Set<[specifier: string, mode: ResolutionMode]>
		| undefined;
	let lateMarkedStatements: LateVisibilityPaintedStatement[] | undefined;
	let lateStatementReplacementMap: Map<
		NodeId,
		VisitResult<
			LateVisibilityPaintedStatement | ExportAssignment | undefined
		>
	>;
	...
	function recordTypeReferenceDirectivesIfNecessary(
		typeReferenceDirectives:
			| readonly [specifier: string, mode: ResolutionMode][]
			| undefined,
	): void {
	...
	function trackReferencedAmbientModuleFromImport(
		node:
			| ImportDeclaration
			| ExportDeclaration
			| ImportEqualsDeclaration
		...
		const symbol =
			moduleSpecifier && resolver.tryFindAmbientModule(moduleSpecifier);
		if (symbol?.declarations) {
			for (const decl of symbol.declarations) {
				if (
					isAmbientModule(decl) &&
					getSourceFileOfNode(decl) !== currentSourceFile
				) {
			...
			if (errorInfo) {
				if (errorInfo.typeName) {
					context.addDiagnostic(
						createDiagnosticForNode(
							symbolAccessibilityResult.errorNode ||
								errorInfo.errorNode,
	...
	function transformRoot(node: SourceFile | Bundle) {
			...
			const bundle = factory.createBundle(
				map(node.sourceFiles, (sourceFile) => {
					...
					const updated = isSourceFileJS(sourceFile)
						? factory.createNodeArray(
								transformDeclarationsForJS(sourceFile),
							)
						: visitNodes(
								sourceFile.statements,
								visitDeclarationStatements,
		...
		function getLibReferences() {
			return arrayFrom(libs.keys(), (lib) => ({
				fileName: lib,
				pos: -1,
				end: -1,
			}));
			...
			if (emittedImports) {
				for (const importStatement of emittedImports) {
					if (
						...
						isExternalModuleReference(
							importStatement.moduleReference,
						)
					) {
						const expr = importStatement.moduleReference.expression;
						if (
							isStringLiteralLike(expr) &&
							expr.text === typeName
						) {
							return undefined;
						}
		...
		function mapReferencesIntoArray(
			...
			return (file) => {
				...
				if (file.isDeclarationFile) {
					...
					if (
						isBundledEmit &&
						contains((node as Bundle).sourceFiles, file)
					)
	...
	function filterBindingPatternInitializers(name: BindingName) {
		...
		function visitBindingElement(
			...
			if (
				elem.propertyName &&
				isComputedPropertyName(elem.propertyName) &&
				isEntityNameExpression(elem.propertyName.expression)
			) {
	...
	function ensureType(
		...
		if (
			!ignorePrivate &&
			hasEffectiveModifier(node, ModifierFlags.Private)
		) {
	...
	function rewriteModuleSpecifier<T extends Node>(
		...
		if (isStringLiteralLike(input)) {
			if (isBundledEmit) {
				...
				const symbol =
					resolver.getSymbolOfExternalModuleSpecifier(input);
				if (symbol) {
					(
						exportedModulesFromDeclarationEmit ||
						(exportedModulesFromDeclarationEmit = [])
	...
	function transformImportDeclaration(decl: ImportDeclaration) {
		...
		if (
			decl.importClause.namedBindings.kind === SyntaxKind.NamespaceImport
		) {
	...
	function transformAndReplaceLatePaintedStatements(
		...
		while (length(lateMarkedStatements)) {
			...
			if (!isLateVisibilityPaintedStatement(i)) {
				return Debug.fail(
					`Late replaced statement was found which is not handled by the declaration transformer!: ${Debug.formatSyntaxKind((i as Node).kind)}`,
				);
			}
		...
		return visitNodes(
			statements,
			visitLateVisibilityMarkedStatements,
			isStatement,
		);
		...
		function visitLateVisibilityMarkedStatements(statement: Statement) {
			if (isLateVisibilityPaintedStatement(statement)) {
				...
				if (lateStatementReplacementMap.has(key)) {
					const result = lateStatementReplacementMap.get(key) as
						| Statement
						| readonly Statement[]
						| undefined;
	...
	function visitDeclarationStatements(
		input: Node,
	): VisitResult<Node | undefined> {
	...
	function updateModuleDeclarationAndKeyword(
		...
		const updated = factory.updateModuleDeclaration(
			node,
			modifiers,
			name,
			body,
		...
		switch (input.kind) {
			...
			case SyntaxKind.FunctionDeclaration: {
				...
				if (
					...
					const props =
						resolver.getPropertiesOfContainerFunction(input);
					...
					let declarations: (
						...
						const nameStr = unescapeLeadingUnderscores(
							p.escapedName,
						);
			...
			case SyntaxKind.ClassDeclaration: {
				...
				const typeParameters = ensureTypeParams(
					input,
					input.typeParameters,
				);
	...
	function transformVariableStatement(input: VariableStatement) {
		...
		if (
			isVarUsing(input.declarationList) ||
			isVarAwaitUsing(input.declarationList)
		) {
			declList = factory.createVariableDeclarationList(
				nodes,
				NodeFlags.Const,
			);
	...
	function checkName(node: DeclarationDiagnosticProducing) {
		...
		Debug.assert(
			resolver.isLateBound(getParseTreeNode(node) as Declaration),
		); // Should only be called with dynamic names
	...
	function ensureModifiers<T extends HasModifiers>(
		node: T,
	...
	function ensureModifierFlags(node: Node): ModifierFlags {
		let mask =
			...
			(ModifierFlags.Public |
				ModifierFlags.Async |
				ModifierFlags.Override); // No async and override modifiers in declaration files
		let additions =
