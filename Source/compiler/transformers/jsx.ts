...
export function transformJsx(
	...
	interface PerFileState {
		...
		utilizedImplicitRuntimeImports?: Map<
			string,
		...
		for (const elem of node.attributes.properties) {
			if (
				...
				(!isObjectLiteralExpression(elem.expression) ||
					elem.expression.properties.some(isSpreadAssignment))
			) {
				spread = true;
			} else if (
				spread &&
				isJsxAttribute(elem) &&
				isIdentifier(elem.name) &&
	...
	function convertJsxChildrenToChildrenPropAssignment(
		...
		if (
			...
			const result = transformJsxChildToExpression(
				nonWhitespaceChildren[0],
			);
			return (
				result && factory.createPropertyAssignment("children", result)
			);
		}
	...
	function transformJsxSpreadAttributeToProps(node: JsxSpreadAttribute) {
		if (
			...
			!hasProto(node.expression)
		) {
			return sameMap(node.expression.properties, (p) =>
				Debug.checkDefined(
					visitNode(p, visitor, isObjectLiteralElementLike),
				),
			);
		}
		...
		const props = flatten(
			spanMap(attrs, isJsxSpreadAttribute, (attrs, isSpread) =>
				flatten(
					map(attrs, (attr) =>
						isSpread
							? transformJsxSpreadAttributeToProps(
	...
	function transformJsxAttributesToExpression(
		...
		for (const attr of attrs) {
			if (isJsxSpreadAttribute(attr)) {
				...
				if (
					isObjectLiteralExpression(attr.expression) &&
					!hasProto(attr.expression)
				) {
				...
				expressions.push(
					Debug.checkDefined(
						visitNode(attr.expression, visitor, isExpression),
					),
				);
				continue;
			}
	...
	function transformJsxAttributeInitializer(
		node: JsxAttributeValue | undefined,
	): Expression {
		if (node === undefined) {
		...
		if (node.kind === SyntaxKind.JsxElement) {
			...
			if (
				...
				return factory.createStringLiteral(
					idText(tagName.namespace) + ":" + idText(tagName.name),
				);
			} else {
	...
	function getAttributeName(node: JsxAttribute): StringLiteral | Identifier {
		...
		return factory.createStringLiteral(
			idText(name.namespace) + ":" + idText(name.name),
		);
	}
