...
export function transformModule(
			...
			for (
				...
				append(
					...
					factory.createExpressionStatement(
						reduceLeft(
							currentModuleInfo.exportedNames!.slice(
								i,
								i + chunkSize,
							),
							(prev, nextId) =>
							...
							[
								...
								jsonSourceFile
									...
									: factory.createFunctionExpression(
											...
											/*type*/ undefined,
											transformAsynchronousModuleBody(
												node,
											),
										),
							],
	...
	function addExportEqualsIfNeeded(
		...
		if (currentModuleInfo.exportEquals) {
			...
			if (expressionResult) {
				if (emitAsReturn) {
					const statement =
						factory.createReturnStatement(expressionResult);
	...
	function topLevelVisitor(node: Node): VisitResult<Node | undefined> {
		switch (node.kind) {
			case SyntaxKind.ImportDeclaration:
				return visitTopLevelImportDeclaration(
					node as ImportDeclaration,
				);

			case SyntaxKind.ImportEqualsDeclaration:
				return visitTopLevelImportEqualsDeclaration(
					node as ImportEqualsDeclaration,
				);

			case SyntaxKind.ExportDeclaration:
				return visitTopLevelExportDeclaration(
	...
	function visitForStatement(node: ForStatement, isTopLevel: boolean) {
		if (
			...
			if (exportStatements) {
				...
				const incrementor = visitNode(
					node.incrementor,
					discardedValueVisitor,
					isExpression,
				);
				const body = visitIterationBody(
					node.statement,
		...
		if (
			...
			if (exportedNames) {
				...
				if (isPrefixUnaryExpression(node)) {
					expression = factory.updatePrefixUnaryExpression(
						node,
						expression,
					);
				} else {
					expression = factory.updatePostfixUnaryExpression(
						node,
						expression,
					);
					if (!valueIsDiscarded) {
						temp = factory.createTempVariable(
							hoistVariableDeclaration,
						);
	...
	function visitImportCallExpression(node: ImportCall): Expression {
		...
		switch (compilerOptions.module) {
			case ModuleKind.AMD:
				return createImportCallExpressionAMD(
					argument,
					containsLexicalThis,
				);
			case ModuleKind.UMD:
	 ...
	 * @param node The node to visit.
	 */
	function visitTopLevelImportEqualsDeclaration(
			...
			for (const specifier of node.exportClause.elements) {
				if (languageVersion === ScriptTarget.ES3) {
					...
					const exportNeedsImportDefault =
						...
						idText(specifier.propertyName || specifier.name) ===
							"default";
					const exportedValue =
						factory.createPropertyAccessExpression(
	 ...
	 * @param node The node to visit.
	 */
	function visitTopLevelExportAssignment(
	...
	function substituteExpressionIdentifier(node: Identifier): Expression {
		if (getEmitFlags(node) & EmitFlags.HelperName) {
			...
			!(
				...
				!(
					node.emitNode.autoGenerate.flags &
			...
			const importDeclaration =
				resolver.getReferencedImportDeclaration(node);
			if (importDeclaration) {
				if (isImportClause(importDeclaration)) {
					...
					const name =
						importDeclaration.propertyName ||
	...
	function substituteBinaryExpression(node: BinaryExpression): Expression {
		...
		if (
			...
			(!isGeneratedIdentifier(node.left) ||
				isFileLevelReservedGeneratedIdentifier(node.left)) &&
	...
	function getExports(name: Identifier): Identifier[] | undefined {
		if (!isGeneratedIdentifier(name)) {
			...
			if (importDeclaration) {
				return currentModuleInfo?.exportedBindings[
					getOriginalNodeId(importDeclaration)
				];
			}
			...
			const exportSpecifiers =
				currentModuleInfo?.exportSpecifiers.get(name);
			if (exportSpecifiers) {
