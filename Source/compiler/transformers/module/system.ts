...
/** @internal */
export function transformSystemModule(
		...
		for (const externalImport of externalImports) {
			...
			if (externalModuleName) {
				...
				if (groupIndex !== undefined) {
					...
					dependencyGroups[groupIndex].externalImports.push(
						externalImport,
					);
				} else {
	...
	function addExportStarIfNeeded(statements: Statement[]) {
		...
		if (
			...
			moduleInfo.exportSpecifiers.size === 0
		) {
		...
		const exportedNamesStorageRef =
			factory.createUniqueName("exportedNames");
		statements.push(
			factory.createVariableStatement(
				/*modifiers*/ undefined,
				factory.createVariableDeclarationList([
	...
	function visitImportEqualsDeclaration(
		...
		return singleOrMany(
			appendExportsOfImportEqualsDeclaration(statements, node),
		);
	}
	...
	function visitVariableStatement(
		...
		// lifetime.
		if (
			isVarUsing(node.declarationList) ||
			isVarAwaitUsing(node.declarationList)
		) {
	...
	function hasExportedReferenceInDestructuringTarget(
		...
		if (isAssignmentExpression(node, /*excludeCompoundAssignment*/ true)) {
			...
			return some(
				node.properties,
				hasExportedReferenceInDestructuringTarget,
			);
	...
	function getExports(name: Identifier) {
		...
		if (valueDeclaration) {
			...
			isFileLevelReservedGeneratedIdentifier(name)
		) {
			const exportSpecifiers = moduleInfo?.exportSpecifiers.get(name);
			if (exportSpecifiers) {
