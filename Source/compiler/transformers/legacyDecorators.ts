...
export function transformLegacyDecorators(
	...
	function decoratorContainsPrivateIdentifierInExpression(
		decorator: Decorator,
	) {
		return !!(
			decorator.transformFlags &
	...
	function hasClassElementWithDecoratorContainingPrivateIdentifierInExpression(
		node: ClassDeclaration,
	) {
		for (const member of node.members) {
			...
			if (
				some(
					allDecorators?.parameters,
					parameterDecoratorsContainPrivateIdentifierInExpression,
				)
			)
				return true;
		}
	...
	function transformDecoratorsOfClassElements(
		node: ClassDeclaration,
	 ...
	 */
	function transformClassDeclarationWithoutClassDecorators(
		node: ClassDeclaration,
		...
		let decorationStatements: Statement[] | undefined = [];
		({ members, decorationStatements } = transformDecoratorsOfClassElements(
			node,
			members,
		));
	...
	function transformClassDeclarationWithClassDecorators(
		...
		const modifiers = visitNodes(
			node.modifiers,
			(node) =>
				isExportOrDefaultModifier(node) || isDecorator(node)
					? undefined
					: node,
			isModifierLike,
		);
			...
			some(
				...
				(member) =>
					(isPropertyDeclaration(member) &&
						hasSyntacticModifier(member, ModifierFlags.Static)) ||
		...
		const varInitializer =
			classAlias && !assignClassAliasInStaticBlock
		...
		if (isExport) {
			if (isDefault) {
				...
				const exportStatement = factory.createExternalModuleExport(
	...
	function transformAllDecoratorsOfDeclaration(
		...
		const { false: decorators, true: metadata } = groupBy(
			allDecorators.decorators,
	...
	function getDecoratedClassElements(
		...
		return filter(node.members, (m) =>
			isDecoratedClassElement(m, isStatic, node),
		);
	}
	...
	function generateClassElementDecorationExpression(
		...
		const decoratorExpressions =
			transformAllDecoratorsOfDeclaration(allDecorators);
	...
	function generateConstructorDecorationExpression(
		...
		const classAlias =
			classAliases && classAliases[getOriginalNodeId(node)];
		...
		const expression = factory.createAssignment(
			localName,
			classAlias
				? factory.createAssignment(classAlias, decorate)
				: decorate,
		);
	...
	function transformDecorator(decorator: Decorator) {
		return Debug.checkDefined(
			visitNode(decorator.expression, visitor, isExpression),
		);
	}
