...
const enum ClassFacts {
	...
	HasAnyDecorators = HasClassOrConstructorParameterDecorators |
		HasMemberDecorators,
...
export function transformTypeScript(context: TransformationContext) {
	 ...
	 */
	function saveStateAndInvoke<T, U extends Node>(
		node: U,
		f: (node: U) => T,
	): T {
		// Save state
	 ...
	 */
	function getClassElementVisitor(
		parent: ClassLikeDeclaration,
	): (node: Node) => VisitResult<Node | undefined> {
		return (node) =>
			saveStateAndInvoke(node, (n) =>
				classElementVisitorWorker(n, parent),
			);
	}
	 ...
	 * @param node The node to visit.
	 */
	function classElementVisitorWorker(
		node: Node,
		parent: ClassLikeDeclaration,
	): VisitResult<Node | undefined> {
		switch (node.kind) {
	...
	function getObjectLiteralElementVisitor(
		...
		return (node) =>
			saveStateAndInvoke(node, (n) =>
				objectLiteralElementVisitorWorker(n, parent),
			);
	}
	...
	function decoratorElidingVisitor(
		node: Node,
	): VisitResult<Node | undefined> {
	...
	function visitClassDeclaration(
		...
		const moveModifiers =
			promoteToIIFE || facts & ClassFacts.IsExportOfNamespace;
		...
		if (moveModifiers) {
			if (facts & ClassFacts.IsExportOfNamespace) {
				return [statement, createExportMemberAssignmentStatement(node)];
			}
	...
	function transformClassMembers(node: ClassDeclaration | ClassExpression) {
		...
		const parametersWithPropertyAssignments =
			...
			filter(
				...
				(p): p is ParameterPropertyDeclaration =>
	...
	function injectClassTypeMetadata(
		...
		if (some(metadata)) {
			...
			addRange(
				modifiersArray,
				takeWhile(modifiers, isExportOrDefaultModifier),
			);
			...
			addRange(
				modifiersArray,
				filter(
					skipWhile(modifiers, isExportOrDefaultModifier),
					isModifier,
				),
			);
	...
	function getOldTypeMetadata(
		...
		if (typeSerializer) {
			...
			if (shouldAddTypeMetadata(node)) {
				...
				decorators = append(
					decorators,
					factory.createDecorator(typeMetadata),
				);
			}
	 ...
	 * @param node The node to test.
	 */
	function shouldAddReturnTypeMetadata(
		node: Declaration,
	...
	function shouldAddParamTypesMetadata(
		...
		| (ClassLikeDeclaration & { _hasConstructorBrand: never })
		| MethodDeclaration
		| AccessorDeclaration {
		switch (node.kind) {
			case SyntaxKind.ClassDeclaration:
	...
	function transformConstructorBody(
		...
		const parametersWithPropertyAssignments =
			...
			(filter(constructor.parameters, (p) =>
				isParameterPropertyDeclaration(p, constructor),
			) as readonly ParameterPropertyDeclaration[] | undefined);
	...
	function transformModuleBody(
		...
		if (node.body) {
			if (node.body.kind === SyntaxKind.ModuleBlock) {
				...
				const result = visitModuleDeclaration(
					node.body as ModuleDeclaration,
				);
				if (result) {
					if (isArray(result)) {
	...
	function visitImportClause(
		node: ImportClause,
	): VisitResult<ImportClause> | undefined {
	...
	): VisitResult<NamedImportBindings> | undefined {
	...
	): VisitResult<ImportSpecifier> | undefined {
	...
	function visitNamedExports(
		node: NamedExports,
		allowEmpty: boolean,
	): VisitResult<NamedExports> | undefined {
	...
	): VisitResult<ExportSpecifier> | undefined {
	...
	function createExportMemberAssignmentStatement(
