...
/** @internal */
export function createRuntimeTypeSerializer(
			...
			case SyntaxKind.PrefixUnaryExpression: {
				...
				switch (operand.kind) {
					...
					case SyntaxKind.BigIntLiteral:
						return serializeLiteralOfLiteralTypeNode(
							operand as NumericLiteral | BigIntLiteral,
						);
					default:
		...
		for (let typeNode of types) {
			...
			if (
				...
				((isLiteralTypeNode(typeNode) &&
					typeNode.literal.kind === SyntaxKind.NullKeyword) ||
			...
			if (
				isIdentifier(serializedConstituent) &&
				serializedConstituent.escapedText === "Object"
			) {
	...
	function equateSerializedTypeNodes(
		...
		return (
			...
			isGeneratedIdentifier(left)
				? isGeneratedIdentifier(right)
				: // entity names
					isIdentifier(left)
					? isIdentifier(right) &&
						left.escapedText === right.escapedText
					: isPropertyAccessExpression(left)
						? isPropertyAccessExpression(right) &&
							equateSerializedTypeNodes(
								left.expression,
								right.expression,
							) &&
							...
							: // `"undefined"` or `"function"` in `typeof` checks
								...
								: // used in `typeof` checks for fallback
									...
									: // parens in `typeof` checks with temps
										isParenthesizedExpression(left)
										? isParenthesizedExpression(right) &&
											equateSerializedTypeNodes(
												left.expression,
												right.expression,
											)
										: // conditionals used in fallback
											isConditionalExpression(left)
											? isConditionalExpression(right) &&
												equateSerializedTypeNodes(
													left.condition,
													right.condition,
												) &&
	...
	function getGlobalConstructor(
		name: string,
		minLanguageVersion: ScriptTarget,
	): SerializedTypeNode {
