...
export function transformES2017(
		...
		switch (node.kind) {
			...
			case SyntaxKind.Identifier:
				if (
					...
					resolver.isArgumentsLocalBinding(node as Identifier)
				) {
	...
	function visitCatchClauseInAsyncBody(node: CatchClause) {
		...
		if (catchClauseUnshadowedNames) {
			...
			enclosingFunctionParameterNames =
				savedEnclosingFunctionParameterNames;
			return result;
		} else {
			return visitEachChild(node, asyncBodyVisitor, context);
		}
	...
	function transformAsyncFunctionBody(
		...
		const hasLexicalArguments =
			...
			0;
		const captureLexicalArguments =
			hasLexicalArguments && !lexicalArgumentsBinding;
			...
			if (isArrowFunction) {
				...
				for (let i = 0; i < node.parameters.length; i++) {
					...
					if (
						...
						originalParameter.dotDotDotToken
					) {
						Debug.assert(i === outerParameters.length - 1);
						parameterBindings.push(
							factory.createSpreadElement(outerParameter.name),
						);
						break;
					}
				...
				argumentsExpression =
					factory.createArrayLiteralExpression(parameterBindings);
			} else {
		...
		if (!isArrowFunction) {
			...
			if (captureLexicalArguments) {
				...
				result = factory.updateBlock(
					block,
					factory.mergeLexicalEnvironment(block.statements, [
						createCaptureArgumentsStatement(),
					]),
				);
			}
