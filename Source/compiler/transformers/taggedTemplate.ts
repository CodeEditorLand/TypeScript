...
export function processTaggedTemplateExpression(
	...
	if (isNoSubstitutionTemplateLiteral(template)) {
		...
		rawStrings.push(
			getRawLiteral(factory, template.head, currentSourceFile),
		);
		for (const templateSpan of template.templateSpans) {
			cookedStrings.push(
				createTemplateCooked(factory, templateSpan.literal),
			);
			rawStrings.push(
				getRawLiteral(factory, templateSpan.literal, currentSourceFile),
			);
...
function createTemplateCooked(
	...
	return template.templateFlags! & TokenFlags.IsInvalid
