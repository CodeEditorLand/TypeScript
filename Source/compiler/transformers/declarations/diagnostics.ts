...
export function createGetSymbolAccessibilityDiagnosticForNode(
	...
	if (
		...
		return Debug.assertNever(
			node,
			`Attempted to set a declaration diagnostic context for unhandled node kind: ${Debug.formatSyntaxKind((node as Node).kind)}`,
		);
	}
	...
	function getParameterDeclarationTypeVisibilityDiagnosticMessage(
		...
		switch (node.parent.kind) {
			...
			default:
				return Debug.fail(
					`Unknown parent for parameter: ${Debug.formatSyntaxKind(node.parent.kind)}`,
				);
		}
	}
	...
	function getTypeParameterConstraintVisibilityError(): SymbolAccessibilityDiagnostic {
		...
		switch (node.parent.kind) {
			...
			case SyntaxKind.InferType:
				diagnosticMessage =
					Diagnostics.Extends_clause_for_inferred_type_0_has_or_is_using_private_name_1;
				break;
