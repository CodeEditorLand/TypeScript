...
export namespace BuilderState {
	...
	function getReferencedFiles(
		...
		function addReferencedFile(referencedPath: Path) {
			(referencedFiles || (referencedFiles = new Set())).add(
				referencedPath,
			);
		}
	}
	...
	export function create(
		...
		const referencedMap =
			options.module !== ModuleKind.None && !isOutFile
		...
		for (const sourceFile of newProgram.getSourceFiles()) {
			...
			const oldUncommittedSignature = useOldState
				? oldState!.oldSignatures?.get(sourceFile.resolvedPath)
				: undefined;
			const signature =
			...
			if (referencedMap) {
				...
				if (useOldState) {
					const oldUncommittedExportedModules =
						oldState!.oldExportedModulesMap?.get(
							sourceFile.resolvedPath,
						);
					const exportedModules =
						oldUncommittedExportedModules === undefined
							? oldState!.exportedModulesMap!.getValues(
									sourceFile.resolvedPath,
								)
							: oldUncommittedExportedModules || undefined;
					if (exportedModules) {
						exportedModulesMap!.set(
							sourceFile.resolvedPath,
							exportedModules,
						);
					}
	...
	export function getFilesAffectedByWithOldState(
		...
		)(state, programOfThisState, sourceFile, cancellationToken, host);
	}
	...
	export function updateShapeSignature(
		...
		if (state.hasCalledUpdateShapeSignature?.has(sourceFile.resolvedPath))
			return false;
		...
		if (latestSignature === undefined) {
			...
			if (state.exportedModulesMap && latestSignature !== prevSignature) {
				(state.oldExportedModulesMap ||= new Map()).set(
					sourceFile.resolvedPath,
					state.exportedModulesMap.getValues(
						sourceFile.resolvedPath,
					) || false,
				);
		...
		(state.hasCalledUpdateShapeSignature ||= new Set()).add(
			sourceFile.resolvedPath,
		);
	...
	export function updateExportedModules(
		...
		(state.oldExportedModulesMap ||= new Map()).set(
			sourceFile.resolvedPath,
			state.exportedModulesMap.getValues(sourceFile.resolvedPath) ||
				false,
		);
	...
	export function getExportedModules(
		...
		exportedModulesFromDeclarationEmit?.forEach((symbol) =>
			getReferencedFilesFromImportedModuleSymbol(symbol).forEach((path) =>
				(exportedModules ??= new Set()).add(path),
			),
		);
	...
	export function getAllDependencies(
		...
		return arrayFrom(
			mapDefinedIterator(
				seenMap.keys(),
				(path) =>
					programOfThisState.getSourceFileByPath(path)?.fileName ??
					path,
			),
		);
	}
	...
	function getFilesAffectedByUpdatedShapeWhenModuleEmit(
		...
		if (
			compilerOptions &&
			(getIsolatedModules(compilerOptions) || outFile(compilerOptions))
		) {
			return [sourceFileWithUpdatedShape];
		}
