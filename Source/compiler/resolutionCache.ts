...
export interface ResolutionCache {
	...
	directoryWatchesOfFailedLookups: Map<
		string,
		DirectoryWatchesOfFailedLookup
	>;
	fileWatchesOfAffectingLocations: Map<
		string,
		FileWatcherOfAffectingLocation
	>;
	...
	setFilesWithInvalidatedNonRelativeUnresolvedImports(
		filesWithUnresolvedImports: Map<Path, readonly string[]>,
	): void;
...
function perceivedOsRootLengthForWatching(
	pathComponents: Readonly<PathPathComponents>,
	length: number,
) {
	// Ignore "/", "c:/"
	...
	if (isDosStyle && !pathComponents[indexAfterOsRoot].match(/^users$/i)) {
 ...
 * @internal
 */
export function canWatchDirectoryOrFile(
	pathComponents: Readonly<PathPathComponents>,
	length?: number,
) {
...
function canWatchAffectedPackageJsonOrNodeModulesOfAtTypes(
	fileOrDirPath: Path,
) {
...
export function getDirectoryToWatchFailedLookupLocation(
	...
	const failedLookupPathComponents: Readonly<PathPathComponents> =
		getPathComponents(failedLookupLocationPath);
	...
	const failedLookupComponents: readonly string[] =
		getPathComponents(failedLookupLocation);
	const perceivedOsRootLength = perceivedOsRootLengthForWatching(
	...
	const nodeModulesIndex = failedLookupPathComponents.indexOf(
		"node_modules" as Path,
	);
	if (
		nodeModulesIndex !== -1 &&
		nodeModulesIndex + 1 <= perceivedOsRootLength + 1
	)
	...
	if (isInDirectoryPath(rootPathComponents, failedLookupPathComponents)) {
		if (failedLookupPathComponents.length > rootPathComponents.length + 1) {
			...
			return getDirectoryOfFailedLookupWatch(
				...
				failedLookupPathComponents,
				Math.max(
					rootPathComponents.length + 1,
					perceivedOsRootLength + 1,
				),
			);
		} else {
...
export function getDirectoryToWatchFailedLookupLocationFromTypeRoot(
	...
	typeRoot = isRootedDiskPath(typeRoot)
		? normalizePath(typeRoot)
	...
	return toWatch && filterCustomPath(toWatch.dirPath)
		? toWatch.dirPath
		: undefined;
}
...
export function getRootDirectoryOfResolutionCache(
	...
	const normalized = getNormalizedAbsolutePath(
...
export function getRootPathSplitLength(rootPath: Path) {
	return (
		rootPath.split(directorySeparator).length -
		(hasTrailingDirectorySeparator(rootPath) ? 1 : 0)
	);
}
...
export type GetResolutionWithResolvedFileName<
	T extends
		ResolutionWithFailedLookupLocations = ResolutionWithFailedLookupLocations,
...
export function createResolutionCache(
	...
	let filesWithInvalidatedNonRelativeUnresolvedImports:
		| ReadonlyMap<Path, readonly string[]>
		| undefined;
	...
	const resolutionsWithFailedLookups =
		new Set<ResolutionWithFailedLookupLocations>();
	...
	const fileWatchesOfAffectingLocations = new Map<
		...
		FileWatcherOfAffectingLocation
	>();
	const rootDir = getRootDirectoryOfResolutionCache(
		rootDirForResolution,
		getCurrentDirectory,
	);
	...
	function clear() {
		...
		typeReferenceDirectiveResolutionCache.update(
			resolutionHost.getCompilationSettings(),
		);
	...
	function isFileWithInvalidatedNonRelativeUnresolvedImports(
		...
		const value =
			filesWithInvalidatedNonRelativeUnresolvedImports.get(path);
	...
	function finishCachingPerDirectoryResolution(
		...
		if (newProgram !== oldProgram) {
			...
			newProgram?.getSourceFiles().forEach((newFile) => {
				const expected = isExternalOrCommonJsModule(newFile)
					? (newFile.packageJsonLocations?.length ?? 0)
					: 0;
				const existing =
					impliedFormatPackageJsons.get(newFile.path) ?? emptyArray;
				...
				if (existing.length > expected) {
					for (let i = expected; i < existing.length; i++) {
						fileWatchesOfAffectingLocations.get(existing[i])!
							.files--;
					}
				}
				if (expected)
					impliedFormatPackageJsons.set(
						newFile.path,
						newFile.packageJsonLocations!,
					);
			...
			impliedFormatPackageJsons.forEach((existing, path) => {
				if (!newProgram?.getSourceFileByPath(path)) {
					existing.forEach(
						(location) =>
							fileWatchesOfAffectingLocations.get(location)!
								.files--,
					);
		...
		fileWatchesOfAffectingLocations.forEach(
	...
	function closeFileWatcherOfAffectingLocation(
		watcher: FileWatcherOfAffectingLocation,
		path: string,
	) {
		if (
			watcher.files === 0 &&
			watcher.resolutions === 0 &&
			!watcher.symlinks?.size
		) {
	...
	>({
		...
		const resolutionsInFile =
			perFileCache.get(path) ||
			perFileCache.set(path, createModeAwareCache()).get(path)!;
		...
		const unmatchedRedirects = oldRedirect
			? !redirectedReference ||
				redirectedReference.sourceFile.path !==
		...
		for (const entry of entries) {
			...
			if (
				...
				const host = getModuleResolutionHost(resolutionHost);
				if (
					isTraceEnabled(options, host) &&
					!seenNamesInFile.has(name, mode)
				) {
					const resolved = getResolutionWithResolvedFileName(
						resolution!,
					);
					trace(
						host,
						...
						resolved?.resolvedFileName,
						resolved?.packageId &&
							packageIdToString(resolved.packageId),
					);
				}
			}
	...
	function resolveModuleNameLiterals(
		...
		return resolveNamesWithLocalCache({
			...
			shouldRetryResolution: (resolution) =>
				...
				!resolutionExtensionIsTSOrJson(
					resolution.resolvedModule.extension,
				),
	...
	function resolveSingleModuleNameWithoutWatching(
		moduleName: string,
		containingFile: string,
	) {
		const path = resolutionHost.toPath(containingFile);
		const resolutionsInFile = resolvedModuleNames.get(path);
		const resolution = resolutionsInFile?.get(
			moduleName,
			/*mode*/ undefined,
		);
		if (resolution && !resolution.isInvalidated) return resolution;
		const data =
			resolutionHost.beforeResolveSingleModuleNameWithoutWatching?.(
				moduleResolutionCache,
			);
		const host = getModuleResolutionHost(resolutionHost);
		...
		resolutionHost.afterResolveSingleModuleNameWithoutWatching?.(
			moduleResolutionCache,
			moduleName,
			containingFile,
			result,
	...
	function watchFailedLookupLocationOfResolution(
		...
		const { failedLookupLocations, affectingLocations, alternateResult } =
			resolution;
		if (
			!failedLookupLocations?.length &&
			!affectingLocations?.length &&
			!alternateResult
		)
			return;
		if (failedLookupLocations?.length || alternateResult)
		...
		watchAffectingLocationsOfResolution(
			resolution,
			!failedLookupLocations?.length && !alternateResult,
		);
	}
	...
	function createFileWatcherOfAffectingLocation(
		...
		const fileWatcher =
			fileWatchesOfAffectingLocations.get(affectingLocation);
		if (fileWatcher) {
		...
		if (resolutionHost.realpath) {
			...
			if (affectingLocation !== locationToWatch) {
				isSymlink = true;
				symlinkWatcher =
					fileWatchesOfAffectingLocations.get(locationToWatch);
			}
		}
		...
		if (!isSymlink || !symlinkWatcher) {
			const watcher: FileWatcherOfAffectingLocation = {
				watcher: canWatchAffectingLocation(
					resolutionHost.toPath(locationToWatch),
				)
					? resolutionHost.watchAffectingFileLocation(
							locationToWatch,
							(fileName, eventKind) => {
		...
		if (isSymlink) {
			...
			const watcher: FileWatcherOfAffectingLocation = {
				watcher: {
					close: () => {
						const symlinkWatcher =
							fileWatchesOfAffectingLocations.get(
								locationToWatch,
							);
						// Close symlink watcher if no ref
						if (
							symlinkWatcher?.symlinks?.delete(
								affectingLocation,
							) &&
							!symlinkWatcher.symlinks.size &&
							!symlinkWatcher.resolutions &&
							!symlinkWatcher.files
						) {
	...
	function invalidateAffectingFileWatcher(
		path: string,
		packageJsonMap: Map<Path, PackageJsonInfoCacheEntry> | undefined,
	) {
		...
		watcher?.symlinks?.forEach((path) =>
			invalidateAffectingFileWatcher(path, packageJsonMap),
		);
	...
	function watchFailedLookupLocationOfNonRelativeModuleResolutions(
		...
		if (
			...
			!program
				.getTypeChecker()
				.tryFindAmbientModuleWithoutAugmentations(name)
		) {
			...
			resolutions.forEach((resolution) =>
				watchAffectingLocationsOfResolution(
					resolution,
					/*addToResolutionsWithOnlyAffectingLocations*/ true,
				),
			);
		}
	}
	...
	>(
		...
		if (resolved && resolved.resolvedFileName) {
			...
			if (resolutions?.delete(resolution) && !resolutions.size)
		...
		return resolutionHost.watchDirectoryOfFailedLookupLocation(
			...
			(fileOrDirectory) => {
				const fileOrDirectoryPath =
					resolutionHost.toPath(fileOrDirectory);
	...
	function invalidateResolutions(
		...
		resolutions.forEach((resolution) => {
			...
			for (const containingFilePath of Debug.checkDefined(
				resolution.files,
			)) {
				(filesWithInvalidatedResolutions ??= new Set()).add(
					containingFilePath,
				);
	...
	function setFilesWithInvalidatedNonRelativeUnresolvedImports(
		filesMap: ReadonlyMap<Path, readonly string[]>,
	) {
		Debug.assert(
	...
	function scheduleInvalidateResolutionOfFailedLookupLocation(
		...
		if (isCreatingWatchedDirectory) {
			...
			if (
				...
				if (packagePath)
					(startsWithPathChecks ||= new Set()).add(
						packagePath as Path,
					);
			}
		}
	...
	function invalidatePackageJsonMap() {
		const packageJsonMap = moduleResolutionCache
			.getPackageJsonInfoCache()
			.getInternalMap();
		if (
			packageJsonMap &&
			(failedLookupChecks || startsWithPathChecks || isInDirectoryChecks)
		) {
			packageJsonMap.forEach((_value, path) =>
				isInvalidatedFailedLookup(path)
					? packageJsonMap.delete(path)
					: undefined,
			);
		}
	}
	...
	function invalidateResolutionsOfFailedLookupLocations() {
		...
		if (affectingPathChecksForFile) {
			resolutionHost
				...
				.forEach((f) => {
					if (
						some(f.packageJsonLocations, (location) =>
							affectingPathChecksForFile!.has(location),
						)
					) {
						(filesWithInvalidatedResolutions ??= new Set()).add(
							f.path,
	...
	function canInvalidateFailedLookupResolution(
		...
		if (
			...
			return false;
		return (
			resolution.failedLookupLocations?.some((location) =>
				isInvalidatedFailedLookup(resolutionHost.toPath(location)),
			) ||
			(!!resolution.alternateResult &&
				isInvalidatedFailedLookup(
					resolutionHost.toPath(resolution.alternateResult),
				))
		);
	}
	...
	function isInvalidatedFailedLookup(locationPath: Path) {
		return (
			failedLookupChecks?.has(locationPath) ||
			firstDefinedIterator(
				startsWithPathChecks?.keys() || [],
				(fileOrDirectoryPath) =>
					startsWith(locationPath, fileOrDirectoryPath)
						? true
						: undefined,
			) ||
			firstDefinedIterator(
				isInDirectoryChecks?.keys() || [],
				(dirPath) =>
					locationPath.length > dirPath.length &&
					startsWith(locationPath, dirPath) &&
					(isDiskPathRoot(dirPath) ||
						locationPath[dirPath.length] === directorySeparator)
						? true
						: undefined,
			)
		);
	...
	function canInvalidatedFailedLookupResolutionWithAffectingLocation(
		...
		return (
			!!affectingPathChecks &&
			resolution.affectingLocations?.some((location) =>
				affectingPathChecks!.has(location),
			)
		);
	}
	...
	function createTypeRootsWatch(typeRoot: string): FileWatcher {
		...
		return canWatchTypeRootPath(typeRoot)
			? resolutionHost.watchTypeRootsDirectory(
					typeRoot,
					(fileOrDirectory) => {
						const fileOrDirectoryPath =
							resolutionHost.toPath(fileOrDirectory);
						...
						const dirPath =
							getDirectoryToWatchFailedLookupLocationFromTypeRoot(
								...
								getCurrentDirectory,
								(dirPath) =>
									directoryWatchesOfFailedLookups.has(
										dirPath,
									),
							);
						if (dirPath) {
