...
function getScriptTransformers(
	...
	if (
		...
		(languageVersion < ScriptTarget.ESNext || !useDefineForClassFields)
	) {
...
export function transformNodes<T extends Node>(
	...
	for (const node of nodes) {
		...
		transformed.push(
			(allowDtsFiles ? transformation : transformRoot)(node),
		);
		tracing?.pop();
	}
	...
	function substituteNode(hint: EmitHint, node: Node) {
		...
		return (
			(node &&
				isSubstitutionEnabled(node) &&
				onSubstituteNode(hint, node)) ||
			node
		);
	}
