...
const commandOptionsWithoutBuild: CommandLineOption[] = [
	...
	},
	{
		...
		category: Diagnostics.Modules,
		description:
			Diagnostics.Allow_imports_to_include_TypeScript_file_extensions_Requires_moduleResolution_bundler_and_either_noEmit_or_emitDeclarationOnly_to_be_set,
		defaultValueDescription: false,
		transpileOptionValue: undefined,
	},
	{
		...
		category: Diagnostics.Modules,
		description:
			Diagnostics.Use_the_package_json_exports_field_when_resolving_package_imports,
		defaultValueDescription:
	...
	},
	{
		...
		category: Diagnostics.Modules,
		description:
			Diagnostics.Use_the_package_json_imports_field_when_resolving_imports,
		defaultValueDescription:
			Diagnostics.true_when_moduleResolution_is_node16_nodenext_or_bundler_otherwise_false,
	},
	{
		...
		category: Diagnostics.Modules,
		description:
			Diagnostics.Conditions_to_set_in_addition_to_the_resolver_specific_defaults_when_resolving_imports,
	},
	...
	// Experimental
	{
		...
		category: Diagnostics.Language_and_Environment,
		description:
			Diagnostics.Enable_experimental_support_for_legacy_experimental_decorators,
		defaultValueDescription: false,
	},
	{
	...
	},
	{
		...
		category: Diagnostics.Modules,
		description:
			Diagnostics.Enable_importing_files_with_any_extension_provided_a_declaration_file_is_present,
		defaultValueDescription: false,
	},
...
function createDiagnosticForInvalidCustomType(
	...
	const stringNames = (
		opt.deprecatedKeys
			? namesOfType.filter((k) => !opt.deprecatedKeys!.has(k))
			: namesOfType
	)
		.map((key) => `'${key}'`)
		.join(", ");
...
export function parseListTypeOption(
	...
	switch (opt.element.type) {
		...
		case "object":
			return Debug.fail(
				`List of ${opt.element.type} is not yet supported.`,
			);
		default:
			return mapDefined(values, (v) =>
...
export function getDiagnosticText(
	...
	return cast(
		createCompilerDiagnostic(message, ...args).messageText,
		isString,
	);
}
...
export function convertToJson(
	...
	return convertPropertyValueToJson(
		rootExpression,
		jsonConversionNotifier?.rootOptions,
	);
	...
	function convertObjectLiteralExpressionToJson(
		...
		for (const element of node.properties) {
			...
			const option = keyText
				? objectOption?.elementOptions?.get(keyText)
				: undefined;
			const value = convertPropertyValueToJson(
				element.initializer,
				option,
			);
			if (typeof keyText !== "undefined") {
				...
				jsonConversionNotifier?.onPropertySet(
					keyText,
					value,
					element,
					objectOption,
...
function isCompilerOptionsValue(
	...
	if (option) {
		...
		if (option.type === "listOrElement") {
			return (
				isArray(value) || isCompilerOptionsValue(option.element, value)
			);
		}
...
export function convertToTSConfig(
	...
	for (const option in computedOptions) {
		if (
			...
			some(
				computedOptions[option as keyof typeof computedOptions]
					.dependencies,
				(dep) => providedKeys.has(dep),
			)
		) {
			const implied = computedOptions[
				option as keyof typeof computedOptions
			].computeValue(configParseResult.options);
			const defaultValue = computedOptions[
				option as keyof typeof computedOptions
			].computeValue({});
			if (implied !== defaultValue) {
				impliedCompilerOptions[option] = computedOptions[
					option as keyof typeof computedOptions
				].computeValue(configParseResult.options);
			}
		}
	...
	assign(
		...
		optionMapToObject(
			serializeCompilerOptions(impliedCompilerOptions, pathOptions),
		),
	);
	return config;
}
...
/** @internal */
export function optionMapToObject(
	optionMap: Map<string, CompilerOptionsValue>,
): object {
	return {
...
function getCustomTypeMapOfCommandLineOption(
	...
	switch (optionDefinition.type) {
		...
		case "list":
		case "listOrElement":
			return getCustomTypeMapOfCommandLineOption(
				optionDefinition.element,
			);
		default:
...
export function generateTSConfig(
	...
	function writeConfigurations() {
		...
		for (const entry of entries) {
			...
			result.push(
				value &&
					`${tab}${tab}${value}${description && makePadding(marginLength - value.length + 2) + description}`,
			);
		}
		if (fileNames.length) {
			result.push(`${tab}},`);
		...
		if (filesSpecs) {
			...
			if (
				...
				if (sourceFile) {
					...
					const nodeValue = forEachTsConfigPropArray(
						sourceFile,
						"files",
...
function parseConfig(
	...
	if (ownConfig.extendedConfigPath) {
		...
		if (isString(ownConfig.extendedConfigPath)) {
			...
			ownConfig.extendedConfigPath.forEach((extendedConfigPath) =>
		...
		if (sourceFile && result.extendedSourceFiles)
			sourceFile.extendedSourceFiles = arrayFrom(
	...
	function applyExtendedConfig(
		...
		if (extendedConfig && isSuccessfulParsedTsconfig(extendedConfig)) {
			...
			const setPropertyInResultIfNotUndefined = (
				propertyName: "include" | "exclude" | "files",
			) => {
		...
		if (parentOption?.name) {
			...
			if (option === extendsOptionDeclaration) {
				...
				if (
					find(
						commandOptionsWithoutBuild,
						(opt) => opt.name === keyText,
					)
				) {
					rootCompilerOptions = append(
						rootCompilerOptions,
						propertyAssignment.name,
					);
				}
...
function getExtendsConfigPath(
	...
	const resolved = nodeNextJsonConfigResolver(
		extendedConfig,
...
function getDefaultTypeAcquisition(configFileName?: string): TypeAcquisition {
	return {
		enable:
			!!configFileName &&
			getBaseFileName(configFileName) === "jsconfig.json",
		include: [],
		exclude: [],
	};
}
...
function convertOptionsFromJson(
	...
	for (const id in jsonOptions) {
		...
		if (opt) {
			(defaultOptions || (defaultOptions = {}))[opt.name] =
				convertJsonOption(opt, jsonOptions[id], basePath, errors);
		} else {
...
export function convertJsonOption(
	...
	if (isCompilerOptionsValue(opt, value)) {
		...
		const validatedValue = validateJsonOptionValue(
			opt,
			value,
			errors,
			valueExpression,
...
function convertJsonOptionOfCustomType(
	...
	if (val !== undefined) {
		...
		errors.push(
			createDiagnosticForInvalidCustomType(opt, (message, ...args) =>
				createDiagnosticForNodeInSourceFileOrCompilerDiagnostic(
					sourceFile,
					valueExpression,
					message,
			...
			if (match) {
				...
				const existingFlags =
					existingPath !== undefined
						? wildcardDirectories[existingPath]
						: undefined;
				if (existingFlags === undefined || existingFlags < flags) {
					wildcardDirectories[
						existingPath !== undefined ? existingPath : path
					] = flags;
...
function toCanonicalKey(
	...
	return (
		useCaseSensitiveFileNames ? path : toFileNameLowerCase(path)
	) as CanonicalKey;
}
...
function getWildcardDirectoryFromSpec(
	...
	if (match) {
		...
		const lastDirectorySeperatorIndex =
			spec.lastIndexOf(directorySeparator);
		return {
...
function hasFileWithHigherPriorityExtension(
	...
	for (const ext of extensionGroup) {
		...
		if (
			fileExtensionIs(file, ext) &&
			(ext !== Extension.Ts || !fileExtensionIs(file, Extension.Dts))
		) {
			return false;
		}
...
function getOptionValueWithEmptyStrings(
	...
	switch (option.type) {
		...
		case "list":
			...
			return isArray(value)
				? mapDefined(value, (v) =>
						getOptionValueWithEmptyStrings(v, elementType),
					)
				: "";
		default:
			return forEachEntry(
