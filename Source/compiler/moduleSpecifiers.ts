...
function getPreferences(
	...
	return {
		relativePreference:
			oldImportSpecifier !== undefined
				? isExternalModuleNameRelative(oldImportSpecifier)
...
function computeModuleSpecifiers(
	...
	for (const modulePath of modulePaths) {
		...
		if (!specifier) {
			...
			if (modulePath.isRedirect) {
				redirectPathsSpecifiers = append(
	...
	return pathsSpecifiers?.length
		? pathsSpecifiers
		: redirectPathsSpecifiers?.length
			? redirectPathsSpecifiers
...
function getInfo(
	...
	importingSourceFileName = getNormalizedAbsolutePath(
		importingSourceFileName,
		host.getCurrentDirectory(),
	);
...
function getLocalModuleSpecifier(
	...
	importMode: ResolutionMode,
	{
		getAllowedEndingsInPreferredOrder: getAllowedEndingsInPrefererredOrder,
		relativePreference,
	}: Preferences,
	pathsOnly?: boolean,
): string | undefined {
	...
	const { sourceDirectory, canonicalSourceDirectory, getCanonicalFileName } =
		info;
	...
	const relativeToBaseUrl = getRelativePathIfInSameVolume(
		moduleFileName,
		baseDirectory,
		getCanonicalFileName,
	);
	...
	const fromPaths =
		pathsOnly || fromPackageJsonImports === undefined
			? paths &&
	...
	const maybeNonRelative =
		...
		(fromPaths === undefined && baseUrl !== undefined
			? processEnding(relativeToBaseUrl, allowedEndings, compilerOptions)
			: fromPaths);
	if (!maybeNonRelative) {
		return relativePath;
	}
	...
	if (
		relativePreference === RelativePreference.NonRelative &&
		!pathIsRelative(maybeNonRelative)
	) {
		return maybeNonRelative;
	}

	if (
		relativePreference === RelativePreference.ExternalNonRelative &&
		!pathIsRelative(maybeNonRelative)
	) {
...
function getAllModulePaths(
	...
	const importingFilePath = toPath(
...
function getAllModulePathsWorker(
	...
	forEachFileNameOfModule(
		...
		(path, isRedirect) => {
			...
			allFileNames.set(path, {
				path: info.getCanonicalFileName(path),
				isRedirect,
				isInNodeModules,
			});
	...
	if (allFileNames.size) {
		const remainingPaths = arrayFrom(
			...
			([fileName, { isRedirect, isInNodeModules }]): ModulePath => ({
				path: fileName,
				isRedirect,
				isInNodeModules,
			}),
	...
	const decl = moduleSymbol.declarations?.find(
		(d) =>
			...
			(!isExternalModuleAugmentation(d) ||
				!isExternalModuleNameRelative(
					getTextOfIdentifierOrLiteral(d.name),
				)),
	...
	const ambientModuleDeclareCandidates = mapDefined(
		...
		(d) => {
			...
			const exportAssignment = (
				topNamespace.parent.parent.symbol.exports?.get(
					"export=" as __String,
...
function tryGetModuleNameFromPaths(
	...
	for (const key in paths) {
		for (const patternText of paths[key]) {
			...
			const candidates: {
				ending: ModuleSpecifierEnding | undefined;
				value: string;
			}[] = allowedEndings.map((ending) => ({
				ending,
				value: processEnding(
					relativeToBaseUrl,
					[ending],
	...
	function validateEnding({
		...
		return (
			ending !== ModuleSpecifierEnding.Minimal ||
			value ===
				processEnding(
					relativeToBaseUrl,
					[ending],
		...
		switch (mode) {
			case MatchingMode.Exact:
				if (
					...
					comparePaths(targetFilePath, pathOrPattern, ignoreCase) ===
						Comparison.EqualTo ||
					(outputFile &&
						comparePaths(outputFile, pathOrPattern, ignoreCase) ===
							Comparison.EqualTo) ||
					(declarationFile &&
						comparePaths(
							declarationFile,
			...
			case MatchingMode.Directory:
				if (
					...
					return {
						moduleFileToTry: getNormalizedAbsolutePath(
							combinePaths(
								combinePaths(packageName, exports),
								fragment,
							),
							/*currentDirectory*/ undefined,
						),
				...
				if (
					outputFile &&
					containsPath(pathOrPattern, outputFile, ignoreCase)
				) {
					const fragment = getRelativePathFromDirectory(
						pathOrPattern,
						outputFile,
					...
					return {
						moduleFileToTry: combinePaths(packageName, fragment),
					};
				}
				if (
					declarationFile &&
					containsPath(pathOrPattern, declarationFile, ignoreCase)
				) {
					const fragment = getRelativePathFromDirectory(
						pathOrPattern,
						declarationFile,
			...
			case MatchingMode.Pattern:
				...
				if (
					...
					endsWith(extensionSwappedTarget, trailingSlice, ignoreCase)
				) {
					...
					return {
						moduleFileToTry: replaceFirstStar(
							packageName,
							starReplacement,
						),
					};
				...
				if (
					startsWith(targetFilePath, leadingSlice, ignoreCase) &&
					endsWith(targetFilePath, trailingSlice, ignoreCase)
				) {
				...
				if (
					outputFile &&
					startsWith(outputFile, leadingSlice, ignoreCase) &&
					endsWith(outputFile, trailingSlice, ignoreCase)
				) {
					const starReplacement = outputFile.slice(
						leadingSlice.length,
						outputFile.length - trailingSlice.length,
					);
					return {
						moduleFileToTry: replaceFirstStar(
							packageName,
				...
				if (
					declarationFile &&
					startsWith(declarationFile, leadingSlice, ignoreCase) &&
...
function tryGetModuleNameFromPackageJsonImports(
	...
	const ancestorDirectoryWithPackageJson =
		getNearestAncestorDirectoryWithPackageJson(host, sourceDirectory);
	...
	if (
		isMissingPackageJsonInfo(cachedPackageJson) ||
		!host.fileExists(packageJsonPath)
	) {
		return undefined;
	}
	const packageJsonContent =
		cachedPackageJson?.contents.packageJsonContent ||
		tryParseJson(host.readFile(packageJsonPath)!);
...
function tryGetModuleNameFromRootDirs(
	...
	const relativePaths = flatMap(normalizedSourcePaths, (sourcePath) => {
		return map(normalizedTargetPaths, (targetPath) =>
			ensurePathIsNonModuleName(
				getRelativePathFromDirectory(
					sourcePath,
					targetPath,
...
function tryGetModuleNameAsNodeModule(
	...
	function tryDirectoryWithPackageJson(packageRootIndex: number): {
		...
		const cachedPackageJson = host
			...
			?.getPackageJsonInfo(packageJsonPath);
		if (
			isPackageJsonInfo(cachedPackageJson) ||
			(cachedPackageJson === undefined &&
				host.fileExists(packageJsonPath))
		) {
			const packageJsonContent: Record<string, any> | undefined =
				cachedPackageJson?.contents.packageJsonContent ||
				tryParseJson(host.readFile!(packageJsonPath)!);
			const importMode =
				overrideMode || importingSourceFile.impliedNodeFormat;
			if (getResolvePackageJsonExports(options)) {
				// The package name that we found in node_modules could be different from the package
				// name in the package.json content via url/filepath dependency specifiers. We need to
				// use the actual directory name, so don't look at `packageJsonContent.name` here.
				const nodeModulesDirectoryName = packageRootPath.substring(
					parts.topLevelPackageNameIndex + 1,
				);
				const packageName = getPackageNameFromTypesPackageName(
					nodeModulesDirectoryName,
			...
			const versionPaths = packageJsonContent?.typesVersions
				? getPackageJsonTypesVersionsPaths(
						packageJsonContent.typesVersions,
					)
				: undefined;
			if (versionPaths) {
			...
			if (
				...
				!(
					...
					matchPatternOrExact(
						tryParsePatterns(versionPaths!.paths),
						mainFileRelative,
					)
				)
			) {
				...
				const canonicalModuleFileToTry =
					getCanonicalFileName(moduleFileToTry);
				if (
					removeFileExtension(mainExportFile) ===
					removeFileExtension(canonicalModuleFileToTry)
				) {
					...
					!fileExtensionIsOneOf(
						canonicalModuleFileToTry,
						extensionsNotSupportingExtensionlessResolution,
					) &&
					startsWith(canonicalModuleFileToTry, mainExportFile) &&
					getDirectoryPath(canonicalModuleFileToTry) ===
						removeTrailingDirectorySeparator(mainExportFile) &&
					removeFileExtension(
						getBaseFileName(canonicalModuleFileToTry),
					) === "index"
				) {
			...
			const fileName = getCanonicalFileName(
				moduleFileToTry.substring(parts.packageRootIndex + 1),
			);
			if (
				fileName === "index.d.ts" ||
				fileName === "index.js" ||
...
function getPathsRelativeToRootDirs(
	...
	return mapDefined(rootDirs, (rootDir) => {
		...
		return relativePath !== undefined &&
			isPathRelativeToParent(relativePath)
			? undefined
			: relativePath;
	});
}
...
function processEnding(
	...
	if (
		fileExtensionIsOneOf(fileName, [Extension.Mts, Extension.Cts]) &&
		tsPriority !== -1 &&
		tsPriority < jsPriority
	) {
		return fileName;
	} else if (
		fileExtensionIsOneOf(fileName, [
		...
		!fileExtensionIsOneOf(fileName, [Extension.Dts]) &&
		fileExtensionIsOneOf(fileName, [Extension.Ts]) &&
		fileName.includes(".d.")
	) {
		...
		case ModuleSpecifierEnding.TsExtension:
			...
			if (isDeclarationFileName(fileName)) {
				const extensionlessPriority = allowedEndings.findIndex(
					(e) =>
						e === ModuleSpecifierEnding.Minimal ||
						e === ModuleSpecifierEnding.Index,
				);
				return extensionlessPriority !== -1 &&
...
export function tryGetRealFileNameForNonJsDeclarationFileName(
	...
	if (
		!endsWith(fileName, Extension.Ts) ||
		!baseName.includes(".d.") ||
...
function getJSExtensionForFile(
	...
	return (
		...
		Debug.fail(
			`Extension ${extensionFromPath(fileName)} is unsupported:: FileName:: ${fileName}`,
		)
	);
}
...
export function tryGetJSExtensionForFile(
	...
	switch (ext) {
		...
		case Extension.Dts:
			return Extension.Js;
		case Extension.Tsx:
			return options.jsx === JsxEmit.Preserve
				? Extension.Jsx
				: Extension.Js;
		case Extension.Js:
		case Extension.Jsx:
