...
export function* flatMapIterator<T, U>(
	iter: Iterable<T>,
	mapfn: (x: T) => readonly U[] | Iterable<U> | undefined,
) {
	for (const x of iter) {
		const iter2 = mapfn(x);
		if (!iter2) continue;
...
): readonly T[]; // eslint-disable-line @typescript-eslint/unified-signatures
/** @internal */
export function concatenate<T>(
...
export function detectSortCaseSensitivity<T>(
	...
	for (
		let i = 1, len = array.length;
		i < len && kind !== SortKind.None;
		i++
	) {
		const element = getString(array[i]);
		if (
			kind & SortKind.CaseSensitive &&
			compareStringsCaseSensitive(prevElement, element) > 0
		) {
			kind &= ~SortKind.CaseSensitive;
		}
		if (
			kind & SortKind.CaseInsensitive &&
			compareStringsCaseInsensitive(prevElement, element) > 0
		) {
			kind &= ~SortKind.CaseInsensitive;
		}
		prevElement = element;
...
export const elementAt: <T>(
	array: readonly T[] | undefined,
	offset: number,
) => T | undefined = !!Array.prototype.at
 ...
 * @internal
 */
export function firstOrUndefined<T>(
	array: readonly T[] | undefined,
): T | undefined {
...
/** @internal */
export function firstOrUndefinedIterator<T>(
 ...
 * @internal
 */
export function memoizeOne<A extends string | number | boolean | undefined, T>(
	callback: (arg: A) => T,
): (arg: A) => T {
 ...
 * @internal
 */
export function memoizeWeak<A extends object, T>(
	callback: (arg: A) => T,
): (arg: A) => T {
...
export function getSpellingSuggestion<T>(
	...
	for (const candidate of candidates) {
		...
		if (
			...
			Math.abs(candidateName.length - name.length) <=
