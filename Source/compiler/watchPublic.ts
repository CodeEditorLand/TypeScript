...
export function readBuilderProgram(
	...
	if (!buildInfo || buildInfo.version !== version || !buildInfo.program)
		return undefined;
	return createBuilderProgramUsingProgramBuildInfo(
		buildInfo,
		buildInfoPath,
		host,
	);
...
type WatchCompilerHostOfFilesAndCompilerOptionsOrConfigFile<
	T extends BuilderProgram,
> =
	| (WatchCompilerHostOfFilesAndCompilerOptions<T> &
			Partial<WatchCompilerHostOfConfigFile<T>>)
	| (WatchCompilerHostOfConfigFile<T> &
			Partial<WatchCompilerHostOfFilesAndCompilerOptions<T>>);
...
export function createWatchProgram<T extends BuilderProgram>(
	...
	let sharedExtendedConfigFileWatchers: Map<
		Path,
	...
	if (
		...
		compilerHost.resolveModuleNameLiterals =
			resolutionCache.resolveModuleNameLiterals.bind(resolutionCache);
	}
	...
	if (
		...
		compilerHost.resolveTypeReferenceDirectiveReferences =
			resolutionCache.resolveTypeReferenceDirectiveReferences.bind(
				resolutionCache,
			);
	}
	...
	function createNewProgram(
		...
		if (projectReferences)
			writeLog(
				`  projectReferences: ${JSON.stringify(projectReferences)}`,
			);
		...
		compilerHost.hasInvalidatedLibResolutions =
			hasInvalidatedLibResolutions;
		...
		if (
			...
			if (hostSourceFile) {
				if (sourceFile) {
					...
					(hostSourceFile as FilePresentOnHost).sourceFile =
						sourceFile;
	...
	function getSourceVersion(
		path: Path,
		readFileWithCache: (file: string) => string | undefined,
	): string | undefined {
		const hostSourceFile = sourceFilesCache.get(path);
		...
		return text !== undefined
			? getSourceFileVersionAsHashFromText(compilerHost, text)
			: undefined;
	}
	...
	function onReleaseOldSourceFile(
		...
		if (hostSourceFileInfo !== undefined) {
			...
			if (isFileMissingOnHost(hostSourceFileInfo)) {
				...
				(hostSourceFileInfo as FilePresentOnHost).sourceFile ===
				oldSourceFile
			) {
	...
	function scheduleProgramUpdate() {
		...
		timerToUpdateProgram = host.setTimeout(
		...
		if (config) {
			...
			if (
				...
				const fileNames = getFileNamesFromConfigSpecs(
					config.parsedCommandLine.options.configFile!
						.configFileSpecs!,
					getNormalizedAbsolutePath(
						getDirectoryPath(configFileName),
						currentDirectory,
				...
				watchFile(
					...
					(_fileName, eventKind) => {
						...
						projects.forEach((projectPath) => {
							if (
								configFileName &&
								toPath(configFileName) === projectPath
							) {
