		...
		if (isBlock(p) || isModuleBlock(p) || isSourceFile(p)) {
			...
			for (const statement of statements) {
				if (nodeHasName(statement, name)) {
					...
					const state = getModuleInstanceStateCached(
						statement,
						visited,
					);
...
function createBinder(): (file: SourceFile, options: CompilerOptions) => void {
	...
	function bindCaseBlock(node: CaseBlock): void {
		...
		const isNarrowingSwitch =
			node.parent.expression.kind === SyntaxKind.TrueKeyword ||
			isNarrowingExpression(node.parent.expression);
	...
	function declareSymbolAndAddToSymbolTable(
		...
		switch (container.kind) {
			...
			case SyntaxKind.SourceFile:
				return declareSourceFileMember(
					node,
					symbolFlags,
					symbolExcludes,
				);
	...
	function delayedBindJSDocTypedefTag() {
		...
		for (const typeAlias of delayedTypeAliases) {
			...
			container =
				(getEnclosingContainer(host) as IsContainer | undefined) ||
				file;
			blockScopeContainer =
				(getEnclosingBlockScopeContainer(host) as
					| IsBlockScopedContainer
					| undefined) || file;
			currentFlow = initFlowNode({ flags: FlowFlags.Start });
			parent = typeAlias;
			...
			if (
				...
				const isTopLevel = isTopLevelNamespaceAssignment(
					declName.parent,
				);
				if (isTopLevel) {
	...
	function checkContextualIdentifier(node: Identifier) {
		...
		if (
			...
			if (
				...
				originalKeywordKind === SyntaxKind.YieldKeyword &&
	...
	function checkStrictModeLabeledStatement(node: LabeledStatement) {
		...
		if (
			inStrictMode &&
			getEmitScriptTarget(options) >= ScriptTarget.ES2015
		) {
			if (
		...
		switch (node.kind) {
			...
			case SyntaxKind.ThisKeyword:
				...
				if (
					...
					(node as Identifier | ThisExpression).flowNode =
						currentFlow;
				}
			...
			case SyntaxKind.LabeledStatement:
				return checkStrictModeLabeledStatement(
	...
	function bindPropertyWorker(node: PropertyDeclaration | PropertySignature) {
		...
		const includes = isAutoAccessor
			? SymbolFlags.Accessor
			: SymbolFlags.Property;
		const excludes = isAutoAccessor
			? SymbolFlags.AccessorExcludes
			: SymbolFlags.PropertyExcludes;
		return bindPropertyOrMethodOrAccessor(
			node,
			includes |
		...
		switch (thisContainer.kind) {
			...
			case SyntaxKind.FunctionExpression:
				...
				if (
					...
					thisContainer.parent.operatorToken.kind ===
						SyntaxKind.EqualsToken
				) {
	...
	function bindPotentiallyNewExpandoMemberToNamespace(
		...
		if (
			isFunctionLikeDeclaration(
				getAssignedExpandoInitializer(declaration)!,
			)
		) {
	...
	function lookupSymbolForPropertyAccess(
		...
		lookupContainer:
			| IsContainer
			| IsBlockScopedContainer
			| EntityNameExpression = container,
	): Symbol | undefined {
	...
	function forEachIdentifierInEntityName(
		...
		if (isExportsOrModuleExportsOrAlias(file, e)) {
			...
			const s = forEachIdentifierInEntityName(
				e.expression,
				parent,
				action,
			);
			...
			return action(
				...
				s &&
					s.exports &&
					s.exports.get(getElementOrPropertyAccessName(e)),
				s,
			);
		}
	}
	...
	function bindCallExpression(node: CallExpression) {
		...
		if (
			!file.commonJsModuleIndicator &&
			isRequireCall(node, /*requireStringLiteralLikeArgument*/ false)
		) {
	...
	function bindTypeParameter(node: TypeParameterDeclaration) {
		if (isJSDocTemplateTag(node.parent)) {
			const container: HasLocals | undefined =
				getEffectiveContainerForJSDocTemplateTag(node.parent);
			if (container) {
				Debug.assertNode(container, canHaveLocals);
			...
			const container: HasLocals | undefined = getInferTypeContainer(
				node.parent,
			);
			if (container) {
				Debug.assertNode(container, canHaveLocals);
...
export function isExportsOrModuleExportsOrAlias(
	...
	while (!q.isEmpty() && i < 100) {
		...
		if (
			...
			isModuleExportsAccessExpression(node)
		) {
			return true;
