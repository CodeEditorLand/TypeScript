...
export class Version {
	...
	constructor(
		...
		Debug.assert(
			every(prereleaseArray, (s) => prereleasePartRegExp.test(s)),
			"Invalid argument: prerelease",
		);
		Debug.assert(
			every(buildArray, (s) => buildPartRegExp.test(s)),
		...
		comparators.push(
			isWildcard(rightResult.minor)
				...
				: isWildcard(rightResult.patch)
					? createComparator(
							"<",
							rightResult.version.increment("minor"),
						)
