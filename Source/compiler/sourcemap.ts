	...
	function appendSourceMap(
		...
		for (const raw of mappingIterator) {
			...
			if (raw.sourceIndex !== undefined) {
				newSourceIndex =
					sourceIndexToNewSourceIndexMap[raw.sourceIndex];
				...
				if (map.names && raw.nameIndex !== undefined) {
					...
					if (newNameIndex === undefined) {
						nameIndexToNewNameIndexMap[raw.nameIndex] =
							newNameIndex = addName(map.names[raw.nameIndex]);
					}
				}
			}

			const rawGeneratedLine =
				raw.generatedLine - (start ? start.line : 0);
...
/** @internal */
export const sourceMapCommentRegExpDontCareLineStart =
	/\/\/[@#] source[M]appingURL=(.+)\r?\n?$/;
/** @internal */
export const sourceMapCommentRegExp =
	/^\/\/[@#] source[M]appingURL=(.+)\r?\n?$/;
			...
			while (!done && pos < mappings.length) {
				...
				if (generatedCharacter < 0)
					return setErrorAndStopIterating(
						"Invalid generatedCharacter found",
					);
