...
export function createCompilerHostWorker(
	...
	function directoryExists(directoryPath: string): boolean {
		...
		if (
			(compilerHost.directoryExists || system.directoryExists)(
				directoryPath,
			)
		) {
	...
	const getSourceFileWithCache: CompilerHost["getSourceFile"] | undefined =
		getSourceFile
			? (
					...
					const key = toPath(fileName);
					const impliedNodeFormat: ResolutionMode =
						typeof languageVersionOrOptions === "object"
							? languageVersionOrOptions.impliedNodeFormat
							: undefined;
					const forImpliedNodeFormat =
						sourceFileCache.get(impliedNodeFormat);
					...
					if (
						...
						sourceFileCache.set(
							impliedNodeFormat,
							(forImpliedNodeFormat || new Map()).set(
								key,
								sourceFile,
							),
						);
...
export function getModeForResolutionAtIndex(
	...
	return getModeForUsageLocationWorker(
		file,
...
function getModeForUsageLocationWorker(
	...
	if (usage.parent.parent && isImportTypeNode(usage.parent.parent)) {
		const override = getResolutionModeOverride(
			usage.parent.parent.attributes,
		);
		if (override) {
			return override;
		}
	...
	if (
		compilerOptions &&
		getEmitModuleKind(compilerOptions) === ModuleKind.Preserve
	) {
		return (usage.parent.parent &&
...
const emptyResolution: ResolvedModuleWithFailedLookupLocations &
	ResolvedTypeReferenceDirectiveWithFailedLookupLocations = {
...
/** @internal */
export const moduleResolutionNameAndModeGetter: ResolutionNameAndModeGetter<
	StringLiteralLike,
	SourceFile
> = {
	getName: getModuleResolutionName,
	getMode: (entry, file, compilerOptions) =>
		getModeForUsageLocation(file, entry, compilerOptions),
};
...
function getTypeReferenceResolutionName<T extends FileReference | string>(
	entry: T,
) {
...
> = {
	...
	getMode: (entry, file) =>
		getModeForFileReference(entry, file?.impliedNodeFormat),
};

/** @internal */
export function createTypeReferenceResolutionLoader<
...
/** @internal */
export function loadWithModeAwareCache<
	Entry,
	SourceFile,
	ResolutionCache,
	Resolution,
...
function forEachProjectReference<T>(
	...
	function worker(
		...
		return forEach(resolvedProjectReferences, (resolvedRef, index) => {
			if (
				resolvedRef &&
				seenResolvedRefs?.has(resolvedRef.sourceFile.path)
			) {
				// ignore recursives
				return undefined;
			}
...
export function getReferencedFileLocation(
	...
	switch (kind) {
		...
		case FileIncludeKind.ReferenceFile:
			...
			break;
		case FileIncludeKind.TypeReferenceDirective:
			({ pos, end, resolutionMode } =
				file.typeReferenceDirectives[index]);
			packageId = program.getResolvedTypeReferenceDirective(
				file,
				toFileNameLowerCase(
					file.typeReferenceDirectives[index].fileName,
				),
...
export function isProgramUptoDate(
	...
	if (
		...
		forEachEntry(program.resolvedLibReferences, (_value, libFileName) =>
...
export function getImpliedNodeFormatForFileWorker(
	...
	function lookupFromPackageJson(): Partial<CreateSourceFileOptions> {
		...
		const impliedNodeFormat =
			packageJsonScope?.contents.packageJsonContent.type === "module"
				? ModuleKind.ESNext
				: ModuleKind.CommonJS;
...
export function createProgram(
	...
	let resolvedTypeReferenceDirectives =
		createModeAwareCache<ResolvedTypeReferenceDirectiveWithFailedLookupLocations>();
	...
	let resolvedModulesProcessing:
		| Map<Path, readonly ResolvedModuleWithFailedLookupLocations[]>
		| undefined;
	...
	const hasInvalidatedResolutions =
		host.hasInvalidatedResolutions || returnFalse;
	...
	const hasInvalidatedLibResolutions =
		host.hasInvalidatedLibResolutions || returnFalse;
	...
	if (structureIsReused !== StructureIsReused.Completely) {
				...
				resolvedProjectReferences?.forEach((parsedRef, index) => {
					...
					if (useSourceOfProjectReferenceRedirect) {
						if (
							...
							for (const fileName of parsedRef.commandLine
								.fileNames) {
		...
		forEach(rootNames, (name, index) =>
			processRootFile(
				...
				{ kind: FileIncludeKind.RootFile, index },
			),
		);
		tracing?.pop();

		// load type declarations specified via 'types' argument or implicitly from types/ and node_modules/@types folders
		automaticTypeDirectiveNames ??= rootNames.length
			...
			for (let i = 0; i < automaticTypeDirectiveNames.length; i++) {
				...
				processTypeReferenceDirective(
					...
					resolutions[i],
					{
						...
						packageId:
							resolutions[i]?.resolvedTypeReferenceDirective
								?.packageId,
					},
				);
			}
			...
			if (!options.lib && defaultLibraryFileName) {
				...
				forEach(options.lib, (libFileName, index) => {
					processRootFile(
						...
						/*isDefaultLib*/ true,
						/*ignoreNoDefaultLib*/ false,
						{ kind: FileIncludeKind.LibFile, index },
					);
				});
			}
		}
	...
	fileProcessingDiagnostics?.forEach((diagnostic) => {
		switch (diagnostic.kind) {
			...
			case FilePreprocessingDiagnosticsKind.ResolutionDiagnostics:
				return diagnostic.diagnostics.forEach((d) =>
					programDiagnostics.add(d),
				);
			default:
	...
	function getResolvedTypeReferenceDirective(
		...
		return resolvedTypeReferenceDirectiveNames
			?.get(file.path)
			?.get(typeDirectiveName, mode);
	}
	...
	function forEachResolution<T>(
		...
		if (file)
			resolutionCache
				?.get(file.path)
				?.forEach((resolution, name, mode) =>
					callback(resolution, name, mode, file.path),
				);
		else
			resolutionCache?.forEach((resolutions, filePath) =>
		...
		forEachResolvedModule(({ resolvedModule }) => {
			if (resolvedModule?.packageId)
				packageMap!.set(
					...
					resolvedModule.extension === Extension.Dts ||
						!!packageMap!.get(resolvedModule.packageId.name),
				);
		});
		return packageMap;
	}
	...
	>(
		...
		const containingSourceFile = !isString(containingFile)
			? containingFile
			: undefined;
		...
		const redirectedReference =
			containingSourceFile &&
			getRedirectReferenceForResolution(containingSourceFile);
		tracing?.push(
			tracing.Phase.Program,
	...
	function getRedirectReferenceForResolution(file: SourceFile) {
		...
		if (
			...
			!file.originalFileName.includes(nodeModulesPathPart)
		)
			return undefined;
		const realDeclarationPath = toPath(
			host.realpath(file.originalFileName),
		);
	...
	function getDefaultLibFilePriority(a: SourceFile) {
		if (
			containsPath(defaultLibraryPath, a.fileName, /*ignoreCase*/ false)
		) {
	...
	function resolveModuleNamesReusingOldState(
		...
		const predictedToResolveToAmbientModuleMarker: ResolvedModuleWithFailedLookupLocations =
			emptyResolution;
		const oldSourceFile =
		...
		for (let i = 0; i < moduleNames.length; i++) {
			...
			if (
				file === oldSourceFile &&
				!hasInvalidatedResolutions(file.path)
			) {
				...
				if (oldResolution?.resolvedModule) {
					...
					(result ??= new Array(moduleNames.length))[i] =
						oldResolution;
		...
		function moduleNameResolvesToAmbientModuleInNonModifiedFile(
			...
			const unmodifiedFile = ambientModuleNameToUnmodifiedFileName.get(
				moduleName.text,
			);
	...
	>(
		...
		for (let i = 0; i < typeDirectiveNames.length; i++) {
			...
			if (canReuseResolutions) {
				...
				const mode = getModeForFileReference(
					entry,
					containingSourceFile?.impliedNodeFormat,
				);
				...
				if (oldResolution?.resolvedTypeReferenceDirective) {
					...
					(result ??= new Array(typeDirectiveNames.length))[i] =
						oldResolution;
	...
	function canReuseProjectReferences(): boolean {
		return !forEachProjectReference(
			...
			(oldResolvedRef, parent, index) => {
				const newRef = (
					parent
						? parent.commandLine.projectReferences
						: projectReferences
				)![index];
				...
				if (oldResolvedRef) {
					...
					return (
						!newResolvedRef ||
						newResolvedRef.sourceFile !==
							oldResolvedRef.sourceFile ||
						!arrayIsEqualTo(
							oldResolvedRef.commandLine.fileNames,
	...
	function tryReuseStructureFromOldProgram(): StructureIsReused {
		...
		if (
			forEachEntry(oldProgram.getMissingFilePaths(), (missingFileName) =>
				host.fileExists(missingFileName),
			)
		) {
			return StructureIsReused.Not;
		}
		...
		for (const oldSourceFile of oldSourceFiles) {
			...
			newSourceFile.packageJsonLocations = sourceFileOptions
			...
			if (packageName !== undefined) {
				...
				if (
					(prevKind !== undefined &&
						newKind === SeenPackageName.Modified) ||
		...
		for (const newSourceFile of modifiedSourceFiles) {
			...
			(resolvedModulesProcessing ??= new Map()).set(
				newSourceFile.path,
				resolutions,
			);
			...
			if (resolutionsChanged)
				structureIsReused = StructureIsReused.SafeModules;
			const typesReferenceDirectives =
				newSourceFile.typeReferenceDirectives;
		...
		if (
			...
			forEachEntry(
				oldProgram.resolvedLibReferences,
				(resolution, libFileName) =>
					pathForLibFileWorker(libFileName).actual !==
					resolution.actual,
			)
		) {
			return StructureIsReused.SafeModules;
		...
		if (host.hasChangedAutomaticTypeDirectiveNames) {
			...
			if (
				!arrayIsEqualTo(
					oldProgram.getAutomaticTypeDirectiveNames(),
					automaticTypeDirectiveNames,
				)
			)
				return StructureIsReused.SafeModules;
		...
		automaticTypeDirectiveNames =
			oldProgram.getAutomaticTypeDirectiveNames();
		automaticTypeDirectiveResolutions =
			oldProgram.getAutomaticTypeDirectiveResolutions();
		...
		resolvedTypeReferenceDirectiveNames =
			oldProgram.resolvedTypeReferenceDirectiveNames;
	...
	function isSourceFileDefaultLibrary(file: SourceFile): boolean {
		...
		if (!options.lib) {
			...
			return some(options.lib, (libFileName) =>
				equalityComparer(
					file.fileName,
					resolvedLibReferences!.get(libFileName)!.actual,
				),
			);
		}
	}
	...
	function getDeclarationDiagnostics(
		...
		if (!sourceFile || outFile(options)) {
			return getDeclarationDiagnosticsWorker(
				sourceFile,
				cancellationToken,
			);
		} else {
	...
	function markPrecedingCommentDirectiveLine(
		...
		let line =
			computeLineAndCharacterOfPosition(lineStarts, start!).line - 1; // TODO: GH#18217
		while (line >= 0) {
		...
		return runWithCancellationToken(() => {
			...
			function walkArray(nodes: NodeArray<Node>, parent: Node) {
				if (canHaveIllegalDecorators(parent)) {
					...
					const decoratorIndex = findIndex(
						parent.modifiers,
						isDecorator,
					);
					if (decoratorIndex >= 0) {
				...
				switch (parent.kind) {
					...
					case SyntaxKind.ArrowFunction:
						...
						if (
							nodes ===
							(parent as DeclarationWithTypeParameterChildren)
								.typeParameters
						) {
							diagnostics.push(
					...
					case SyntaxKind.PropertyDeclaration:
						...
						if (
							nodes === (parent as PropertyDeclaration).modifiers
						) {
					...
					case SyntaxKind.Parameter:
						...
						if (
							nodes ===
								(parent as ParameterDeclaration).modifiers &&
							some(nodes, isModifier)
						) {
							diagnostics.push(
								createDiagnosticForNodeArray(
									nodes,
					...
					case SyntaxKind.TaggedTemplateExpression:
						...
						if (
							nodes ===
							(parent as NodeWithTypeArguments).typeArguments
						) {
							diagnostics.push(
								createDiagnosticForNodeArray(
									nodes,
		...
		forEachResolvedProjectReference((resolvedRef) => {
			diagnostics = concatenate(
				...
				programDiagnostics.getDiagnostics(
					resolvedRef.sourceFile.fileName,
				),
			);
		});
		return diagnostics;
			...
			if (isAnyImportOrReExport(node)) {
				...
				if (
					...
					if (
						...
						(inAmbientModule &&
							!isExternalModuleNameRelative(nameText))
					) {
						(
						...
						if (file.isDeclarationFile) {
							...
							(ambientModules || (ambientModules = [])).push(
								nameText,
							);
						}
						...
						const body = (node as ModuleDeclaration)
							.body as ModuleBlock;
						if (body) {
	...
	function getLibFileFromReference(ref: FileReference) {
		...
		const actualFileName =
			libFileName && resolvedLibReferences?.get(libFileName)?.actual;
		return actualFileName !== undefined
			? getSourceFile(actualFileName)
			: undefined;
	}
		...
		if (hasExtension(fileName)) {
			...
			if (
				...
				!forEach(
					...
					(extension) =>
						fileExtensionIs(canonicalFileName, extension),
				)
			) {
				if (fail) {
	...
	function getCreateSourceFileOptions(
		...
		const setExternalModuleIndicator =
	...
	function findSourceFileWorker(
		...
		if (filesByName.has(path)) {
			...
			if (file && !(options.forceConsistentCasingInFileNames === false)) {
				...
				if (isRedirect) {
					fileName =
						getProjectReferenceRedirect(fileName) || fileName;
				}
				// Check if it differs only in drive letters its ok to ignore that error:
				const checkedAbsolutePath =
					getNormalizedAbsolutePathWithoutRoot(
						checkedName,
						currentDirectory,
					);
		...
		if (isReferencedFile(reason) && !useSourceOfProjectReferenceRedirect) {
			const redirectProject =
				getProjectReferenceRedirectProject(fileName);
			if (redirectProject) {
		...
		if (packageId) {
			...
			if (fileFromPackageId) {
				...
				sourceFileToPackageName.set(
					path,
					packageIdToPackageName(packageId),
				);
	...
	function updateFilesByNameMap(
		fileName: string,
		path: Path,
		file: SourceFile | false | undefined,
	) {
	...
	function getResolvedProjectReferenceToRedirect(fileName: string) {
		...
		const referencedProjectPath =
			mapFromFileToProjectReferenceRedirects.get(toPath(fileName));
		return (
	...
	function forEachResolvedProjectReference<T>(
		...
		return ts_forEachResolvedProjectReference(
			resolvedProjectReferences,
			cb,
		);
	}
	...
	function processTypeReferenceDirectives(file: SourceFile) {
		...
		const resolutions =
			resolvedTypeReferenceDirectiveNamesProcessing?.get(file.path) ||
			resolveTypeReferenceDirectiveNamesReusingOldState(
				typeDirectives,
				file,
			);
		const resolutionsInFile =
			createModeAwareCache<ResolvedTypeReferenceDirectiveWithFailedLookupLocations>();
		(resolvedTypeReferenceDirectiveNames ??= new Map()).set(
			file.path,
			resolutionsInFile,
		);
	...
	function pathForLibFileWorker(libFileName: string): LibResolution {
		...
		if (
			...
			const oldResolution =
				oldProgram.resolvedLibReferences?.get(libFileName);
			if (oldResolution) {
				if (oldResolution.resolution && isTraceEnabled(options, host)) {
					const libraryName =
						getLibraryNameFromLibFileName(libFileName);
					const resolveFrom = getInferredLibraryNameResolveFrom(
						options,
						currentDirectory,
						libFileName,
					);
				...
				(resolvedLibProcessing ??= new Map()).set(
					libFileName,
					oldResolution,
				);
		...
		const resolution = actualResolveLibrary(
			libraryName,
			resolveFrom,
			options,
			libFileName,
	...
	function processLibReferenceDirectives(file: SourceFile) {
		forEach(file.libReferenceDirectives, (libReference, index) => {
			const { libName, libFileName } =
				getLibFileNameFromLibReference(libReference);
			if (libFileName) {
	...
	function processImportedModules(file: SourceFile) {
		...
		if (file.imports.length || file.moduleAugmentations.length) {
			...
			const resolutions =
				resolvedModulesProcessing?.get(file.path) ||
			...
			for (let index = 0; index < moduleNames.length; index++) {
				...
				const isFromNodeModulesSearch =
					resolution.isExternalLibraryImport;
				const isJsFile = !resolutionExtensionIsTSOrJson(
					resolution.extension,
				);
				const isJsFileFromNodeModules =
					isFromNodeModulesSearch &&
					isJsFile &&
					(!resolution.originalPath ||
						pathContainsNodeModules(resolution.resolvedFileName));
	...
	function checkSourceFilesBelongToPath(
		...
		for (const sourceFile of sourceFiles) {
			if (!sourceFile.isDeclarationFile) {
				...
				if (
					absoluteSourceFilePath.indexOf(
	...
	function parseProjectReferenceConfigFile(
		...
		if (host.getParsedCommandLine) {
			...
			Debug.assert(
				!sourceFile.path || sourceFile.path === sourceFilePath,
			);
			addFileToFilesByName(
				sourceFile,
				sourceFilePath,
		...
		const resolvedRef: ResolvedProjectReference = {
			commandLine,
			sourceFile,
		};
	...
	function verifyCompilerOptions() {
		...
		if (options.verbatimModuleSyntax) {
			...
			if (options.preserveValueImports) {
				createRedundantOptionDiagnostic(
					"preserveValueImports",
					"verbatimModuleSyntax",
				);
			}
		...
		if (
			options.resolvePackageJsonExports &&
			!moduleResolutionSupportsPackageJsonExportsAndImports(
				moduleResolution,
			)
		) {
		...
		if (
			options.resolvePackageJsonImports &&
			!moduleResolutionSupportsPackageJsonExportsAndImports(
				moduleResolution,
			)
		) {
		...
		if (!options.noEmit && !options.suppressOutputPathCheck) {
			...
			forEachEmittedFile(emitHost, (emitFileNames) => {
				...
				verifyEmitFilePath(
					emitFileNames.declarationFilePath,
					emitFilesSeen,
				);
			});
		}
	...
	function checkDeprecations(
		...
		const typescriptVersion = new Version(
			typeScriptVersion || versionMajorMinor,
		);
		const ignoreDeprecationsVersion = getIgnoreDeprecationsVersion();

		const mustBeRemoved = !(
			removedInVersion.compareTo(typescriptVersion) ===
			Comparison.GreaterThan
		);
		const canBeSilenced =
			!mustBeRemoved &&
			ignoreDeprecationsVersion.compareTo(deprecatedInVersion) ===
				Comparison.LessThan;
	...
	function createDiagnosticExplainingFile(
		...
		const location =
			locationReason &&
			getReferencedFileLocation(program, locationReason);
		...
		const redirectInfo =
			file && explainIfFileIsRedirectAndImpliedFormat(file);
		const chain = chainDiagnosticMessages(
			redirectInfo
	...
	function fileIncludeReasonToRelatedInformation(
		...
		switch (reason.kind) {
			case FileIncludeKind.RootFile:
				...
				const matchedByInclude = getMatchedIncludeSpec(
					program,
					fileName,
				);
			...
			case FileIncludeKind.OutputFromProjectReference:
				...
				return referencesSyntax &&
					referencesSyntax.elements.length > index
					? createDiagnosticForNodeInSourceFile(
							sourceFile,
			...
			case FileIncludeKind.LibFile:
				...
				const target = forEachEntry(
					...
					(value, key) =>
						value === getEmitScriptTarget(options)
							? key
							: undefined,
				);
	...
	function verifyProjectReferences() {
		...
		forEachProjectReference(
			...
			(resolvedRef, parent, index) => {
				...
				const parentFile =
					parent && (parent.sourceFile as JsonSourceFile);
	...
	function forEachOptionPathsSyntax<T>(
		callback: (prop: PropertyAssignment) => T | undefined,
	) {
	...
	function getOptionsSyntaxByValue(name: string, value: string) {
		return forEachOptionsSyntaxByName(name, (property) =>
	...
	function createDiagnosticForOption(
		...
		if (needCompilerDiagnostic) {
			...
			if ("messageText" in message) {
				programDiagnostics.add(
					createCompilerDiagnosticFromMessageChain(message),
				);
			} else {
				programDiagnostics.add(
	 ...
	 */
	function createRedundantOptionDiagnostic(
		errorOnOption: string,
		redundantWithOption: string,
	) {
		const compilerOptionsObjectLiteralSyntax =
			getCompilerOptionsObjectLiteralSyntax();
	...
	function isEmittedFile(file: string): boolean {
		...
		if (
			...
			isDeclarationFileName(filePath)
		) {
...
function updateHostForUseSourceOfProjectReferenceRedirect(
	...
	function directoryExistsIfProjectReferenceDeclDir(dir: string) {
		...
		return forEachKey(
			...
			(declDirPath) =>
				...
				startsWith(
					declDirPath,
					dirPathWithTrailingDirectorySeparator,
				) ||
	...
	function fileOrDirectoryExistsUsingSource(
		...
		if (
			isFile &&
			symlinkCache.getSymlinkedFiles()?.has(fileOrDirectoryPath)
		)
			return true;
...
export function parseConfigHostFromCompilerHostLike<T extends BuilderProgram>(
	host: (CompilerHost | ProgramHost<T>) & {
	...
	return {
		...
		realpath: maybeBind(
			directoryStructureHost,
			directoryStructureHost.realpath,
		),
		useCaseSensitiveFileNames: host.useCaseSensitiveFileNames(),
...
export function getResolutionDiagnostic(
	...
	function needAllowArbitraryExtensions() {
		...
		return isDeclarationFile || options.allowArbitraryExtensions
			? undefined
