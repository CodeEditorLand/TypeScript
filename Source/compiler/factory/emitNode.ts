...
export function getOrCreateEmitNode(node: Node): EmitNode {
	if (!node.emitNode) {
		if (isParseTreeNode(node)) {
			...
			const sourceFile =
				getSourceFileOfNode(
					getParseTreeNode(getSourceFileOfNode(node)),
 ...
 * @internal
 */
export function setInternalEmitFlags<T extends Node>(
	node: T,
	emitFlags: InternalEmitFlags,
) {
 ...
 * @internal
 */
export function addInternalEmitFlags<T extends Node>(
	node: T,
	emitFlags: InternalEmitFlags,
) {
	const emitNode = getOrCreateEmitNode(node);
...
export function moveEmitHelpers(
	...
	for (let i = 0; i < sourceEmitHelpers.length; i++) {
		...
		if (predicate(helper)) {
			helpersRemoved++;
			targetEmitNode.helpers = appendIfUnique(
...
export function ignoreSourceNewlines<T extends Node>(node: T): T {
	getOrCreateEmitNode(node).internalFlags |=
		InternalEmitFlags.IgnoreSourceNewlines;
	return node;
}
...
/** @internal */
export function getIdentifierAutoGenerate(
	node: Identifier | PrivateIdentifier,
): AutoGenerateInfo | undefined {
	return node.emitNode?.autoGenerate;
}

/** @internal */
export function setIdentifierGeneratedImportReference<
	T extends Identifier | PrivateIdentifier,
