...
export function createNodeConverters(factory: NodeFactory): NodeConverters {
	...
	function convertToFunctionExpression(node: FunctionDeclaration) {
		...
		const updated = factory.createFunctionExpression(
			getModifiers(node)?.filter(
				(modifier) =>
					!isExportModifier(modifier) && !isDefaultModifier(modifier),
			),
			node.asteriskToken,
			node.name,
			node.typeParameters,
		...
		if (isBindingElement(element)) {
			if (element.dotDotDotToken) {
				...
				return setOriginalNode(
					setTextRange(
						factory.createSpreadElement(element.name),
						element,
					),
					element,
				);
	...
	function convertToObjectAssignmentElement(
		...
		if (isBindingElement(element)) {
			if (element.dotDotDotToken) {
				...
				return setOriginalNode(
					setTextRange(
						factory.createSpreadAssignment(element.name),
						element,
					),
					element,
				);
			...
			if (element.propertyName) {
				const expression = convertToAssignmentElementTarget(
					element.name,
				);
				return setOriginalNode(
					setTextRange(
