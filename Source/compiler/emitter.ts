...
export function getOutputDeclarationFileName(
	...
	return getOutputDeclarationFileNameWorker(
		inputFileName,
		configFile.options,
		ignoreCase,
		getCommonSourceDirectory,
...
export function emitFiles(
	...
	var sourceMapDataList: SourceMapEmitResult[] | undefined =
		...
		getAreDeclarationMapsEnabled(compilerOptions)
			? []
			: undefined;
	var emittedFilesList: string[] | undefined =
	...
	var { enter, exit } = performance.createTimer(
		"printTime",
		"beforePrint",
		"afterPrint",
	);
	...
	function emitBuildInfo(
		...
		const buildInfo =
			host.getBuildInfo(bundle) ||
			createBuildInfo(/*program*/ undefined, bundle);
	...
	function printSourceFileOrBundle(
		...
		if (sourceMapGenerator) {
			...
			if (sourceMapFilePath) {
				...
				if (printer.bundleFileInfo)
					printer.bundleFileInfo.mapHash = computeSignature(
						sourceMap,
						host,
					);
			}
		...
		writeFile(
			...
			sourceFiles,
			{ sourceMapUrlPos, diagnostics: transform.diagnostics },
		);
...
export function getBuildInfo(buildInfoFile: string, buildInfoText: string) {
	return readJsonOrUndefined(buildInfoFile, buildInfoText) as
		| BuildInfo
		| undefined;
}
...
export function emitUsingBuildInfo(
	...
	const result = emitUsingBuildInfoWorker(
		config,
		host,
		getCommandLine,
		customTransformers,
...
function emitUsingBuildInfoWorker(
	...
	if (
		!buildInfo.bundle ||
		!buildInfo.bundle.js ||
		(declarationFilePath && !buildInfo.bundle.dts)
	)
	...
	if (
		sourceMapFilePath &&
		computeSignature(sourceMapText!, host) !== buildInfo.bundle.js.mapHash
	)
	...
	if (
		declarationFilePath &&
		computeSignature(declarationText!, host) !== buildInfo.bundle.dts!.hash
	)
	...
	if (
		declarationMapPath &&
		computeSignature(declarationMapText!, host) !==
			buildInfo.bundle.dts!.mapHash
	)
	...
	const emitHost: EmitHost = {
		...
		getBuildInfo: (bundle) => {
			...
			if (
				program &&
				changedDtsText !== undefined &&
				config.options.composite
			) {
				// Update the output signature
				(program as ProgramBundleEmitBuildInfo).outSignature =
					computeSignature(changedDtsText, host, changedDtsData);
			}
...
/** @internal */
export const createPrinterWithDefaults = /* @__PURE__ */ memoize(() =>
	createPrinter({}),
);

/** @internal */
export const createPrinterWithRemoveComments = /* @__PURE__ */ memoize(() =>
	createPrinter({ removeComments: true }),
);

/** @internal */
export const createPrinterWithRemoveCommentsNeverAsciiEscape =
	/* @__PURE__ */ memoize(() =>
		createPrinter({ removeComments: true, neverAsciiEscape: true }),
	);

/** @internal */
export const createPrinterWithRemoveCommentsOmitTrailingSemicolon =
	/* @__PURE__ */ memoize(() =>
...
export function createPrinter(
	...
	var bundleFileInfo = printerOptions.writeBundleFileInfo
		...
		: undefined;
	var relativeToBuildInfo = bundleFileInfo
	...
	var typeArgumentParenthesizerRuleSelector: OrdinalParentheizerRuleSelector<TypeNode> =
		{
			select: (index) =>
				index === 0
					? parenthesizer.parenthesizeLeadingTypeArgument
					: undefined,
		};
	...
	function writeBundle(
			...
			if (bundleFileInfo) {
				...
				if (prepend.oldFileOfCurrentEmit)
					...
					bundleFileInfo.sections.push({
						...
						data: relativeToBuildInfo!(
							(prepend as UnparsedSource).fileName,
						),
		...
		if (bundleFileInfo && bundle.sourceFiles.length) {
			...
			if (recordBundleFileTextLikeSection(end)) {
				// Store prologues
				const prologues =
					getPrologueDirectivesFromBundledSourceFiles(bundle);
				if (prologues) {
	...
	function getCurrentLineMap() {
		return (
			currentLineMap ||
			(currentLineMap = getLineStarts(
				Debug.checkDefined(currentSourceFile),
			))
		);
	}

	function emit<T extends Node>(
		node: T,
		parenthesizerRule?: (node: T) => T,
	): void;
	function emit<T extends Node>(
		node: T | undefined,
		parenthesizerRule?: (node: T) => T,
	...
	function emitExpression<T extends Expression>(
		node: T,
		parenthesizerRule?: (node: T) => T,
	): void;
	function emitExpression<T extends Expression>(
	...
	function beforeEmitNode(node: Node) {
		if (
			...
			getInternalEmitFlags(node) & InternalEmitFlags.IgnoreSourceNewlines
		) {
	...
	function pipelineEmit<T extends Node>(
		emitHint: EmitHint,
		node: T,
		parenthesizerRule?: (node: T) => T,
	) {
		...
		switch (phase) {
			case PipelinePhase.Notification:
				if (
					onEmitNode !== noEmitNotification &&
					(!isEmitNotificationEnabled ||
						isEmitNotificationEnabled(node))
				) {
			...
			case PipelinePhase.Substitution:
				if (
					substituteNode !== noEmitSubstitution &&
					(lastSubstitution =
						substituteNode(emitHint, node) || node) !== node
				) {
					if (currentParenthesizerRule) {
						lastSubstitution =
							currentParenthesizerRule(lastSubstitution);
					}
	...
	function pipelineEmitWithHintWorker(
		...
		if (hint === EmitHint.MappedTypeParameter)
			return emitMappedTypeParameter(
				cast(node, isTypeParameterDeclaration),
			);
		...
		if (hint === EmitHint.Expression) {
			switch (node.kind) {
				...
				case SyntaxKind.SyntheticExpression:
					return Debug.fail(
						"SyntheticExpression should never be printed.",
					);
				case SyntaxKind.MissingDeclaration:
					return;
		...
		Debug.fail(
			`Unhandled SyntaxKind: ${Debug.formatSyntaxKind(node.kind)}.`,
		);
	}
			...
			if (helpers) {
				for (const helper of helpers) {
					...
					if (typeof helper.text === "string") {
						...
						writeLines(
							helper.text(makeFileLevelOptimisticUniqueName),
						);
					}
					if (bundleFileInfo)
						bundleFileInfo.sections.push({
	...
	function emitComputedPropertyName(node: ComputedPropertyName) {
		...
		pushPrivateNameGenerationScope(
			savedPrivateNameTempFlags,
			savedReservedMemberNames,
		);
	}
	...
	function emitTypeLiteral(node: TypeLiteralNode) {
		...
		pushPrivateNameGenerationScope(
			TempFlags.Auto,
			/*newReservedMemberNames*/ undefined,
		);
	...
	function emitArrayType(node: ArrayTypeNode) {
		emit(
			node.elementType,
			parenthesizer.parenthesizeNonArrayTypeOfPostfixType,
		);
		writePunctuation("[");
		writePunctuation("]");
	}
	...
	function emitConditionalType(node: ConditionalTypeNode) {
		emit(
			node.checkType,
			parenthesizer.parenthesizeCheckTypeOfConditionalType,
		);
		writeSpace();
		writeKeyword("extends");
		writeSpace();
		emit(
			node.extendsType,
			parenthesizer.parenthesizeExtendsTypeOfConditionalType,
		);
		writeSpace();
		writePunctuation("?");
		writeSpace();
	...
	function emitPropertyAccessExpression(node: PropertyAccessExpression) {
		...
		const linesBeforeDot = getLinesBetweenNodes(
			node,
			node.expression,
			token,
		);
	...
	function mayNeedDotDotForPropertyAccess(expression: Expression) {
		...
		if (isNumericLiteral(expression)) {
			...
			return (
				!(expression.numericLiteralFlags & TokenFlags.WithSpecifier) &&
	...
	function emitCallExpression(node: CallExpression) {
		const indirectCall =
			getInternalEmitFlags(node) & InternalEmitFlags.IndirectCall;
		if (indirectCall) {
			writePunctuation("(");
			writeLiteral("0");
			writePunctuation(",");
		...
		function onExit(node: BinaryExpression, state: WorkArea) {
			...
			if (state.stackIndex > 0) {
				...
				const savedContainerPos =
					state.containerPosStack[state.stackIndex];
				const savedContainerEnd =
					state.containerEndStack[state.stackIndex];
	...
	function emitTokenWithComment(
		...
		if (isSimilarNode && contextNode.end !== pos) {
			const isJsxExprContext =
				contextNode.kind === SyntaxKind.JsxExpression;
			emitTrailingCommentsOfPosition(
				pos,
	...
	function parenthesizeExpressionForNoAsi(node: Expression) {
		if (
			...
			if (parseNode && isParenthesizedExpression(parseNode)) {
				...
				const parens = factory.createParenthesizedExpression(
					node.expression,
				);
	...
	function emitVariableDeclaration(node: VariableDeclaration) {
		...
		emitInitializer(
			node.initializer,
			node.type?.end ??
				node.name.emitNode?.typeNode?.end ??
				node.name.end,
			node,
			parenthesizer.parenthesizeExpressionForDisallowedComma,
		);
	...
	function emitSignatureAndBody<T extends FunctionLikeDeclaration>(
		node: T,
		emitSignatureHead: (node: T) => void,
	) {
	...
	function emitClassDeclarationOrExpression(
		...
		emitTokenWithComment(
			...
			moveRangePastModifiers(node).pos,
			writeKeyword,
			node,
		);
		if (node.name) {
	...
	function emitImportTypeNodeAttributes(node: ImportAttributes) {
		...
		writeSpace();
		writeKeyword(
			node.token === SyntaxKind.AssertKeyword ? "assert" : "with",
		);
		writePunctuation(":");
		writeSpace();
		const elements = node.elements;
	...
	function hasCommentsAtPosition(pos: number) {
		return (
			hasTrailingCommentsAtPosition(pos) ||
			hasLeadingCommentsAtPosition(pos)
		);
	}
	...
	function emitJsxExpression(node: JsxExpression) {
		if (
			...
			const isMultiline =
				currentSourceFile &&
				!nodeIsSynthesized(node) &&
				getLineAndCharacterOfPosition(currentSourceFile, node.pos)
					.line !==
					getLineAndCharacterOfPosition(currentSourceFile, node.end)
						.line;
			if (isMultiline) {
		...
		if (modifiers?.length) {
			...
			if (every(modifiers, isDecorator)) {
				if (allowDecorators) {
					...
					return emitDecoratorList(
						node,
						modifiers as NodeArray<Decorator>,
					);
				}
				return node.pos;
			}
	...
	function emitList<Child extends Node, Children extends NodeArray<Child>>(
		...
		emitNodeList(
			...
			children,
			format |
				(parentNode && getEmitFlags(parentNode) & EmitFlags.MultiLine
					? ListFormat.PreferNewLine
					: 0),
			parenthesizerRule,
			start,
	...
	function emitExpressionList<
		Child extends Node,
		Children extends NodeArray<Child>,
	>(
		parentNode: Node | undefined,
		children: Children | undefined,
		format: ListFormat,
	...
	function emitNodeListItems<Child extends Node>(
		...
		for (let i = 0; i < count; i++) {
			...
			if (format & ListFormat.AsteriskDelimited) {
				...
				if (
					...
					const previousSiblingEmitFlags =
						getEmitFlags(previousSibling);
					if (
						!(
				...
				if (separatingLineTerminatorCount > 0) {
					...
					if (
						(format &
							(ListFormat.LinesMask | ListFormat.Indented)) ===
		...
		if (
			...
			emitLeadingCommentsOfPosition(
				emitTrailingComma && childrenTextRange?.end
	...
	function getLeadingLineTerminatorCount(
		...
		if (format & ListFormat.PreserveLines || preserveSourceNewlines) {
			...
			if (
				...
				(!firstChild.parent ||
					getOriginalNode(firstChild.parent) ===
						getOriginalNode(parentNode))
			) {
				if (preserveSourceNewlines) {
		...
		if (format & ListFormat.PreserveLines || preserveSourceNewlines) {
			...
			if (
				...
				if (preserveSourceNewlines) {
					const end =
						childrenTextRange &&
						!positionIsSynthesized(childrenTextRange.end)
							? childrenTextRange.end
							: lastChild.end;
					return getEffectiveLines((includeComments) =>
						getLinesBetweenPositionAndNextNonWhitespaceCharacter(
	...
	function getTextOfNode(
		...
		const canUseSourceFile =
			!!sourceFile && !!node.parent && !nodeIsSynthesized(node);
		if (isMemberName(node)) {
			if (
				!canUseSourceFile ||
				getSourceFileOfNode(node) !== getOriginalNode(sourceFile)
			) {
				return idText(node);
			}
	...
	function getLiteralTextOfNode(
		...
		if (
			...
			if (
				isIdentifier(textSourceNode) ||
				isPrivateIdentifier(textSourceNode) ||
		...
		const flags =
			...
			(printerOptions.terminateUnterminatedLiterals
				...
				: 0) |
			(printerOptions.target &&
			printerOptions.target === ScriptTarget.ESNext
	...
	function reservePrivateNameInNestedScopes(name: string) {
		if (
			...
			reservedPrivateNames === lastOrUndefined(reservedPrivateNamesStack)
		) {
	...
	function generateNames(node: Node | undefined) {
		...
		switch (node.kind) {
			...
			case SyntaxKind.ForInStatement:
				...
				generateNames(
					(node as ForStatement | ForInOrOfStatement).statement,
				);
				break;
			case SyntaxKind.SwitchStatement:
			...
			case SyntaxKind.DefaultClause:
				forEach(
					(node as CaseOrDefaultClause).statements,
					generateNames,
				);
				break;
			case SyntaxKind.TryStatement:
			...
			case SyntaxKind.FunctionDeclaration:
				...
				if (getEmitFlags(node) & EmitFlags.ReuseTempVariableScope) {
					forEach(
						(node as FunctionDeclaration).parameters,
						generateNames,
					);
	...
	function generateNameIfNeeded(name: DeclarationName | undefined) {
		if (name) {
			if (
				isGeneratedIdentifier(name) ||
				isGeneratedPrivateIdentifier(name)
			) {
				generateName(name);
	...
	function isUniqueName(name: string, privateName: boolean): boolean {
		return (
			isFileLevelUniqueNameInCurrentFile(name, privateName) &&
	 ...
	 */
	function isFileLevelUniqueNameInCurrentFile(
		name: string,
		_isPrivate: boolean,
	) {
		return currentSourceFile
			? isFileLevelUniqueName(currentSourceFile, name, hasGlobalName)
			: true;
	}
	...
	function isUniqueLocalName(
		name: string,
		container: HasLocals | undefined,
	): boolean {
		for (
			let node = container;
			node && isNodeDescendantOf(node, container);
	...
	function getTempFlags(formattedNameKey: string) {
		switch (formattedNameKey) {
			...
			default:
				return (
					formattedNameTempFlags?.get(formattedNameKey) ??
					TempFlags.Auto
				);
		}
	}
	...
	function makeTempVariableName(
		...
		if (flags && !(tempFlags & flags)) {
			...
			const fullName = formatGeneratedName(
				privateName,
				prefix,
				name,
				suffix,
	...
	function makeUniqueName(
		...
		if (
			baseName.length > 0 &&
			baseName.charCodeAt(0) === CharacterCodes.hash
		) {
	...
	function generateNameForModuleOrEnum(
		...
		return isUniqueLocalName(name, tryCast(node, canHaveLocals))
			? name
			: makeUniqueName(
					name,
					isUniqueName,
	...
	function generateNameForNode(
		...
		switch (node.kind) {
			...
			case SyntaxKind.ClassDeclaration: {
				Debug.assert(!prefix && !suffix && !privateName);
				const name = (node as ClassDeclaration | FunctionDeclaration)
					.name;
	...
	function makeName(name: GeneratedIdentifier | GeneratedPrivateIdentifier) {
		const autoGenerate = name.emitNode.autoGenerate;
		const prefix = formatGeneratedNamePart(
			autoGenerate.prefix,
			generateName,
		);
		...
		switch (autoGenerate.flags & GeneratedIdentifierFlags.KindMask) {
			...
			case GeneratedIdentifierFlags.Unique:
				return makeUniqueName(
					idText(name),
					autoGenerate.flags & GeneratedIdentifierFlags.FileLevel
						? isFileLevelUniqueNameInCurrentFile
						: isUniqueName,
					!!(
	...
	function emitLeadingComments(pos: number, isEmittedNode: boolean) {
		...
		if (isEmittedNode) {
			if (pos === 0 && currentSourceFile?.isDeclarationFile) {
				forEachLeadingCommentToEmit(
					pos,
					emitNonTripleSlashLeadingComment,
				);
			} else {
	...
	function emitLeadingComment(
		...
		if (
			!currentSourceFile ||
			!shouldWriteComment(currentSourceFile.text, commentPos)
		)
			return;
	...
	function forEachLeadingCommentToEmit(
		...
		if (
			currentSourceFile &&
			(containerPos === -1 || pos !== containerPos)
		) {
	...
	function emitDetachedCommentsAndUpdateCommentsInfo(range: TextRange) {
		const currentDetachedCommentInfo =
			currentSourceFile &&
			emitDetachedComments(
				currentSourceFile.text,
				getCurrentLineMap(),
...
type ParenthesizerRuleOrSelector<T extends Node> =
	| OrdinalParentheizerRuleSelector<T>
	| ParenthesizerRule<T>;

type EmitFunction = <T extends Node>(
	node: T,
	parenthesizerRule?: ParenthesizerRule<T>,
) => void;
...
function emitListItemWithParenthesizerRuleSelector(
	node: Node,
	emit: EmitFunction,
	parenthesizerRuleSelector:
		| OrdinalParentheizerRuleSelector<Node>
		| undefined,
	index: number,
) {
...
function getEmitListItem<T extends Node>(
	...
	return emit.length === 1
		? (emitListItemNoParenthesizer as EmitListItemFunction<T>)
		: typeof parenthesizerRule === "object"
			? (emitListItemWithParenthesizerRuleSelector as EmitListItemFunction<T>)
