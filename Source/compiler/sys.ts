...
function createDynamicPriorityPollingWatchFile(host: {
	...
	interface PollingIntervalQueue
		extends Array<WatchedFileWithUnchangedPolls> {
		pollingInterval: PollingInterval;
		pollIndex: number;
	...
	function createPollingIntervalQueue(
		...
		const queue =
			[] as WatchedFileWithUnchangedPolls[] as PollingIntervalQueue;
...
function createDirectoryWatcherSupportingRecursive({
	...
	function updateChildWatches(
		...
		>(
			...
			(child, childWatcher) =>
				filePathComparer(child, childWatcher.dirName),
...
export function createSystemWatchFunctions({
		...
		switch (watchFileKind) {
			...
			case WatchFileKind.UseFsEventsOnParentDirectory:
				if (!nonPollingWatchFile) {
					nonPollingWatchFile =
						createUseFsEventsOnParentDirectoryWatchFile(
							fsWatch,
							useCaseSensitiveFileNames,
						);
				}
	...
	function updateOptionsForWatchDirectory(
		...
		switch (tscWatchDirectory) {
			...
			case "RecursiveDirectoryUsingDynamicPriorityPolling":
				// Use polling interval but change the interval depending on file changes and their default polling interval
				return {
					watchDirectory: WatchDirectoryKind.DynamicPriorityPolling,
				};
			default:
				const defaultFallbackPolling = options?.fallbackPolling;
				return {
	...
	function pollingWatchFile(
		...
		return createSingleWatcherPerName(
			...
			callback,
			(cb) =>
				pollingWatchFileWorker(fileName, cb, pollingInterval, options),
		);
	}
	function fsWatch(
		fileOrDirectory: string,
			...
			(eventName, relativeFileName, currentModifiedTime) => {
				if (eventName === "change") {
					currentModifiedTime ||=
						...
						missingFileModifiedTime;
					if (
						currentModifiedTime.getTime() === modifiedTime.getTime()
					)
						return;
				}
				modifiedTime =
...
export let sys: System = (() => {
	...
	function getNodeSystem(): System {
		...
		const fsSupportsRecursiveFsWatch =
			process.platform === "win32" || isMacOs;
		...
		function cleanupPaths(profile: import("inspector").Profiler.Profile) {
			...
			const normalizedDir = normalizeSlashes(
				_path.dirname(executingFilePath),
			);
		...
		function getAccessibleFileSystemEntries(
			...
			try {
				...
				for (const dirent of entries) {
					...
					const entry =
						typeof dirent === "string" ? dirent : dirent.name;
