...
export namespace tracingEnabled {
	...
	function writeStackEvent(index: number, endTime: number, results?: Args) {
		...
		if (separateBeginAndEnd) {
			Debug.assert(
				!results,
				"`results` are not supported for events with `separateBeginAndEnd`",
			);
	...
	function getLocation(node: Node | undefined) {
		...
		return !file
			...
			: {
					path: file.path,
					start: indexFromOne(
						getLineAndCharacterOfPosition(file, node!.pos),
					),
					end: indexFromOne(
						getLineAndCharacterOfPosition(file, node!.end),
					),
				};
		...
		for (let i = 0; i < numTypes; i++) {
			...
			if (type.flags & TypeFlags.Conditional) {
				...
				conditionalProperties = {
					...
					conditionalTrueType:
						conditionalType.resolvedTrueType?.id ?? -1,
					conditionalFalseType:
						conditionalType.resolvedFalseType?.id ?? -1,
				};
			}
			...
			if (objectFlags & ObjectFlags.ReverseMapped) {
				...
				reverseMappedProperties = {
					...
					reverseMappedConstraintType:
						reverseMappedType.constraintType?.id,
				};
			}
			...
			if (objectFlags & ObjectFlags.EvolvingArray) {
				...
				evolvingArrayProperties = {
					...
					evolvingArrayFinalType:
						evolvingArrayType.finalArrayType?.id,
				};
			}
