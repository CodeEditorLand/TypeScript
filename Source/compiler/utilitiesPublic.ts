		...
		case SyntaxKind.ExpressionStatement:
			...
			switch (expr.kind) {
				...
				case SyntaxKind.ElementAccessExpression:
					const arg = (expr as ElementAccessExpression)
						.argumentExpression;
					if (isIdentifier(arg)) {
						return arg;
					}
...
export function nodeHasName(statement: Node, name: Identifier) {
	...
	if (
		...
		some(statement.declarationList.declarations, (d) =>
			nodeHasName(d, name),
		)
	) {
		return true;
...
export function isTypeOnlyImportDeclaration(
	...
	switch (node.kind) {
		case SyntaxKind.ImportSpecifier:
			return (
				(node as ImportSpecifier).isTypeOnly ||
				(node as ImportSpecifier).parent.parent.isTypeOnly
			);
...
export function isTypeOnlyExportDeclaration(
	...
	switch (node.kind) {
		case SyntaxKind.ExportSpecifier:
			return (
				(node as ExportSpecifier).isTypeOnly ||
				(node as ExportSpecifier).parent.parent.isTypeOnly
			);
...
export function isCallLikeOrFunctionLikeExpression(
	...
	return (
		isCallLikeExpression(node) || isFunctionExpressionOrArrowFunction(node)
	);
}
...
export function isInternalDeclaration(node: Node, sourceFile?: SourceFile) {
	...
	if (parseTreeNode && parseTreeNode.kind === SyntaxKind.Parameter) {
		...
		return (
			some(commentRanges) &&
			hasInternalAnnotation(last(commentRanges), sourceFile)
		);
	}
	const leadingCommentRanges =
		parseTreeNode &&
		getLeadingCommentRangesOfNode(parseTreeNode, sourceFile);
