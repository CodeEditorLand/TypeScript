...
/** @internal */
export type SerializedDiagnosticMessageChain = Omit<
	DiagnosticMessageChain,
	"next" | "repopulateInfo"
> & {
	next?: ReusableDiagnosticMessageChain[];
...
/** @internal */
export type ReusableDiagnosticMessageChain =
	| SerializedDiagnosticMessageChain
	| ReusableRepopulateModuleNotFoundChain;
...
export interface ReusableBuilderProgramState extends BuilderState {
	...
	semanticDiagnosticsPerFile?:
		| Map<Path, readonly ReusableDiagnostic[] | readonly Diagnostic[]>
		| undefined;
...
/** @internal */
export type SavedBuildProgramEmitState = Pick<
	BuilderProgramState,
...
export function getPendingEmitKind(
	...
	const oldEmitKind =
		oldOptionsOrEmitKind &&
		(isNumber(oldOptionsOrEmitKind)
			? oldOptionsOrEmitKind
			: getBuilderFileEmit(oldOptionsOrEmitKind));
	const emitKind = isNumber(optionsOrEmitKind)
		? optionsOrEmitKind
		: getBuilderFileEmit(optionsOrEmitKind);
	...
	if (diff & BuilderFileEmit.AllDts)
		result = result | (emitKind & BuilderFileEmit.AllDts);
	return result;
}
...
function createBuilderProgramState(
	...
	if (!outFilePath) {
		...
		oldState?.outSignature &&
		outFilePath === outFile(oldState?.compilerOptions)
	) {
		state.outSignature =
			oldState.outSignature &&
	...
	state.latestChangedDtsFile = compilerOptions.composite
		? oldState?.latestChangedDtsFile
		: undefined;
	...
	if (useOldState) {
		...
		oldState!.changedFilesSet?.forEach((value) =>
			state.changedFilesSet.add(value),
		);
		if (!outFilePath && oldState!.affectedFilesPendingEmit?.size) {
			state.affectedFilesPendingEmit = new Map(
				oldState!.affectedFilesPendingEmit,
			);
	...
	state.fileInfos.forEach((info, sourceFilePath) => {
		...
		if (
			...
			(newReferences &&
				forEachKey(
					newReferences,
					(path) =>
						!state.fileInfos.has(path) &&
						oldState!.fileInfos.has(path),
				))
		) {
			...
			const emitDiagnostics =
				oldState!.emitDiagnosticsPerFile?.get(sourceFilePath);
			if (emitDiagnostics) {
				(state.emitDiagnosticsPerFile ??= new Map()).set(
					sourceFilePath,
					oldState!.hasReusableDiagnostic
						? convertToDiagnostics(
								emitDiagnostics as readonly ReusableDiagnostic[],
								newProgram,
							)
						: repopulateDiagnostics(
								emitDiagnostics as readonly Diagnostic[],
								newProgram,
							),
				);
			}
			...
			if (canCopySemanticDiagnostics) {
				...
				if (diagnostics) {
					...
					(state.semanticDiagnosticsFromOldState ??= new Set()).add(
						sourceFilePath,
					);
				}
			}
		...
		if (canCopyEmitSignatures) {
			const oldEmitSignature =
				oldState.emitSignatures.get(sourceFilePath);
			if (oldEmitSignature) {
	...
	if (
		...
		const pendingEmitKind = compilerOptionsAffectEmit(
			compilerOptions,
			oldCompilerOptions,
		)
		...
		if (pendingEmitKind !== BuilderFileEmit.None) {
			if (!outFilePath) {
				...
				Debug.assert(
					!state.seenAffectedFiles || !state.seenAffectedFiles.size,
				);
...
>(
	...
	return next === chain.next
		? (chain as DiagnosticMessageChain)
		: { ...(chain as DiagnosticMessageChain), next };
}
...
function convertToDiagnosticRelatedInformation(
	...
	const sourceFile = file
		? newProgram.getSourceFileByPath(toPath(file))
		: undefined;
	return {
		...diagnostic,
		file: sourceFile,
		messageText: isString(diagnostic.messageText)
			...
			: convertOrRepopulateDiagnosticMessageChain(
					...
					newProgram,
					(chain) =>
						(chain as ReusableRepopulateModuleNotFoundChain).info,
				),
	};
}
...
function backupBuilderProgramEmitState(
	...
	return {
		affectedFilesPendingEmit:
			...
			new Map(state.affectedFilesPendingEmit),
		seenEmittedFiles:
		...
		changedFilesSet: outFilePath
			? new Set(state.changedFilesSet)
			: undefined,
		buildInfoEmitPending: state.buildInfoEmitPending,
		emitDiagnosticsPerFile:
			state.emitDiagnosticsPerFile &&
			new Map(state.emitDiagnosticsPerFile),
	};
}

function restoreBuilderProgramEmitState(
	state: BuilderProgramState,
	savedEmitState: SavedBuildProgramEmitState,
) {
...
function getNextAffectedFilePendingEmit(
	...
	return forEachEntry(state.affectedFilesPendingEmit, (emitKind, path) => {
		...
		if (
			!affectedFile ||
			!sourceFileMayBeEmitted(affectedFile, state.program!)
		) {
		...
		if (emitOnlyDtsFiles)
			pendingKind = pendingKind & BuilderFileEmit.AllDts;
...
function getNextPendingEmitDiagnosticsFile(state: BuilderProgramState) {
	...
	return forEachEntry(state.emitDiagnosticsPerFile, (diagnostics, path) => {
		...
		const seenKind =
			state.seenEmittedFiles?.get(affectedFile.resolvedPath) ||
			BuilderFileEmit.None;
...
function isChangedSignature(state: BuilderProgramState, path: Path) {
	const oldSignature =
		Debug.checkDefined(state.oldSignatures).get(path) || undefined;
...
function handleDtsMayChangeOfGlobalScope(
	...
	BuilderState.getAllFilesExcludingDefaultLibraryFile(
		...
		handleDtsMayChangeOf(state, file.resolvedPath, cancellationToken, host),
	);
	removeDiagnosticsOfLibraryFiles(state);
	return true;
}
...
function handleDtsMayChangeOfReferencingExportOfAffectedFile(
	...
	state.exportedModulesMap
		...
		?.forEach((exportedFromPath) => {
			...
			return (
				references &&
				forEachKey(references, (filePath) =>
...
function handleDtsMayChangeOfFileAndExportsOfFile(
	...
	// If exported modules has path, all files referencing file exported from are affected
	state
		.exportedModulesMap!.getKeys(filePath)
		?.forEach((exportedFromPath) =>
			handleDtsMayChangeOfFileAndExportsOfFile(
				state,
				exportedFromPath,
...
export type ProgramBuilderInfoFilePendingEmit =
	| ProgramBuildInfoFileId
	| [fileId: ProgramBuildInfoFileId]
	| [fileId: ProgramBuildInfoFileId, emitKind: BuilderFileEmit];
/** @internal */
...
export type ProgramBuildInfoEmitSignature =
	| ProgramBuildInfoFileId
	| [fileId: ProgramBuildInfoFileId, signature: EmitSignature | []];
/**
 ...
 * @internal
 */
export type ProgramMultiFileEmitBuildInfoFileInfo =
	| string
	| ProgramMultiFileEmitBuildInfoBuilderStateFileInfo;
/** @internal */
export type ProgramBuildInfoRootStartEnd = [
 ...
 * @internal
 */
export type ProgramBuildInfoRoot =
	| ProgramBuildInfoRootStartEnd
	| ProgramBuildInfoFileId;
/** @internal */
...
/** @internal */
export type ProgramBuildInfo =
	| ProgramMultiFileEmitBuildInfo
	| ProgramBundleEmitBuildInfo;

/** @internal */
export function isProgramBundleEmitBuildInfo(
	info: ProgramBuildInfo,
): info is ProgramBundleEmitBuildInfo {
...
function getBuildInfo(
	...
	const latestChangedDtsFile = state.latestChangedDtsFile
		? relativeToBuildInfoEnsuringAbsolutePath(state.latestChangedDtsFile)
		: undefined;
	...
	if (outFile(state.compilerOptions)) {
		...
		const fileInfos = arrayFrom(
			...
			([key, value]): ProgramBundleEmitBuildInfoFileInfo => {
				// Ensure fileId
		...
		state.bundle = bundle = {
			...
			js:
				js ||
				(!state.compilerOptions.emitDeclarationOnly
					? state.bundle?.js
					: undefined),
			dts:
				dts ||
	...
	let fileNamesToFileIdListId:
		| Map<string, ProgramBuildInfoFileIdListId>
		| undefined;
		...
		([key, value]): ProgramMultiFileEmitBuildInfoFileInfo => {
			...
			if (state.compilerOptions.composite) {
				...
				if (
					...
					sourceFileMayBeEmitted(file, state.program!)
				) {
						...
						(emitSignatures ||= []).push(
							emitSignature === undefined
								...
								: // fileId, signature: emptyArray if signature only differs in dtsMap option than our own compilerOptions otherwise EmitSignature
									[
										fileId,
										!isString(emitSignature) &&
										emitSignature[0] === actualSignature
											? (emptyArray as [])
											: emitSignature,
									],
						);
			...
			(key) => {
				...
				if (oldValue === undefined)
					return [
						toFileId(key),
						toFileIdListId(
							state.exportedModulesMap!.getValues(key)!,
						),
					];
	...
	if (state.affectedFilesPendingEmit?.size) {
		...
		for (const path of arrayFrom(
			state.affectedFilesPendingEmit.keys(),
		).sort(compareStringsCaseSensitive)) {
			if (tryAddToSet(seenFiles, path)) {
				...
				const fileId = toFileId(path),
					pendingEmit = state.affectedFilesPendingEmit.get(path)!;
	...
	const emitDiagnosticsPerFile = convertToProgramBuildInfoDiagnostics(
		state.emitDiagnosticsPerFile,
	);
	...
	function toFileIdListId(
		...
		if (fileIdListId === undefined) {
			...
			(fileNamesToFileIdListId ||= new Map()).set(
				key,
				(fileIdListId =
					fileIdsList.length as ProgramBuildInfoFileIdListId),
			);
		}
		return fileIdListId;
	}
	...
	function tryAddRoot(path: Path, fileId: ProgramBuildInfoFileId) {
		...
		if (
			!state
				.program!.getFileIncludeReasons()
				.get(file.path)!
				.some((r) => r.kind === FileIncludeKind.RootFile)
		)
			return;
		// First fileId as is
	...
	function convertToProgramBuildInfoCompilerOptions(
		...
		for (const name of getOwnKeys(options).sort(
			compareStringsCaseSensitive,
		)) {
	...
	function convertToProgramBuildInfoDiagnostics(
		diagnostics: Map<Path, readonly Diagnostic[]> | undefined,
	) {
		let result: ProgramBuildInfoDiagnostic[] | undefined;
		if (diagnostics) {
			for (const key of arrayFrom(diagnostics.keys()).sort(
				compareStringsCaseSensitive,
			)) {
				const value = diagnostics.get(key)!;
				(result ||= []).push(
					value.length
						? [toFileId(key), convertToReusableDiagnostics(value)]
						: toFileId(key),
				);
			}
		}
	...
	function convertToReusableDiagnostics(
		...
		return diagnostics.map((diagnostic) => {
			const result: ReusableDiagnostic =
				convertToReusableDiagnosticRelatedInformation(diagnostic);
			result.reportsUnnecessary = diagnostic.reportsUnnecessary;
			...
			result.relatedInformation = relatedInformation
				? relatedInformation.length
					? relatedInformation.map((r) =>
							convertToReusableDiagnosticRelatedInformation(r),
						)
					: []
				: undefined;
			return result;
	...
	function convertToReusableDiagnosticRelatedInformation(
		...
		return {
			...
			messageText: isString(diagnostic.messageText)
				? diagnostic.messageText
				: convertToReusableDiagnosticMessageChain(
						diagnostic.messageText,
					),
		};
	}

	function convertToReusableDiagnosticMessageChain(
	...
	function convertToReusableDiagnosticMessageChainArray(
		...
		return (
			forEach(array, (chain, index) => {
				const reusable = convertToReusableDiagnosticMessageChain(chain);
				if (chain === reusable) return undefined;
				const result: ReusableDiagnosticMessageChain[] =
					index > 0 ? array.slice(0, index - 1) : [];
				result.push(reusable);
				for (let i = index + 1; i < array.length; i++) {
					result.push(
						convertToReusableDiagnosticMessageChain(array[i]),
					);
				}
				return result;
			}) || array
...
export function getBuilderCreationParameters(
	...
	return {
		...
		oldProgram,
		configFileParsingDiagnostics:
			configFileParsingDiagnostics || emptyArray,
	};
}

function getTextHandlingSourceMapForSignature(
	text: string,
	data: WriteFileCallbackData | undefined,
) {
	return data?.sourceMapUrlPos !== undefined
...
export function computeSignatureWithDiagnostics(
	...
	function flattenDiagnosticMessageText(
		diagnostic: string | DiagnosticMessageChain | undefined,
	): string {
		return isString(diagnostic)
			...
			: diagnostic === undefined
				...
				: !diagnostic.next
					...
					: diagnostic.messageText +
						diagnostic.next
							.map(flattenDiagnosticMessageText)
							.join("\n");
	}
...
export function computeSignature(
	...
	return (host.createHash ?? generateDjb2Hash)(
...
export function createBuilderProgram(
	...
	builderProgram.restoreEmitState = (saved) =>
		restoreBuilderProgramEmitState(state, saved);
	builderProgram.hasChangedEmitSignature = () =>
		!!state.hasChangedEmitSignature;
	...
	function emitNextAffectedFile(
		...
		if (!affected) {
			if (!outFile(state.compilerOptions)) {
				...
				if (!pendingAffectedFile) {
					const pendingForDiagnostics =
						getNextPendingEmitDiagnosticsFile(state);
					if (pendingForDiagnostics) {
						(state.seenEmittedFiles ??= new Map()).set(
							pendingForDiagnostics.affectedFile.resolvedPath,
							pendingForDiagnostics.seenKind |
								BuilderFileEmit.AllDts,
						);
						return {
							result: {
								emitSkipped: true,
								diagnostics: pendingForDiagnostics.diagnostics,
							},
				...
				if (emitOnlyDtsFiles)
					emitKind = emitKind & BuilderFileEmit.AllDts;
		...
		if (affected !== state.program) {
			...
			const existing =
				state.seenEmittedFiles?.get(affectedSourceFile.resolvedPath) ||
				BuilderFileEmit.None;
			(state.seenEmittedFiles ??= new Map()).set(
				affectedSourceFile.resolvedPath,
				emitKind | existing,
			);
			const existingPending =
				state.affectedFilesPendingEmit?.get(
					affectedSourceFile.resolvedPath,
				) || programEmitKind;
			const pendingKind = getPendingEmitKind(
				existingPending,
				emitKind | existing,
			);
			if (pendingKind)
				(state.affectedFilesPendingEmit ??= new Map()).set(
					affectedSourceFile.resolvedPath,
					pendingKind,
				);
			else
	...
	function getWriteFileCallback(
		...
		return (
				...
				if (!outFile(state.compilerOptions)) {
						...
						if (info.signature === file.version) {
							...
							if (signature !== file.version) {
								...
								if (host.storeFilesChangingSignatureDuringEmit)
									(state.filesChangingSignature ??=
										new Set()).add(file.resolvedPath);
								if (state.exportedModulesMap)
								...
								if (state.affectedFiles) {
									...
									const existing = state.oldSignatures?.get(
										file.resolvedPath,
									);
					...
					if (state.compilerOptions.composite) {
						...
						emitSignature = handleNewSignature(
							state.emitSignatures?.get(filePath),
							emitSignature,
						);
			...
			function handleNewSignature(
				...
				const oldSignature =
					!oldSignatureFormat || isString(oldSignatureFormat)
						? oldSignatureFormat
		...
		if (!targetSourceFile) {
			if (
				...
				while (
					...
					emitSkipped =
						emitSkipped || affectedEmitResult.result.emitSkipped;
	...
	function getSemanticDiagnosticsOfNextAffectedFile(
		...
		while (true) {
			...
			if (!affected)
				...
				if (
					!ignoreSourceFile ||
					!ignoreSourceFile(affectedSourceFile)
				) {
					result = getSemanticDiagnosticsOfFile(
						state,
						affectedSourceFile,
						cancellationToken,
					);
				...
				state.changedFilesSet.clear();
				state.programEmitPending = getBuilderFileEmit(
					state.compilerOptions,
				);
			}
			return { result, affected };
...
function addToAffectedFilesPendingEmit(
	...
	const existingKind =
		state.affectedFilesPendingEmit?.get(affectedFilePendingEmit) ||
		BuilderFileEmit.None;
	(state.affectedFilesPendingEmit ??= new Map()).set(
...
export function createBuilderProgramUsingProgramBuildInfo(
	...
	const latestChangedDtsFile = program.latestChangedDtsFile
	...
	if (isProgramBundleEmitBuildInfo(program)) {
		...
		filePathsSetList = program.fileIdsList?.map(
			(fileIds) => new Set(fileIds.map(toFilePath)),
		);
		const fileInfos = new Map<Path, BuilderState.FileInfo>();
		const emitSignatures =
			program.options?.composite && !outFile(program.options)
				? new Map<Path, EmitSignature>()
				: undefined;
		program.fileInfos.forEach((fileInfo, index) => {
			const path = toFilePath((index + 1) as ProgramBuildInfoFileId);
		...
		const fullEmitForOptions = program.affectedFilesPendingEmit
			? getBuilderFileEmit(program.options || {})
			: undefined;
		state = {
			fileInfos,
			compilerOptions: program.options
			...
			semanticDiagnosticsPerFile: toPerFileDiagnostics(
				program.semanticDiagnosticsPerFile,
			),
			emitDiagnosticsPerFile: toPerFileDiagnostics(
				program.emitDiagnosticsPerFile,
			),
			hasReusableDiagnostic: true,
			affectedFilesPendingEmit:
...
export function getBuildInfoFileVersionMap(
	...
	host: Pick<
		ReadBuildProgramHost,
		"useCaseSensitiveFileNames" | "getCurrentDirectory"
	>,
) {
	const buildInfoDirectory = getDirectoryPath(
		getNormalizedAbsolutePath(buildInfoPath, host.getCurrentDirectory()),
...
export function createRedirectedBuilderProgram(
	...
	return {
		...
		getDeclarationDiagnostics: (sourceFile, cancellationToken) =>
			getProgram().getDeclarationDiagnostics(
				sourceFile,
				cancellationToken,
			),
