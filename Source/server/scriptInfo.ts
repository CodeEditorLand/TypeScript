...
export class TextStorage {
	...
	public reloadWithFileText(tempFileName?: string) {
		const { text: newText, fileSize } =
			tempFileName || !this.info.isDynamicOrHasMixedContent()
	...
	public getSnapshot(): IScriptSnapshot {
		return (
			this.tryUseScriptVersionCache()?.getSnapshot() ||
			(this.textSnapshot ??= ScriptSnapshot.fromString(
				Debug.checkDefined(this.text),
			))
		);
	}
	...
	public getAbsolutePositionAndLineText(
		...
		return oneBasedLine <= lineMap.length
			? {
					...
					lineText: this.text!.substring(
						lineMap[oneBasedLine - 1],
						lineMap[oneBasedLine],
					),
				}
			: {
	...
	private getFileTextAndSize(tempFileName?: string): {
		...
		const getText = () =>
			text === undefined
				? (text = this.host.readFile(fileName) || "")
				: text;
	...
	private tryUseScriptVersionCache(): ScriptVersionCache | undefined {
		...
		if (this.isOpen) {
			if (!this.svc && !this.textSnapshot) {
				this.svc = ScriptVersionCache.fromString(
					Debug.checkDefined(this.text),
				);
				this.textSnapshot = undefined;
			}
	...
	private getLineMap() {
		...
		return (
			this.lineMap ||
			(this.lineMap = computeLineStarts(Debug.checkDefined(this.text)))
		);
	}
	...
	getLineInfo(): LineInfo {
		...
		if (svc) {
			return {
				...
				getLineText: (line) =>
					svc.getAbsolutePositionAndLineText(line + 1).lineText!,
			};
		}
		const lineMap = this.getLineMap();
...
export class ScriptInfo {
	...
	getDefaultProject() {
		switch (this.containingProjects.length) {
			...
			default:
				...
				for (
					let index = 0;
					index < this.containingProjects.length;
					index++
				) {
					const project = this.containingProjects[index];
					if (isConfiguredProject(project)) {
						if (
							!project.isSourceOfProjectReferenceRedirect(
								this.fileName,
							)
						) {
							...
							if (
								...
								defaultConfiguredProject =
									project.projectService.findDefaultConfiguredProject(
										this,
									) || false;
							}
	...
	isContainedByBackgroundProject() {
		return some(this.containingProjects, isBackgroundProject);
	}
	...
	public isJavaScript() {
		return (
			...
			this.scriptKind === ScriptKind.JSX
		);
	}
