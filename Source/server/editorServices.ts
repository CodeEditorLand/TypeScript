...
const fileNamePropertyReader: FilePropertyReader<string> = {
	...
	hasMixedContent: (fileName, extraFileExtensions) =>
		some(
			extraFileExtensions,
			(ext) =>
				ext.isMixedContent && fileExtensionIs(fileName, ext.extension),
		),
};
...
export function forEachResolvedProjectReferenceProject<T>(
	...
	let seenResolvedRefs:
		| Map<string, ProjectReferenceProjectLoadKind>
		| undefined;
	...
	if (possibleDefaultRef) {
		...
		if (child) {
			...
			const result = forEachResolvedProjectReferenceProjectWorker(
				...
				(ref, loadKind) =>
					possibleDefaultRef === ref
						? callback(ref, loadKind)
						: undefined,
				projectReferenceProjectLoadKind,
		...
		const child =
			...
			(loadKind === ProjectReferenceProjectLoadKind.Find
				...
				: loadKind === ProjectReferenceProjectLoadKind.FindCreate
					? project.projectService.createConfiguredProject(
							configFileName,
						)
					: loadKind ===
...
function forEachResolvedProjectReferenceProjectWorker<T>(
	...
	return forEach(resolvedProjectReferences, (ref) => {
		...
		const configFileName = toNormalizedPath(ref.sourceFile.fileName);
		const canonicalPath =
			projectService.toCanonicalFileName(configFileName);
...
function forEachAnyProjectReferenceKind<T>(
	...
	cbPotentialProjectRef: (
		potentialProjectReference: NormalizedPath,
	) => T | undefined,
): T | undefined {
...
export interface ParsedConfig {
	...
	updateLevel?:
		| ProgramUpdateLevel.RootNamesAndUpdate
...
function createWatchFactoryHostUsingWatchEvents(
	service: ProjectService,
	canUseWatchEvents: boolean | undefined,
): WatchFactoryHost | undefined {
	...
	const watchedDirectoriesRecursive =
		getHostWatcherMap<DirectoryWatcherCallback>();
	let ids = 1;
	service.session.addProtocolHandler(
		protocol.CommandTypes.WatchChange,
		(req) => {
	...
	function watchFile(
		...
		return getOrCreateFileWatcher(watchedFiles, path, callback, (id) => ({
			eventName: CreateFileWatcherEvent,
			data: { id, path },
		}));
	}
	function watchDirectory(
		...
		return getOrCreateFileWatcher(
			...
			(id) => ({
				...
				data: { id, path, recursive: !!recursive },
			}),
		);
	}
	function getOrCreateFileWatcher<T>(
		...
		event: (
			id: number,
		) => CreateFileWatcherEvent | CreateDirectoryWatcherEvent,
	) {
		...
		return {
			close() {
				...
				service.eventHandler!({
					eventName: CloseFileWatcherEvent,
					data: { id },
				});
			},
		};
	...
	function onWatchChange({
		...
		path,
		eventType,
	}: protocol.WatchChangeRequestArgs) {
...
export class ProjectService {
	...
	/** @internal */
	readonly newAuxiliaryProjectName = createProjectNameFactoryWithCounter(
		makeAuxiliaryProjectName,
	);
	/**
	 * Open files: with value being project root path, and key being Path of the file that is open
	 */
	readonly openFiles: Map<string, NormalizedPath | undefined> = new Map<
		Path,
	 ...
	 * @internal
	 */
	readonly configFileExistenceInfoCache = new Map<
		NormalizedPath,
	...
	private pendingPluginEnablements?: Map<
		Project,
		Promise<BeginEnablePluginResult>[]
	>;
	...
	constructor(opts: ProjectServiceOptions) {
		...
		this.currentDirectory = toNormalizedPath(
		...
		this.watchFactory =
			this.serverMode !== LanguageServiceMode.Semantic
				...
				: getWatchFactory(
						createWatchFactoryHostUsingWatchEvents(
							this,
							opts.canUseWatchEvents,
						) || this.host,
						watchLogLevel,
						log,
						getDetailWatchInfo,
		...
		try {
			...
			for (const key in raw.simpleMap) {
				if (hasProperty(raw.simpleMap, key)) {
					this.legacySafelist.set(
						key,
						raw.simpleMap[key].toLowerCase(),
					);
				}
			}
		} catch (e) {
	...
	watchTypingLocations(response: WatchTypingLocations) {
		this.findProject(response.projectName)?.watchTypingLocations(
			response.files,
		);
	}
	...
	findProject(projectName: string): Project | undefined {
		...
		return (
			...
			this.findConfiguredProjectByProjectName(
				toNormalizedPath(projectName),
			)
		);
	}
	...
	tryGetDefaultProjectForFile(
		fileNameOrScriptInfo: NormalizedPath | ScriptInfo,
	): Project | undefined {
		const scriptInfo = isString(fileNameOrScriptInfo)
			? this.getScriptInfoForNormalizedPath(fileNameOrScriptInfo)
	...
	ensureDefaultProjectForFile(
		...
		return (
			this.tryGetDefaultProjectForFile(fileNameOrScriptInfo) ||
	...
	private doEnsureDefaultProjectForFile(
		...
		return scriptInfo
			...
			: (this.logErrorForScriptInfoNotFound(
					isString(fileNameOrScriptInfo)
						? fileNameOrScriptInfo
						: fileNameOrScriptInfo.fileName,
				),
			...
			(fileOrDirectory) => {
				...
				if (
					...
					((fsResult && fsResult.fileExists) ||
						(!fsResult && this.host.fileExists(fileOrDirectory)))
				) {
					const file =
						this.getNormalizedAbsolutePath(fileOrDirectory);
					this.logger.info(
						`Config: ${configFileName} Detected new package.json: ${file}`,
					);
					this.packageJsonCache.addOrUpdate(
						file,
						fileOrDirectoryPath,
					);
					this.watchPackageJsonFile(
						file,
						fileOrDirectoryPath,
						result,
					);
				...
				if (
					isIgnoredFileFromWildCardWatching({
						...
						getScriptKind: configuredProjectForConfig
							? (fileName) =>
									configuredProjectForConfig.getScriptKind(
										fileName,
									)
							: undefined,
					})
				...
				config.projects.forEach(
					(watchWildcardDirectories, projectCanonicalPath) => {
						...
						if (
							project.pendingUpdateLevel !== undefined &&
						...
						if (this.openFiles.has(fileOrDirectoryPath)) {
							...
							if (info.isAttached(project)) {
								const loadLevelToSet = Math.max(
									updateLevel,
									project.openFileWatchTriggered.get(
										fileOrDirectoryPath,
									) || ProgramUpdateLevel.Update,
	...
	assignOrphanScriptInfoToInferredProject(
					...
					: getDirectoryPath(
							isRootedDiskPath(info.fileName)
								...
								: getNormalizedAbsolutePath(
										info.fileName,
										projectRootPath
											? this.getNormalizedAbsolutePath(
													projectRootPath,
												)
											: this.currentDirectory,
									),
		...
		if (!this.useSingleInferredProject && !project.projectRootPath) {
			...
			for (const inferredProject of this.inferredProjects) {
				...
				Debug.assert(
					roots.length === 1 || !!inferredProject.projectRootPath,
				);
				if (
					roots.length === 1 &&
					forEach(
						...
						(p) =>
							p !== roots[0].containingProjects[0] &&
							!p.isOrphan(),
					)
				) {
	...
	private closeOpenFile(
		...
		if (
			this.openFilesWithNonRootedDiskPath.get(canonicalFileName) === info
		) {
		...
		for (const p of info.containingProjects) {
			if (isConfiguredProject(p)) {
				...
				if (updateLevel !== undefined) {
					...
					if (
						p.pendingUpdateLevel !== undefined &&
	...
	private deleteScriptInfo(info: ScriptInfo) {
		...
		this.filenameToScriptInfoVersion.set(
			info.path,
			info.textStorage.version,
		);
	...
	private configFileExists(
		...
		if (configFileExistenceInfo) {
			...
			if (
				...
				!configFileExistenceInfo.openFilesImpactedByConfigFile?.has(
					info.path,
				)
			) {
				(configFileExistenceInfo.openFilesImpactedByConfigFile ||=
	...
	private stopWatchingConfigFilesForClosedScriptInfo(info: ScriptInfo) {
		...
		this.forEachConfigFileLocation(info, (canonicalConfigFilePath) => {
			const configFileExistenceInfo =
				this.configFileExistenceInfoCache.get(canonicalConfigFilePath);
			if (configFileExistenceInfo) {
				...
				if (
					...
					this.configFileExistenceInfoCache.delete(
						canonicalConfigFilePath,
					);
				}
			}
		...
		this.forEachConfigFileLocation(
			...
			(canonicalConfigFilePath, configFileName) => {
				let configFileExistenceInfo =
					this.configFileExistenceInfoCache.get(
						canonicalConfigFilePath,
					);
				if (!configFileExistenceInfo) {
					// Create the cache
				...
				configFileExistenceInfo.watcher ||= canWatchDirectoryOrFile(
					getPathComponents(
						getDirectoryPath(canonicalConfigFilePath) as Path,
					),
				)
	...
	private forEachConfigFileLocation(
		...
		const anySearchPathOk =
			!projectRootPath || !isSearchPathInProjectRoot();
		...
		for (const f of fileNames) {
			...
			if (
				...
				const top5LargestFiles = fileNames
					...
					.map((name) => ({
						name,
						size: this.host.getFileSize!(name),
					}))
	...
	sendProjectTelemetry(project: ExternalProject | ConfiguredProject): void {
		...
		function configFileName(): ProjectInfoTelemetryEventData["configFileName"] {
			...
			return (
				getBaseConfigFileName(project.getConfigFilePath()) || "other"
			);
		}
	...
	private loadConfiguredProject(project: ConfiguredProject, reason: string) {
		...
		if (!project.projectOptions) {
			project.projectOptions = {
				configHasExtendsProperty:
					parsedCommandLine.raw.extends !== undefined,
				configHasFilesProperty:
					parsedCommandLine.raw.files !== undefined,
				configHasIncludeProperty:
					parsedCommandLine.raw.include !== undefined,
				configHasExcludeProperty:
					parsedCommandLine.raw.exclude !== undefined,
			};
		}
		...
		const filesToAdd = parsedCommandLine.fileNames.concat(
			project.getExternalFiles(ProgramUpdateLevel.Full),
		);
	...
	ensureParsedConfigUptoDate(
		...
		if (configFileExistenceInfo.config) {
			...
			if (
				configFileExistenceInfo.config.updateLevel ===
				ProgramUpdateLevel.RootNamesAndUpdate
			) {
	...
	reloadFileNamesOfConfiguredProject(project: ConfiguredProject) {
		...
		this.updateNonInferredProjectFiles(
			project,
			fileNames.concat(
				project.getExternalFiles(ProgramUpdateLevel.RootNamesAndUpdate),
			),
			fileNamePropertyReader,
		);
		project.markAsDirty();
	...
	private reloadFileNamesOfParsedConfig(
		...
		if (config.updateLevel === undefined)
	...
	private getOrCreateInferredProjectForProjectRootPathIfEnabled(
		...
		for (const project of this.inferredProjects) {
			...
			if (
				bestMatch &&
				bestMatch.projectRootPath!.length >
					project.projectRootPath.length
			)
				continue;
			bestMatch = project;
		}
	...
	private getOrCreateSingleInferredWithoutProjectRoot(
		...
		for (const inferredProject of this.inferredProjects) {
			if (
				...
				inferredProject.canonicalCurrentDirectory ===
	...
	getSymlinkedProjects(
		...
		if (this.realpathToScriptInfos) {
			...
			if (realpath) {
				forEach(
					this.realpathToScriptInfos.get(realpath),
					combineProjects,
				);
			}
		...
		if (
			...
			if (!this.host.getModifiedTime || indexOfNodeModules === -1) {
				...
				info.fileWatcher = this.watchClosedScriptInfoInNodeModules(
					info.fileName.substring(0, indexOfNodeModules),
				);
			}
		}
	}
	...
	watchPackageJsonsInNodeModules(dir: string, project: Project): FileWatcher {
		...
		const watcher =
			this.nodeModulesWatchers.get(dirPath) ||
			this.createNodeModulesWatcher(dir, dirPath);
		Debug.assert(
			!watcher.affectedModuleSpecifierCacheProjects?.has(project),
		);
	...
	private watchClosedScriptInfoInNodeModules(dir: string): FileWatcher {
		...
		const watcher =
			this.nodeModulesWatchers.get(watchDirPath) ||
			this.createNodeModulesWatcher(watchDir, watchDirPath);
	...
	private getModifiedTime(info: ScriptInfo) {
		return (
			this.host.getModifiedTime!(info.fileName) || missingFileModifiedTime
		).getTime();
	}
	...
	getDocumentPositionMapper(
		...
		if (sourceMapFileInfo) {
			...
			declarationInfo.sourceMapFilePath = {
				...
				sourceInfos: this.addSourceInfoToSourceMap(
					sourceFileName,
					project,
				),
			};
		...
		if (
			...
			if (sourceMapInfo) {
				(
					...
					(sourceMapInfo.sourceInfos = new Set())
				).add(info.path);
			}
		}
	...
	setHostConfiguration(args: protocol.ConfigureRequestArguments) {
		if (args.file) {
			...
			if (info) {
				...
				this.logger.info(
					`Host configuration update for file ${args.file}`,
				);
			}
		} else {
				...
				if (
					...
					this.configuredProjects.forEach((project) => {
						if (
							...
							project.pendingUpdateLevel ===
								ProgramUpdateLevel.Full &&
							!this.pendingProjectUpdates.has(
			...
			if (args.watchOptions) {
				...
				this.logger.info(
					`Host watch options changed to ${JSON.stringify(this.hostConfiguration.watchOptions)}, it will be take effect for next watches.`,
				);
			}
		}
	}
							...
							const referencedProject =
								forEachResolvedProjectReferenceProject(
									...
									(child) => {
										...
										return projectContainsInfoDirectly(
											child,
											info,
										);
									},
	...
	getOriginalLocationEnsuringConfiguredProject(
		...
		if (
			...
			configuredProject = forEachResolvedProjectReferenceProject(
				...
				(child) => {
					...
					return projectContainsOriginalInfo(child)
						? child
						: undefined;
				},
				ProjectReferenceProjectLoadKind.FindCreateLoad,
		...
		if (
			!originalScriptInfo ||
			!originalScriptInfo.containingProjects.length
		)
			return undefined;
		...
		if (!project && this.serverMode === LanguageServiceMode.Semantic) {
			...
			if (configFileName) {
				...
				if (projectForConfigFileDiag) {
					configFileName =
						projectForConfigFileDiag.getConfigFilePath();
					if (
						projectForConfigFileDiag !== project ||
						defaultConfigProjectIsCreated
					) {
						configFileErrors =
							projectForConfigFileDiag.getAllProjectErrors();
	...
	private ensureProjectChildren(
		...
		for (const child of children) {
			...
			const referencedProject = forEachResolvedProjectReference(
				child.references,
				(ref) =>
					forProjects.has(ref.sourceFile.path) ? ref : undefined,
			);
	...
	private removeOrphanConfiguredProjects(
		...
		this.configuredProjects.forEach((project) => {
			...
			if (project.hasOpenRef()) {
				...
				forEachReferencedProject(
					project,
					(ref) =>
						isRetained(ref) && retainConfiguredProject(project),
				);
			}
		});
		...
		function retainConfiguredProject(project: ConfiguredProject) {
			if (
				toRemoveConfiguredProjects.delete(
					project.canonicalConfigFilePath,
				)
			) {
	...
	private removeOrphanScriptInfos() {
		...
		this.filenameToScriptInfo.forEach((info) => {
			...
			if (
				!info.isScriptOpen() &&
				info.isOrphan() &&
				!info.isContainedByBackgroundProject()
			) {
				...
				if (
					!forEachKey(sourceInfos, (path) => {
						const info = this.getScriptInfoForPath(path);
						return (
							!!info && (info.isScriptOpen() || !info.isOrphan())
						);
					})
				) {
					return;
	...
	applyChangesInOpenFiles(
		...
		if (openScriptInfos) {
			retainProjects = flatMap(
				openScriptInfos,
				(info) =>
					this.assignProjectToOpenedScriptInfo(info).retainProjects,
			);
		}
			...
			for (const root of normalizedNames) {
				if (rule.match.test(root)) {
					...
					if (rule.exclude) {
						...
						const escaped =
							ProjectService.escapeFilenameForRegex(root);
	...
	openExternalProject(proj: protocol.ExternalProject): void {
		...
		if (externalProject) {
			...
			if (!tsConfigFiles) {
				...
				const oldConfigFiles =
					this.externalProjectToConfiguredProjectMap.get(
						proj.projectFileName,
					)!;
				let iNew = 0;
				let iOld = 0;
				while (
					iNew < tsConfigFiles.length &&
					iOld < oldConfigFiles.length
				) {
					...
					if (oldConfig < newConfig) {
						...
						(exisingConfigFiles || (exisingConfigFiles = [])).push(
							oldConfig,
						);
						iOld++;
						iNew++;
		...
		if (tsConfigFiles) {
			...
			for (const tsconfigFile of tsConfigFiles) {
				let project =
					this.findConfiguredProjectByProjectName(tsconfigFile);
				if (!project) {
			...
			this.externalProjectToConfiguredProjectMap.delete(
				proj.projectFileName,
			);
	...
	requestEnablePlugin(
		...
		if (
			...
			this.logger.info(
				`Skipped loading plugin ${pluginConfigEntry.name || JSON.stringify(pluginConfigEntry)} because only package name is allowed plugin name`,
			);
			return;
		}
	...
	private endEnablePlugin(
		...
		if (resolvedModule) {
			const configurationOverride =
				this.currentPluginConfigOverrides?.get(pluginConfigEntry.name);
	...
	private async enableRequestedPluginsAsync() {
		...
		this.currentPluginEnablementPromise =
			this.enableRequestedPluginsWorker(entries);
		await this.currentPluginEnablementPromise;
	}

	private async enableRequestedPluginsWorker(
		pendingPlugins: [Project, Promise<BeginEnablePluginResult>[]][],
	) {
		...
		await Promise.all(
			map(pendingPlugins, ([project, promises]) =>
				this.enableRequestedPluginsForProjectAsync(project, promises),
			),
		);
	...
	getPackageJsonsVisibleToFile(
		...
		const processDirectory = (directory: string): boolean | undefined => {
			switch (packageJsonCache.directoryHasPackageJson(directory)) {
				...
				// Check package.json
				case Ternary.True:
					const packageJsonFileName = combinePaths(
						directory,
						"package.json",
					);
					this.watchPackageJsonFile(
						packageJsonFileName,
						this.toPath(packageJsonFileName),
						project,
					);
		...
		return forEachAncestorDirectory(fileName, (directory) => {
			switch (this.packageJsonCache.directoryHasPackageJson(directory)) {
				...
				case Ternary.False:
					return undefined;
				case Ternary.Maybe:
					return this.host.fileExists(
						combinePaths(directory, "package.json"),
					)
						? directory
						: undefined;
			}
	...
	private watchPackageJsonFile(
		file: string,
		path: Path,
		project: Project | WildcardWatcher,
	) {
	...
	private onPackageJsonChange(result: PackageJsonWatcher) {
		result.projects.forEach((project) =>
			(project as Project).onPackageJsonChange?.(),
		);
	}
