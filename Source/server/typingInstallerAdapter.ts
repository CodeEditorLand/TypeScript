...
export abstract class TypingsInstallerAdapter implements ITypingsInstaller {
	...
	private packageInstalledPromise:
		| Map<number, PackageInstallPromise>
		| undefined;
	...
	installPackage(
		...
		const request: InstallPackageRequest = {
			kind: "installPackage",
			...options,
			id: this.packageInstallId,
		};
		const promise = new Promise<ApplyCodeActionCommandResult>(
			(resolve, reject) => {
				(this.packageInstalledPromise ??= new Map()).set(
					this.packageInstallId,
					{ resolve, reject },
				);
			},
	...
	enqueueInstallTypingsRequest(
		...
		if (this.logger.hasLevel(LogLevel.verbose)) {
			this.logger.info(
				`TIAdapter:: Scheduling throttled operation:${stringifyIndented(request)}`,
			);
		}
		...
		if (this.activeRequestCount < this.maxActiveRequestCount) {
			...
			if (this.logger.hasLevel(LogLevel.verbose)) {
				this.logger.info(
					`TIAdapter:: Deferring request for: ${request.projectName}`,
				);
			}
	...
	handleMessage(
		...
		if (this.logger.hasLevel(LogLevel.verbose)) {
			this.logger.info(
				`TIAdapter:: Received response:${stringifyIndented(response)}`,
			);
		}
		...
		switch (response.kind) {
			case EventTypesRegistry:
				this.typesRegistryCache = new Map(
					Object.entries(response.typesRegistry),
				);
				break;
			case ActionPackageInstalled: {
				...
				Debug.assertIsDefined(
					promise,
					"Should find the promise for package install",
				);
			...
			case ActionSet: {
				...
				while (!this.requestQueue.isEmpty()) {
					...
					if (
						this.requestMap.get(queuedRequest.projectName) ===
						queuedRequest
					) {
					...
					if (this.logger.hasLevel(LogLevel.verbose)) {
						this.logger.info(
							`TIAdapter:: Skipping defunct request for: ${queuedRequest.projectName}`,
						);
					}
				}
	...
	scheduleRequest(request: DiscoverTypings) {
		if (this.logger.hasLevel(LogLevel.verbose)) {
			this.logger.info(
				`TIAdapter:: Scheduling request for: ${request.projectName}`,
			);
		}
		this.activeRequestCount++;
		this.host.setTimeout(
			() => {
				if (this.logger.hasLevel(LogLevel.verbose)) {
					this.logger.info(
						`TIAdapter:: Sending request:${stringifyIndented(request)}`,
					);
				}
				this.installer.send(request);
			},
