...
export function createPackageJsonCache(host: ProjectService): PackageJsonCache {
	...
	return {
		...
		getInDirectory: (directory) => {
			return (
				packageJsons.get(
					host.toPath(combinePaths(directory, "package.json")),
				) || undefined
			);
		},
		directoryHasPackageJson: (directory) =>
			directoryHasPackageJson(host.toPath(directory)),
		searchDirectoryAndAncestors: (directory) => {
			forEachAncestorDirectory(directory, (ancestor) => {
				...
				if (tryFileExists(host, packageJsonFileName)) {
					addOrUpdate(
						packageJsonFileName,
						combinePaths(ancestorPath, "package.json") as Path,
					);
				} else {
	...
	function directoryHasPackageJson(directory: Path) {
		return packageJsons.has(combinePaths(directory, "package.json") as Path)
			? Ternary.True
