...
export function countEachFileTypes(
	...
	for (const info of infos) {
		const fileSize = includeSizes
			? info.textStorage.getTelemetryFileSize()
			: 0;
	...
	implements LanguageServiceHost, ModuleResolutionHost
{
	...
	public static resolveModule(
		...
		return Project.importServicePluginSync(
			{ name: moduleName },
			[initialDir],
			host,
			log,
	...
	public static importServicePluginSync<T = {}>(
		...
		for (const initialDir of searchPaths) {
			...
			log(
				`Loading ${pluginConfigEntry.name} from ${initialDir} (resolved to ${resolvedPath})`,
			);
			...
			(errorLogs ??= []).push(
				`Failed to load module '${pluginConfigEntry.name}' from ${resolvedPath}: ${err}`,
			);
		}
	...
	public static async importServicePluginAsync<T = {}>(
		...
		for (const initialDir of searchPaths) {
			...
			log(
				`Dynamically importing ${pluginConfigEntry.name} from ${initialDir} (resolved to ${resolvedPath})`,
			);
			let result: ModuleImportResult;
			try {
				result = await host.importPlugin(
					resolvedPath,
	...
	constructor(
		...
		this.currentDirectory =
			this.projectService.getNormalizedAbsolutePath(currentDirectory);
	...
	getScriptKind(fileName: string) {
		const info = this.projectService.getScriptInfoForPath(
	...
	getScriptSnapshot(filename: string): IScriptSnapshot | undefined {
		const scriptInfo =
			this.getOrCreateScriptInfoAndAttachToProject(filename);
		if (scriptInfo) {
	...
	resolveLibrary(
		...
		return this.resolutionCache.resolveLibrary(
			libraryName,
			resolveFrom,
			options,
			libFileName,
	...
	emitFile(
		...
		if (!emitSkipped) {
			...
			if (
				this.builderState &&
				getEmitDeclarations(this.compilerOptions)
			) {
				const dtsFiles = outputFiles.filter((f) =>
					isDeclarationFileName(f.name),
				);
		...
		return sort(
			flatMap(this.plugins, (plugin) => {
				if (typeof plugin.module.getExternalFiles !== "function")
					return;
				try {
					return plugin.module.getExternalFiles(
						this,
	...
	getFileNames(
		...
		if (!this.languageServiceEnabled) {
			...
			if (this.compilerOptions) {
				const defaultLibrary = getDefaultLibFilePath(
	...
	watchTypingLocations(files: readonly string[] | undefined) {
		...
		const createProjectWatcher = (
			path: string,
			typingsWatcherType: TypingWatcherType,
		) => {
						...
						? this.projectService.watchFactory.watchFile(
								...
								() =>
									!this.typingWatchers!.isInvoked
										...
										: this.writeLog(
												`TypingWatchers already invoked`,
											),
								PollingInterval.High,
								this.projectService.getWatchOptions(this),
						...
						: this.projectService.watchFactory.watchDirectory(
								...
								(f) => {
									...
									if (!fileExtensionIs(f, Extension.Json))
										return this.writeLog(
											`Ignoring files that are not *.json`,
										);
									if (
										comparePaths(
											f,
										...
										return this.writeLog(
											`Ignoring package.json change at global typings location`,
										);
									this.onTypingInstallerWatchInvoke();
								},
		...
		for (const file of files) {
			...
			if (
				containsPath(
					this.projectService.typingsInstaller
						.globalTypingsCacheLocation!,
					file,
					this.currentDirectory,
					!this.useCaseSensitiveFileNames(),
	...
	private updateGraphWorker() {
		...
		const { hasInvalidatedResolutions, hasInvalidatedLibResolutions } =
			this.resolutionCache.createHasInvalidatedResolutions(
				returnFalse,
				returnFalse,
			);
		...
		if (
			...
			if (oldProgram) {
				for (const f of oldProgram.getSourceFiles()) {
					const newFile = this.program.getSourceFileByPath(
						f.resolvedPath,
					);
					if (
						!newFile ||
						(f.resolvedPath === f.path &&
							newFile.resolvedPath !== f.path)
					) {
			...
			updateMissingFilePathsWatch(
				...
				(missingFilePath, missingFileName) =>
					this.addMissingFileWatcher(
						missingFilePath,
						missingFileName,
					),
			);
				...
				if (isGeneratedFileWatcher(this.generatedFilesMap)) {
					...
					if (outPath) {
						...
						this.generatedFilesMap.forEach((watcher, source) => {
							const sourceFile =
								this.program!.getSourceFileByPath(source);
							if (
								!sourceFile ||
		...
		if (this.exportMapCache && !this.exportMapCache.isEmpty()) {
			...
			if (
				...
				forEachKey(this.changedFilesForExportMapCache, (fileName) => {
					const oldSourceFile =
						oldProgram.getSourceFileByPath(fileName);
					const sourceFile =
						this.program!.getSourceFileByPath(fileName);
		...
		this.writeLog(
			`Finishing updateGraphWorker: Project: ${this.getProjectName()} Version: ${this.getProjectVersion()} structureChanged: ${hasNewProgram}${this.program ? ` structureIsReused:: ${(ts as any).StructureIsReused[this.program.structureIsReused]}` : ""} Elapsed: ${elapsed}ms`,
		);
	...
	private detachScriptInfoFromProject(
		...
		if (scriptInfoToDetach) {
			...
			if (!noRemoveResolution) {
				this.resolutionCache.removeResolutionsOfFile(
					scriptInfoToDetach.path,
	...
	setCompilerOptions(compilerOptions: CompilerOptions) {
		if (compilerOptions) {
			...
			this.setInternalCompilerOptionsForEmittingJsFiles();
			this.noDtsResolutionProject?.setCompilerOptions(
	...
	getChangesSinceVersion(
		...
		if (
			...
			return {
				info,
				changes: {
					added: includeProjectReferenceRedirectInfoIfRequested(
						added,
					),
					removed:
						includeProjectReferenceRedirectInfoIfRequested(removed),
					updated: includeProjectReferenceRedirectInfo
						? updated.map(
								(
									...
									fileName,
									isSourceOfProjectReferenceRedirect:
										this.isSourceOfProjectReferenceRedirect(
											fileName,
										),
								}),
							)
	...
	protected getGlobalPluginSearchPaths() {
		...
		return [
			...
			combinePaths(
				this.projectService.getExecutingFilePath(),
				"../../..",
			),
		];
	}
	...
	protected enablePlugin(
		...
		this.projectService.requestEnablePlugin(
			this,
			pluginConfigEntry,
			searchPaths,
		);
		...
		if (this.program) {
			return {
				...
				realpath:
					...
					this.projectService.host.realpath?.bind(
						this.projectService.host,
					),
				...
				trace: this.projectService.host.trace?.bind(
					this.projectService.host,
				),
				useCaseSensitiveFileNames:
					this.program.useCaseSensitiveFileNames(),
				readDirectory: this.projectService.host.readDirectory.bind(
					this.projectService.host,
				),
			};
		}
...
export class InferredProject extends Project {
	...
	constructor(
		...
		this.projectRootPath =
			projectRootPath &&
			projectService.toCanonicalFileName(projectRootPath);
	...
	override addRoot(info: ScriptInfo) {
		...
		this.projectService.startWatchingConfigFilesForInferredProjectRoot(
			info,
		);
	...
	override close() {
		forEach(this.getRootScriptInfos(), (info) =>
			this.projectService.stopWatchingConfigFilesForInferredProjectRoot(
				info,
			),
		);
		super.close();
	...
	static getRootFileNames(
					...
					(directory) => {
						if (directory) {
							...
							if (typesPackageJson) {
								...
								rootNames = concatenate(rootNames, entrypoints);
								dependenciesAdded += entrypoints?.length
									? 1
									: 0;
								return true;
							}
		...
		function addDependency(dependency: string) {
			if (!startsWith(dependency, "@types/")) {
				(dependencyNames || (dependencyNames = new Set())).add(
					dependency,
				);
			}
		}
		...
		function getRootNamesFromPackageJson(
			...
			if (entrypoints) {
				...
				const isSymlink =
					realPath &&
					realPath !==
						hostProject.toPath(packageJson.packageDirectory);
				if (isSymlink) {
					symlinkCache.setSymlinkedDirectory(
						packageJson.packageDirectory,
						{
							real: ensureTrailingDirectorySeparator(real!),
							realPath:
								ensureTrailingDirectorySeparator(realPath),
						},
					);
				}
...
export class ConfiguredProject extends Project {
	...
	private releaseParsedConfig(canonicalConfigFilePath: NormalizedPath) {
		this.projectService.stopWatchingWildCards(
			canonicalConfigFilePath,
			this,
		);
	...
	override updateGraph(): boolean {
		...
		switch (updateLevel) {
			case ProgramUpdateLevel.RootNamesAndUpdate:
				...
				result =
					this.projectService.reloadFileNamesOfConfiguredProject(
						this,
					);
				break;
	...
	/** @internal */
	override getResolvedProjectReferenceToRedirect(
		fileName: string,
	): ResolvedProjectReference | undefined {
		const program = this.getCurrentProgram();
		return (
			program && program.getResolvedProjectReferenceToRedirect(fileName)
		);
	}
	...
	enablePluginsWithOptions(options: CompilerOptions): void {
		...
		if (
			...
			!this.projectService.globalPlugins.length
		)
			return;
		const host = this.projectService.host;
		if (!host.require && !host.importPlugin) {
		...
		return (
			(!!configFileExistenceInfo.openFilesImpactedByConfigFile &&
				forEachEntry(
					...
					(_value, infoPath) => {
						const info =
							this.projectService.getScriptInfoForPath(infoPath)!;
						return (
...
/**@internal */
export function isBackgroundProject(
	project: Project,
): project is AutoImportProviderProject | AuxiliaryProject {
	return (
		project.projectKind === ProjectKind.AutoImportProvider ||
