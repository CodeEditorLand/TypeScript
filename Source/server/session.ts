...
function combineProjectOutput<T, U>(
	...
	if (!isArray(projects) && projects.symLinkedProjects) {
		projects.symLinkedProjects.forEach((projects, path) => {
			...
			outputs.push(
				...flatMap(projects, (project) => action(project, value)),
			);
		});
	}
...
function getRenameLocationsWorker(
	...
	perProjectResults.forEach((projectResults, project) => {
		for (const result of projectResults) {
			...
			if (
				!seen.has(result) &&
				!getMappedLocationForProject(
					documentSpanLocation(result),
					project,
				)
			) {
				results.push(result);
...
function getReferencesWorker(
	...
	const perProjectResults = getPerProjectReferences(
		...
		(project, position) => {
			...
			return project
				.getLanguageService()
				.findReferences(position.fileName, position.pos);
		},
		(referencedSymbol, cb) => {
			...
			const definition: ReferencedSymbolDefinitionInfo =
				mappedDefinitionFile === undefined
					...
					: {
							...
							contextSpan: getMappedContextSpanForProject(
								referencedSymbol.definition,
								project,
							),
						};

			let symbolToAddTo = find(results, (o) =>
				documentSpansEqual(
					o.definition,
					definition,
					useCaseSensitiveFileNames,
...
function forEachProjectInProjects(
	...
	if (!isArray(projects) && projects.symLinkedProjects) {
		projects.symLinkedProjects.forEach(
			(symlinkedProjects, symlinkedPath) => {
...
function getPerProjectReferences<TResult>(
	...
	forPositionInResult: (
		result: TResult,
		cb: (location: DocumentPosition) => void,
	) => void,
		...
		for (const result of projectResults) {
			forPositionInResult(result, (position) => {
				...
				for (const project of originalScriptInfo.containingProjects) {
					if (!project.isOrphan() && !resultsMap.has(project)) {
				...
				if (symlinkedProjectsMap) {
					symlinkedProjectsMap.forEach(
						(symlinkedProjects, symlinkedPath) => {
							for (const symlinkedProject of symlinkedProjects) {
								if (
									!symlinkedProject.isOrphan() &&
									!resultsMap.has(symlinkedProject)
								) {
									// Optimization: don't enqueue if will be discarded
									queue.enqueue({
...
function getMappedLocationForProject(
	location: DocumentPosition,
	project: Project,
): DocumentPosition | undefined {
	return getMappedLocation(location, project.getSourceMapper(), (p) =>
		project.projectService.fileExists(p as NormalizedPath),
	);
}

function getMappedDocumentSpanForProject(
	documentSpan: DocumentSpan,
	project: Project,
): DocumentSpan | undefined {
	return getMappedDocumentSpan(documentSpan, project.getSourceMapper(), (p) =>
		project.projectService.fileExists(p as NormalizedPath),
	);
}

function getMappedContextSpanForProject(
	documentSpan: DocumentSpan,
	project: Project,
): TextSpan | undefined {
	return getMappedContextSpan(documentSpan, project.getSourceMapper(), (p) =>
		project.projectService.fileExists(p as NormalizedPath),
	);
}
...
export class Session<TMessage = string> implements EventSender {
				...
				this.event<protocol.ConfigFileDiagnosticEventBody>(
					{
						...
						diagnostics: map(event.data.diagnostics, (diagnostic) =>
	...
	private projectsUpdatedInBackgroundEvent(openFiles: string[]): void {
		this.projectService.logger.info(
			`got projects updated in background ${openFiles}`,
		);
		if (openFiles.length) {
			if (
				...
				this.projectService.logger.info(
					`Queueing diagnostics update for ${openFiles}`,
				);
	...
	private logErrorWorker(
		...
		if (this.logger.hasLevel(LogLevel.verbose)) {
			if (fileRequest) {
				try {
					const { file, project } =
						this.getFileAndProject(fileRequest);
					const scriptInfo =
						project.getScriptInfoForNormalizedPath(file);
					if (scriptInfo) {
	...
	public send(msg: protocol.Message) {
		if (msg.type === "event" && !this.canUseEvents) {
			if (this.logger.hasLevel(LogLevel.verbose)) {
				this.logger.info(
					`Session does not support events: ignored event: ${stringifyIndented(msg)}`,
				);
			}
			return;
		}
	...
	private semanticCheck(file: NormalizedPath, project: Project) {
		tracing?.push(tracing.Phase.Session, "semanticCheck", {
			file,
			configFilePath: (project as ConfiguredProject)
				.canonicalConfigFilePath,
		}); // undefined is fine if the cast fails
		const diags = isDeclarationFileInJSOnlyNonConfiguredProject(
			project,
			file,
		)
			? emptyArray
	...
	private updateErrorCheck(
		...
		const checkOne = () => {
			...
			if (
				project.projectService.serverMode !==
	...
	private getConfigFileAndProject(args: protocol.FileRequestArgs): {
		...
		return {
			configFile:
				project && project.hasConfigFile(file) ? file : undefined,
			project,
		};
	}
	...
	private convertToDiagnosticsWithLinePositionFromDiagnosticFile(
		...
		return diagnostics.map<protocol.DiagnosticWithLinePosition>((d) => ({
			...
			startLocation: (d.file &&
				convertToLocation(
					getLineAndCharacterOfPosition(d.file, d.start!),
				))!, // TODO: GH#18217
			endLocation: (d.file &&
				convertToLocation(
	...
	private getDefinition(
		...
		const definitions = this.mapDefinitionInfoLocations(
			project
				.getLanguageService()
				.getDefinitionAtPosition(file, position) || emptyArray,
			project,
		);
	...
	private mapDefinitionInfoLocations(
		...
		return definitions.map((info): DefinitionInfo => {
			const newDocumentSpan = getMappedDocumentSpanForProject(
				info,
				project,
			);
	...
	private findSourceDefinition(
		...
		const { file, project } = this.getFileAndProject(args);
		const position = this.getPositionInFile(args, file);
		const unmappedDefinitions = project
			.getLanguageService()
			.getDefinitionAtPosition(file, position);
		let definitions: readonly DefinitionInfo[] =
			this.mapDefinitionInfoLocations(
				unmappedDefinitions || emptyArray,
				project,
			).slice();
		const needsJsResolution =
			this.projectService.serverMode === LanguageServiceMode.Semantic &&
			(!some(
				definitions,
				(d) => toNormalizedPath(d.fileName) !== file && !d.isAmbient,
			) ||
		...
		if (needsJsResolution) {
			...
			if (some(jsDefinitions)) {
				...
				const ambientCandidates = definitions.filter(
					(d) => toNormalizedPath(d.fileName) !== file && d.isAmbient,
				);
				for (const candidate of some(ambientCandidates)
					? ambientCandidates
					: getAmbientCandidatesByClimbingAccessChain()) {
					const fileNameToSearch =
						findImplementationFileFromDtsFileName(
							candidate.fileName,
							file,
							noDtsProject,
						);
					...
					const fileToSearch = Debug.checkDefined(
						noDtsProgram.getSourceFile(fileNameToSearch),
					);
					for (const match of searchForDeclaration(
						candidate.name,
						fileToSearch,
						noDtsProgram,
					)) {
		...
		definitions = definitions.filter(
			(d) => !d.isAmbient && !d.failedAliasResolution,
		);
		...
		function findImplementationFileFromDtsFileName(
			...
			if (
				...
				fileName.lastIndexOf(nodeModulesPathPart) ===
					nodeModulesPathParts.topLevelNodeModulesIndex
			) {
				// Second check ensures the fileName only contains one `/node_modules/`. If there's more than one I give up.
				const packageDirectory = fileName.substring(
					0,
					nodeModulesPathParts.packageRootIndex,
				);
				const packageJsonCache = project
				...
				const packageName = getPackageNameFromTypesPackageName(
					unmangleScopedPackageName(packageNamePathPart),
				);
				const path = project.toPath(fileName);
				if (
					entrypoints &&
					some(entrypoints, (e) => project.toPath(e) === path)
				) {
					...
					const pathToFileInPackage = fileName.substring(
						nodeModulesPathParts.packageRootIndex + 1,
					);
		...
		function getAmbientCandidatesByClimbingAccessChain(): readonly {
			name: string;
			fileName: string;
		}[] {
			const ls = project.getLanguageService();
			const program = ls.getProgram()!;
			const initialNode = getTouchingPropertyName(
				program.getSourceFile(file)!,
				position,
			);
			if (
				(isStringLiteralLike(initialNode) ||
					isIdentifier(initialNode)) &&
					...
					forEachNameInAccessChainWalkingLeft(
						...
						(nameInChain) => {
							...
							const candidates = ls
								...
								?.filter(
									(d) =>
										toNormalizedPath(d.fileName) !== file &&
										d.isAmbient,
								)
								.map((d) => ({
									fileName: d.fileName,
									name: getTextOfIdentifierOrLiteral(
										initialNode,
									),
								}));
							if (some(candidates)) {
		...
		function tryRefineDefinition(
			definition: DefinitionInfo,
			program: Program,
			noDtsProgram: Program,
		) {
			const fileToSearch = noDtsProgram.getSourceFile(
				definition.fileName,
			);
			if (!fileToSearch) {
				return undefined;
			...
			const symbol = program
				.getTypeChecker()
				.getSymbolAtLocation(initialNode);
			const importSpecifier =
				symbol &&
				getDeclarationOfKind<ImportSpecifier>(
					symbol,
		...
		function searchForDeclaration(
			declarationName: string,
			fileToSearch: SourceFile,
			noDtsProgram: Program,
		) {
			const matches =
				FindAllReferences.Core.getTopMostDeclarationNamesInFile(
					declarationName,
					fileToSearch,
				);
			return mapDefined(matches, (match) => {
				const symbol = noDtsProgram
					.getTypeChecker()
					.getSymbolAtLocation(match);
	...
	private getLinkedEditingRange(
		...
		const { file, languageService } =
			this.getFileAndLanguageServiceForSyntacticOperation(args);
		const position = this.getPositionInFile(args, file);
		const linkedEditInfo = languageService.getLinkedEditingRangeAtPosition(
			file,
			position,
		);
		const scriptInfo =
				...
				displayParts: displayParts?.map(({ text, span, file }) => {
					if (span) {
						Debug.assertIsDefined(
							file,
							"Target file should be defined together with its span.",
						);
						const scriptInfo =
						...
						return {
							text,
							span: {
								start: scriptInfo.positionToLineOffset(
									span.start,
								),
	...
	private getRenameInfo(args: protocol.FileLocationRequestArgs): RenameInfo {
		...
		return project
			.getLanguageService()
			.getRenameInfo(file, position, preferences);
	}
		...
		const refs: readonly protocol.ReferencesResponseItem[] = flatMap(
			...
			(referencedSymbol) => {
				return referencedSymbol.references.map((entry) =>
					referenceEntryToReferencesResponseItem(
						this.projectService,
						entry,
						preferences,
					),
	...
	private getNameOrDottedNameSpan(args: protocol.FileLocationRequestArgs) {
		const { file, languageService } =
			this.getFileAndLanguageServiceForSyntacticOperation(args);
		const position = this.getPositionInFile(args, file);
		return languageService.getNameOrDottedNameSpan(
			file,
			position,
			position,
		);
		...
		if (simplifiedResult) {
			...
			return {
				...
				start: scriptInfo.positionToLineOffset(
					...
					textSpanEnd(quickInfo.textSpan),
				),
				displayString,
	...
	private getFormattingEditsAfterKeystroke(
		...
		const scriptInfo =
			this.projectService.getScriptInfoForNormalizedPath(file)!;
		const position = scriptInfo.lineOffsetToPosition(
			args.line,
			args.offset,
		);
		...
		if (
			...
			const { lineText, absolutePosition } =
				scriptInfo.textStorage.getAbsolutePositionAndLineText(
					args.line,
				);
			if (lineText && lineText.search("\\S") < 0) {
				const preferredIndent =
		...
		const completions = project
			...
			.getCompletionsAtPosition(
				...
				position,
				{
					...
					triggerKind: args.triggerKind as
						| CompletionTriggerKind
						| undefined,
					includeExternalModuleExports:
						args.includeExternalModuleExports,
	...
	private getCompletionEntryDetails(
		...
		return fullResult
			...
			: result.map((details) => ({
					...
					documentation: this.mapDisplayParts(
		...
		return combineProjectOutput(
			...
			(project, info) => {
				...
				if (
					...
					(isDeclarationFileName(info.fileName) &&
	...
	private toPendingErrorCheck(
		...
		const project =
			this.projectService.tryGetDefaultProjectForFile(fileName);
	...
	private reload(args: protocol.ReloadRequestArgs, reqSeq: number) {
		const file = toNormalizedPath(args.file);
		const tempFileName =
			args.tmpfile === undefined
				? undefined
				: toNormalizedPath(args.tmpfile);
	...
	private mapLocationNavigationBarItems(
		...
		return map(items, (item) => ({
			...
			spans: item.spans.map((span) =>
				toProtocolTextSpan(span, scriptInfo),
			),
		...
		return !items
			...
			: simplifiedResult
				? this.mapLocationNavigationBarItems(
						items,
						this.projectService.getScriptInfoForNormalizedPath(
							file,
						)!,
					)
				: items;
		...
		return !simplifiedResult
			...
			: flatMap(full, ({ project, navigateToItems }) =>
					navigateToItems.map((navItem) => {
						const scriptInfo = project.getScriptInfo(
							navItem.fileName,
						)!;
						...
						if (
							navItem.kindModifiers &&
							navItem.kindModifiers !== ""
						) {
							bakedItem.kindModifiers = navItem.kindModifiers;
						}
						if (
							navItem.containerName &&
							navItem.containerName.length > 0
						) {
							bakedItem.containerName = navItem.containerName;
						}
						if (
							navItem.containerKind &&
							navItem.containerKind.length > 0
						) {
	...
	private getSupportedCodeFixes(
		args: Partial<protocol.FileRequestArgs> | undefined,
	): readonly string[] {
	...
	private isLocation(
		...
		return (
			(locationOrSpan as protocol.FileLocationRequestArgs).line !==
			undefined
		);
	}
	...
	private extractPositionOrRange(
		...
		return Debug.checkDefined(
			position === undefined ? textRange : position,
		);
		...
		const changes = project.getLanguageService().organizeImports(
			{
				fileName: file,
				mode:
					(args.mode as OrganizeImportsMode | undefined) ??
	...
	private applyCodeActionCommand(
		...
		const commands = args.command as
			| CodeActionCommand
			| CodeActionCommand[]; // They should be sending back the command we sent them.
	...
	private mapTextChangesToCodeEdits(
		...
		return textChanges.map((change) =>
			this.mapTextChangeToCodeEdit(change),
		);
	}
	...
	private getBraceMatching(
		...
		const spans = languageService.getBraceMatchingAtPosition(
			file,
			position,
		);
		return !spans
	...
	private getDiagnosticsForProject(
		...
		const { fileNames, languageServiceDisabled } =
			this.getProjectInfoWorker(
				fileName,
		...
		for (const fileNameInProject of fileNamesInProject) {
			if (
				...
				const info =
					this.projectService.getScriptInfo(fileNameInProject)!; // TODO: GH#18217
		...
		const checkList = sortedFiles.map((fileName) => ({
			fileName,
			project,
		}));
	...
	private prepareCallHierarchy(
		...
		if (scriptInfo) {
			...
			return (
				result &&
				mapOneOrMany(result, (item) =>
					this.toProtocolCallHierarchyItem(item),
				)
			);
		}
		return undefined;
	...
	private handlers = new Map(
		Object.entries<(request: any) => HandlerResponse>({
			...
			[protocol.CommandTypes.SynchronizeProjectList]: (
				...
				if (
					!result.some(
						(p) => p.projectErrors && p.projectErrors.length !== 0,
					)
				) {
				...
				const converted = map(result, (p) => {
					...
					return {
						...
						files: p.files,
						projectErrors:
							this.convertToDiagnosticsWithLinePosition(
								p.projectErrors,
								/*scriptInfo*/ undefined,
							),
					};
			...
			[protocol.CommandTypes.UpdateOpen]: (
				request: protocol.UpdateOpenRequest,
			) => {
				this.changeSeq++;
				this.projectService.applyChangesInOpenFiles(
					request.arguments.openFiles &&
						mapIterator(request.arguments.openFiles, (file) => ({
							fileName: file.file,
					...
					request.arguments.changedFiles &&
						mapIterator(request.arguments.changedFiles, (file) => ({
							fileName: file.fileName,
							changes: mapDefinedIterator(
								arrayReverseIterator(file.textChanges),
								(change) => {
									const scriptInfo = Debug.checkDefined(
										this.projectService.getScriptInfo(
											file.fileName,
										),
									);
									const start =
			...
			[protocol.CommandTypes.ApplyChangedToOpenFiles]: (
			...
			[protocol.CommandTypes.DefinitionAndBoundSpan]: (
				request: protocol.DefinitionAndBoundSpanRequest,
			) => {
				return this.requiredResponse(
			...
			[protocol.CommandTypes.FindSourceDefinition]: (
				request: protocol.FindSourceDefinitionRequest,
			) => {
				return this.requiredResponse(
					this.findSourceDefinition(request.arguments),
				);
			},
			[protocol.CommandTypes.EmitOutput]: (
				request: protocol.EmitOutputRequest,
			) => {
				return this.requiredResponse(
					this.getEmitOutput(request.arguments),
				);
			},
			[protocol.CommandTypes.TypeDefinition]: (
			...
			[protocol.CommandTypes.RenameInfoFull]: (
				...
				return this.requiredResponse(
					this.getRenameInfo(request.arguments),
				);
			},
			...
			[protocol.CommandTypes.GetOutliningSpans]: (
			...
			[protocol.CommandTypes.GetOutliningSpansFull]: (
				request: protocol.FileRequest,
			) => {
				return this.requiredResponse(
					this.getOutliningSpans(
			...
			[protocol.CommandTypes.TodoComments]: (
				request: protocol.TodoCommentRequest,
			) => {
				return this.requiredResponse(
					this.getTodoComments(request.arguments),
				);
			},
			[protocol.CommandTypes.Indentation]: (
				request: protocol.IndentationRequest,
			) => {
				return this.requiredResponse(
					this.getIndentation(request.arguments),
				);
			},
			[protocol.CommandTypes.NameOrDottedNameSpan]: (
				request: protocol.FileLocationRequest,
			) => {
				return this.requiredResponse(
			...
			[protocol.CommandTypes.BreakpointStatement]: (
				request: protocol.FileLocationRequest,
			) => {
				return this.requiredResponse(
			...
			[protocol.CommandTypes.BraceCompletion]: (
			...
			[protocol.CommandTypes.DocCommentTemplate]: (
			...
			[protocol.CommandTypes.GetSpanOfEnclosingComment]: (
			...
			[protocol.CommandTypes.FileReferencesFull]: (
				request: protocol.FileReferencesRequest,
			) => {
				return this.requiredResponse(
					this.getFileReferences(
			...
			[protocol.CommandTypes.Format]: (
				request: protocol.FormatRequest,
			) => {
				return this.requiredResponse(
					this.getFormattingEditsForRange(request.arguments),
				);
			},
			[protocol.CommandTypes.Formatonkey]: (
			...
			[protocol.CommandTypes.FormatFull]: (
				request: protocol.FormatRequest,
			) => {
				return this.requiredResponse(
			...
			[protocol.CommandTypes.FormatonkeyFull]: (
				request: protocol.FormatOnKeyRequest,
			) => {
				return this.requiredResponse(
					this.getFormattingEditsAfterKeystrokeFull(
						request.arguments,
					),
				);
			},
			[protocol.CommandTypes.FormatRangeFull]: (
				request: protocol.FormatRequest,
			) => {
				return this.requiredResponse(
			...
			[protocol.CommandTypes.SyntacticDiagnosticsSync]: (
			...
			[protocol.CommandTypes.Geterr]: (
				request: protocol.GeterrRequest,
			) => {
				this.errorCheck.startNew((next) =>
					this.getDiagnostics(
						next,
			...
			[protocol.CommandTypes.Change]: (
				request: protocol.ChangeRequest,
			) => {
				this.change(request.arguments);
				return this.notRequired();
			},
			[protocol.CommandTypes.Configure]: (
			...
			[protocol.CommandTypes.Reload]: (
				request: protocol.ReloadRequest,
			) => {
			...
			[protocol.CommandTypes.Saveto]: (request: protocol.Request) => {
				const savetoArgs =
					request.arguments as protocol.SavetoRequestArgs;
			...
			[protocol.CommandTypes.Brace]: (
				request: protocol.FileLocationRequest,
			) => {
				return this.requiredResponse(
					this.getBraceMatching(
						request.arguments,
						/*simplifiedResult*/ true,
					),
				);
			},
			[protocol.CommandTypes.BraceFull]: (
				request: protocol.FileLocationRequest,
			) => {
				return this.requiredResponse(
					this.getBraceMatching(
			...
			[protocol.CommandTypes.NavBarFull]: (
				request: protocol.FileRequest,
			) => {
				return this.requiredResponse(
					this.getNavigationBarItems(
			...
			[protocol.CommandTypes.NavTreeFull]: (
				request: protocol.FileRequest,
			) => {
				return this.requiredResponse(
					this.getNavigationTree(
			...
			[protocol.CommandTypes.ProjectInfo]: (
				request: protocol.ProjectInfoRequest,
			) => {
				return this.requiredResponse(
					this.getProjectInfo(request.arguments),
				);
			},
			[protocol.CommandTypes.ReloadProjects]: () => {
			...
			[protocol.CommandTypes.JsxClosingTag]: (
			...
			[protocol.CommandTypes.LinkedEditingRange]: (
				request: protocol.LinkedEditingRangeRequest,
			) => {
				return this.requiredResponse(
					this.getLinkedEditingRange(request.arguments),
				);
			},
			[protocol.CommandTypes.GetCodeFixes]: (
				request: protocol.CodeFixRequest,
			) => {
				return this.requiredResponse(
					this.getCodeFixes(
						request.arguments,
			...
			[protocol.CommandTypes.GetSupportedCodeFixes]: (
				request: protocol.GetSupportedCodeFixesRequest,
			) => {
				return this.requiredResponse(
					this.getSupportedCodeFixes(request.arguments),
				);
			},
			[protocol.CommandTypes.GetApplicableRefactors]: (
			...
			[protocol.CommandTypes.GetMoveToRefactoringFileSuggestions]: (
				...
				return this.requiredResponse(
					this.getMoveToRefactoringFileSuggestions(request.arguments),
				);
			},
			[protocol.CommandTypes.GetEditsForRefactorFull]: (
				request: protocol.GetEditsForRefactorRequest,
			...
			[protocol.CommandTypes.ConfigurePlugin]: (
			...
			[protocol.CommandTypes.SelectionRange]: (
			...
			[protocol.CommandTypes.SelectionRangeFull]: (
				request: protocol.SelectionRangeRequest,
			) => {
				return this.requiredResponse(
					this.getSmartSelectionRange(
			...
			[protocol.CommandTypes.ToggleLineComment]: (
			...
			[protocol.CommandTypes.ToggleLineCommentFull]: (
				request: protocol.ToggleLineCommentRequest,
			) => {
				return this.requiredResponse(
					this.toggleLineComment(
			...
			[protocol.CommandTypes.ToggleMultilineComment]: (
			...
			[protocol.CommandTypes.ToggleMultilineCommentFull]: (
				request: protocol.ToggleMultilineCommentRequest,
			) => {
				return this.requiredResponse(
					this.toggleMultilineComment(
			...
			[protocol.CommandTypes.CommentSelection]: (
			...
			[protocol.CommandTypes.CommentSelectionFull]: (
				request: protocol.CommentSelectionRequest,
			) => {
				return this.requiredResponse(
					this.commentSelection(
			...
			[protocol.CommandTypes.UncommentSelection]: (
			...
			[protocol.CommandTypes.UncommentSelectionFull]: (
				request: protocol.UncommentSelectionRequest,
			) => {
				return this.requiredResponse(
					this.uncommentSelection(
			...
			[protocol.CommandTypes.ProvideInlayHints]: (
	...
	public executeCommand(request: protocol.Request): HandlerResponse {
		...
		if (handler) {
			const response = this.executeWithRequestId(request.seq, () =>
				handler(request),
			);
	...
	public onMessage(message: TMessage) {
		...
		if (this.logger.hasLevel(LogLevel.requestTime)) {
			...
			if (this.logger.hasLevel(LogLevel.verbose)) {
				this.logger.info(
					`request:${indent(this.toStringMessage(message))}`,
				);
			}
		}
		...
		try {
			...
			relevantFile =
				request.arguments &&
				(request as protocol.FileRequest).arguments.file
			...
			if (this.logger.hasLevel(LogLevel.requestTime)) {
				const elapsedTime = hrTimeToMilliseconds(
					this.hrtime(start),
				).toFixed(4);
				if (responseRequired) {
					this.logger.perftrc(
...
function referenceEntryToReferencesResponseItem(
	...
	const lineText = disableLineTextInReferences
		? undefined
		: getLineText(scriptInfo, span);
	return {
		file: fileName,
		...span,
		lineText,
...
function getLineText(
	scriptInfo: ScriptInfo,
	span: protocol.TextSpanWithContext,
) {
