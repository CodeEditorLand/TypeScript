...
function getOptionsForHelp(commandLine: ParsedCommandLine) {
	...
	return !!commandLine.options.all
		...
		: filter(
				optionDeclarations.slice(),
				(v) => !!v.showInSimplifiedHelpView,
			);
}
	...
	const defaultValueDescription =
		typeof option.defaultValueDescription === "object"
			...
			: formatDefaultValue(
					...
					option.type === "list" || option.type === "listOrElement"
						? option.element.type
						: option.type,
				);
...
function getHeader(sys: System, message: string) {
	...
	const tsIconSecondLine = colors.blueBackground(
		colors.brightWhite("TS ".padStart(tsIconLength)),
	);
...
export function isBuild(commandLineArgs: readonly string[]) {
	if (
		...
		const firstOption = commandLineArgs[0]
			.slice(
				commandLineArgs[0].charCodeAt(1) === CharacterCodes.minus
					? 2
					: 1,
			)
			.toLowerCase();
...
function performBuild(
	...
	if (buildOptions.watch) {
		...
		const solutionPerformance = enableSolutionPerformance(
			sys,
			buildOptions,
		);
...
function updateSolutionBuilderHost(
	...
	buildHost.beforeEmitBundle = (config) =>
		enableStatisticsAndTracing(sys, config.options, /*isBuildMode*/ true);
...
function reportSolutionBuilderTimes(
	...
	reportSolutionBuilderCountStatistic(
		"SolutionBuilder::Timestamps only updates",
	);
