...
export class CompilerBaselineRunner extends RunnerBase {
	...
	public enumerateTestFiles() {
		...
		return (this.testFiles ??= this.enumerateFiles(
			this.basePath,
			/\.tsx?$/,
			{ recursive: true },
		));
...
class CompilerTest {
	...
	constructor(
		...
		if (tsConfigOptions && tsConfigOptions.configFilePath !== undefined) {
			tsConfigOptions.configFile!.fileName =
	...
	public verifyModuleResolution() {
		if (this.options.traceResolution) {
			Baseline.runBaseline(
				...
				JSON.stringify(
					this.result.traces.map(
						Utils.sanitizeTraceResolutionLogEntry,
					),
					undefined,
					4,
				),
	...
	private createHarnessTestFile(
		unit: TestCaseParser.TestUnitData,
	): Compiler.TestFile {
		return {
			unitName: unit.name,
