...
export function start() {
	...
	class ProgressBars {
		...
		constructor(options?: Partial<ProgressBarsOptions>) {
			...
			const maxWidth =
				Base.window.width - open.length - close.length - 34;
		...
		update(
			...
			progress += this._color(
				"progress",
				fill(this._options.incomplete, i),
			);
	...
	function startDelayed(
		...
		for (let i = 0; i < workerCount; i++) {
			...
			const killChild = (timeout: TaskTimeout) => {
				worker.process.kill();
				console.error(
					`Worker exceeded ${timeout.duration}ms timeout ${worker.currentTasks && worker.currentTasks.length ? `while running test '${worker.currentTasks[0].file}'.` : `during test setup.`}`,
				);
    ...
    ${worker.accumulatedOutput}`);
            ...
            Stack: ${data.payload.stack}`);
					...
					case "result": {
						...
						if (data.payload.errors.length) {
							errorResults = errorResults.concat(
								data.payload.errors,
							);
							passingResults = passingResults.concat(
								data.payload.passes,
							);
							failingFiles++;
						} else {
						...
						newPerfData[
							hashName(
								data.payload.task.runner,
								data.payload.task.file,
							)
						] = data.payload.duration;

						const progress =
							(failingFiles + passingFiles) / totalFiles;
						...
						if (data.type === "result") {
							...
							while (
								tasks.length &&
								taskList.reduce((p, c) => p + c.size, 0) <
									chunkSize
							) {
							...
							if (taskList.length === 1) {
								worker.process.send({
									type: "test",
									payload: taskList[0],
								} satisfies ParallelHostMessage); // TODO: GH#18217
							} else {
								worker.process.send({
									type: "batch",
									payload: taskList,
		...
		if (totalFiles > 1000 && batchSize > 0) {
			...
			if (unknownValue) {
				...
				console.log(
					`${prefix} with approximate total ${perfData ? "time" : "file sizes"} of ${perfData ? ms(batchSize) : `${Math.floor(batchSize)} bytes`} in each group. (${((scheduledTotal / totalCost) * 100).toFixed(1)}% of total tests batched)`,
				);
			}
		...
		function outputFinalResult() {
			...
			let failedTestReporter:
				| import("../../../scripts/failed-tests.cjs")
				| undefined;
			...
			replaySuite(
				replayRunner,
				rebuildSuite(errorResults, passingResults),
			);
