		...
		for (const { name, payload } of ProjectTestCase.getConfigurations(
			...
			describe(
				...
				() => {
					...
					before(() => {
						projectTestCase = new ProjectTestCase(
							testCaseFileName,
							payload,
						);
					});
					...
					it(`Correct JS output for ${testCaseFileName}`, () =>
						projectTestCase &&
						projectTestCase.verifyJavaScriptOutput());
					...
					it(`Correct declarations for ${testCaseFileName}`, () =>
						projectTestCase &&
						projectTestCase.verifyDeclarations());
					after(() => {
...
class ProjectTestCase {
	...
	public static getConfigurations(
		...
		function makeFileSystem() {
			const fs = vfs.createFromFileSystem(
				Harness.IO,
				/*ignoreCase*/ false,
			);
			fs.mountSync(
	...
	public verifyDeclarations() {
		if (!this.compilerResult.errors.length && this.testCase.declaration) {
			const dTsCompileResult = this.compileDeclarations(
				this.compilerResult,
			);
	...
	private compileDeclarations(
		...
		ts.forEach(compilerResult.program.getSourceFiles(), (sourceFile) => {
			if (sourceFile.isDeclarationFile) {
				...
				if (!ts.contains(allInputFiles, outputDtsFile)) {
					...
					rootFiles.unshift(
						outputDtsFile.meta.get("fileName") ||
							outputDtsFile.file,
					);
				}
			}
		...
		const _vfs = vfs.createFromFileSystem(
			Harness.IO,
			/*ignoreCase*/ false,
			{
		...
		function findOutputDtsFile(fileName: string) {
			return ts.forEach(compilerResult.outputFiles, (outputFile) =>
				outputFile.meta.get("fileName") === fileName
					? outputFile
					: undefined,
			);
		}
