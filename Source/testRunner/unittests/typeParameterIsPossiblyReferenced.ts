...
describe("unittests :: internalApi :: typeParameterIsPossiblyReferenced", () => {
	it("with type parameter aliasing", () => {
		...
		const typeQueryNode = (
			(file.statements[0] as ts.FunctionDeclaration).parameters[1] // function f<T> // f
				.type! as ts.FunctionTypeNode
		).type as ts.TypeQueryNode; // <T>(a: typeof b) => typeof a // typeof a
		const typeParameterDecl = (file.statements[0] as ts.FunctionDeclaration)
			.typeParameters![0]; // T in f<T>
		const typeParameter = checker.getTypeAtLocation(
			typeParameterDecl,
		) as ts.TypeParameter;
		const isReferenced = checker.isTypeParameterPossiblyReferenced(
			typeParameter,
			typeQueryNode,
		);
		assert.ok(
			isReferenced,
			"Type parameter is referenced in type query node",
		);
	});
});
