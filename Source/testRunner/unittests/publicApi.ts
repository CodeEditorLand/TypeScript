...
describe("unittests:: Public APIs", () => {
	function verifyApi(fileName: string) {
		...
		before(() => {
			...
			if (!fileContent)
				throw new Error(
					`File ${fileName} was not present in built/local`,
				);
...
function test() {}`;
		...
		const funcDec = testSourceFile.statements.find(
...
describe("unittests:: Public APIs:: getTypeAtLocation", () => {
	...
	it("works on ExpressionWithTypeArguments", () => {
		...
		assert.equal(
			checker.getTypeAtLocation(declaration.initializer!).flags,
			ts.TypeFlags.Object,
		);
	});
	...
	it("returns an errorType for VariableDeclaration with BindingPattern name", () => {
		...
		assert.equal(
			checker.getTypeAtLocation(declaration).flags,
			ts.TypeFlags.Any,
		);
	});
});
...
class C {
    ...
    prop = 1;
		...
		assert.equal(
			...
			ts.getSyntacticModifierFlags(propNode),
		);
		assert.equal(
			ts.ModifierFlags.None,
			ts.getEffectiveModifierFlags(propNode),
		);
		assert.equal(
			ts.ModifierFlags.None,
			ts.getSyntacticModifierFlags(propNode),
