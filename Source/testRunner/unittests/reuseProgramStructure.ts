...
describe("unittests:: Reuse program structure:: General", () => {
	...
	function baselineProgram(
		...
		baselines.push(
			`Program Reused:: ${(ts as any).StructureIsReused[program.structureIsReused]}`,
		);
		...
		host.getTrace().forEach((trace) =>
			baselines.push(Utils.sanitizeTraceResolutionLogEntry(trace)),
		);
		host.clearTrace();
		baselines.push("");
		baselines.push(
			`MissingPaths:: ${jsonToReadableText(ts.arrayFrom(program.getMissingFilePaths().values()))}`,
		);
		baselines.push("");
		baselines.push(
			ts.formatDiagnostics(program.getSemanticDiagnostics(), {
...
`,
	...
	it("successful if change affects a single module of a package", () => {
		const files = [
			...
			},
			{
				...
				text: SourceText.New(
					"",
					"",
					jsonToReadableText({ name: "b", version: "1.2.3" }),
				),
			},
		];
		...
		const program2 = updateProgram(
			...
			options,
			(files) => {
				files[2].text = files[2].text.updateProgram(
					"export const b = 2;",
				);
			},
		);
...
/// <reference types="typerefs1" />`;
	...
	it("fails if module kind changes", () => {
		...
		const program2 = updateProgram(
			program1,
			["a.ts"],
			{ target, module: ts.ModuleKind.AMD },
			ts.noop,
		);
		baselineProgram(baselines, program2);
	...
	it("set the resolvedImports after re-using an ambient external module declaration", () => {
		...
		const program2 = updateProgram(
			...
			options,
			(files) => {
				files[0].text = files[0].text.updateProgram(
					'import * as aa from "a";',
				);
			},
		);
		baselineProgram(baselines, program2);
		runBaseline(
			"resolvedImports after re-using an ambient external module declaration",
			baselines,
		);
	});
	...
	it("works with updated SourceFiles", () => {
		...
		baselines.push(
			`parent pointers are updated: ${sourceFile.statements[2].getSourceFile() === sourceFile}`,
		);
		...
		baselines.push(
			`parent pointers are not altered: ${sourceFile.statements[2].getSourceFile() === sourceFile}`,
		);
	...
	it("resolved type directives cache follows type directives", () => {
		...
		baselineProgram(baselines, program4);
		runBaseline(
			"resolved type directives cache follows type directives",
			baselines,
		);
	});
	...
	it("can reuse module resolutions from non-modified files", () => {
		...
		const program7 = updateProgram(
			...
			(f) => {
				const newSourceText =
					f[indexOfF1].text.updateImportsAndExports("");
				f[indexOfF1] = { name: "f1.ts", text: newSourceText };
			},
		);
		baselineProgram(baselines, program7);
		runBaseline(
			"can reuse module resolutions from non-modified files",
			baselines,
		);
	});
			...
			const files: NamedSourceText[] = [
				...
				},
				{
					...
					text: SourceText.New(
						"",
						"",
						jsonToReadableText({ name: "x", version: "1.2.3" }),
					),
				},
				{
					name: "/node_modules/b/index.d.ts",
				...
				const program2 = updateRedirectProgram(
					...
					(files) => {
						...
						updateProgramText(
							files,
							bxPackage,
							jsonToReadableText({ name: "x", version: "1.2.4" }),
						);
					},
					useGetSourceFileByPath,
				);
...
describe("unittests:: Reuse program structure:: isProgramUptoDate", () => {
	...
	describe("should return true when there is no change in compiler options and", () => {
		...
		function verifyProgramWithConfigFile(
			...
			const program = ts
				.createWatchProgram(
					ts.createWatchCompilerHostOfConfigFile({
						configFileName,
						system,
					}),
				)
		...
		it("has paths specified in the options", () => {
			...
			const module1: File = {
				...
				content:
					'import classc from "module2/file2";export default classc;',
			};
			const module2: File = {
