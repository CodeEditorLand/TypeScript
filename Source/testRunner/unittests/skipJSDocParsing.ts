...
describe("unittests:: skipJSDocParsing", () => {
	...
	const filenames = ["file.ts", "file.js"];
	function diffSourceFiles(name: string, content: string) {
		for (const jsDocParsingMode of kinds) {
			const kindName = ts.Debug.formatEnum(
				jsDocParsingMode,
				(ts as any).JSDocParsingMode,
			);
			for (const filename of filenames) {
				...
				it(testName, () => {
					...
					const sourceFileSkipped = ts.createSourceFile(
						filename,
						content,
						{
							languageVersion: ts.ScriptTarget.ESNext,
					...
					assert.isTrue(
						sourceFileSkipped &&
							sourceFileSkipped.parseDiagnostics.length === 0,
						"no errors issued",
					);
					...
					Harness.Baseline.runBaseline(
						"skipJSDocParsing/" + testName + ".diff",
						patch,
					);
				});
			}
