...
'change'`,
	...
	emitsCorrectly(
		...
		],
		{
			before: [
				(context) => {
					const transformSourceFile: ts.Transformer<ts.SourceFile> = (
						node,
					) =>
						ts.visitNode(
							node,
							function visitor(node: ts.Node): ts.Node {
								if (
									...
									const newNode =
										ts.factory.createIdentifier("changed");
					...
					return {
						...
						transformBundle: (node) =>
							ts.factory.createBundle(
								ts.map(node.sourceFiles, transformSourceFile),
								node.prepends,
							),
					};
				},
