...
describe("unittests:: TransformAPI", () => {
			...
			if (
				...
				node = ts.factory.createPartiallyEmittedExpression(
					ts.addSyntheticTrailingComment(
						ts.setTextRange(ts.factory.createVoidZero(), node),
	...
	function replaceNumberWith2(context: ts.TransformationContext) {
		...
		return (file: ts.SourceFile) =>
			ts.visitNode(file, visitor, ts.isSourceFile);
	}
	...
	function replaceIdentifiersNamedOldNameWithNewName2(
		...
		return (node: ts.SourceFile) =>
			ts.visitNode(node, visitor, ts.isSourceFile);
	}
	...
	testBaseline("issue44068", () => {
		return transformSourceFile(
			...
			[
				(context) => (file) => {
					const firstVarName = (
						file.statements[0] as ts.VariableStatement
	...
	testBaseline("transformParameterProperty", () => {
		...
		function transformAddParameterProperty(
						...
						ts.factory.createConstructorDeclaration(
							...
							[
								ts.factory.createParameterDeclaration(
									[
										ts.factory.createDecorator(
											ts.factory.createIdentifier("Dec"),
										),
	...
	function addSyntheticComment(nodeFilter: (node: ts.Node) => boolean) {
		return (context: ts.TransformationContext) => {
			...
			function rootTransform<T extends ts.Node>(
				node: T,
			): ts.VisitResult<T> {
				if (nodeFilter(node)) {
...
`,
			{
				transformers: {
					before: [
						addSyntheticComment((n) => ts.isModuleDeclaration(n)),
					],
				},
				compilerOptions: {
					target: ts.ScriptTarget.ES2015,
