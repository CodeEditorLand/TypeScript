...
export function createTestCompilerHost(
	...
	const files = ts.arrayToMap(
		texts,
		(t) => t.name,
		(t) => {
			if (oldProgram) {
				let oldFile = oldProgram.getSourceFile(
					t.name,
				) as SourceFileWithText;
	...
	const useCaseSensitiveFileNames =
		ts.sys && ts.sys.useCaseSensitiveFileNames;
	...
	if (useGetSourceFileByPath) {
		const filesByPath = ts.mapEntries(files, (fileName, file) => [
			ts.toPath(fileName, "", getCanonicalFileName),
			file,
		]);
