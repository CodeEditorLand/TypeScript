...
describe("unittests:: config:: tsconfigParsingWatchOptions:: parseConfigFileTextToJson", () => {
	...
	function verifyWatchOptions(
		subScenario: string,
		scenario: () => VerifyWatchOptions[],
	) {
		baselineParseConfig({
			...
			input: () =>
				scenario().map(
					({ json, additionalFiles, existingWatchOptions }) => {
						...
						return {
							...
							baselineParsed: (baseline, parsed) => {
								...
								baseline.push(
									jsonToReadableText(parsed.watchOptions),
								);
							},
						};
					},
	...
	verifyWatchOptions(
		"when extending config file without watchOptions",
		() => [
			{
				json: {
					extends: "./base.json",
	...
	verifyWatchOptions(
		"watch options extending passed in watch options",
		() => [
			{
				json: { watchOptions: { watchFile: "UseFsEvents" } },
			},
