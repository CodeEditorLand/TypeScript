...
describe("unittests:: config:: tsconfigParsing:: parseConfigFileTextToJson", () => {
	...
	function baselineParseResult(name: string, jsonTexts: () => string[]) {
		it(name, () => {
			...
			Harness.Baseline.runBaseline(
				`config/tsconfigParsing/${name} jsonParse.js`,
				baseline.join("\n"),
			);
		});
	}
	...
	function baselinedParsed(
		subScenario: string,
		scenario: () => VerifyConfig[],
		skipJson?: true,
	) {
		baselineParseConfig({
			...
			input: () =>
				scenario().map(
					({ jsonText, configFileName, basePath, allFileList }) => ({
						createHost: () => {
							const files = allFileList.reduce(
								(files, value) => ((files[value] = ""), files),
								{} as vfs.FileSet,
							);
							files[ts.combinePaths(basePath, configFileName)] =
								jsonText;
	...
	baselineParseResult(
		"returns object with error when json is invalid",
		() => ["invalid"],
	);
	...
	baselinedParsed("exclude outDir unless overridden", () => {
		...
		return [
			{
				jsonText: tsconfigWithoutExclude,
				configFileName: "tsconfig.json",
				basePath,
				allFileList,
			},
			{
				jsonText: tsconfigWithExclude,
				configFileName: "tsconfig.json",
				basePath,
				allFileList,
			},
	...
	it("parse and re-emit tsconfig.json file with diagnostics", () => {
		...
		const configJsonObject = ts.convertToObject(
			result,
	...
	baselinedParsed(
		"generates errors for when invalid comment type present in tsconfig",
		() => [
			{
				jsonText: `{
              "compilerOptions": {
	...
	baselinedParsed(
		"generates errors when commandline option is in tsconfig",
		() => [
			{
				jsonText: jsonToReadableText({
					compilerOptions: {
			...
			input: () =>
				scenario().map(({ jsonText, configFileName, basePath }) => ({
					...
					baselineParsed: (baseline, parsed) => {
						...
						ts.getOwnKeys(parsed.wildcardDirectories!).forEach(
							(dir) =>
								baseline.push(
	...
	baselineWildcards(
		"parses wildcard directories even when parent directories have dots",
		() => [
			{
