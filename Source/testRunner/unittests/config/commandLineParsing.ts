...
describe("unittests:: config:: commandLineParsing:: parseCommandLine", () => {
	...
	// --lib es6 0.ts
	assertParseResult("Parse single option of library flag", [
		"--lib",
		"es6",
		"0.ts",
	]);
	assertParseResult("Handles may only be used with --build flags", [
		"--clean",
		"--dry",
		"--force",
		"--verbose",
	...
	assertParseResult("Handles did you mean for misspelt flags", [
		"--declarations",
		"--allowTS",
	]);
	// --lib es5,es2015.symbol.wellknown 0.ts
	assertParseResult("Parse multiple options of library flags", [
		"--lib",
		"es5,es2015.symbol.wellknown",
		"0.ts",
	]);
	// --lib es5,invalidOption 0.ts
	assertParseResult("Parse invalid option of library flags", [
		"--lib",
		"es5,invalidOption",
		"0.ts",
	]);
	...
	// 0.ts --newLine
	assertParseResult("Parse empty options of --newLine", [
		"0.ts",
		"--newLine",
	]);
	// 0.ts --target
	...
	assertParseResult("Parse empty options of --moduleResolution", [
		"0.ts",
		"--moduleResolution",
	]);
	// 0.ts --lib
	...
	// --lib es5, es7 0.ts
	assertParseResult("Parse --lib option with extra comma", [
		"--lib",
		"es5,",
		"es7",
		"0.ts",
	...
	// --lib es5, es7 0.ts
	assertParseResult("Parse --lib option with trailing white-space", [
		"--lib",
		"es5, ",
		"es7",
		"0.ts",
	...
	assertParseResult("Parse explicit boolean flag value", [
		"--strictNullChecks",
		"false",
		"0.ts",
	]);
	assertParseResult("Parse non boolean argument after boolean flag", [
		"--noImplicitAny",
		"t",
		"0.ts",
	]);
	assertParseResult("Parse implicit boolean flag value", [
		"--strictNullChecks",
	]);
	assertParseResult("parse --incremental", ["--incremental", "0.ts"]);
	assertParseResult("parse --tsBuildInfoFile", [
		"--tsBuildInfoFile",
		"build.tsbuildinfo",
		"0.ts",
	]);
	...
	describe("parses command line null for tsconfig only option", () => {
				...
				workerDiagnostic: () => {
					const optionDeclarations: ts.CommandLineOption[] = [
							...
							.optionDeclarations,
						{
							...
							description:
								ts.Diagnostics.Enable_project_compilation,
					...
					return {
						...
						optionDeclarations,
						getOptionsNameMap: () =>
							ts.createOptionNameMap(optionDeclarations),
					};
				},
			});
		}
		...
		describe("option of type boolean", () => {
			assertParseResult("allows setting option type boolean to false", [
				"--composite",
				"false",
				"0.ts",
			]);
	...
	describe("Watch options", () => {
		assertParseResult("parse --watchFile", [
			"--watchFile",
			"UseFsEvents",
			"0.ts",
		]);
		assertParseResult("parse --watchDirectory", [
			"--watchDirectory",
			"FixedPollingInterval",
			"0.ts",
		]);
		assertParseResult("parse --fallbackPolling", [
			"--fallbackPolling",
			"PriorityInterval",
			"0.ts",
		]);
		assertParseResult("parse --synchronousWatchDirectory", [
			"--synchronousWatchDirectory",
			"0.ts",
		]);
		assertParseResult("errors on missing argument to --fallbackPolling", [
			"0.ts",
			"--fallbackPolling",
		]);
		assertParseResult("parse --excludeDirectories", [
			"--excludeDirectories",
			"**/temp",
			"0.ts",
		]);
		assertParseResult("errors on invalid excludeDirectories", [
			"--excludeDirectories",
			"**/../*",
			"0.ts",
		]);
		assertParseResult("parse --excludeFiles", [
			"--excludeFiles",
			"**/temp/*.ts",
			"0.ts",
		]);
		assertParseResult("errors on invalid excludeFiles", [
			"--excludeFiles",
			"**/../*",
			"0.ts",
		]);
...
describe("unittests:: config:: commandLineParsing:: parseBuildOptions", () => {
	...
	assertParseResult("Parse multiple options", [
		"--verbose",
		"--force",
		"tests",
	]);
	assertParseResult("Parse option with invalid option", [
		"--verbose",
		"--invalidOption",
	]);
	assertParseResult("Parse multiple flags with input projects at the end", [
		"--force",
		"--verbose",
		"src",
		"tests",
	...
	assertParseResult("parse build with --incremental", [
		"--incremental",
		"tests",
	]);
	assertParseResult("parse build with --locale en-us", [
		"--locale",
		"en-us",
		"src",
	]);
	assertParseResult("parse build with --tsBuildInfoFile", [
		"--tsBuildInfoFile",
		"build.tsbuildinfo",
		"tests",
	]);
	...
	describe("Combining options that make no sense together", () => {
		function verifyInvalidCombination(
			flag1: keyof ts.BuildOptions,
			flag2: keyof ts.BuildOptions,
		) {
			assertParseResult(`--${flag1} and --${flag2} together is invalid`, [
				`--${flag1}`,
				`--${flag2}`,
			]);
		}
	...
	describe("Watch options", () => {
		...
		assertParseResult("errors on missing argument", [
			"--verbose",
			"--fallbackPolling",
		]);
		assertParseResult("errors on invalid excludeDirectories", [
