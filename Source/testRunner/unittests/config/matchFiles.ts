...
const caseInsensitiveHostWithSameFileNamesWithDifferentExtensions =
		...
		input: () => [
			{
				...
				baselineParsed: (baseline, parsed) => {
					const wildcardDirectories = parsed.wildcardDirectories
						? ({} as ts.MapLike<string>)
						: undefined;
					if (parsed.wildcardDirectories)
						ts.getOwnKeys(parsed.wildcardDirectories).forEach(
							(dir) =>
								(wildcardDirectories![dir] =
									`WatchDirectoryFlags.${(ts as any).WatchDirectoryFlags[parsed.wildcardDirectories![dir]]}`),
						);
					baseline.push(
						"Result",
						jsonToReadableText({
...
describe("unittests:: config:: matchFiles", () => {
	...
	describe("when recursive symlinked directories are present", () => {
		...
		const json = {};
		baselineMatches(
			"when recursive symlinked directories are present",
			json,
			host,
			caseInsensitiveBasePath,
		);
