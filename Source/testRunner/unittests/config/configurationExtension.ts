	...
	return new vfs.FileSystem(ignoreCase, {
		...
		files: {
			[root]: {
				...
				"dev/extends2.json": jsonToReadableText({
					extends: "configs/base",
				}),
				...
				"dev/extendsArrayFails2.json": jsonToReadableText({
					extends: [42],
				}),
			},
		},
	...
	ts.forEach<[string, string, fakes.ParseConfigHost], void>(
		...
		([testName, basePath, host]) => {
			...
			baselineParsedCommandLine(
				...
				"tsconfig.extendsFoo.json",
			);

			baselineExtendsSourceFile(
				"adds extendedSourceFiles only once",
				"configs/fourth.json",
			);
			baselineExtendsSourceFile(
				"adds extendedSourceFiles from an array only once",
				"configs/extendsArrayFifth.json",
			);
			...
			function baselineExtendsSourceFile(name: string, entry: string) {
				it(name, () => {
					...
					const sourceFile = ts.readJsonConfigFile(entry, (path) =>
						host.readFile(path),
					);
					...
					ts.parseJsonSourceFileConfigFileContent(
						...
						dir,
						{},
						ts.getBaseFileName(entry),
					);
					baseline.push(
						"ExtendedSourceFiles::",
						...sourceFile.extendedSourceFiles!,
					);
					ts.parseJsonSourceFileConfigFileContent(
						sourceFile,
						host,
