...
export function baselineParseConfig(input: BaselineParseConfigInput) {
	if (!input.skipJson)
		baselineParseConfigWith("with json api", getParsedCommandJson, input);
	baselineParseConfigWith(
		"with jsonSourceFile api",
		getParsedCommandJsonSourceFile,
		input,
	);
}

export function baselineParseConfigHost(
	baseline: string[],
	host: fakes.ParseConfigHost,
) {
	baseline.push(
		"Fs::",
		vfs.formatPatch(
			host.sys.vfs.diff(/*base*/ undefined, {
	...
	describe(subScenario, () => {
		it(jsonTest, () => {
			...
			Harness.Baseline.runBaseline(
				`config/${scenario}/${subScenario} ${jsonTest}.js`,
				baseline.join("\n"),
			);
		});
	});
