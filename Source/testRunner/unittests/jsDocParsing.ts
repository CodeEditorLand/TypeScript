...
describe("unittests:: JSDocParsing", () => {
	describe("TypeExpressions", () => {
		function parsesCorrectly(name: string, content: string) {
			it(name, () => {
				const typeAndDiagnostics =
					ts.parseJSDocTypeExpressionForTests(content);
				assert.isTrue(
					typeAndDiagnostics &&
						typeAndDiagnostics.diagnostics.length === 0,
					"no errors issued",
				);

				Harness.Baseline.runBaseline(
					"JSDocParsing/TypeExpressions.parsesCorrectly." +
						name +
						".json",
					Utils.sourceFileToJSON(
						typeAndDiagnostics!.jsDocTypeExpression.type,
					),
				);
			});
		}
	...
	describe("DocComments", () => {
		function parsesCorrectly(name: string, content: string) {
			it(name, () => {
				...
				Harness.Baseline.runBaseline(
					"JSDocParsing/DocComments.parsesCorrectly." +
						name +
						".json",
					JSON.stringify(
						comment.jsDoc,
 ...
 */`,
	...
	describe("parseIsolatedJSDocComment", () => {
		it("doesn't create a 1-element array with missing type parameter in jsDoc", () => {
			...
			assert.equal(
				(doc?.jsDoc.tags?.[0] as ts.JSDocTemplateTag).typeParameters
					.length,
				0,
			);
		});
