...
function withChange(
	...
	const newContents =
		...
		newText +
		contents.substring(start + length);
...
function reusedElements(
	oldNode: ts.SourceFile,
	newNode: ts.SourceFile,
): number {
	const allOldElements = collectElements(oldNode);
	const allNewElements = collectElements(newNode);

	return ts.filter(allOldElements, (v) => ts.contains(allNewElements, v))
		.length;
}
...
0;";
    ...
    bar3();`;
			...
			function getIndexOfTsIgnoreComment(atIndex: number) {
				...
				for (let i = 0; i <= atIndex; i++) {
					index = textWithIgnoreComment.indexOf(
						tsIgnoreComment,
						index,
					);
				}
			...
			function verifyDelete(atIndex: number, singleIgnore?: true) {
				...
				const oldText = ts.ScriptSnapshot.fromString(
					textWithIgnoreCommentFrom(
						textWithIgnoreComment,
						singleIgnore,
					),
				);
				const newTextAndChange = withDelete(
			...
			function verifyInsert(atIndex: number, singleIgnore?: true) {
				...
				const source = textWithIgnoreCommentFrom(
					textWithIgnoreComment.slice(0, index) +
						textWithIgnoreComment.slice(
							index + tsIgnoreComment.length,
						),
					singleIgnore,
				);
				const oldText = ts.ScriptSnapshot.fromString(source);
    ...
    foo3();`;
				...
				const newText = oldSubStr.replace(
					letStr,
					`let yn : string = x;`,
				);
				const newTextAndChange = withChange(
