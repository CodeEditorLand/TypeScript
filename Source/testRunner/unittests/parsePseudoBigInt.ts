				...
				for (let leadingZeros = 0; leadingZeros < 10; leadingZeros++) {
					assert.equal(
						ts.parsePseudoBigInt(
							"0".repeat(leadingZeros) + testNumber + "n",
						),
						String(testNumber),
					);
				}
			...
			for (const testNumber of testNumbers) {
				for (let leadingZeros = 0; leadingZeros < 10; leadingZeros++) {
					...
					for (const prefix of ["0b", "0B"]) {
						assert.equal(
							ts.parsePseudoBigInt(prefix + binary),
							String(testNumber),
						);
					}
				}
			...
			for (const testNumber of testNumbers) {
				for (let leadingZeros = 0; leadingZeros < 10; leadingZeros++) {
					...
					for (const prefix of ["0o", "0O"]) {
						assert.equal(
							ts.parsePseudoBigInt(prefix + octal),
							String(testNumber),
						);
					}
				}
			...
			for (const testNumber of testNumbers) {
				for (let leadingZeros = 0; leadingZeros < 10; leadingZeros++) {
					...
					for (const prefix of ["0x", "0X"]) {
						for (const hexCase of [
							hex.toLowerCase(),
							hex.toUpperCase(),
						]) {
							assert.equal(
								ts.parsePseudoBigInt(prefix + hexCase),
								String(testNumber),
							);
						}
					}
