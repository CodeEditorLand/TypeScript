		...
		const runOnce = (name: string, testSettings: TranspileTestSettings) =>
			describe(name, () => {
				...
				if (transpileOptions.compilerOptions.target === undefined) {
					transpileOptions.compilerOptions.target =
						ts.ScriptTarget.ES3;
				}
				...
				const justName =
					...
					(transpileOptions.compilerOptions.jsx
						? ts.Extension.Tsx
						: ts.Extension.Ts);
				const toBeCompiled = [
					{
				...
				const canUseOldTranspile =
					!transpileOptions.renamedDependencies;

				before(() => {
					transpileResult = ts.transpileModule(
						input,
						transpileOptions,
					);
...
var x = 0;`,
	...
	transpilesCorrectly(
		...
		{
			options: {
				compilerOptions: { module: 123 as any as ts.ModuleKind },
			},
		},
	);
...
export * as alias from './file';`,
	...
	transpilesCorrectly(
		"Can transpile .ts extensions without error",
