...
export interface TestProjectServiceOptions
	extends ts.server.ProjectServiceOptions {
	host: TestServerHost;
}
export type TestProjectServicePartialOptionsAndHost = Partial<
	Omit<TestProjectServiceOptions, "typingsInstaller" | "logger" | "host">
> &
	Pick<TestProjectServiceOptions, "host">;

export class TestProjectService extends ts.server.ProjectService {
	constructor(
		optsOrHost: TestServerHost | TestProjectServicePartialOptionsAndHost,
	) {
		// eslint-disable-next-line local/no-in-operator
		const opts = "host" in optsOrHost ? optsOrHost : { host: optsOrHost };
		super({
...
export function extractTest(source: string): Test {
	...
	while (pos < source.length) {
		if (
			...
			if (source.charCodeAt(pos) === ts.CharacterCodes.bar) {
				...
				const name =
					s === e
						? source.charCodeAt(saved + 1) ===
							ts.CharacterCodes.hash
							? "selection"
							: "extracted"
	...
	function consumeIdentifier() {
		while (
			ts.isIdentifierPart(source.charCodeAt(pos), ts.ScriptTarget.Latest)
		) {
			pos++;
		}
	}
...
export function testExtractSymbol(
	...
	function runBaseline(extension: ts.Extension) {
		...
		const infos =
			ts.refactor.extractSymbol.getRefactorActionsToExtractSymbol(
				context,
			);
		const actions = ts.find(
			infos,
		...
		for (const action of actions) {
			const { renameLocation, edits } =
				ts.refactor.extractSymbol.getRefactorEditsToExtractSymbol(
					context,
					action.name,
				)!;
...
export function testExtractSymbolFailed(
	...
	it(caption, () => {
		...
		assert.isUndefined(
			ts.find(infos, (info) => info.description === description.message),
		);
	});
}
