...
describe("unittests:: services:: textChanges", () => {
	...
	function runSingleFileTest(
		...
		it(caption, () => {
			...
			const changeTracker = new ts.textChanges.ChangeTracker(
				newLineCharacter,
				rulesProvider,
			);
			...
			const modified = ts.textChanges.applyChanges(
				sourceFile.text,
    ...
    namespace M2
    {
		...
		runSingleFileTest(
			...
			(sourceFile, changeTracker) => {
				const statements = (
					findChild("foo", sourceFile) as ts.FunctionDeclaration
		...
		runSingleFileTest(
			...
			(sourceFile, changeTracker) => {
				deleteNode(
					...
					{
						leadingTriviaOption:
							ts.textChanges.LeadingTriviaOption.Exclude,
					},
				);
			},
		);
				...
				deleteNode(
					...
					findVariableStatementContaining("y", sourceFile),
					{
						trailingTriviaOption:
							ts.textChanges.TrailingTriviaOption.Exclude,
					},
				);
			},
		);
