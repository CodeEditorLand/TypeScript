...
import { newLineCharacter, TestProjectService } from "./extract/helpers";
...
describe("unittests:: services:: organizeImports", () => {
	describe("Sort imports", () => {
		...
		function assertSortsBefore(
			importString1: string,
			importString2: string,
		) {
			const [
			...
			assert.equal(
				ts.OrganizeImports.compareModuleSpecifiers(
					moduleSpecifier1,
					moduleSpecifier2,
					/*ignoreCase*/ true,
				),
	...
	describe("Coalesce imports", () => {
		it("No imports", () => {
			assert.isEmpty(
				ts.OrganizeImports.coalesceImports([], /*ignoreCase*/ true),
			);
		});
	...
	describe("Coalesce exports", () => {
		it("No exports", () => {
			assert.isEmpty(
				ts.OrganizeImports.coalesceExports([], /*ignoreCase*/ true),
			);
		});
		...
		it("Sort specifiers - case-insensitive", () => {
			...
			const actualCoalescedExports = ts.OrganizeImports.coalesceExports(
				sortedExports,
				/*ignoreCase*/ true,
			);
...
`,
		...
		function makeLanguageService(...files: File[]) {
			...
			const projectService = new TestProjectService({
				host,
	...
	function assertListEqual(
		list1: readonly ts.Node[],
		list2: readonly ts.Node[],
	) {
