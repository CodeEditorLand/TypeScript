...
describe("unittests:: tsserver:: packageJsonInfo::", () => {
	it("detects new package.json files that are added, caches them, and watches them", () => {
		...
		let packageJsonInfo =
			projectService.packageJsonCache.getInDirectory("/")!;
	...
	it("finds multiple package.json files when present", () => {
		...
		assert.lengthOf(
			projectService.getPackageJsonsVisibleToFile("/a.ts", project),
			1,
		);
		assert.lengthOf(
			projectService.getPackageJsonsVisibleToFile("/src/b.ts", project),
			2,
		);
		baselineTsserverLogs(
			"packageJsonInfo",
			"finds multiple package.json files when present",
			session,
		);
	});
	...
	it("handles errors in json parsing of package.json", () => {
		...
		baselineTsserverLogs(
			"packageJsonInfo",
			"handles errors in json parsing of package.json",
			session,
		);
	});
...
function setup(files: readonly File[] = [tsConfig, packageJson]) {
	...
	const getPackageJsonsVisibleToFile =
		projectService.getPackageJsonsVisibleToFile;
	projectService.getPackageJsonsVisibleToFile = (
		fileName,
		project,
		...
		const result = getPackageJsonsVisibleToFile.call(
			projectService,
			fileName,
			project,
			rootDir,
