...
describe("unittests:: tsserver:: with project references and tsbuild", () => {
	describe("with container project", () => {
		function setup(tempFile?: File) {
			...
			const host = createHostWithSolutionBuild(files, [
		...
		it("does not error on container only project", () => {
			...
			session.executeCommandSeq<ts.server.protocol.OpenExternalProjectsRequest>(
				{
					command:
						ts.server.protocol.CommandTypes.OpenExternalProjects,
					arguments: {
						projects: [
							{
								projectFileName:
									"/user/username/projects/container/container",
								rootFiles: files.map((f) => ({
									fileName: f.path,
								})),
								options: {},
							},
			...
			files.forEach((f) => {
				const args: ts.server.protocol.FileRequestArgs = {
					file: f.path,
					projectFileName: ts.endsWith(f.path, "tsconfig.json")
						? f.path
						: undefined,
				};
			...
			const containerProject = session
				.getProjectService()
				.configuredProjects.get(containerConfig.path)!;
			session.executeCommandSeq<ts.server.protocol.CompilerOptionsDiagnosticsRequest>(
				{
					...
					arguments: {
						projectFileName: containerProject.projectName,
					},
				},
			);
			baselineTsserverLogs(
		...
		it("can successfully find references with --out options", () => {
			...
			const myConstStart = protocolLocationFromSubstring(
				containerCompositeExecIndex.content,
				"myConst",
			);
		...
		it("ancestor and project ref management", () => {
			...
			closeFilesForSession(
				[containerCompositeExecIndex, tempFile],
				session,
			);
			openFilesForSession([tempFile], session);
			baselineTsserverLogs(
...
`,
	...
	it("reusing d.ts files from composite and non composite projects", () => {
		...
		const projectA = session
			.getProjectService()
			.configuredProjects.get(configA.path)!;
...
testCompositeFunction('why hello there', 42);`,
	...
	it("when finding references in overlapping projects", () => {
		...
		session.executeCommandSeq<ts.server.protocol.ReferencesRequest>({
			command: ts.server.protocol.CommandTypes.References,
			arguments: protocolFileLocationFromSubstring(bFile, "I", {
				index: 1,
			}),
		});
		...
		baselineTsserverLogs(
			"projectReferences",
			`finding references in overlapping projects`,
			session,
		);
	});
...
//# sourceMappingURL=main.d.ts.map`,
		...
		const mainDtsMap: File = {
			...
			content: jsonToReadableText({
				...
				names: [],
				mappings:
					"AAAA,OAAO,EAAE,GAAG,EAAE,MAAM,mBAAmB,CAAC;AAExC,OAAO,EAAC,GAAG,EAAC,CAAC",
			}),
		};
...
export function bar() {}`,
		...
		function setup({
			...
			const tsconfig: File = {
				...
				content: jsonToReadableText({
					...(solutionOptions
						? { compilerOptions: solutionOptions }
						: {}),
		...
		function verifySolutionScenario(input: Setup) {
			...
			const info = session
				.getProjectService()
				.getScriptInfoForPath(main.path as ts.Path)!;
...
bar;`,
				...
				const { tsconfigIndirect, indirect } = getIndirectProject("1");
				const {
					tsconfigIndirect: tsconfigIndirect2,
					indirect: indirect2,
				} = getIndirectProject("2");
				verifySolutionScenario({
					scenario:
