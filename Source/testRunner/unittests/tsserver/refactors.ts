...
describe("unittests:: tsserver:: refactors", () => {
	...
	it("handles text changes in tsconfig.json", () => {
		...
		baselineTsserverLogs(
			"refactors",
			"handles text changes in tsconfig",
			session,
		);
	});
	...
	it("handles canonicalization of tsconfig path", () => {
		...
		baselineTsserverLogs(
			"refactors",
			"handles canonicalization of tsconfig path",
			session,
		);
	});
    ...
    const a = 1;`,
		...
		baselineTsserverLogs(
			"refactors",
			"handles moving statement to an existing file",
			session,
		);
	});
	...
	it("handles moving statements to a non-TS file", () => {
		...
		baselineTsserverLogs(
			"refactors",
			"handles moving statements to a non-TS file",
			session,
		);
	});
