...
describe("unittests:: tsserver:: Open-file", () => {
	it("can be reloaded with empty content", () => {
		...
		baselineTsserverLogs(
			"openfile",
			"realoaded with empty content",
			session,
		);
	});

	function verifyOpenFileWorks(
		subScenario: string,
		...
		it(subScenario, () => {
			...
			const host = createServerHost(
				[file1, file2, configFile, configFile2],
				{
	...
	verifyOpenFileWorks(
		"project root is used with case-sensitive system",
		/*useCaseSensitiveFileNames*/ true,
	);
	verifyOpenFileWorks(
		"project root is used with case-insensitive system",
	...
	it("uses existing project even if project refresh is pending", () => {
		...
		baselineTsserverLogs(
			"openfile",
			"uses existing project even if project refresh is pending",
			session,
		);
	});
	...
	it("can open same file again", () => {
		...
		const session = new TestSession(host);
		openFilesForSession(
			[{ file: aFile, content: aFile.content, projectRootPath }],
			session,
		);
		openFilesForSession(
			[
				{
					file: aFile,
					content: `${aFile.content}export const y = 10;`,
					projectRootPath,
				},
			],
			session,
...
bar();`,
	...
	describe("opening file and refreshing program", () => {
		...
		it("file opening does not refresh sourceFile", () => {
			...
			session
				.getProjectService()
				.configuredProjects.get("/project/tsconfig.json")!
				.updateGraph();
		...
		it("file opening with different content refreshes sourceFile", () => {
			...
			openFilesForSession(["/project/a.ts"], session);
			openFilesForSession(
				[{ file: "/project/b.ts", content: "export const newB = 10;" }],
				session,
			);
			applyEdit("/project/a.ts", session);
			session
			...
			baselineTsserverLogs(
				"openfile",
				"different content refreshes sourceFile",
				session,
			);
		});
		...
		it("edits on file and then close refreshes sourceFile", () => {
			...
			baselineTsserverLogs(
				"openfile",
				"edits on file and then close refreshes sourceFile",
				session,
			);
		});
