...
describe("unittests:: tsserver:: exportMapCache", () => {
	it("caches auto-imports in the same file", () => {
		...
		baselineTsserverLogs(
			"exportMapCache",
			"caches auto-imports in the same file",
			session,
		);
	});
	...
	it("invalidates the cache when new files are added", () => {
		...
		baselineTsserverLogs(
			"exportMapCache",
			"invalidates the cache when new files are added",
			session,
		);
	});
	...
	it("invalidates the cache when files are deleted", () => {
		...
		baselineTsserverLogs(
			"exportMapCache",
			"invalidates the cache when files are deleted",
			session,
		);
	});
	...
	it("does not store transient symbols through program updates", () => {
		...
		assert.notEqual(
			symbolIdBefore,
			ts.getSymbolId(sigintPropAfter![0].symbol),
		);
		baselineTsserverLogs(
			"exportMapCache",
			"does not store transient symbols through program updates",
			session,
		);
	});
	...
	it("invalidates the cache when a file is opened with different contents", () => {
		...
		openFilesForSession(
			[
				{
					file: utilsTs.path,
					content: utilsTs.content.replace("render", "render2"),
				},
			],
			session,
		);
