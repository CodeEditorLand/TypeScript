...
describe("unittests:: tsserver:: auxiliaryProject::", () => {
	it("AuxiliaryProject does not remove scrips from InferredProject", () => {
		...
		const auxProject = inferredProject.getNoDtsResolutionProject(
			aTs.path as ts.server.NormalizedPath,
		);
		...
		const bJsScriptInfo = ts.Debug.checkDefined(
			session.getProjectService().getScriptInfo(bJs.path),
		);
		...
		assert(bJsScriptInfo.isContainedByBackgroundProject());
		assert.equal(
			bJsScriptInfo.getDefaultProject().projectKind,
			ts.server.ProjectKind.Inferred,
		);
		baselineTsserverLogs(
			"auxiliaryProject",
			"does not remove scrips from InferredProject",
			session,
		);
	});
	...
	it("file is added later through finding definition", () => {
		...
		baselineTsserverLogs(
			"auxiliaryProject",
			"file is added later through finding definition",
			session,
		);
	});
	...
	it("resolution is reused from different folder", () => {
		...
		baselineTsserverLogs(
			"auxiliaryProject",
			"resolution is reused from different folder",
			session,
		);
	});
});
