...
describe("Test Suite 1", () => {
    ...
    it("Test B", () => {
		...
		openFilesForSession(
			[{ file: app.path, content: app.content }],
			session,
		);
		...
		openFilesForSession(
			[{ file: unitTest1.path, content: unitTest1.content }],
			session,
		);
...
export function Test2() {
    ...
    assert.ok(false, "This should fail");
		...
		openFilesForSession(
			[
				{
					file: unitTest1WithChangedContent,
					content: unitTest1WithChangedContent.content,
				},
			],
			session,
		);
		...
		baselineTsserverLogs(
			"syntaxOperations",
			"file is removed and added with different content",
			session,
		);
	});
});
