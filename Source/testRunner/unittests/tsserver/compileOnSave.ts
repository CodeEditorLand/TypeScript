...
describe("unittests:: tsserver:: compileOnSave:: affected list", () => {
	describe("for configured projects", () => {
		...
		it("should contains only itself if a module file's shape didn't change, and all files referencing it if its shape changed", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects module shape changed",
				session,
			);
		});
		...
		it("should be up-to-date with the reference map changes", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects uptodate with reference map changes",
				session,
			);
		});
		...
		it("should be up-to-date with changes made in non-open files", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects uptodate with changes in non open files",
				session,
			);
		});
		...
		it("should be up-to-date with deleted files", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects uptodate with deleted files",
				session,
			);
		});
		...
		it("should be up-to-date with newly created files", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects uptodate with new files",
				session,
			);
		});
		...
		it("should detect changes in non-root files", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects detect changes in non-root files",
				session,
			);
		});
		...
		it("should return all files if a global file changed shape", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects global file shape changed",
				session,
			);
		});
		...
		it("should return empty array if CompileOnSave is not enabled", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects compileOnSave disabled",
				session,
			);
		});
		...
		it("should save when compileOnSave is enabled in base tsconfig.json", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects compileOnSave in base tsconfig",
				session,
			);
		});
		...
		it("should always return the file itself if '--isolatedModules' is specified", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects isolatedModules",
				session,
			);
		});
		...
		it("should always return the file itself if '--out' or '--outFile' is specified", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects outFile",
				session,
			);
		});

		it("should return cascaded affected file list", () => {
			const { moduleFile1, file1Consumer1, globalFile3, configFile } =
				files();
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects cascaded affected file list",
				session,
			);
		});
		...
		it("should work fine for files with circular references", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects circular references",
				session,
			);
		});
		...
		it("should return results for all projects if not specifying projectFileName", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects all projects without projectPath",
				session,
			);
		});
		...
		it("should detect non-existing code file", () => {
			...
			baselineTsserverLogs(
				"compileOnSave",
				"configProjects non existing code",
				session,
			);
		});
	});
	...
	describe("tsserverProjectSystem emit with outFile or out setting", () => {
		...
		test(
			"compileOnSaveAffectedFileList projectUsesOutFile should not be returned if not set",
			{},
		);
		test(
			"compileOnSaveAffectedFileList projectUsesOutFile should be true if outFile is set",
			{ outFile: "/a/out.js" },
		);
		test(
			"compileOnSaveAffectedFileList projectUsesOutFile should be true if out is set",
			{ out: "/a/out.js" },
		);
	});
});
...
describe("unittests:: tsserver:: compileOnSave:: EmitFile test", () => {
	it("should respect line endings", () => {
		...
		function test(newLine: string, logger: LoggerWithInMemoryLogs) {
			...
			session.executeCommandSeq<ts.server.protocol.CompileOnSaveEmitFileRequest>(
				{
					command:
						ts.server.protocol.CommandTypes.CompileOnSaveEmitFile,
	...
	it("shoud not emit js files in external projects", () => {
		...
		baselineTsserverLogs(
			"compileOnSave",
			"should not emit js files in external projects",
			session,
		);
	});
	...
	it("should use project root as current directory so that compile on save results in correct file mapping", () => {
		...
		baselineTsserverLogs(
			"compileOnSave",
			"use projectRoot as current directory",
			session,
		);
	});
	...
	describe("compile on save emit with and without richResponse", () => {
		...
		function verify(richResponse: boolean | undefined) {
			...
			baselineTsserverLogs(
				"compileOnSave",
				`emit with richRepsonse as ${richResponse}`,
				session,
			);
		}
	});
...
function bar() {
    return "world";
			...
			const files = [
				...
				file2,
				file3,
				...(hasModule ? [module] : ts.emptyArray),
			];
...
describe("unittests:: tsserver:: compileOnSave:: CompileOnSaveAffectedFileListRequest with and without projectFileName in request", () => {
	...
	function logDirtyOfProjects(session: TestSession) {
		session.logger.log(
			`Project1 is dirty: ${session.getProjectService().configuredProjects.get(`/user/username/projects/myproject/app1/tsconfig.json`)!.dirty}`,
		);
		session.logger.log(
			`Project2 is dirty: ${session.getProjectService().configuredProjects.get(`/user/username/projects/myproject/app2/tsconfig.json`)!.dirty}`,
		);
	}

	function verify(
		subScenario: string,
		commandArgs: ts.server.protocol.FileRequestArgs,
	) {
		it(subScenario, () => {
			const core: File = {
	...
	verify(
		"CompileOnSaveAffectedFileListRequest when projectFile is specified",
		{
	...
	verify(
		"CompileOnSaveAffectedFileListRequest when projectFile is not specified",
		{
			file: `/user/username/projects/myproject/core/core.ts`,
		},
	);
