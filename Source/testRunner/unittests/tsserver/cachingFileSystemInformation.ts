...
describe("unittests:: tsserver:: CachingFileSystemInformation:: tsserverProjectSystem CachingFileSystemInformation", () => {
	...
	function createLoggerTrackingHostCalls(host: TestServerHost) {
		...
		const calledMaps: Record<
			...
			ts.MultiMap<string, true>
		> &
			Record<
				CalledMapsWithFiveArgs,
				ts.MultiMap<string, CalledWithFiveArgs>
			> = {
		...
		(host as IncrementalVerifierCallbacks).beforeVerification =
			storeAndSetToOriginal;
		(host as IncrementalVerifierCallbacks).afterVerification =
			revertCallbacks;
		...
		function setCallsTrackingWithSingleArgFn(
			prop: CalledMapsWithSingleArg,
		) {
		...
		function logCacheEntry(
			...
			const result = Array.from<
				[string, (true | CalledWithFiveArgs)[]],
				{ key: string; count: number }
			>(calledMaps[callback].entries(), ([key, arr]) => ({
				key,
				count: arr.length,
			}));
	...
	function logSemanticDiagnostics(
		session: TestSession,
	...
	it("works using legacy resolution logic", () => {
		...
		baselineTsserverLogs(
			...
			"works using legacy resolution logic",
			session,
		);
	...
	it("when calling goto definition of module", () => {
		...
		baselineTsserverLogs(
			"cachingFileSystemInformation",
			"when calling goto definition of module",
			session,
		);
	});
	...
	describe("WatchDirectories for config file with", () => {
		function verifyWatchDirectoriesCaseSensitivity(
			...
			it(`watchDirectories for config file with case ${useCaseSensitiveFileNames ? "" : "in"}sensitive file system`, () => {
				const frontendDir =
					"/Users/someuser/work/applications/frontend";
				const file1: File = {
	...
	describe("Verify npm install in directory with tsconfig file works when", () => {
		function verifyNpmInstall(timeoutDuringPartialInstallation: boolean) {
			...
			const getRootedFileOrFolder = <T extends File | Folder>(
				fileOrFolder: T,
			) => {
...
`,
	...
	it("when creating new file in symlinked folder", () => {
		...
		baselineTsserverLogs(
			"cachingFileSystemInformation",
			"when creating new file in symlinked folder",
			session,
		);
	});
});
