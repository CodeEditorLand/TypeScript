...
describe("unittests:: tsserver:: autoImportProvider", () => {
	it("Auto import provider program is not created without dependencies listed in package.json", () => {
		...
		assert.isUndefined(
			session
				.getProjectService()
				.configuredProjects.get(tsconfig.path)!
				.getLanguageService()
				.getAutoImportProvider(),
		);
		baselineTsserverLogs(
			"autoImportProvider",
			"without dependencies listed",
			session,
		);
	});
	...
	it("Auto import provider program is not created if dependencies are already in main program", () => {
		...
		baselineTsserverLogs(
			"autoImportProvider",
			"dependencies are already in main program",
			session,
		);
	});
	...
	it("Auto-import program is not created for projects already inside node_modules", () => {
		...
		baselineTsserverLogs(
			"autoImportProvider",
			"projects already inside node_modules",
			session,
		);
	});
	...
	it("Auto-importable file is in inferred project until imported", () => {
		...
		openFilesForSession([angularFormsDts], session);
		session.logger.log(
			`Default Project for ${angularFormsDts.path}:: ${session.getProjectService().getDefaultProjectForFile(angularFormsDts.path as ts.server.NormalizedPath, /*ensureProject*/ true)?.projectName}`,
		);
	...
	it("Responds to package.json changes", () => {
		...
		baselineTsserverLogs(
			"autoImportProvider",
			"Responds to package_json changes",
			session,
		);
	});
	...
	it("Reuses autoImportProvider when program structure is unchanged", () => {
		...
		openFilesForSession([indexTs], session);
		const autoImportProvider = session
			.getProjectService()
			.configuredProjects.get(tsconfig.path)!
			.getLanguageService()
			.getAutoImportProvider();
	...
	it("Does not schedule ensureProjectForOpenFiles on AutoImportProviderProject creation", () => {
		...
		assert.isFalse(
			session.getProjectService().pendingEnsureProjectForOpenFiles,
		);
	...
	it("Responds to automatic changes in node_modules", () => {
		...
		assert.isTrue(
			completionsBefore?.entries.some(
				(c) => c.name === "PatternValidator",
			),
		);
		...
		assert.isFalse(
			completionsAfter?.entries.some(
				(c) => c.name === "PatternValidator",
			),
		);
		baselineTsserverLogs(
			"autoImportProvider",
			"Responds to automatic changes in node_modules",
			session,
		);
	});
	...
	it("Responds to manual changes in node_modules", () => {
		...
		assert.isTrue(
			completionsAfter?.entries.some(
				(c) => c.name === "ValidatorPattern",
			),
		);
		baselineTsserverLogs(
			"autoImportProvider",
			"Responds to manual changes in node_modules",
			session,
		);
	});
	...
	it("Recovers from an unparseable package.json", () => {
		...
		baselineTsserverLogs(
			"autoImportProvider",
			"Recovers from an unparseable package_json",
			session,
		);
	});
	...
	it("Does not create an auto import provider if there are too many dependencies", () => {
		...
		const packageJson: File = {
			path: "/package.json",
			content: jsonToReadableText(dependencies),
		};
		const { session } = setup([
			...ts.flatten(packages),
			indexTs,
	...
	it("Shared source files between AutoImportProvider and main program do not cause duplicate entries in export info map", () => {
		const files = [
			...
			},
			{
				...
				content: `export declare class Stats {}`,
			},
		...
		const project = session
			.getProjectService()
			.configuredProjects.get("/tsconfig.json")!;
...
describe("unittests:: tsserver:: autoImportProvider - monorepo", () => {
	it("Does not create auto import providers upon opening projects for find-all-references", () => {
		...
		assert.isUndefined(
			session
				.getProjectService()
				.configuredProjects.get("/packages/a/tsconfig.json")!
				.getLanguageService()
				.getAutoImportProvider(),
		);
		baselineTsserverLogs(
	...
	it("Can use the same document registry bucket key as main program", () => {
		for (const option of ts.sourceFileAffectingCompilerOptions) {
			assert(
				!ts.hasProperty(
					ts.server.AutoImportProviderProject
