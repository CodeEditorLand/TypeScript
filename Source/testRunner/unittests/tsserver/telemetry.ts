...
describe("unittests:: tsserver:: project telemetry", () => {
	it("does nothing for inferred project", () => {
		...
		baselineTsserverLogs(
			"telemetry",
			"does nothing for inferred project",
			session,
		);
	});
	...
	it("sends telemetry for typeAcquisition settings", () => {
		...
		baselineTsserverLogs(
			"telemetry",
			"sends telemetry for typeAcquisition settings",
			session,
		);
	});
	...
	it("sends telemetry for file sizes", () => {
		...
		baselineTsserverLogs(
			"telemetry",
			"sends telemetry for file sizes",
			session,
		);
	});
	...
	it("detects whether language service was disabled", () => {
		...
		baselineTsserverLogs(
			"telemetry",
			"detects whether language service was disabled",
			session,
		);
	});
	...
	describe("open files telemetry", () => {
		it("sends event for inferred project", () => {
			...
			baselineTsserverLogs(
				"telemetry",
				"sends event for inferred project",
				session,
			);
		});
		...
		it("even for project with 'ts-check' in config", () => {
			...
			baselineTsserverLogs(
				"telemetry",
				"even for project with ts-check in config",
				session,
			);
		});
	});
...
function makeFile(path: string, content: {} = ""): File {
	return {
		path,
		content: ts.isString(content) ? content : jsonToReadableText(content),
	};
}
