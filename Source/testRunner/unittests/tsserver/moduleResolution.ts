...
describe("unittests:: tsserver:: moduleResolution", () => {
	describe("package json file is edited", () => {
		function setup(packageFileContents: string) {
			...
			return {
				...
				packageFile,
				verifyErr: () =>
					verifyGetErrRequest({ files: [fileA], session }),
			};
		}
	...
	describe("using referenced project", () => {
		...
		function verify(built?: boolean) {
			...
			const { end } = protocolTextSpanFromSubstring(
				indexContent,
				"package-a",
			);
			...
			baselineTsserverLogs(
				"moduleResolution",
				`using referenced project${built ? " built" : ""}`,
				session,
			);
		}
