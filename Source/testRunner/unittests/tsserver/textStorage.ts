...
describe("unittests:: tsserver:: Text storage", () => {
	...
	it("text based storage should be have exactly the same as script version cache", () => {
		...
		const ts1 = new ts.server.TextStorage(
			host,
			getDummyScriptInfo(ts.server.asNormalizedPath(f.path)),
		);
		const ts2 = new ts.server.TextStorage(
			host,
			getDummyScriptInfo(ts.server.asNormalizedPath(f.path)),
		);
		...
		for (let pos = 0; pos < f.content.length; pos++) {
			const { line: line1, offset: offset1 } =
				ts1.positionToLineOffset(pos);
			const { line: line2, offset: offset2 } =
				ts2.positionToLineOffset(pos);
			assert.strictEqual(
				line1,
				line2,
	...
	it("should switch to script version cache if necessary", () => {
		...
		assert.isFalse(
			ts1.hasScriptVersionCache_TestOnly(),
			"should not have script version cache - 3",
		);
	});
	...
	it("should be able to return the file size when a JS file is too large to load into text", () => {
		...
		assert.strictEqual(
			largeFile.content.length,
			ts1.getTelemetryFileSize(),
		);
		baselineTsserverLogs(
			"textStorage",
	...
	it("should return the file size without reloading the file", () => {
		...
		const ts1 = new ts.server.TextStorage(
			host,
			getDummyScriptInfo(ts.server.asNormalizedPath(changingFile.path)),
		);
