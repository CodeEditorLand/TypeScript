...
describe("unittests:: tsserver:: inferredProjects", () => {
	...
	it("project settings for inferred projects", () => {
		...
		setCompilerOptionsForInferredProjectsRequestForSession(
			{
				moduleResolution:
					ts.server.protocol.ModuleResolutionKind.Classic,
			},
			session,
		);
		...
		baselineTsserverLogs(
			"inferredProjects",
			"project settings for inferred projects",
			session,
		);
	});
	...
	it("should support files without extensions", () => {
		...
		openFilesForSession(
			[{ file: f.path, content: f.content, scriptKindName: "JS" }],
			session,
		);
		baselineTsserverLogs(
			"inferredProjects",
			"should support files without extensions",
			session,
		);
	});
	...
	it("inferred projects per project root", () => {
		...
		baselineTsserverLogs(
			"inferredProjects",
			"inferred projects per project root",
			session,
		);
	});
	...
	it("when existing inferred project has no root files", () => {
		...
		baselineTsserverLogs(
			"inferredProjects",
			"when existing inferred project has no root files",
			session,
		);
	});
});
