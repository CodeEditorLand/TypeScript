...
describe("unittests:: tsserver:: typeOnlyImportChains", () => {
	it("named export -> type-only namespace import -> named export -> named import", () => {
		...
		assertUsageError(
			"namedExport typeOnlyNamespaceImport namedExport namedImport",
			[a, b, c],
			c,
		);
	});
	...
	it("named export -> type-only named import -> named export -> named import", () => {
		...
		assertUsageError(
			"namedExport typeOnlyNamedImport namedExport namedImport",
			[a, b, c],
			c,
		);
	});
	...
	it("named export -> type-only namespace import -> export equals -> import equals", () => {
		...
		assertUsageError(
			"namedExport typeOnlyNamespaceImport exportEquals importEquals",
			[a, b, c],
			c,
		);
	});
	...
	it("named export -> type-only export from -> export star from -> named import", () => {
		...
		assertUsageError(
			"namedExport typeOnlyExportFrom exportStarFrom namedImport",
			[a, b, c, d],
			d,
		);
	});
	...
	it("named export -> type-only export from -> export namespace from -> named import", () => {
		...
		assertUsageError(
			"namedExport typeonlyExportFrom exportNamespaceFrom namedImport",
			[a, b, c, d],
			d,
		);
	});
