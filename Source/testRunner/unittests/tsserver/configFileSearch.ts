...
describe("unittests:: tsserver:: configFileSearch:: searching for config file", () => {
	it("should stop at projectRootPath if given", () => {
		...
		baselineTsserverLogs(
			"configFileSearch",
			"should stop at projectRootPath if given",
			session,
		);
	});
	...
	describe("when the opened file is not from project root", () => {
		...
		function openClientFile(files: File[]) {
			...
			const session = new TestSession(host);
			openFilesForSession(
				[{ file, projectRootPath: "/a/b/projects/proj" }],
				session,
			);
			return { host, session };
		}
		...
		it("tsconfig for the file does not exist", () => {
			...
			baselineTsserverLogs(
				"configFileSearch",
				"tsconfig for the file does not exist",
				session,
			);
		});
	});
	...
	describe("should not search and watch config files from directories that cannot be watched", () => {
		function verifyConfigFileWatch(
			scenario: string,
		...
		verifyConfigFileWatch(
			"when projectRootPath is not present",
			/*projectRootPath*/ undefined,
		);
		verifyConfigFileWatch(
			"when projectRootPath is present but file is not from project root",
			"/a/b",
		);
	});
});
