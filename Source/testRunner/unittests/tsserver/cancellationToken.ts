...
describe("unittests:: tsserver:: cancellationToken", () => {
	...
	it("Lower priority tasks are cancellable", () => {
		...
		baselineTsserverLogs(
			"cancellationT",
			"Lower priority tasks are cancellable",
			session,
		);

		function verifyExecuteCommandSeqIsCancellable<
			T extends ts.server.protocol.Request,
		>(request: TestSessionRequest<T>) {
			...
			session.serverCancellationToken.setRequestToCancel(
				session.getNextSeq(),
			);
