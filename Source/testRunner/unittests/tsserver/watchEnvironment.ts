	...
	function verifyCompletionListWithNewFileInSubFolder(
		...
		it(scenario, () => {
			...
			session.executeCommandSeq<ts.server.protocol.CompletionsRequest>({
				...
				arguments: protocolFileLocationFromSubstring(index, '"', {
					index: 1,
				}),
			});
...
describe("unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem Watched recursive directories with windows style file system", () => {
	...
	verifyWatchedDirectories(
		"files at windows style root",
		"c:/",
		/*useProjectAtRoot*/ true,
	);
	verifyWatchedDirectories(
		"files not at windows style root",
		"c:/",
		/*useProjectAtRoot*/ false,
	);
...
describe("unittests:: tsserver:: watchEnvironment:: networkStylePaths", () => {
	it("unittests:: tsserver:: watchEnvironment:: tsserverProjectSystem watching files with network style paths", () => {
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`watching files with network style paths`,
			{ logger },
		);

		function verifyFilePathStyle(
			path: string,
			...
			const host = createServerHost([libFile, file], {
				windowsStyleRoot,
			});
			logger.host = host;
...
describe("unittests:: tsserver:: watchEnvironment:: handles watch compiler options", () => {
	it("with watchFile option as host configuration", () => {
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`with watchFile option as host configuration`,
			session,
		);
	});
	...
	it("with watchDirectory option as host configuration", () => {
		...
		session.executeCommandSeq<ts.server.protocol.ConfigureRequest>({
			...
			arguments: {
				watchOptions: {
					watchDirectory:
						ts.server.protocol.WatchDirectoryKind.UseFsEvents,
				},
			},
		});
		openFilesForSession(
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`with watchDirectory option as host configuration`,
			session,
		);
	});
	...
	it("with fallbackPolling option as host configuration", () => {
		...
		session.executeCommandSeq<ts.server.protocol.ConfigureRequest>({
			...
			arguments: {
				watchOptions: {
					fallbackPolling:
						ts.server.protocol.PollingWatchKind.PriorityInterval,
				},
			},
		});
		openFilesForSession(
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`with fallbackPolling option as host configuration`,
			session,
		);
	});
	...
	it("with watchDirectory option in configFile", () => {
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`with watchDirectory option in configFile`,
			session,
		);
	});
	...
	it("with fallbackPolling option in configFile", () => {
		...
		baselineTsserverLogs(
			"watchEnvironment",
			`with fallbackPolling option in configFile`,
			session,
		);
	});
	...
	describe("excludeDirectories", () => {
		...
		it("with excludeDirectories option in configFile", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`with excludeDirectories option in configFile`,
				session,
			);
		});
		...
		it("with excludeDirectories option in configuration", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`with excludeDirectories option in configuration`,
				session,
			);
		});
		...
		it("external project watch options", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`external project watch options`,
				session,
			);
		});
		...
		it("external project watch options in host configuration", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`external project watch options in host configuration`,
				session,
			);
		});
		...
		it("external project watch options errors", () => {
			...
			session.logger.info(
				jsonToReadableText(project.getAllProjectErrors()),
			);
			baselineTsserverLogs(
				"watchEnvironment",
				`external project watch options errors`,
				session,
			);
		});
		...
		it("inferred project watch options", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`inferred project watch options`,
				session,
			);
		});
		...
		it("inferred project watch options in host configuration", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`inferred project watch options in host configuration`,
				session,
			);
		});
		...
		it("inferred project watch options errors", () => {
			...
			baselineTsserverLogs(
				"watchEnvironment",
				`inferred project watch options errors`,
				session,
			);
		});
	});
...
describe("unittests:: tsserver:: watchEnvironment:: file names on case insensitive file system", () => {
	...
	verifyFileNames(
		"project with ascii file names",
		"/User/userName/Projects/I",
	);
	verifyFileNames(
		"project with ascii file names with i",
		"/User/userName/Projects/i",
	);
	verifyFileNames(
		"project with unicode file names",
		"/User/userName/Projects/İ",
	);
});
...
describe("unittests:: tsserver:: watchEnvironment:: perVolumeCasing", () => {
	it("new file addition", () => {
		...
		host.getCanonicalFileName = (s) =>
			ts.startsWith(s, "/Volumes/") ? s : ts.toFileNameLowerCase(s);
		host.ensureFileOrFolder({
			path: "/Volumes/git/projects/project/foo.ts",
		...
		baselineTsserverLogs(
			"watchEnvironment",
			"perVolumeCasing and new file addition",
			session,
		);
	});
});
