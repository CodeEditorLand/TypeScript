...
import { createWatchUtils, WatchUtils } from "../../../../harness/watchUtils";
import {
...
describe("unittests:: tsserver:: events:: watchEvents", () => {
	interface TestServerHostWithCustomWatch extends TestServerHost {
		factoryData: {
			...
			>;
			watchFile(
				data: ts.server.protocol.CreateFileWatcherEventBody,
			): void;
			watchDirectory(
				data: ts.server.protocol.CreateDirectoryWatcherEventBody,
			): void;
			closeWatcher(
				data: ts.server.protocol.CloseFileWatcherEventBody,
			): void;
		};
	}
	...
	function createTestServerHostWithCustomWatch(
		...
		function watchFile(
			...
			const result = host.factoryData.watchUtils.pollingWatch(
				data.path,
				data,
			);
			idToClose.set(data.id, () => {
				logger.log(
					`Custom watchFile:: Close:: ${data.id}: ${data.path}`,
				);
				result.close();
			});
		}
		...
		function watchDirectory(
			...
			logger.log(
				`Custom watchDirectory: ${data.id}: ${data.path} ${data.recursive}`,
			);
			ts.Debug.assert(!idToClose.has(data.id));
			const result = host.factoryData.watchUtils.fsWatch(
				data.path,
				data.recursive,
				data,
			);
			idToClose.set(data.id, () => {
				logger.log(
					`Custom watchDirectory:: Close:: ${data.id}: ${data.path} ${data.recursive}`,
				);
				result.close();
			});
		}
		...
		function serializeWatches(baseline: string[] = []) {
			if (host.factoryData.watchUtils.getHasWatchChanges())
				host.watchUtils.setHasWatchChanges();
	...
	function updateFileOnHost(session: TestSession, file: string, log: string) {
		...
		session.host.writeFile(
			file,
			session.host.readFile("/user/username/projects/myproject/a.ts")!,
		);
	...
	function addFile(session: TestSession, path: string) {
		...
		(
			session.logger.host as TestServerHostWithCustomWatch
		).factoryData.watchUtils.fsWatchesRecursive.forEach(
	...
	function changeFile(session: TestSession, path: string) {
		...
		(
			session.logger.host as TestServerHostWithCustomWatch
		).factoryData.watchUtils.pollingWatches.forEach(path, (data) =>
			session.executeCommandSeq<ts.server.protocol.WatchChangeRequest>({
				command: ts.server.protocol.CommandTypes.WatchChange,
	...
	it("canUseWatchEvents without canUseEvents", () => {
		...
		baselineTsserverLogs(
			"events/watchEvents",
			`canUseWatchEvents without canUseEvents`,
			session,
		);
	});
});
