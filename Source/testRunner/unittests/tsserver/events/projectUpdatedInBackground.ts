...
describe("unittests:: tsserver:: events:: ProjectsUpdatedInBackground", () => {
	function verifyProjectsUpdatedInBackgroundEvent(
		...
		it("when adding new file", () => {
			...
			baselineTsserverLogs(
				...
				`${scenario} and when adding new file`,
				session,
			);
		});
		...
		describe("with --out or --outFile setting", () => {
			...
			verifyEventWithOutSettings("when --out is set", {
				out: "/a/out.js",
			});
			verifyEventWithOutSettings("when --outFile is set", {
				outFile: "/a/out.js",
			});
		});

		describe("with modules and configured project", () => {
			const file1Consumer1Path =
				"/users/username/projects/project/file1Consumer1.ts";
			const moduleFile1Path =
			...
			function getInitialState({
				...
				const filesToReload =
					firstReloadFileList?.map(
						(fileName) =>
							ts.find(files, (file) => file.path === fileName)!,
					) || files;
				...
				function updateContentOfOpenFile(
					file: File,
					newContent: string,
				) {
			...
			it("should be up-to-date with deleted files", () => {
				const { host, moduleFile1, file1Consumer2, session } =
					getInitialState();
			...
			it("should detect non-existing code file", () => {
				...
				const { host, moduleFile2, updateContentOfOpenFile, session } =
					getInitialState({
						getAdditionalFileOrFolder: () => [referenceFile1],
						firstReloadFileList: [
							referenceFile1.path,
	...
	describe("when event handler is not set but session is created with canUseEvents = true", () => {
		describe("without noGetErrOnBackgroundUpdate, diagnostics for open files are queued", () => {
			verifyProjectsUpdatedInBackgroundEvent(
				"without noGetErrOnBackgroundUpdate",
				(host) =>
					new TestSession({
						host,
		...
		describe("with noGetErrOnBackgroundUpdate, diagnostics for open file are not queued", () => {
			verifyProjectsUpdatedInBackgroundEvent(
				"with noGetErrOnBackgroundUpdate",
				(host) => new TestSession(host),
			);
		});
	});
});
