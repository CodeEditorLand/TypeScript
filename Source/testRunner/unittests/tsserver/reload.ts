...
describe("unittests:: tsserver:: reload", () => {
	it("should work with temp file", () => {
		...
		const snap1 = session
			.getProjectService()
			.getScriptInfo(f1.path)!
			.getSnapshot();
		session.logger.log(
			`Content of ${f1.path}:: ${ts.getSnapshotText(snap1)}`,
		);
		...
		const snap2 = session
			...
			.getSnapshot();
		session.logger.log(
			`Content of ${f1.path}:: ${ts.getSnapshotText(snap2)}`,
		);
	...
	it("should work when script info doesnt have any project open", () => {
		...
		baselineTsserverLogs(
			"reload",
			"should work when script info doesnt have any project open",
			session,
		);
		...
		function checkInferredProjectIsOrphan() {
			...
			session.logger.log(
				`info:: ${info.path}:: ${info.containingProjects.map((p) => p.projectName).join(",")}`,
			);
		}

		function checkScriptInfoAndProjects(captionForContents: string) {
			assert.strictEqual(
				session.getProjectService().getScriptInfo(f1.path),
				info,
			);
		...
		function checkScriptInfoContents(captionForContents: string) {
			const snap = info.getSnapshot();
			session.logger.log(
				`${captionForContents}:: Content of ${info.fileName}:: ${ts.getSnapshotText(snap)}`,
			);
		}
	});
});
