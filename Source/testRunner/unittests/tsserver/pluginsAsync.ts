...
describe("unittests:: tsserver:: pluginsAsync:: async loaded plugins", () => {
	...
	it("plugins are not loaded immediately", async () => {
		...
		session.logger.log(
			`hasNewPluginEnablementRequests:: ${projectService.hasNewPluginEnablementRequests()}`,
		);
		...
		session.logger.log(
			`hasPendingPluginEnablements:: ${projectService.hasPendingPluginEnablements()}`,
		);
		...
		session.logger.log(
			`pluginModuleInstantiated:: ${pluginModuleInstantiated}`,
		);
		...
		baselineTsserverLogs(
			"pluginsAsync",
			"plugins are not loaded immediately",
			session,
		);
	});
	...
	it("plugins evaluation in correct order even if imports resolve out of order", async () => {
		...
		host.importPlugin = async (
			...
			const promise =
				moduleName === "plugin-a"
					? pluginADeferred.promise
					: pluginBDeferred.promise;
			await promise;
	...
	it("sends projectsUpdatedInBackground event", async () => {
		...
		baselineTsserverLogs(
			"pluginsAsync",
			"sends projectsUpdatedInBackground event",
			session,
		);
	});
	...
	it("adds external files", async () => {
		...
		session.logger.log(
			`External files before plugin is loaded: ${project.getExternalFiles().join(",")}`,
		);
	...
	it("project is closed before plugins are loaded", async () => {
		...
		baselineTsserverLogs(
			"pluginsAsync",
			"project is closed before plugins are loaded",
			session,
		);
	});
});
