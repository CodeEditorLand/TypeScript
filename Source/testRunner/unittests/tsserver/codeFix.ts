...
describe("unittests:: tsserver:: codeFix::", () => {
	function setup() {
		...
		const actions =
			session.executeCommandSeq<ts.server.protocol.GetCombinedCodeFixRequest>(
				{
	...
	it("install package", () => {
		...
		actions.commands?.forEach((command) =>
			session.executeCommandSeq<ts.server.protocol.ApplyCodeActionCommandRequest>(
				{
					command:
						ts.server.protocol.CommandTypes.ApplyCodeActionCommand,
					arguments: {
						command,
					},
				},
	...
	it("install package when serialized", () => {
		...
		baselineTsserverLogs(
			"codeFix",
			"install package when serialized",
			session,
		);
	});
});
