...
import { baselineTsserverLogs, TestSession } from "../helpers/tsserver";
...
`,
		...
		const loadingIndicatorScriptInfo = session
			.getProjectService()
			.getScriptInfo(files[3].path)!;
		...
		assert(
			loadingIndicatorScriptInfo.containingProjects.find(
				(p) => p.projectName === "/packages/babel-loader/tsconfig.json",
			),
		);
