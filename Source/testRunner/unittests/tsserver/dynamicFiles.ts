...
describe("unittests:: tsserver:: dynamicFiles:: Untitled files", () => {
	...
	it("Can convert positions to locations", () => {
		...
		baselineTsserverLogs(
			"dynamicFiles",
			"untitled can convert positions to locations",
			session,
		);
	});
	...
	it("opening untitled files", () => {
		...
		verifyDynamic(
			session,
			`/user/username/projects/myproject/${untitledFile}`,
		);
	...
	it("when changing scriptKind of the untitled files", () => {
		...
		const program = session
			.getProjectService()
			.inferredProjects[0].getCurrentProgram()!;
		...
		baselineTsserverLogs(
			"dynamicFiles",
			"when changing scriptKind of the untitled files",
			session,
		);
	});
});
...
describe("unittests:: tsserver:: dynamicFiles:: ", () => {
	it("dynamic file without external project", () => {
		...
		openFilesForSession(
			[{ file: file.path, content: "var x = 10;" }],
			session,
		);
		...
		baselineTsserverLogs(
			"dynamicFiles",
			"dynamic file without external project",
			session,
		);
	});
	...
	describe("dynamic file with projectRootPath", () => {
		...
		it("with useInferredProjectPerProjectRoot", () => {
			...
			verifyDynamic(
				session,
				`/user/username/projects/myproject/${file.path}`,
			);
			...
			openFilesForSession(
				[{ file: file2Path, content: file.content }],
				session,
			);
			baselineTsserverLogs(
				"dynamicFiles",
