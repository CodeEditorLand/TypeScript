...
describe("unittests:: tsserver:: completionsIncomplete", () => {
	it("works", () => {
		const excessFileCount =
			ts.Completions.moduleSpecifierResolutionLimit + 50;
		const exportingFiles = createExportingModuleFiles(
			`/lib/a`,
		...
		typeToTriggerCompletions(indexFile.path, "a", (completions) => {
			...
			assert.lengthOf(
				completions.entries.filter(
					(entry) =>
						entry.source && !(entry.data as any)?.moduleSpecifier,
				),
				excessFileCount,
			);
	...
	it("ambient module specifier resolutions do not count against the resolution limit", () => {
		...
		typeToTriggerCompletions(indexFile.path, "a", (completions) => {
			...
			assert.lengthOf(
				completions.entries.filter(
					(e) => (e.data as any)?.moduleSpecifier,
				),
	...
	it("works with PackageJsonAutoImportProvider", () => {
		...
		baselineTsserverLogs(
			"completionsIncomplete",
			"works with PackageJsonAutoImportProvider",
			session,
		);
	});
	...
	it("works for transient symbols between requests", () => {
		...
		baselineTsserverLogs(
			"completionsIncomplete",
			"works for transient symbols between requests",
			session,
		);
	});
});
...
function setup(files: File[]) {
	...
	function typeToTriggerCompletions(
		...
		function type(
			typedCharacters: string,
			cb:
				| ((completions: ts.server.protocol.CompletionInfo) => void)
				| undefined,
				...
				session.executeCommandSeq<ts.server.protocol.CompletionsRequest>(
					{
						...
						arguments: {
							...
							triggerKind: isIncompleteContinuation
								? ts.server.protocol.CompletionTriggerKind
			...
			return {
				...
				continueTyping: (
					typedCharacters: string,
					cb: (
						completions: ts.server.protocol.CompletionInfo,
					) => void,
				) => {
		...
		function backspace(n = 1) {
			...
			const startLineCharacter = ts.getLineAndCharacterOfPosition(
				file,
				file.text.length - n,
			);
			const endLineCharacter = ts.getLineAndCharacterOfPosition(
				file,
				file.text.length,
			);
