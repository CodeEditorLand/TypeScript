...
describe("unittests:: tsserver:: Session:: General functionality", () => {
	...
	describe("executeCommand", () => {
		...
		it("should handle literal types in request", () => {
			...
			assert.equal(
				session
					.getProjectService()
					.getFormatCodeOptions("" as ts.server.NormalizedPath)
					.indentStyle,
			...
			const setOptionsRequest: ts.server.protocol.SetCompilerOptionsForInferredProjectsRequest =
				{
					...
					arguments: {
						options: {
							...
							moduleResolution:
								ts.server.protocol.ModuleResolutionKind.Node,
						},
					},
				};
	...
	describe("onMessage", () => {
		const allCommandNames: ts.server.protocol.CommandTypes[] =
			Object.values((ts.server.protocol as any).CommandTypes);
	...
	describe("send", () => {
		it("is an overrideable handle which sends protocol messages over the wire", () => {
			const msg: ts.server.protocol.Request = {
				seq: 0,
				type: "request",
				command: "",
			};
	...
	it("has access to the project service", () => {
		new (class extends TestSession {
			constructor() {
				...
				expect(this.projectService).to.be.instanceOf(
...
describe("unittests:: tsserver:: Session:: an example of using the Session API to create an in-process server", () => {
	class InProcSession extends ts.server.Session {
		...
		constructor(private client: InProcClient) {
			...
			this.addProtocolHandler(
				"echo",
				(req: ts.server.protocol.Request) => ({
		...
		handleRequest(msg: ts.server.protocol.Request) {
			...
			try {
				response = this.executeCommand(msg)
					.response as ts.server.protocol.Response;
			} catch (e) {
				this.doOutput(
					/*info*/ undefined,
					msg.command,
	...
	class InProcClient {
		...
		execute(
			command: string,
			args: any,
			callback: (resp: ts.server.protocol.Response) => void,
		): void {
			if (!this.server) {
				return;
			}
...
describe("unittests:: tsserver:: Session:: helpers", () => {
	it(ts.server.getLocationInNewDocument.name, () => {
		...
		const res = ts.server.getLocationInNewDocument(
			text,
			fileName,
