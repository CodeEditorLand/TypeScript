...
describe("unittests:: tsserver:: ConfiguredProjects", () => {
	it("create configured project without file list", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"create configured project without file list",
			session,
		);
	});
	...
	it("create configured project with the file list", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"create configured project with the file list",
			session,
		);
	});
	...
	it("handle recreated files correctly", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"handle recreated files correctly",
			session,
		);
	});
	...
	it("files explicitly excluded in config file", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"files explicitly excluded in config file",
			session,
		);
	});
	...
	it("should properly handle module resolution changes in config file", () => {
		...
		const session = new TestSession(host);
		openFilesForSession(
			[file1, nodeModuleFile, classicModuleFile],
			session,
		);
	...
	it("files are properly detached when language service is disabled", () => {
		...
		host.getFileSize = (filePath: string) =>
			filePath === f2.path
				? ts.server.maxProgramSizeForNonTsFiles + 1
		...
		for (const f of [f1, f2, f3]) {
			...
			const scriptInfo = session
				.getProjectService()
				.getScriptInfoForNormalizedPath(
					ts.server.toNormalizedPath(f.path),
				)!;
			session.logger.log(
				`Containing projects for ${f.path}:: ${scriptInfo.containingProjects.map((p) => p.projectName).join(",")}`,
			);
		}
...
declare var console: {
	...
	it("when file name starts with ^", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"when file name starts with caret",
			session,
		);
	});
	...
	describe("when creating new file", () => {
		...
		function verifySession(input: VerifySession) {
			it("when error on new file are asked before old one", () => {
				verifySessionWorker(
					input,
					/*errorOnNewFileBeforeOldFile*/ true,
				);
			});
			...
			it("when error on new file are asked after old one", () => {
				verifySessionWorker(
					input,
					/*errorOnNewFileBeforeOldFile*/ false,
				);
			});
		}
...
describe("unittests:: tsserver:: ConfiguredProjects:: non-existing directories listed in config file input array", () => {
	it("should be tolerated without crashing the server", () => {
		...
		baselineTsserverLogs(
			"configuredProjects",
			"should be tolerated without crashing the server",
			session,
		);
	});
