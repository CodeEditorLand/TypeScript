...
describe("unittests:: tsserver:: plugins:: loading", () => {
				...
				module: () => ({
					create(info: ts.server.PluginCreateInfo) {
						info.session?.addProtocolHandler(
							...
							(request) => {
								session.logger.log(
									`addProtocolHandler: ${jsonToReadableText(request)}`,
								);
	...
	it("With local plugins", () => {
		...
		const tsconfig: File = {
			...
			content: jsonToReadableText({
				compilerOptions: {
					plugins: [
						...[...expectedToLoad, ...notToLoad].map((name) => ({
							name,
						})),
	...
	it("With global plugins", () => {
		...
		const { session } = createHostWithPlugin(
			[aTs, tsconfig, libFile],
			[...expectedToLoad, ...notToLoad],
		);
		openFilesForSession([aTs], session);
	...
	it("With session and custom protocol message", () => {
		...
		baselineTsserverLogs(
			"plugins",
			"With session and custom protocol message",
			session,
		);
	});
	...
	it("when plugins use LS to get program and update is pending", () => {
		...
		const { session, host } = createHostWithPlugin([
			aTs,
			tsconfig,
			libFile,
		]);
		...
		baselineTsserverLogs(
			"plugins",
			"when plugins use LS to get program and update is pending",
			session,
		);
	});
	...
	it("gets external files with config file reload", () => {
		...
		baselineTsserverLogs(
			"plugins",
			"gets external files with config file reload",
			session,
		);
	});
});
...
describe("unittests:: tsserver:: plugins:: overriding getSupportedCodeFixes", () => {
	it("getSupportedCodeFixes can be proxied", () => {
		...
		host.require = () => {
			return {
				module: () => ({
					create(info: ts.server.PluginCreateInfo) {
						const proxy =
							Harness.LanguageService.makeDefaultProxy(info);
						proxy.getSupportedCodeFixes = (fileName) => {
							switch (fileName) {
								...
								default:
									...
									return info.languageService.getSupportedCodeFixes(
										fileName,
									);
							}
						};
		...
		baselineTsserverLogs(
			"plugins",
			"getSupportedCodeFixes can be proxied",
			session,
		);
	});
});
...
describe("unittests:: tsserver:: plugins:: supportedExtensions::", () => {
	it("new files with non ts extensions and wildcard matching", () => {
			...
			return {
				module: () => ({
					create(info: ts.server.PluginCreateInfo) {
						const proxy =
							Harness.LanguageService.makeDefaultProxy(info);
						const originalScriptKind =
							info.languageServiceHost.getScriptKind!.bind(
					...
					getExternalFiles: (
						...
						const parseHost: ts.ParseConfigHost = {
							useCaseSensitiveFileNames:
								project.useCaseSensitiveFileNames(),
		...
		baselineTsserverLogs(
			"plugins",
			"new files with non ts extensions with wildcard matching",
			session,
		);
	});
});
