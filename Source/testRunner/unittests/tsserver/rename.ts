...
describe("unittests:: tsserver:: rename", () => {
	...
	it("works with prefixText and suffixText when enabled", () => {
		...
		baselineTsserverLogs(
			"rename",
			"works with prefixText and suffixText when enabled",
			session,
		);
	});
	...
	it("rename behavior is based on file of rename initiation", () => {
		...
		baselineTsserverLogs(
			"rename",
			"rename behavior is based on file of rename initiation",
			session,
		);
	});
	...
	it("with symlinks and case difference", () => {
		...
		const host = createServerHost(
            ...
            `,
				...
				"C:/temp/test/node_modules/project1": {
					symLink: "c:/temp/test/project1",
				},
		...
		baselineTsserverLogs(
			"rename",
			"with symlinks and case difference",
			session,
		);
	});
	...
	it("rename TS file with js extension", () => {
		...
		const bTs: File = {
			path: "/b.ts",
			content: `import * as foo from './a.js';`,
		};
		...
		baselineTsserverLogs(
			"rename",
			"rename TS file with js extension",
			session,
		);
	});
});
