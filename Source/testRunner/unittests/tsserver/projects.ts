...
import { commonFile1, commonFile2 } from "../helpers/tscWatch";
import {
...
describe("unittests:: tsserver:: projects::", () => {
	...
	it("should disable features when the files are too large", () => {
		...
		baselineTsserverLogs(
			"projects",
			"should disable features when the files are too large",
			session,
		);
	});
	...
	describe("ignoreConfigFiles", () => {
		...
		it("loose file included in config file (openClientFile)", () => {
			...
			openFilesForSession(
				[{ file: file1, content: file1.content }],
				session,
			);
			baselineTsserverLogs(
				"projects",
				"loose file included in config file (openClientFile)",
				session,
			);
		});
		...
		it("loose file included in config file (applyCodeChanges)", () => {
			...
			session.executeCommandSeq<ts.server.protocol.ApplyChangedToOpenFilesRequest>(
				{
					...
					arguments: {
						openFiles: [
							{ fileName: file1.path, content: file1.content },
						],
					},
				},
			);
			...
			baselineTsserverLogs(
				"projects",
				"loose file included in config file (applyCodeChanges)",
				session,
			);
		});
	});
	...
	it("reload regular file after closing", () => {
		...
		openFilesForSession(
			[f1, { file: f2.path, content: "let x: string" }],
			session,
		);
		...
		baselineTsserverLogs(
			"projects",
			"reload regular file after closing",
			session,
		);
	});
	...
	it("clear mixed content file after closing", () => {
		...
		openFilesForSession(
			[f1, { file: f2.path, content: "let somelongname: string" }],
			session,
		);
		...
		baselineTsserverLogs(
			"projects",
			"clear mixed content file after closing",
			session,
		);
	});
	...
	it("changes in closed files are reflected in project structure", () => {
		...
		baselineTsserverLogs(
			"projects",
			"changes in closed files are reflected in project structure",
			session,
		);
	});
	...
	it("deleted files affect project structure", () => {
		...
		baselineTsserverLogs(
			"projects",
			"deleted files affect project structure",
			session,
		);
	});
	...
	it("ignores files excluded by a custom safe type list", () => {
		...
		baselineTsserverLogs(
			"projects",
			"ignores files excluded by a custom safe type list",
			session,
		);
	});
	...
	it("ignores files excluded by the default type list", () => {
		...
		baselineTsserverLogs(
			"projects",
			"ignores files excluded by the default type list",
			session,
		);
	});
	...
	it("ignores files excluded by a legacy safe type list", () => {
		...
		baselineTsserverLogs(
			"projects",
			"ignores files excluded by a legacy safe type list",
			session,
		);
	});
	...
	it("correctly migrate files between projects", () => {
		...
		baselineTsserverLogs(
			"projects",
			"correctly migrate files between projects",
			session,
		);
	});
	...
	it("regression test for crash in acquireOrUpdateDocument", () => {
		...
		openFilesForSession(
			[{ file: "/a/b/file1.js", content: "var x = 10;" }],
			session,
		);
		baselineTsserverLogs(
			"projects",
			"regression test for crash in acquireOrUpdateDocument",
			session,
		);
	});
	...
	it("loading files with correct priority", () => {
		...
		baselineTsserverLogs(
			"projects",
			"loading files with correct priority",
			session,
		);
	});
	...
	it("no tsconfig script block diagnostic errors", () => {
		...
		verfiy(
			...
			createServerHost([file1, file2, config1, libFile], {
				executingFilePath: ts.combinePaths(
					ts.getDirectoryPath(libFile.path),
					"tsc.js",
				),
			}),
		);
		...
		baselineTsserverLogs(
			"projects",
			"no tsconfig script block diagnostic errors",
			session,
		);
	...
	it("files with mixed content are handled correctly", () => {
		...
		openFilesForSession(
			[{ file: file1.path, content: file1.content }],
			session,
		);
		...
		baselineTsserverLogs(
			"projects",
			"files with mixed content are handled correctly",
			session,
		);
	});
	...
	it("syntax tree cache handles changes in project settings", () => {
		...
		baselineTsserverLogs(
			"projects",
			"syntax tree cache handles changes in project settings",
			session,
		);
	});
	...
	it("File in multiple projects at opened and closed correctly", () => {
		...
		baselineTsserverLogs(
			"projects",
			"File in multiple projects at opened and closed correctly",
			session,
		);
	});
	...
	it("snapshot from different caches are incompatible", () => {
		...
		baselineTsserverLogs(
			"projects",
			"snapshot from different caches are incompatible",
			session,
		);
	});
	...
	it("Properly handle Windows-style outDir", () => {
		...
		baselineTsserverLogs(
			"projects",
			"Properly handle Windows-style outDir",
			session,
		);
	});
	...
	it("files opened and closed affecting multiple projects", () => {
		...
		baselineTsserverLogs(
			"projects",
			"files opened and closed affecting multiple projects",
			session,
		);
	});
	...
	describe("includes deferred files in the project context", () => {
		function verifyDeferredContext(
			...
			if (lazyConfiguredProjectsFromExternalProject) {
				...
				session.logger.info(
					"Calling ensureInferredProjectsUpToDate_TestOnly",
				);
				session
					.getProjectService()
	...
	it("Orphan source files are handled correctly on watch trigger", () => {
		...
		baselineTsserverLogs(
			"projects",
			"Orphan source files are handled correctly on watch trigger",
			session,
		);

		function verifyFile2InfoIsOrphan() {
			const info = ts.Debug.checkDefined(
				session
					.getProjectService()
					.getScriptInfoForPath(file2.path as ts.Path),
			);
			session.logger.log(
  ...
  "references": [
    { "path": "../A" }
		...
		baselineTsserverLogs(
			"projects",
			"synchronizeProjectList provides redirect info when requested",
			session,
		);
	});
  ...
  "include": [
	...
	it("does not look beyond node_modules folders for default configured projects", () => {
		...
		const rootProjectPath = ts.server.asNormalizedPath(
			"/project/tsconfig.json",
		);
		const nodeModulesFilePath1 = ts.server.asNormalizedPath(
			"/project/node_modules/@types/a/index.d.ts",
		);
		const nodeModulesProjectPath1 = ts.server.asNormalizedPath(
			"/project/node_modules/@types/a/tsconfig.json",
		);
		const nodeModulesFilePath2 = ts.server.asNormalizedPath(
		...
		openFilesForSession(
			[rootFilePath, nodeModulesFilePath1, nodeModulesFilePath2],
			session,
		);
		baselineTsserverLogs(
			"projects",
	...
	describe("file opened is in configured project that will be removed", () => {
		...
		it("js file opened is in configured project that will be removed", () => {
			...
			baselineTsserverLogs(
				"projects",
				"js file opened is in configured project that will be removed",
				session,
			);
		});
	});
