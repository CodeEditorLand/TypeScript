...
import { compilerOptionsToConfigJson, libContent } from "../helpers/contents";
...
declare global {
    namespace JSX {
			...
			fs: () =>
				loadProjectFromFiles({
					"/src/project/node_modules/react/jsx-runtime.js":
						"export {}", // js needs to be present so there's a resolution result
		...
		verifyTsc({
			scenario: "react-jsx-emit-mode",
			subScenario:
				"with no backing types found doesn't crash under --strict",
			fs: () =>
				loadProjectFromFiles({
					"/src/project/node_modules/react/jsx-runtime.js":
						"export {}", // js needs to be present so there's a resolution result
	...
	verifyTsc({
		...
		edits: [
			...
			},
			{
				...
				edit: (fs) =>
					fs.unlinkSync("/src/projects/project1/class3.d.ts"),
				discrepancyExplanation: () => [
	...
	verifyTsc({
		scenario: "incremental",
		subScenario:
			"ts file with no-default-lib that augments the global scope",
		fs: () =>
			loadProjectFromFiles({
				"/src/project/src/main.ts": Utils.dedent`
	...
	verifyTsc({
		scenario: "incremental",
		subScenario:
			"change to type that gets used as global through export in another file",
		commandLineArgs: ["-p", `src/project`],
		fs: () =>
			loadProjectFromFiles({
				"/src/project/tsconfig.json": jsonToReadableText({
...
console.log(a);`,
	...
	verifyTsc({
		scenario: "incremental",
		subScenario:
			"change to type that gets used as global through export in another file through indirect import",
		commandLineArgs: ["-p", `src/project`],
		fs: () =>
			loadProjectFromFiles({
				"/src/project/tsconfig.json": jsonToReadableText({
...
console.log(a);`,
		...
		edits: [
			{
				...
				edit: (fs) =>
					replaceText(fs, "/src/project/a.ts", "x", "x: 20"),
			...
			},
			{
				...
				edit: (fs) =>
					replaceText(fs, "/src/project/a.ts", "x: 20", "x"),
				commandLineArgs: ["--p", "/src/project", "--declarationMap"],
			},
		],
	});
	...
	describe("different options::", () => {
		...
		function withOptionChangeAndDiscrepancyExplanation(
			caption: string,
			option: string,
		): TestTscEdit {
			return {
		...
		function withEmitDeclarationOnlyChangeAndDiscrepancyExplanation(
			caption: string,
		): TestTscEdit {
			const edit = withOptionChangeAndDiscrepancyExplanation(
				caption,
				"--emitDeclarationOnly",
			);
			...
			return {
				...
				edit: (fs) => {
					const config = JSON.parse(
						fs.readFileSync("/src/project/tsconfig.json", "utf-8"),
					);
					config.compilerOptions.declarationMap = true;
					fs.writeFileSync(
						"/src/project/tsconfig.json",
						jsonToReadableText(config),
					);
				},
			};
		}
		verifyTsc({
			...
			edits: [
				...
				noChangeWithSubscenario(
					"should re-emit only js so they dont contain sourcemap",
				),
				...
				noChangeWithSubscenario(
					"should re-emit only dts so they dont contain sourcemap",
				),
				withOptionChangeAndDiscrepancyExplanation(
				...
				withOptionChange(
					"with sourceMap should not emit d.ts",
					"--sourceMap",
				),
			],
			baselinePrograms: true,
