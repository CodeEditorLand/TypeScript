...
import { baselineBuildInfo, CommandLineProgram } from "../helpers/baseline";
import { applyEdit, createBaseline, watchBaseline } from "../helpers/tscWatch";
import {
	createWatchedSystem,
	File,
	libFile,
...
describe("unittests:: tsc:: builder cancellationToken", () => {
	...
	function verifyCancellation(useBuildInfo: boolean, scenario: string) {
		it(scenario, () => {
			...
			const { sys, baseline } = createBaseline(
				createWatchedSystem(
			...
			try {
				...
				sys.write(
					`Operation ws cancelled:: ${e instanceof ts.OperationCanceledException}\r\n`,
				);
			}
			cancel = false;
