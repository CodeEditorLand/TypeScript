...
describe("unittests:: compilerCore", () => {
	describe("equalOwnProperties", () => {
		it("correctly equates objects", () => {
			...
			assert.isTrue(
				ts.equalOwnProperties({ a: 1, b: 2 }, { b: 2, a: 1 }),
			);
		});
		it("correctly identifies unmatched objects", () => {
			assert.isFalse(
				ts.equalOwnProperties({}, { a: 1 }),
			...
			assert.isFalse(
				ts.equalOwnProperties({ a: 1 }, { a: 2 }),
				"differing property",
			);
		});
		it("correctly identifies undefined vs hasOwnProperty", () => {
			assert.isFalse(
				ts.equalOwnProperties({}, { a: undefined }),
				"missing left property",
			);
			assert.isFalse(
				ts.equalOwnProperties({ a: undefined }, {}),
		...
		it("truthiness", () => {
			...
			assert.isFalse(
				ts.equalOwnProperties({}, { a: 1 }, trythyTest),
				"missing left truthy property",
			);
			assert.isFalse(
				ts.equalOwnProperties({}, { a: 0 }, trythyTest),
		...
		it("all equal", () => {
			assert.isFalse(
				ts.equalOwnProperties({}, { a: 1 }, () => true),
				"missing left property",
			);
			assert.isFalse(
				ts.equalOwnProperties({ a: 1 }, {}, () => true),
			...
			assert.isTrue(
				ts.equalOwnProperties({ a: 1 }, { a: 2 }, () => true),
				"valid equality",
			);
		});
	});
	describe("customSet", () => {
		it("mutation", () => {
			const set = ts.createSet<number, number>(
				(x) => x % 2,
				(x, y) => x % 4 === y % 4,
			);
		...
		it("string hash code", () => {
			...
			const set = ts.createSet<Thing, string>(
				(t) => t.y,
				(t, u) => t.x === u.x && t.y === u.y,
			);
