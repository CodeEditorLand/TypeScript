...
describe("unittests:: tsbuild:: outFile::", () => {
	...
	before(() => {
		outFileFs = loadProjectFromFiles({
			...
			"/src/first/tsconfig.json": jsonToReadableText({
				...
				files: ["first_PART1.ts", "first_part2.ts", "first_part3.ts"],
				references: [],
			}),
	...
	function getOutFileFsAfterBuild() {
		...
		const builder = ts.createSolutionBuilder(host, ["/src/third"], {
			dry: false,
			force: false,
			verbose: true,
		});
	...
	verifyTsc({
		...
		modifyFs: (fs) =>
			fs.unlinkSync("/src/first/bin/first-output.tsbuildinfo"),
	});
	...
	verifyTsc({
		scenario: "outFile",
		subScenario:
			"rebuilds completely when command line incremental flag changes between non dts changes",
		fs: () => outFileFs,
	...
	describe("Prepend output with .tsbuildinfo", () => {
		...
		describe("emitHelpers", () => {
			...
			verifyOutFileScenario({
				...
				modifyAgainFs: (fs) =>
					changeStubToRest(fs, "first", "first_PART1"),
				ignoreDtsChanged: true,
				baselineOnly: true,
			});
...
${internal} enum internalEnum { a, b, c }`,
			...
			verifyOutFileScenario({
				subScenario:
					"stripInternal jsdoc style with comments emit enabled",
				modifyFs: (fs) =>
					stripInternalScenario(
						fs,
    ...
    export interface SourceFile {
        someProp: string;
			...
			verifyOutFileScenario({
				subScenario:
					"stripInternal when few members of enum are internal",
				modifyFs: (fs) => {
					stripInternalOfThird(fs);
					prependText(
						fs,
...
`,
			...
			verifyOutFileScenario({
				subScenario:
					"stripInternal when prepend is completely internal",
				baselineOnly: true,
				ignoreDtsChanged: true,
				ignoreDtsUnchanged: true,
				modifyFs: (fs) => {
