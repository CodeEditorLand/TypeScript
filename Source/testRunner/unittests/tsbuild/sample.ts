...
import { libContent, libPath } from "../helpers/contents";
import {
...
describe("unittests:: tsbuild:: on 'sample1' project", () => {
	...
	function getSampleFsAfterBuild() {
		...
		const host = createSolutionBuilderHostForBaseline(
			sys as TscCompileSystem,
		);
	...
	describe("sanity check of clean build of 'sample1' project", () => {
		...
		verifyTsc({
			...
			modifyFs: (fs) =>
				replaceText(fs, "core/tsconfig.json", `"composite": true,`, ""),
		});
	});
	...
	describe("can detect when and what to rebuild", () => {
		verifyTsc({
			...
			edits: [
				// Update a file in the leaf node (tests), only it should rebuild the last one
				{
					...
					edit: (fs) =>
						fs.writeFileSync("tests/index.ts", "const m = 10;"),
				},
		...
		verifyTsc({
			...
			modifyFs: (fs) => {
				fs.writeFileSync(
					...
					jsonToReadableText({ compilerOptions: { target: "es3" } }),
				);
				replaceText(
					fs,
					"tests/tsconfig.json",
		...
		it("building using getNextInvalidatedProject", () => {
			...
			function verifyBuildNextResult() {
				...
				baseline.push(
					`Project Result:: ${jsonToReadableText({ project: project?.project, result })}`,
				);
				system.serializeState(
					baseline,
	...
	describe("project invalidation", () => {
		it("invalidates projects correctly", () => {
			...
			const status = builder.getUpToDateStatusOfProject(
				"logic/tsconfig.json",
			);
			baseline.push(
				`Project should still be upto date: ${ts.UpToDateStatusType[status.type]}`,
			);
			...
			Harness.Baseline.runBaseline(
				`tsbuild/sample1/invalidates-projects-correctly.js`,
				baseline.join("\r\n"),
			);
			...
			edits: [
				{
					caption: "incremental-declaration-changes",
					edit: (fs) =>
						replaceText(fs, "core/tsconfig.json", "esnext", "es5"),
				},
			],
		});
		...
		verifyTsc({
			...
			modifyFs: (fs) =>
				fs.writeFileSync(
					"tests/tsconfig.json",
					jsonToReadableText({
						references: [{ path: "../core" }, { path: "../logic" }],
						files: ["index.ts"],
						compilerOptions: {
							composite: true,
							declaration: true,
