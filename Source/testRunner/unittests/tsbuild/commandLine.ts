...
import { noChangeRun, TestTscEdit, verifyTsc } from "../helpers/tsc";
import { appendText, loadProjectFromFiles, replaceText } from "../helpers/vfs";
...
describe("unittests:: tsbuild:: commandLine::", () => {
	...
	describe("emitDeclarationOnly::", () => {
		...
		function verifyWithIncremental(options: ts.CompilerOptions) {
			verifyTsc({
				...
				edits: [
					...
					},
					{
						...
						discrepancyExplanation:
							options.incremental && options.outFile
								? undefined
								: () => [
			...
			verifyTsc({
				...
				subScenario: subScenario(
					"emitDeclarationOnly false on commandline",
				),
		...
		verifyTsc({
			scenario: "commandLine",
			subScenario:
				"emitDeclarationOnly false on commandline with declaration",
		...
		verifyTsc({
			scenario: "commandLine",
			subScenario:
				"emitDeclarationOnly on commandline with declaration with outFile",
			fs: () =>
				fs({
					declaration: true,
					outFile: "../outFile.js",
		...
		verifyTsc({
			scenario: "commandLine",
			subScenario:
				"emitDeclarationOnly false on commandline with declaration with outFile",
			fs: () =>
				fs({
					declaration: true,
