...
describe("unittests:: tsbuild:: fileDelete::", () => {
	function fs(
		childOptions: ts.CompilerOptions,
		mainOptions?: ts.CompilerOptions,
	) {
			...
			...(mainOptions
                ...
                `,
						"/src/main/tsconfig.json": jsonToReadableText({
							compilerOptions:
								compilerOptionsToConfigJson(mainOptions),
	...
	verifyTsc({
		...
		fs: () =>
			fs({ outFile: "../childResult.js", module: ts.ModuleKind.AMD }),
		edits: [
			{
				caption: "delete child2 file",
				edit: (fs) => fs.rimrafSync("/src/child/child2.ts"),
