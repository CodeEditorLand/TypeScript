...
import { noChangeRun, verifyTsc } from "../helpers/tsc";
import { loadProjectFromFiles } from "../helpers/vfs";

describe("unittests:: tsbuild:: noEmit", () => {
	function verifyNoEmitWorker(
		subScenario: string,
		aTsContent: string,
		...
		verifyTsc({
			...
			edits: [
				noChangeRun,
				{
					caption: "Fix error",
					edit: (fs) =>
						fs.writeFileSync("/src/a.ts", `const a = "hello"`),
				},
				noChangeRun,
			],
	...
	function verifyNoEmit(subScenario: string, aTsContent: string) {
		verifyNoEmitWorker(subScenario, aTsContent, [
			"--b",
			"/src/tsconfig.json",
			"-v",
		]);
