...
describe("unittests:: tsc-watch:: watchEnvironment:: tsc-watch with different polling/non polling options", () => {
		...
		edits: [
			{
				...
				timeouts: (sys, programs) => {
					...
					const mediumPollingIntervalThreshold =
						ts.unchangedPollThresholds[ts.PollingInterval.Medium];
					for (
						let index = 0;
			...
			},
			{
				...
				timeouts: (sys, programs) => {
					...
					const mediumPollingIntervalThreshold =
						ts.unchangedPollThresholds[ts.PollingInterval.Medium];
					const newThreshold =
						ts.unchangedPollThresholds[ts.PollingInterval.Low] +
	...
	verifyTscWatch({
		...
		edits: [
			...
			},
			{
				...
				// Make a change to file
				edit: (sys) =>
					sys.writeFile(commonFile1.path, "var zz30 = 100;"),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
			{
	...
	describe("tsc-watch when watchDirectories implementation", () => {
		...
		verifyTscWatch({
			scenario,
			subScenario:
				"watchDirectories/with non synchronous watch directory",
			commandLineArgs: [
				"--w",
				"-p",
			...
			edits: [
				{
					caption:
						"Directory watch updates because of file1.js creation",
					edit: ts.noop,
					timeouts: (sys) => {
	...
	describe("handles watch compiler options", () => {
		...
		verifyTscWatch({
			scenario,
			subScenario:
				"watchOptions/with watchFile as watch options to extend",
			commandLineArgs: [
				"-w",
				"-p",
				"/a/b/tsconfig.json",
			...
			function verifyWorker(...additionalFlags: string[]) {
				...
				verifyTscWatch({
					...
					edits: [
						{
							caption:
								"Directory watch updates because of main.js creation",
							edit: ts.noop,
	...
	verifyTscWatch({
		...
		edits: [
			{
				caption:
					"Introduce error such that when callback happens file is already appeared",
	...
	describe("with fsWatch on inodes", () => {
		verifyTscWatch({
			...
			edits: [
				{
					caption:
						"Replace file with rename event that introduces error",
					edit: (sys) =>
						sys.modifyFile(
					...
					createWatchedSystem(
						{
							...
							"/user/username/projects/myproject/tsconfig.json":
								jsonToReadableText({ files: ["main.ts"] }),
						},
						{
							currentDirectory:
	...
	verifyTscWatch({
		...
		edits: [
			...
			{
				caption:
					"receive another change event without modifying the file",
				edit: (sys) =>
					sys.invokeFsWatches(
						"/user/username/projects/project/main.ts",
						"change",
