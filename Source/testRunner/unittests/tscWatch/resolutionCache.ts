...
describe("unittests:: tsc-watch:: resolutionCache:: tsc-watch module resolution caching", () => {
	...
	it("loads missing files from disk", () => {
		...
		const { sys, baseline, cb, getPrograms } = createBaseline(
			createWatchedSystem([root, libFile]),
		);
		const host =
			createWatchCompilerHostOfFilesAndCompilerOptionsForBaseline({
				rootFiles: [root.path],
			...
			edits: [
				{
					...
					timeouts: (sys) => {
						sys.runQueuedTimeoutCallbacks();
						assert.isTrue(
							fileExistsCalledForBar,
							"'fileExists' should be called.",
						);
					},
				},
			],
	...
	it("should compile correctly when resolved module goes missing and then comes back (module is not part of the root)", () => {
		...
		runWatchBaseline({
			scenario: "resolutionCache",
			subScenario:
				"should compile correctly when resolved module goes missing and then comes back",
			commandLineArgs: ["--w", root.path],
			sys,
			baseline,
			getPrograms,
		...
		function verifyIgnore(
			...
			verifyTscWatch({
				...
				edits: [
					{
						caption:
							"npm install file and folder that start with '.'",
						edit: (sys) =>
							sys.ensureFileOrFolder({
		...
		verifyTscWatch({
			...
			edits: [
				...
				},
				{
					...
					edit: (sys) =>
						sys.ensureFileOrFolder({
							path: `/user/username/projects/myproject/node_modules/@types/node`,
						}),
					timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
				},
				{
