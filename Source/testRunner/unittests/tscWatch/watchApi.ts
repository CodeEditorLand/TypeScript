...
describe("unittests:: tsc-watch:: watchAPI:: tsc-watch with custom module resolution", () => {
	it("verify that module resolution with json extension works when returned without extension", () => {
		...
		const { sys, baseline, cb, getPrograms } = createBaseline(
			createWatchedSystem([libFile, mainFile, config, settingsJson], {
		...
		const parsedCommandResult = ts.parseJsonConfigFileContent(
			configFileJson,
			sys,
			config.path,
		);
		...
		runWatchBaseline({
			scenario: "watchApi",
			subScenario:
				"verify that module resolution with json extension works when returned without extension",
	...
	describe("hasInvalidatedResolutions", () => {
		function verifyWatch(
			subScenario: string,
			implementHasInvalidatedResolution: boolean,
		) {
			it(subScenario, () => {
				const { sys, baseline, cb, getPrograms } = createBaseline(
					createWatchedSystem(
				...
				runWatchBaseline({
					...
					edits: [
						...
						{
							caption:
								"write other with same contents but write ts file",
							edit: (sys) => {
								sys.appendFile(
									`/user/username/projects/myproject/other.d.ts`,
									"",
		...
		verifyWatch(
			"host implements hasInvalidatedResolutions",
			/*implementHasInvalidatedResolution*/ true,
		);
	});
});
...
describe("unittests:: tsc-watch:: watchAPI:: tsc-watch expose error count to watch status reporter", () => {
	it("verify that the error count is correctly passed down to the watch status reporter", () => {
		...
		const { sys, baseline, cb, getPrograms } = createBaseline(
			createWatchedSystem([libFile, mainFile, config], {
				currentDirectory: "/user/username/projects/myproject",
			}),
		);
		const host = createWatchCompilerHostOfConfigFileForBaseline({
		...
		runWatchBaseline({
			scenario: "watchApi",
			subScenario:
				"verify that the error count is correctly passed down to the watch status reporter",
			commandLineArgs: ["--w", "--p", config.path],
			sys,
			baseline,
			getPrograms,
...
describe("unittests:: tsc-watch:: watchAPI:: when watchHost uses createSemanticDiagnosticsBuilderProgram", () => {
	...
	function createSystemForBuilderTest(configText: string, mainText: string) {
		...
		const { sys: emitSys, baseline: emitBaseline } = createSystem(
			configText,
			mainText,
		);
	...
	it("verifies that noEmit is handled on createSemanticDiagnosticsBuilderProgram and typechecking happens only on affected files", () => {
		const { sys, baseline, cb, getPrograms, config, mainFile } =
			createSystem("{}", "export const x = 10;");
		const host = createWatchCompilerHostOfConfigFileForBaseline({
			configFileName: config.path,
		...
		runWatchBaseline({
			scenario: "watchApi",
			subScenario:
				"verifies that noEmit is handled on createSemanticDiagnosticsBuilderProgram",
			commandLineArgs: ["--w", "--p", config.path],
			sys,
			baseline,
			getPrograms,
	...
	it("SemanticDiagnosticsBuilderProgram emitDtsOnly does not update affected files pending emit", () => {
		...
		applyEdit(
			sys,
			baseline,
			(sys) => sys.writeFile(mainFile.path, "export const x = 10;"),
			"Fix error",
		);
		...
		const reportWatchStatus = ts.createWatchStatusReporter(
			sys,
			/*pretty*/ true,
		);
		...
		Harness.Baseline.runBaseline(
			`tscWatch/watchApi/semantic-builder-emitOnlyDts.js`,
			baseline.join("\r\n"),
		);
	});
});
...
describe("unittests:: tsc-watch:: watchAPI:: when creating program with project references but not config file", () => {
	...
	it("when watching referenced project when there is no config file name", () => {
		...
		runWatchBaseline({
			scenario: "watchApi",
			subScenario:
				"when watching referenced project when there is no config file name",
	...
	it("when watching referenced project with extends when there is no config file name", () => {
		...
		runWatchBaseline({
			scenario: "watchApi",
			subScenario:
				"when watching referenced project with extends when there is no config file name",
			commandLineArgs: ["--w", "-p", ".", "--extendedDiagnostics"],
			...baseline,
			edits: [
				{
