...
import { TscWatchCompileChange, verifyTscWatch } from "../helpers/tscWatch";
import {
	createWatchedSystem,
	File,
	libFile,
...
getPoint().c.x;`,
		...
		verifyEmitAndErrorUpdates({
			subScenario:
				"file not exporting a deep multilevel import that changes",
				...
				changes: [
					{
						...
						edit: (sys) =>
							sys.writeFile(
								...
								lib1ToolsInterface.content.replace(
									"title",
									"title2",
								),
							),
					...
					{
						caption:
							"Rename property title to title2 of interface ITest",
						edit: (sys) =>
							sys.writeFile(
								lib1ToolsInterface.path,
								lib1ToolsInterface.content.replace(
