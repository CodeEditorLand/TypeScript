...
describe("unittests:: tsc-watch:: projects with references: invoking when references are already built", () => {
	verifyTscWatch({
		...
		sys: () =>
			solutionBuildWithBaseline(getSysForSampleProjectReferences(), [
				"tests",
			]),
		commandLineArgs: [
			"-w",
		...
		edits: [
			...
			{
				caption:
					"change in project reference config file builds correctly",
				edit: (sys) => {
					sys.writeFile(
	...
	verifyTscWatch({
		...
		sys: () =>
			solutionBuildWithBaseline(
				createWatchedSystem(getFsContentsForTransitiveReferences(), {
		...
		edits: [
			...
			},
			{
				...
				edit: (sys) => {
					...
					changeCompilerOpitonsPaths(sys, "tsconfig.c.json", {
						"@ref/*": ["./nrefs/*"],
					});
				},
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			...
			},
			{
				...
				edit: (sys) =>
					changeCompilerOpitonsPaths(sys, "tsconfig.b.json", {
						"@ref/*": ["./nrefs/*"],
					}),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
		...
		edits: [
			...
			},
			{
				...
				edit: (sys) => {
					...
					changeCompilerOpitonsPaths(sys, "c/tsconfig.json", {
						"@ref/*": ["../nrefs/*"],
					});
				},
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			...
			},
			{
				caption: "edit in referenced config file",
				edit: (sys) =>
					changeCompilerOpitonsPaths(sys, "b/tsconfig.json", {
						"@ref/*": ["../nrefs/*"],
					}),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
