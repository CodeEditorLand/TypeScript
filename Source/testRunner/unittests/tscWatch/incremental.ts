...
describe("unittests:: tsc-watch:: emit file --incremental", () => {
	...
	function verifyIncrementalWatchEmitWorker(
		...
		function build() {
			const closer = ts.executeCommandLine(sys, cb, argsToPass);
			...
			it("verify that state is read correctly", () => {
				...
				ts.performIncrementalCompilation({
					...
					configFileParsingDiagnostics:
						ts.getConfigFileParsingDiagnostics(parsedConfig),
					reportDiagnostic,
					system,
				});
				...
				assert.equal(
					ts.arrayFrom(state.referencedMap!.keys()).length,
					0,
				);
				assert.equal(
					ts.arrayFrom(state.exportedModulesMap!.keys()).length,
					0,
				);

				assert.equal(state.semanticDiagnosticsPerFile!.size, 3);
				assert.deepEqual(
					state.semanticDiagnosticsPerFile!.get(
						libFile.path as ts.Path,
					),
					ts.emptyArray,
				);
				assert.deepEqual(
					state.semanticDiagnosticsPerFile!.get(
						file1.path as ts.Path,
					),
					ts.emptyArray,
				);
				assert.deepEqual(
					state.semanticDiagnosticsPerFile!.get(
						file2.path as ts.Path,
					),
					[
						{
							file: state.program!.getSourceFileByPath(
								file2.path as ts.Path,
							)!,
							start: 13,
							length: 1,
							code: ts.Diagnostics
								.Type_0_is_not_assignable_to_type_1.code,
							category:
								ts.Diagnostics
...
`,
	...
	verifyIncrementalWatchEmit({
		subScenario:
			"when file with ambient global declaration file is deleted",
		files: () => [
			...
			{
				path: `${project}/index.ts`,
				content: `console.log(Config.value);`,
			},
			{
				path: configFile.path,
				content: jsonToReadableText({
