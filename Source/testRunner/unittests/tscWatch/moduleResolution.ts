...
describe("unittests:: tsc-watch:: moduleResolution", () => {
	verifyTscWatch({
		...
		edits: [
			...
			{
				caption:
					"removes those errors when a package file is changed back",
				edit: (sys) =>
					sys.replaceFileText(
						`/user/username/projects/myproject/packages/pkg2/package.json`,
						`other.js`,
	...
	describe("package json file is edited", () => {
		function getSys(packageFileContents: string) {
			...
			return createWatchedSystem(
				[
					...
					fileB,
					packageFile,
					{ ...libFile, path: "/a/lib/lib.es2016.full.d.ts" },
				],
				{ currentDirectory: "/user/username/projects/myproject" },
			);
		}
		verifyTscWatch({
			...
			sys: () =>
				getSys(jsonToReadableText({ name: "app", version: "1.0.0" })),
			edits: [
				{
	...
	verifyTscWatch({
		scenario: "moduleResolution",
		subScenario:
			"module resolutions from files with partially used import attributes",
		sys: () =>
			createWatchedSystem(
				[
					{
	...
	verifyTscWatch({
		...
		sys: () =>
			createWatchedSystem(getFsContentsForAlternateResult(), {
