...
describe("unittests:: tsc-watch:: program updates", () => {
		...
		edits: [
			{
				...
				edit: (sys) =>
					sys.appendFile(commonFile2.path, ";let xy = 10;"),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
			{
	...
	verifyTscWatch({
		scenario,
		subScenario:
			"Updates diagnostics when '--allowArbitraryExtensions' changes",
		commandLineArgs: ["-w", "-p", "/tsconfig.json"],
		sys: () => {
			const aTs: File = {
				path: "/a.ts",
...
export class A {
	...
	it("two watch programs are not affected by each other", () => {
		...
		baseline.push(
			`${sys.getExecutingFilePath()} --w ${file2.path} ${file3.path}`,
		);
		watchBaseline({
			baseline,
			getPrograms,
		...
		const { cb: cb2, getPrograms: getPrograms2 } =
			commandLineCallbacks(sys);
		...
		baseline.push(
			`First program is not updated:: ${getPrograms() === ts.emptyArray}`,
		);
		baseline.push(
			`Second program is not updated:: ${getPrograms2() === ts.emptyArray}`,
		);
...
declare const eval: any`,
	...
	verifyTscWatch({
		...
		edits: [changeModuleFileToModuleFile1, changeModuleFile1ToModuleFile],
	});
				...
				sys: () => {
					...
					const config = {
						...
						content: jsonToReadableText({
							compilerOptions: {
								types: ["node"],
								typeRoots: includeTypeRoots ? [] : undefined,
							},
						}),
					};
					const node = {
	...
	verifyTscWatch({
		...
		sys: () => {
			...
			return createWatchedSystem([f, config, t1, t2, libFile], {
				currentDirectory: ts.getDirectoryPath(f.path),
			});
		},
	});
	...
	it("should support files without extensions", () => {
		...
		const { sys, baseline, cb, getPrograms } = createBaseline(
			createWatchedSystem([f, libFile]),
		);
		const watch = ts.createWatchProgram(
			createWatchCompilerHostOfFilesAndCompilerOptionsForBaseline({
				rootFiles: [f.path],
				system: sys,
				options: { allowNonTsExtensions: true },
			...
			verifyTscWatch({
				...
				sys: () => {
					...
					const tsconfig: File = {
						...
						content: jsonToReadableText({
							compilerOptions:
								compilerOptionsToConfigJson(options),
						}),
					};
					return createWatchedSystem(
						[file1, file2, libFile, tsconfig],
						{
							currentDirectory:
		...
		edits: [
			...
			},
			{
				...
				edit: (sys) =>
					sys.writeFile(
						`/user/username/projects/myproject/tsconfig.json`,
						jsonToReadableText({ compilerOptions: {} }),
					),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
		],
...
export function f(p: C) { return p; }`,
  ...
  type foo = number;
			...
			},
			{
				caption: "Delete b.ts",
				edit: (sys) =>
					sys.deleteFile(`/user/username/projects/myproject/b.ts`),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
		],
	});
...
interface Document {
    readonly ${field}: boolean;
		...
		function verifyLibFileErrorsWith(subScenario: string, aFile: File) {
			function verifyLibErrors(
				...
				verifyTscWatch({
					...
					edits: [
						...
						{
							caption:
								"Rever the file to contain document declaration",
							edit: (sys) =>
			...
			verifyLibErrors(
				`${subScenario}/with default options`,
				ts.emptyArray,
			);
			verifyLibErrors(`${subScenario}/with skipLibCheck`, [
				"--skipLibCheck",
			]);
...
import { x } from "../b";`,
		...
		edits: [
			{
				...
				edit: (sys) =>
					sys.writeFile(`/user/username/projects/myproject/foo`, ``),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
		],
	});
	...
	verifyTscWatch({
		scenario,
		subScenario:
			"when changing `allowImportingTsExtensions` of config file",
		commandLineArgs: ["-w", "-p", ".", "--extendedDiagnostics"],
		sys: () => {
			const module1: File = {
				path: `/user/username/projects/myproject/a.ts`,
