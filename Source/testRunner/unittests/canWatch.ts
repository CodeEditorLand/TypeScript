...
describe("unittests:: canWatch::", () => {
	baselineCanWatch(
		...
		(paths, longestPathLength, baseline) => {
			...
			const maxLengths = [
				longestPathLength + "/package.json".length,
				testType.length,
			] as const;
			pushHeader(baseline, ["Directory", testType], maxLengths);
			paths.forEach((path) =>
				baselineCanWatchDirectoryOrFile(path, maxLengths),
			);
			pushHeader(baseline, ["File", testType], maxLengths);
			paths.forEach((path) =>
				baselineCanWatchDirectoryOrFile(
					combinePaths(path, "package.json"),
					maxLengths,
				),
			);
			baseline.push("", "");
			function baselineCanWatchDirectoryOrFile(
				path: ts.Path,
				...
				pushRow(
					...
					[
						path,
						`${ts.canWatchDirectoryOrFile(ts.getPathComponents(path))}`,
					],
					maxLengths,
				);
			}
	...
	baselineCanWatch(
		...
		(paths, longestPathLength, baseline) => {
			...
			const maxLengths = [
				longestPathLength + "/node_modules/@types".length,
				testType.length,
			] as const;
			pushHeader(baseline, ["Directory", testType], maxLengths);
			paths.forEach((path) => {
	...
	baselineCanWatch(
		...
		(paths, longestPathLength, baseline) => {
			...
			paths.forEach((path) => {
				...
				pushRow(
					baseline,
					[path, `${ts.canWatchAffectingLocation(path)}`],
					maxLengths,
				);
			});
			baseline.push("", "");
	...
	baselineGetDirectoryToWatchOfFailedLookup(
	...
	function baselineGetDirectoryToWatchOfFailedLookup(
		...
		baselineCanWatch(
			...
			(paths, longestPathLength, baseline) => {
				...
				const maxLength =
					longestPathLength +
					ts.combinePaths(forPath, "dir/subdir/somefile.d.ts").length;
				const maxLengths = [
					maxLength,
					maxLength,
					recursive.length,
				] as const;
				baselineCanWatchForRoot(
					paths,
					baseline,
					(rootPathCompoments, root) => {
						pushHeader(
							baseline,
							[
								"Location",
			...
			baselineCanWatchForRoot(
				...
				(rootPathCompoments, root) => {
					...
					paths.forEach((path) => {
						...
						const result =
							ts.getDirectoryToWatchFailedLookupLocationFromTypeRoot(
								path,
								path,
								root,
								rootPathCompoments,
	...
	function baselineCanWatchForRoot(
		...
		paths.forEach((rootDirForResolution) => {
			const root = ts.getRootDirectoryOfResolutionCache(
	...
	function baselineCanWatchForOsRoot(
		...
		Baseline.runBaseline(
			`canWatch/${scenario}${suffix}.baseline.md`,
			baseline.join("\r\n"),
		);
	...
	function pushRow(
		...
		columns.forEach((header, index) => {
			...
			if (addDivider)
				divider += " " + "-".repeat(maxLengths[index]) + " |";
		});
