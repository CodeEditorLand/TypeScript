...
export function loggerToTypingsInstallerLog(
	logger: LoggerWithInMemoryLogs,
): ts.server.typingsInstaller.Log {
...
export class TestTypingsInstallerWorker extends ts.server.typingsInstaller
	.TypingsInstaller {
	readonly typesRegistry: Map<string, ts.MapLike<string>>;
	constructor(readonly testTypingInstaller: TestTypingsInstallerAdapter) {
		const log = loggerToTypingsInstallerLog(
			testTypingInstaller.session.logger,
		);
		ts.Debug.assert(testTypingInstaller.session.host.patched);
		testTypingInstaller.session.host.baselineHost(
		...
		this.ensurePackageDirectoryExists(
			testTypingInstaller.globalTypingsCacheLocation,
		);
		...
		this.log.writeLine(
			`npm install --ignore-scripts ${typesRegistryPackageName}@${this.latestDistTag}`,
		);
	...
	installWorker(
		...
		this.log.writeLine(
			`#${requestId} with cwd: ${cwd} arguments: ${jsonToReadableText(packageNames)}`,
		);
		if (typeof this.testTypingInstaller.installAction === "boolean") {
			...
			const typingFiles = this.testTypingInstaller.installAction.filter(
				(f) =>
					!f.package ||
					packageNames.includes(
						ts.server.typingsInstaller.typingsName(f.package),
					),
			);
			this.scheduleInstall(
				requestId,
		...
		this.testTypingInstaller.session.host.scheduleInstall(
			(pendingInstallInfo) => {
				for (const file of typingFiles) {
					this.testTypingInstaller.session.host.ensureFileOrFolder(
						file,
					);
				}
...
export class TestTypingsInstallerAdapter extends ts.server
	...
	constructor(options: TestTypingsInstallerOptions) {
		const globalTypingsCacheLocation =
			options.globalTypingsCacheLocation ||
		...
		super(
			...
			options.throttledRequests === undefined
				? 10
				: options.throttledRequests,
		);
		this.throttleLimit = options.throttleLimit || 5;
		this.installAction =
			options.installAction !== undefined ? options.installAction : true;
	...
	protected override createInstallerProcess(): ts.server.TypingsInstallerWorkerProcess {
		return {
			send: (req) =>
				(this.worker ??= new TestTypingsInstallerWorker(
					this,
				)).handleRequest(req),
		};
	}
...
function createTypesRegistryFileContent(
...
export function createTypesRegistry(...list: string[]) {
	return new Map(
		Object.entries(createTypesRegistryFileContent(list).entries),
	);
}
