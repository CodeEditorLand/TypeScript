...
export interface TscWatchCompileBase<
	T extends ts.BuilderProgram = ts.EmitAndSemanticDiagnosticsBuilderProgram,
...
function tscWatchCompile(input: TscWatchCompile) {
	it("tsc-watch:: Generates files matching the baseline", () => {
		...
		const watchOrSolution = ts.executeCommandLine(sys, cb, commandLineArgs);
		runWatchBaseline({
			scenario,
			subScenario,
			commandLineArgs,
...
function changeToTestServerHostWithTimeoutLogging(
	...
	host.timeoutCallbacks.switchToBaseliningInvoke(
		logger,
		SerializeOutputOrder.BeforeDiff,
	);
...
export function createBaseline(
	...
	const sys = changeToTestServerHostWithTimeoutLogging(
		changeToHostTrackingWrittenFiles(initialSys),
		baseline,
	);
	baseline.push(
...
interface CreateWatchCompilerHostOfConfigFileForBaseline<
...
>(input: CreateWatchCompilerHostOfConfigFileForBaseline<T>) {
	const host = ts.createWatchCompilerHostOfConfigFile({
		...input,
		reportDiagnostic: ts.createDiagnosticReporter(
			input.system,
			/*pretty*/ true,
		),
		reportWatchStatus: ts.createWatchStatusReporter(
			input.system,
			/*pretty*/ true,
		),
	});
...
export interface RunWatchBaseline<T extends ts.BuilderProgram>
	extends BaselineBase,
		TscWatchCompileBase<T> {
	sys: TscWatchSystem;
...
export function isWatch(commandLineArgs: readonly string[]) {
	return ts.forEach(commandLineArgs, (arg) => {
		...
		const option = arg
			.slice(arg.charCodeAt(1) === ts.CharacterCodes.minus ? 2 : 1)
			.toLowerCase();
...
function verifyProgramStructureAndResolutionCache(
	...
	compilerHost.readFile = (fileName) => {
		...
		ts.Debug.assert(
			sys.toPath(sys.realpath(fileName)) !== sys.toPath(fileName),
		);
