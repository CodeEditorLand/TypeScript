...
function baselineProgram(
	...
	if (program !== oldProgram?.[0]) {
		...
		baseline.push(
			`Program root files: ${jsonToReadableText(program.getRootFileNames())}`,
		);
	...
	if (builderProgram !== oldProgram?.[1]) {
		...
		if (state.semanticDiagnosticsPerFile?.size) {
			...
			for (const file of program.getSourceFiles()) {
				if (
					...
					!internalState.semanticDiagnosticsFromOldState.has(
						file.resolvedPath,
					)
				) {
					baseline.push(file.fileName);
		...
		if (internalState) {
			...
			if (internalState.hasCalledUpdateShapeSignature?.size) {
				...
				internalState.hasCalledUpdateShapeSignature.forEach(
					(path: ts.Path) => {
						const info = state.fileInfos.get(path);
						if (
							info?.version === info?.signature ||
							!info?.signature
						) {
							baseline.push(path + " (used version)");
						} else if (
							internalState.filesChangingSignature?.has(path)
						) {
							baseline.push(
								path + " (computed .d.ts during emit)",
							);
						} else {
...
export function generateSourceMapBaselineFiles(
	sys: ts.System & { writtenFiles: ts.ReadonlyCollection<ts.Path> },
) {
	const mapFileNames = ts.mapDefinedIterator(sys.writtenFiles.keys(), (f) =>
...
function generateBundleFileSectionInfo(
	...
	if (!ts.length(bundleFileInfo && bundleFileInfo.sections) && !outFile)
	...
	function writeSectionHeader(section: ts.BundleFileSection) {
		baselineRecorder.WriteLine(
			`${section.kind}: (${section.pos}-${section.end})${section.data ? ":: " + section.data : ""}${section.kind === ts.BundleFileSectionKind.Prepend ? " texts:: " + section.texts.length : ""}`,
		);
	}
}

export type ReadableProgramBuildInfoDiagnostic =
	| string
	| [string, readonly ts.ReusableDiagnostic[]];
...
export type ReadableProgramBuildInfoEmitSignature =
	| string
	| [string, ts.EmitSignature | []];
export type ReadableProgramBuildInfoFileInfo<T> = Omit<
...
> & {
	...
	semanticDiagnosticsPerFile:
		| readonly ReadableProgramBuildInfoDiagnostic[]
		| undefined;
	emitDiagnosticsPerFile:
		| readonly ReadableProgramBuildInfoDiagnostic[]
		| undefined;
	...
	emitSignatures:
		| readonly ReadableProgramBuildInfoEmitSignature[]
		| undefined;
};
...
export type ReadableProgramBundleEmitBuildInfo = Omit<
	...
	"fileInfos" | "root" | "pendingEmit"
> & {
	fileInfos: ts.MapLike<
		string | ReadableProgramBuildInfoFileInfo<ts.BuilderState.FileInfo>
	>;
	root: readonly ReadableProgramBuildInfoRoot[];
	pendingEmit: ReadableProgramBuildInfoBundlePendingEmit | undefined;
};

export type ReadableProgramBuildInfo =
...
function generateBuildInfoProgramBaseline(
	...
	if (
		buildInfo.program &&
		ts.isProgramBundleEmitBuildInfo(buildInfo.program)
	) {
		const fileInfos: ReadableProgramBundleEmitBuildInfo["fileInfos"] = {};
		buildInfo.program?.fileInfos?.forEach(
			(fileInfo, index) =>
				(fileInfos[
					toFileName((index + 1) as ts.ProgramBuildInfoFileId)
				] = ts.isString(fileInfo)
					? fileInfo
		...
		program = {
			...
			pendingEmit:
				pendingEmit === undefined
					? undefined
					: [
							toReadableBuilderFileEmit(
								ts.toProgramEmitPending(
									pendingEmit,
		...
		fileNamesList = buildInfo.program.fileIdsList?.map((fileIdsListId) =>
		...
		program = buildInfo.program && {
			...
			referencedMap: toMapOfReferencedSet(
			...
			affectedFilesPendingEmit:
				buildInfo.program.affectedFilesPendingEmit?.map((value) =>
					toReadableProgramBuilderInfoFilePendingEmit(
						value,
						fullEmitForOptions!,
					),
				),
			...
			emitSignatures: buildInfo.program.emitSignatures?.map((s) =>
				ts.isNumber(s) ? toFileName(s) : [toFileName(s[0]), s[1]],
			),
	...
	function toReadableFileInfo<T>(
		...
		return {
			...
			impliedFormat:
				info.impliedFormat &&
				ts.getNameOfCompilerOptionValue(
					info.impliedFormat,
					ts.moduleOptionDeclaration.type,
				),
		};
	}

	function toReadableProgramBuildInfoRoot(
		original: ts.ProgramBuildInfoRoot,
	): ReadableProgramBuildInfoRoot {
		if (!ts.isArray(original)) return [original, toFileName(original)];
		const readable: string[] = [];
		for (let index = original[0]; index <= original[1]; index++)
			readable.push(toFileName(index));
	...
	function toReadableProgramBuilderInfoFilePendingEmit(
		...
		return [
			...
			toReadableBuilderFileEmit(
				ts.toBuilderFileEmit(value, fullEmitForOptions),
			),
		];
	}

	function toReadableBuilderFileEmit(
		emit: ts.BuilderFileEmit | undefined,
	...
	function toReadableProgramBuildInfoDiagnosticsPerFile(
		...
		return diagnostics?.map((d) =>
			ts.isNumber(d) ? toFileName(d) : [toFileName(d[0]), d[1]],
		);
	}
}
...
export function toPathWithSystem(sys: ts.System, fileName: string): ts.Path {
	return ts.toPath(
		...
		ts.createGetCanonicalFileName(sys.useCaseSensitiveFileNames),
	);
}
...
export function baselineBuildInfo(
	...
	if (!buildInfo)
		return sys.writeFile(
			...
			"Error reading valid buildinfo file",
		);
	generateBuildInfoProgramBaseline(sys, buildInfoPath, buildInfo);

	if (!ts.outFile(options)) return;
	const { jsFilePath, declarationFilePath } = ts.getOutputPathsForBundle(
		options,
		/*forceDtsPaths*/ false,
	);
	...
	if (
		!bundle ||
		(!ts.length(bundle.js && bundle.js.sections) &&
			!ts.length(bundle.dts && bundle.dts.sections))
	)
		return;
