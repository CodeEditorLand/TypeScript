...
export function createWatchedSystem(
	fileOrFolderList:
		...
		| readonly FileOrFolderOrSymLink[],
...
class Callbacks {
	...
	private nextId = 1;
	invoke: (invokeKey?: number) => void = (invokeKey) =>
	...
	log(logChanges?: boolean) {
		...
		return (
			`${this.callbackType} callback:: count: ${this.map.size}` +
			(deleted.length ? "\r\n" + deleted.join("\r\n") : "") +
			(details.length ? "\r\n" + details.join("\r\n") : "")
		);
	}
	...
	switchToBaseliningInvoke(
		logger: StateLogger,
		serializeOutputOrder: SerializeOutputOrder,
	) {
		this.invoke = (invokeKey) => {
			...
			this.invokeWorker(invokeKey);
			logger.log(
				`After running ${this.callbackType} callback:: count: ${this.map.size}`,
			);
			this.host.serializeState(logger.logs, serializeOutputOrder);
		};
	}
	...
	implements server.ServerHost, FormatDiagnosticsHost, ModuleResolutionHost
{
	...
	readonly timeoutCallbacks = new Callbacks(
		this,
		"Timeout",
		/*swallowExitException*/ true,
	);
	...
	constructor(
		...
		this.watchUtils = createWatchUtils("PolledWatches", "FsWatches", (s) =>
			this.getCanonicalFileName(s),
		);
		this.toPath = (s) =>
	...
	private reloadFS(
		...
		this.ensureFileOrFolder({
			path: !this.windowsStyleRoot ? "/" : this.getHostSpecificPath("/"),
		});
	}
	...
	modifyFile(
		...
		if (options && options.invokeFileDeleteCreateAsPartInsteadOfChange) {
			...
			if (options && options.invokeDirectoryWatcherInsteadOfFileChanged) {
				const directoryFullPath = getDirectoryPath(
	...
	private fsWatchWorker(
		...
		if (this.runWithFallbackPolling)
			throw new Error(
				"Need to use fallback polling instead of file system native watching",
			);
	...
	invokeFileWatcher(
		...
		this.watchUtils.pollingWatches.forEach(fileFullPath, ({ cb }) =>
			cb(fileFullPath, eventKind, modifiedTime),
		);
	}
	...
	private fsWatchCallback(
		...
		watches.forEach(path, ({ cb, inode }) => {
			...
			if (
				...
				return;
			let relativeFileName = entryFullPath
	...
	serializeState(baseline: string[], serializeOutput: SerializeOutputOrder) {
		if (serializeOutput === SerializeOutputOrder.BeforeDiff)
			this.serializeOutput(baseline);
		this.diff(baseline);
		if (serializeOutput === SerializeOutputOrder.AfterDiff)
			this.serializeOutput(baseline);
	...
	realpath(s: string): string {
		...
		const realFullPath = combinePaths(
			dirFullPath,
			getBaseFileName(fullPath),
		);
	...
	getEnvironmentVariable(name: string) {
		return (
			(this.environmentVariables &&
				this.environmentVariables.get(name)) ||
			""
		);
	}
}
...
function diffFsSymLink(
	baseline: string[],
	fsEntry: FsSymLink,
	newInode: number | undefined,
) {
	baseline.push(
		`//// [${fsEntry.fullPath}] symlink(${fsEntry.symLink})${inodeString(newInode)}`,
	);
}
		...
		if (isFsFile(newFsEntry)) {
			if (oldFsEntry.content !== newFsEntry.content) {
				...
				if (oldFsEntry.fullPath !== newFsEntry.fullPath) {
					...
					baseline.push(
						`//// [${file}] file changed its modified time${inodeString(newInode)}`,
					);
				} else {
					baseline.push(
						`//// [${file}] file written with same contents${inodeString(newInode)}`,
					);
				}
			}
		} else {
		...
		if (isFsSymLink(newFsEntry)) {
			if (oldFsEntry.symLink !== newFsEntry.symLink) {
				...
				if (oldFsEntry.fullPath !== newFsEntry.fullPath) {
					...
					baseline.push(
						`//// [${file}] symlink changed its modified time${inodeString(newInode)}`,
					);
				} else {
					baseline.push(
						`//// [${file}] symlink written with same link${inodeString(newInode)}`,
					);
				}
			}
		} else {
