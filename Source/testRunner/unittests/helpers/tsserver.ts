...
export function toExternalFile(
	fileName: string,
): ts.server.protocol.ExternalFile {
	return { fileName };
}
...
export function patchHostTimeouts(
	...
	host.timeoutCallbacks.switchToBaseliningInvoke(
		logger,
		SerializeOutputOrder.None,
	);
	host.immediateCallbacks.switchToBaseliningInvoke(
		logger as StateLogger,
		SerializeOutputOrder.None,
	);
	host.pendingInstalls.switchToBaseliningInvoke(
		logger,
		SerializeOutputOrder.None,
	);
...
export interface TestSessionOptions
	extends ts.server.SessionOptions,
		TestTypingsInstallerOptions {
	host: TestServerHost;
	logger: LoggerWithInMemoryLogs;
...
export type TestSessionPartialOptionsAndHost = Partial<
	Omit<TestSessionOptions, "typingsInstaller" | "cancellationToken">
> &
	Pick<TestSessionOptions, "host">;
export type TestSessionConstructorOptions =
	| TestServerHost
	| TestSessionPartialOptionsAndHost;
export type TestSessionRequest<T extends ts.server.protocol.Request> = Pick<
	T,
	"command" | "arguments"
>;
...
function getTestSessionPartialOptionsAndHost(
	...
	return "host" in optsOrHost ? optsOrHost : { host: optsOrHost };
}
export class TestSession extends ts.server.Session {
	...
	constructor(optsOrHost: TestSessionConstructorOptions) {
		...
		const typingsInstaller = !opts.disableAutomaticTypingAcquisition
			? new TestTypingsInstallerAdapter(opts)
			: undefined;
		const cancellationToken = opts.useCancellationToken
			? new TestServerCancellationToken(
					opts.logger,
					ts.isNumber(opts.useCancellationToken)
						? opts.useCancellationToken
						: undefined,
				)
		...
		this.serverCancellationToken =
			cancellationToken as TestServerCancellationToken;
		patchHostTimeouts(
	...
	public executeCommandSeq<T extends ts.server.protocol.Request>(
		inputRequest: TestSessionRequest<T>,
	) {
		this.seq++;
	...
	function eventHandler(event: ts.server.ProjectServiceEvent) {
		...
		switch (event.eventName) {
			...
			case ts.server.ConfigFileDiagEvent:
				data = {
					...data,
					diagnostics: ts.map(event.data.diagnostics, (diagnostic) =>
						ts.server.formatDiagnosticToProtocol(
							diagnostic,
...
export function protocolToLocation(
	text: string,
): (pos: number) => ts.server.protocol.Location {
	...
	implements ts.server.ServerCancellationToken
{
	...
	constructor(
		...
		private cancelAfterRequest = 0,
	) {}

	setRequest(requestId: number) {
		this.currentId = requestId;
		this.logger.log(
			`TestServerCancellationToken:: Cancellation Request id:: ${requestId}`,
		);
	}

	setRequestToCancel(requestId: number) {
		this.logger.log(
			`TestServerCancellationToken:: Setting request to cancel:: ${requestId}`,
		);
		this.resetToken();
...
export function openExternalProjectForSession(
	project: ts.server.protocol.ExternalProject,
	session: TestSession,
) {
...
export function openExternalProjectsForSession(
	projects: ts.server.protocol.ExternalProject[],
	session: TestSession,
) {
	...
	session.executeCommandSeq<ts.server.protocol.SetCompilerOptionsForInferredProjectsRequest>(
		{
			...
			arguments:
				"options" in options // eslint-disable-line local/no-in-operator
...
export function logDiagnostics(
	...
	diagnostics.forEach((d) =>
		sessionOrService.logger.info(ts.formatDiagnostic(d, project)),
	);
}
...
function checkAllErrors({
	...
	for (let i = 0; i < files.length; i++) {
		session.host.runQueuedTimeoutCallbacks(
			existingTimeouts ? session.host.getNextTimeoutId() - 1 : undefined,
		);
...
export function verifyDynamic(session: TestSession, path: string) {
	session.logger.log(
		`${path} isDynamic:: ${session.getProjectService().filenameToScriptInfo.get(path)!.isDynamic}`,
	);
}
...
export function logInferredProjectsOrphanStatus(session: TestSession) {
	session
		.getProjectService()
		.inferredProjects.forEach((inferredProject) =>
			session.logger.log(
