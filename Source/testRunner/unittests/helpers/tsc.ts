...
export function testTscCompileLike(input: TestTscCompileLike) {
	...
	worker(sys);
	sys.write(
		`exitCode:: ExitStatus.${ts.ExitStatus[sys.exitCode as ts.ExitStatus]}\n`,
	);
...
export function makeSystemReadyForBaseline(
	sys: TscCompileSystem,
	...
	sys.writeFile = (fileName, content, writeByteOrderMark) => {
		...
		ts.Debug.assert(
			!writtenFiles.has(path) || ts.endsWith(path, "baseline.txt"),
		);
	...
	function commandLineCompile(sys: TscCompileSystem) {
		...
		if (input.baselineReadFileCalls) {
			sys.readFile = (path) => {
				...
				if (
					!path.startsWith(
						ts.getDirectoryPath(sys.getExecutingFilePath()),
					)
				) {
					actualReadFileMap![path] =
						(ts.getProperty(actualReadFileMap!, path) || 0) + 1;
				}
		...
		ts.executeCommandLine(sys, result.cb, input.commandLineArgs);
				...
				(signature, sourceFiles) => {
					const exportedModules = ts.BuilderState.getExportedModules(
						state.exportedModulesMap &&
							sourceFiles[0].exportedModulesFromDeclarationEmit,
					);
...
function verifyTscEditDiscrepancies({
	...
	for (const outputFile of sys.writtenFiles.keys()) {
		...
		if (ts.isBuildInfoFile(outputFile)) {
			...
			if (!sys.fileExists(`${outputFile}.readable.baseline.txt`))
				addBaseline(
					`Readable baseline not present in clean build:: File:: ${outputFile}`,
				);
			if (!newSys.fileExists(`${outputFile}.readable.baseline.txt`))
				addBaseline(
					`Readable baseline not present in incremental build:: File:: ${outputFile}`,
				);
			...
			verifyMapLike(
				incrementalReadableBuildInfo?.program
					?.fileInfos as ReadableProgramMultiFileEmitBuildInfo["fileInfos"],
				cleanReadableBuildInfo?.program
					?.fileInfos as ReadableProgramMultiFileEmitBuildInfo["fileInfos"],
			...
			if (
				...
				if (
					...
					incrementalReadableBuildInfo.program.affectedFilesPendingEmit.forEach(
						([actualFileOrArray]) => {
							const actualFile = ts.isString(actualFileOrArray)
								? actualFileOrArray
								: actualFileOrArray[0];
							expectedIndex = ts.findIndex(
								(
									cleanReadableBuildInfo!
										.program! as ReadableProgramMultiFileEmitBuildInfo
								).affectedFilesPendingEmit,
								([expectedFileOrArray]) =>
									actualFile ===
									(ts.isString(expectedFileOrArray)
										? expectedFileOrArray
				...
				if (
					...
					incrementalReadableBuildInfo.program.emitDiagnosticsPerFile.forEach(
						([actualFileOrArray]) => {
							const actualFile = ts.isString(actualFileOrArray)
								? actualFileOrArray
								: actualFileOrArray[0];
	...
	function verifyTextEqual(
		...
		if (incrementalText !== cleanText)
			writeNotEqual(incrementalText, cleanText, message);
	}
	...
	function verifyMapLike<T>(
		...
		verifyPresenceAbsence(
			incremental,
			clean,
			`Incremental and clean do not match:: ${message}`,
		);
		...
		cleanMap.forEach((cleanValue, key) => {
			const result = verifyValue(
				key,
				incrementalMap.get(key),
				cleanValue,
			);
	...
	if (readableBuildInfo.program?.fileInfos) {
		...
		for (const id in readableBuildInfo.program.fileInfos) {
			if (ts.hasProperty(readableBuildInfo.program.fileInfos, id)) {
				...
				sanitizedFileInfos[id] = ts.isString(info)
					? info
	...
	return {
		buildInfo: jsonToReadableText({
			...
			program: readableBuildInfo.program && {
				...
				latestChangedDtsFile: readableBuildInfo.program
					...
					(editsSys || (editsSys = [])).push(
						testTscCompile({
							...
							fs: () =>
								index === 0
									? sys.vfs
									: editsSys![index - 1].vfs,
							commandLineArgs:
								editCommandLineArgs || commandLineArgs,
							modifyFs: edit,
							baselineSourceMap,
							baselineReadFileCalls,
							baselinePrograms,
