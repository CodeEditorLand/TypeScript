...
describe("unittests:: tsbuildWatch:: watchMode:: program updates", () => {
	...
	it("verify building references watches only those projects", () => {
		...
		runWatchBaseline({
			scenario: "programUpdates",
			subScenario:
				"verify building references watches only those projects",
			commandLineArgs: ["--b", "--w"],
			sys,
			baseline,
			getPrograms,
        ...
        private p = 12
			...
			const changeFileWithoutError: TscWatchCompileChange = {
				...
				edit: (sys) =>
					sys.writeFile(
						...
						fileWithoutError.content.replace(
							/myClass/g,
							"myClass2",
						),
					),
			...
			describe("when reporting errors on introducing error", () => {
				...
				verifyTscWatch({
					...
					edits: [introduceError, changeFileWithoutError],
				});
			});
		});
	});
	...
	verifyTscWatch({
		...
		edits: [
			noopChange,
			{
				...
				edit: (sys) =>
					sys.writeFile("core/file3.ts", `export const y = 10;`),
				timeouts: (sys) => sys.runQueuedTimeoutCallbacks(),
			},
			noopChange,
		],
