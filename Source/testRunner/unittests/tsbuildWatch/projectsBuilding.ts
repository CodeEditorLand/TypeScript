		...
		return [
			...
			},
			{
				...
				content: jsonToReadableText({
					...
					references: index === 0 ? undefined : [{ path: `../pkg0` }],
				}),
			},
		];
	}
	...
	verifyTscWatch({
		...
		sys: () =>
			createWatchedSystem(
				[
					libFile,
					...ts.flatMap(pkgs(pkgFiles, 3), ts.identity),
					solution(3),
				],
	...
	verifyTscWatch({
		...
		sys: () =>
			createWatchedSystem(
				[
					libFile,
					...ts.flatMap(pkgs(pkgFiles, 5), ts.identity),
					solution(5),
				],
				{ currentDirectory: "/user/username/projects/myproject" },
			),
	...
	verifyTscWatch({
		...
		sys: () =>
			createWatchedSystem(
				[
					libFile,
					...ts.flatMap(pkgs(pkgFiles, 8), ts.identity),
					solution(8),
				],
				{ currentDirectory: "/user/username/projects/myproject" },
			),
	...
	verifyTscWatch({
		...
		sys: () =>
			createWatchedSystem(
				[
					libFile,
					...ts.flatMap(pkgs(pkgFiles, 23), ts.identity),
					solution(23),
				],
				{ currentDirectory: "/user/username/projects/myproject" },
			),
