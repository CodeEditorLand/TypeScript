...
function verifyMissingFilePaths(
	missing: ReturnType<ts.Program["getMissingFilePaths"]>,
	...
	const notFound = ts.arrayFrom(
		ts.mapDefinedIterator(map.keys(), (k) => (map.has(k) ? k : undefined)),
	);
	assert.equal(
		notFound.length,
		0,
...
describe("unittests:: programApi:: Program.getMissingFilePaths", () => {
	...
	it("normalizes file paths", () => {
		const program0 = ts.createProgram(
			["./nonexistent.ts", "./NONEXISTENT.ts"],
			options,
			testCompilerHost,
		);
	...
	it("should not have missing file paths", () => {
		...
		const host: ts.CompilerHost = {
			getSourceFile: (
				...
				return fileName === "test.ts"
					? ts.createSourceFile(fileName, testSource, languageVersion)
					: undefined;
			},
		...
		assert(
			program.getMissingFilePaths().size === 0,
...
describe("unittests:: Program.isSourceFileFromExternalLibrary", () => {
	it("works on redirect files", () => {
		...
		const fooPackageJsonText = jsonToReadableText({
			name: "foo",
			version: "1.2.3",
		});
...
describe("unittests:: Program.getNodeCount / Program.getIdentifierCount", () => {
	it("works on projects that have .json files", () => {
		...
		const pkg = new documents.TextDocument(
			"/package.json",
			jsonToReadableText({ version: "1.0.0" }),
		);
...
describe("unittests:: programApi:: Program.getTypeChecker / Program.getSemanticDiagnostics", () => {
	...
	it("getSymbolAtLocation does not cause additional error to be added on module resolution failure", () => {
		...
		typeChecker.getSymbolAtLocation(
			(sourceFile.statements[0] as ts.ImportDeclaration).moduleSpecifier,
		);
