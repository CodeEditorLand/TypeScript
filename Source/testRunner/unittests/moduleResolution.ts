...
function runBaseline(scenario: string, baselines: readonly string[]) {
	Harness.Baseline.runBaseline(
		`moduleResolution/${scenario.split(" ").join("-")}.js`,
		baselines.join("\n"),
	);
}
...
describe("unittests:: moduleResolution:: Node module resolution - relative paths", () => {
	...
	const autoExtensions = ts.filter(
		ts.supportedTSExtensionsFlat,
		(e) => !nonImplicitExtensions.includes(e),
	);
	...
	it("load as file", () => {
		...
		function testLoadAsFile(
			...
			function test(ext: string, hasDirectoryExists: boolean) {
				...
				baselines.push(
					`Resolution:: ${jsonToReadableText(resolution)}`,
				);
				baselines.push("");
			}
		}
		...
		function testLoadingFromPackageJson(
			...
			function test(hasDirectoryExists: boolean) {
				...
				const packageJson = {
					...
					content: jsonToReadableText({ typings: fieldRef }),
				};
	...
	it("module name as directory - handle invalid 'typings'", () => {
		...
		runBaseline(
			"relative module name as directory with invalid typings",
			baselines,
		);
		...
		function testTypingsIgnored(typings: any): void {
			...
			function test(hasDirectoryExists: boolean) {
				...
				const packageJson = {
					...
					content: jsonToReadableText({ typings }),
				};
	...
	it("module name as directory - load index.d.ts", () => {
		...
		function test(hasDirectoryExists: boolean) {
			...
			const packageJson = {
				...
				content: jsonToReadableText({ main: "/c/d" }),
			};
			const indexFile = { name: "/a/b/foo/index.d.ts" };
			baselines.push(
				`Resolving "./foo" from ${containingFile.name}${hasDirectoryExists ? "" : " with host that doesnt have directoryExists"}`,
			);
			const resolution = ts.nodeModuleNameResolver(
				"./foo",
				containingFile.name,
...
describe("unittests:: moduleResolution:: Node module resolution - non-relative paths", () => {
	it("computes correct commonPrefix for moduleName cache", () => {
		...
		let cache = resolutionCache.getOrCreateCacheForNonRelativeName(
			"a",
			/*mode*/ undefined,
		);
		cache.set("/sub", {
		...
		cache = resolutionCache.getOrCreateCacheForNonRelativeName(
			"b",
			/*mode*/ undefined,
		);
	...
	it("load module as file - ts files not loaded", () => {
		...
		test(/*hasDirectoryExists*/ false);
		runBaseline(
			"non relative module name as file ts files not loaded",
			baselines,
		);
	...
	it("preserveSymlinks", () => {
		...
		function testPreserveSymlinks(preserveSymlinks: boolean) {
			...
			baselines.push(
				`Resolving "linked" from /app/app.ts when preserveSymlinks is ${preserveSymlinks}`,
			);
			const resolution = ts.nodeModuleNameResolver(
				"linked",
				"/app/app.ts",
	...
	it("preserves originalPath on cache hit", () => {
		...
		runBaseline(
			"non relative preserves originalPath on cache hit",
			baselines,
		);
		...
		it(scenario, () => {
			...
			const host: ts.CompilerHost = {
				getSourceFile: (
					...
					const path = ts.normalizePath(
						ts.combinePaths(currentDirectory, fileName),
					);
					const file = files.get(path);
					return file
						? ts.createSourceFile(fileName, file, languageVersion)
						: undefined;
				},
			...
			program
				.getSourceFiles()
				.forEach((file) => baselines.push(file.fileName));

			baselines.push("", "Syntactic Diagnostics::");
			baselines.push(
				ts.formatDiagnostics(program.getSyntacticDiagnostics(), host),
				"",
			);

			baselines.push("Semantic Diagnostics::");
			baselines.push(
				ts.formatDiagnostics(program.getSemanticDiagnostics(), host),
				"",
			);
		...
		function test(hasDirectoryExists: boolean) {
			...
			for (const moduleResolution of [
				...
				};
				{
					...
					baselines.push(
						`Resolution:: ${jsonToReadableText(result)}`,
					);
					baselines.push("");
				}
				{
	...
	it("node + baseUrl", () => {
		...
		function test(hasDirectoryExists: boolean) {
			...
			function check(name: string, caller: File) {
				baselines.push(
					`Resolving "${name}" from ${caller.name}${hasDirectoryExists ? "" : " with host that doesnt have directoryExists"}`,
				);
				const result = ts.resolveModuleName(
					name,
					caller.name,
	...
	it("node + baseUrl + path mappings", () => {
		...
		function test(hasDirectoryExists: boolean) {
			...
			function check(name: string) {
				baselines.push(
					`Resolving "${name}" from ${main.name}${hasDirectoryExists ? "" : " with host that doesnt have directoryExists"}`,
				);
				const result = ts.resolveModuleName(
					name,
					main.name,
	...
	it("nested node module", () => {
		...
		function test(hasDirectoryExists: boolean) {
			...
			const libsPackage: File = {
				...
				content: jsonToReadableText({ typings: "dist/guid.d.ts" }),
			};
			...
			baselines.push(
				`Resolving "libs/guid" from ${app.name}${hasDirectoryExists ? "" : " with host that doesnt have directoryExists"}`,
			);
			const result = ts.resolveModuleName(
				"libs/guid",
				app.name,
...
describe("unittests:: moduleResolution:: Type reference directive resolution: ", () => {
	...
	it("Can be resolved from secondary location", () => {
		...
		}
		{
			...
			const packageFile = {
				...
				content: jsonToReadableText({ typings: "typings/lib.d.ts" }),
			};
			test(
				baselines,
				/*typesRoot*/ "/root/src/types",
	...
	it("Reused program keeps errors", () => {
		...
		const sourceFiles = ts.arrayToMap(
			ts.map(files, (f) =>
				ts.createSourceFile(f.name, f.content, ts.ScriptTarget.ES2015),
			),
			(f) => f.fileName,
		);
		...
		baselines.push(
			ts.formatDiagnostics(
				program1.getOptionsDiagnostics(),
				compilerHost,
			),
			"",
		);
		...
		baselines.push(
			`Program Reused:: ${(ts as any).StructureIsReused[program2.structureIsReused]}`,
		);
	...
	it("can be resolved when typeReferenceDirective is relative and in a sibling folder", () => {
		...
		runBaseline(
			"typeReferenceDirective is relative and in a sibling folder",
			baselines,
		);
	});
});
