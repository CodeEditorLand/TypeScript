		...
		for (const runner of runners) {
			if (
				...
				for (const full of runner.enumerateTestFiles()) {
					...
					if (
						seen.has(base) &&
						!/fourslash\/(shim|server)/.test(full)
					) {
		...
		if (runnerConfig && runnerConfig.length > 0) {
			...
			for (const option of runnerConfig) {
				...
				switch (option) {
					case "compiler":
						runners.push(
							new CompilerBaselineRunner(
								CompilerTestType.Conformance,
							),
						);
						runners.push(
							new CompilerBaselineRunner(
								CompilerTestType.Regressions,
							),
						);
						break;
					case "conformance":
					...
					case "fourslash":
						runners.push(
							new FourSlashRunner(
								FourSlash.FourSlashTestType.Native,
							),
						);
						break;
					case "fourslash-server":
						runners.push(
							new FourSlashRunner(
								FourSlash.FourSlashTestType.Server,
							),
						);
						break;
					case "fourslash-generated":
						runners.push(
							new GeneratedFourslashRunner(
								FourSlash.FourSlashTestType.Native,
							),
						);
						break;
