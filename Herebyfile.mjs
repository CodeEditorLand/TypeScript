...
const libs = memoize(() => {
	...
	const libs = libraries.libs.map((lib) => {
		...
		const relativeTarget =
			(libraries.paths && libraries.paths[lib]) || "lib." + lib + ".d.ts";
		const sources = relativeSources.map((s) =>
			path.posix.join("src/lib", s),
		);
...
function entrypointBuildTask(options) {
	...
	const watch = task({
		...
		dependencies: (options.buildDeps ?? [])
			.concat(options.mainDeps ?? [])
			.concat(cmdLineOptions.bundle ? [] : [shim]),
		run: () => {
...
export const dtsServices = task({
	...
	run: async () => {
		if (
			...
			await runDtsBundler(
				"./built/local/typescript/typescript.d.ts",
				"./built/local/typescript.d.ts",
			);
		}
	},
...
const lssl = task({
	...
	run: async () => {
		await fs.promises.writeFile(
			...
			await fileContentsWithCopyright(lsslJs),
		);
	},
});
...
export const dtsLssl = task({
	...
	run: async () => {
		await fs.promises.writeFile(
			...
			await fileContentsWithCopyright(lsslDts),
		);
		await fs.promises.writeFile(
...
export const lint = task({
	...
	run: async () => {
		const folder = ".";
		const formatter = cmdLineOptions.ci
			? "stylish"
...
export const checkFormat = task({
	...
	run: () =>
		exec(process.execPath, ["node_modules/dprint/bin.js", "check"], {
			ignoreStdout: true,
		}),
});

const { main: cancellationToken, watch: watchCancellationToken } =
	entrypointBuildTask({
...
const { main: typingsInstaller, watch: watchTypingsInstaller } =
	entrypointBuildTask({
		...
		builtEntrypoint:
			"./built/local/typingsInstaller/nodeTypingsInstaller.js",
...
const builtLocalDiagnosticMessagesGeneratedJson =
...
const copyBuiltLocalDiagnosticMessages = task({
	...
	run: async () => {
		...
		JSON.parse(contents); // Validates that the JSON parses.
		await fs.promises.writeFile(
			builtLocalDiagnosticMessagesGeneratedJson,
			contents,
		);
	},
});
...
export const otherOutputs = task({
	name: "other-outputs",
	description:
		"Builds miscelaneous scripts and documents distributed with the LKG",
	dependencies: [
		cancellationToken,
		typingsInstaller,
		watchGuard,
...
const runtestsDeps = [tests, generateLibs].concat(
	cmdLineOptions.typecheck ? [dts] : [],
);
...
export const runTestsAndWatch = task({
	...
	run: async () => {
		if (!cmdLineOptions.tests && !cmdLineOptions.failed) {
			console.log(
				chalk.redBright(
					`You must specifiy either --tests/-t or --failed to use 'runtests-watch'.`,
				),
			);
			return;
		}
		...
		let testsChangedDeferred = /** @type {Deferred<void>} */ (
			new Deferred()
		);
		...
		const testCaseWatcher = chokidar.watch(
			["tests/cases/**/*.*", "tests/lib/**/*.*", "tests/projects/**/*.*"],
			{
		...
		while (watching) {
			...
			if (watching) {
				console.log(
					chalk.yellowBright(
						`[watch] test run complete, waiting for changes...`,
					),
				);
				await promise;
			}
		...
		function beginRunTests(path) {
			if (testsChangedDebouncer.empty) {
				console.log(
					chalk.yellowBright(
						`[watch] tests changed due to '${path}', restarting...`,
					),
				);
				if (running) {
					console.log(
						chalk.yellowBright(
							"[watch] aborting in-progress test run...",
						),
					);
				}
		...
		function endRunTests() {
			...
			testsChangedDeferred = /** @type {Deferred<void>} */ (
				new Deferred()
			);
		}
		...
		function endWatchMode() {
			if (watching) {
				watching = false;
				console.log(
					chalk.yellowBright("[watch] exiting watch mode..."),
				);
				testsChangedCancelSource.cancel();
...
const doRunTestsParallel = task({
	...
	run: () =>
		runConsoleTests(
			testRunner,
			"min",
			/*runInParallel*/ cmdLineOptions.workers > 1,
		),
});
...
export const runTestsParallel = task({
	...
	description:
		"Runs all the tests in parallel using the built run.js file, linting in parallel if --lint=true.",
	dependencies: [doRunTestsParallel].concat(
		cmdLineOptions.lint ? [lint] : [],
	),
});
...
export const testBrowserIntegration = task({
	...
	description:
		"Runs scripts/browserIntegrationTest.mjs which tests that typescript.js loads in a browser",
	dependencies: [services],
...
export const diff = task({
	...
	run: () =>
		exec(getDiffTool(), [refBaseline, localBaseline], {
			ignoreExitCode: true,
			waitForExit: false,
		}),
});
	...
	run: async () => {
		for (const file of [
			...
			await fs.promises.copyFile(
				file,
				path.resolve(
					"../TypeScript-Sublime-Plugin/tsserver/",
					path.basename(file),
				),
			);
		}
...
export const configureNightly = task({
	...
	description:
		"Runs scripts/configurePrerelease.mjs to prepare a build for nightly publishing",
	run: () =>
...
export const configureInsiders = task({
	...
	description:
		"Runs scripts/configurePrerelease.mjs to prepare a build for insiders publishing",
	run: () =>
		exec(process.execPath, [
			"scripts/configurePrerelease.mjs",
			"insiders",
...
export const configureExperimental = task({
	...
	description:
		"Runs scripts/configurePrerelease.mjs to prepare a build for experimental publishing",
	run: () =>
		exec(process.execPath, [
			"scripts/configurePrerelease.mjs",
			"experimental",
